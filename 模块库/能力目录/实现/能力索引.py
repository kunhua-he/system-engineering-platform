...
def 定位数据根(项目根: Path) -> list[Path]:
    ...
    **2026-09-21 审计续修（同一病根第三次复发）**：仅补技能库 是不够的——实测 `/健康`
...
def 是否可发现包(声明: dict) -> bool:
    ...
    **2026-09-21 实测取证（残差清零，本条从此可引用）**：装配口径 `745` =
    发现面 `673` + 被排除的 `19 个适配层包 / 72 条能力`（差额逐条对得上，不是约数）。
    被排除的 72 条**全部**有可发现的双胞胎（按「去掉包前缀后的尾部同名」匹配：
    `Git操作.提交` → `版本控制支持库.Git操作.提交`），**72/72、零漏网** ——
    即「能调得动但搜不到」的只有这 72 个**短名别名**，而它们指向的功能都有可发现的落点，
    所以 Agent 靠「搜索能力」仍能到达全部功能。
