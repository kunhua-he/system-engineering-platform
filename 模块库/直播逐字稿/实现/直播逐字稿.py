...
def 转写媒体文件(文件路径: str, 输出目录: str, 分片秒数: int = 默认分片秒数,
    ...
    # 空凭证统一落成空串（不是 None）：写腿 开工ID 声明为 必填 文本型，显式 None 会被
    # 唯一校验点 `运行核心/统一网关/协议/类型规格.py::校验能力参数` 判「必须是 文本型」，
    # 连非受管路径也写不成；本形参默认值保持 None（与契约 `默认值: null` 对齐）。
    _底座("文件系统支持库.文件操作.创建目录",
        {"目录路径": str(输出根), "递归": 真, "开工ID": 开工ID or ""})
    ...
    转录底稿目录 = 输出根 / "02_转录底稿"
    _底座("文件系统支持库.文件操作.创建目录",
        {"目录路径": str(元数据目录), "递归": 真, "开工ID": 开工ID or ""})
    _底座("文件系统支持库.文件操作.创建目录",
        {"目录路径": str(转录底稿目录), "递归": 真, "开工ID": 开工ID or ""})
