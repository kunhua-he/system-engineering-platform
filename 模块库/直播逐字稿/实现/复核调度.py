...
区间音频格式 = "mp3"
# 超短区间的识别下限：实测 0.3 秒的疑难区间会让 ASR 三轮里有一轮返回空文本，
# 进而整区间判「部分失败」、整场质检被拦（31 分钟场次的稿子因 0.3 秒碎片全军覆没）。
最小复核时长 = 2.0
轮次提示表 = (
...
def _执行一轮(调用能力, 源音频路径: str, 超时: float, 模型配置, 序号: int) -> dict:
    ...
    参数 = {"文件路径": 源音频路径, "超时秒": 超时,
            "附加术语": 提示, "返回分段": 真}
    # 与 `分片转写._转写一片` 同一口径：「配置」声明为 字典型，**显式 None 会被
    # 唯一校验点判类型不符**（HTTP 腿与进程内腿同一把尺子），未配置模型时省略该键。
    if 模型配置 is not None:
        参数["配置"] = 模型配置
    try:
        结果 = 调用能力(转写能力id, 参数)
...
def _保证最小时长(开始: float, 结束: float, 最小时长: float = 最小复核时长) -> tuple:
    """把过短区间向两侧扩到最小时长（不早于 0 秒），返回 (开始, 结束)。

    为什么不直接放弃短区间：疑难点的归属时间不变，只是把窗口扩到 ASR 能稳定识别的长度，
    证据里同时记录扩窗后的实际区间，不伪造、不丢点。
    """
    try:
        长 = float(结束) - float(开始)
    except (TypeError, ValueError):
    ...
    if 长 >= 最小时长:
        return 开始, 结束
    中心 = (float(开始) + float(结束)) / 2.0
    新开始 = max(0.0, 中心 - 最小时长 / 2.0)
    return 新开始, 新开始 + 最小时长


def _区间可用(轮次: list, 开始, 结束) -> tuple:
    """判定一份复核是否算「完成」；返回 (是否完成, 标注说明)。

    「无可用内容」是事实不是失败：极短区间（< 最小复核时长）各轮都拿不到文本，
    说明该疑难点处没有可转写内容；按失败记账会把 0.3 秒碎片升级成整场不合格。
    除这种情况外，任何一轮失败仍如实判失败（不静默降级）。
    """
...
def _截取区间(调用能力, 源音频路径: str, 开始: float, 结束: float,
    ...
    try:
        结果 = 调用能力(截取能力id, {"文件路径": 源音频路径, "开始秒": 开始, "结束秒": 结束,
                                    ...
                                    # 同 分片转写：源时长预检走底座默认 7200 秒，长场次必须显式放宽
                                    "最大时长秒": 43200.0})
    except Exception as 错误:  # 底层异常不得外泄给调用方
        return "", f"截取异常: {错误}"
    if not bool(getattr(结果, "成功", 假)):
...
def _读已有复核(复核目录: str, 区间id: int) -> dict | None:
    ...
    # 不把「不完整」的缓存当成果复用（2026-09-17 实测）：失败多因瞬时（那一轮模型未返回文本），
    # 复用会让该区间永久卡死——报告里只剩「区间5: 」这种空原因，重跑多少次都不变。
    # 返回 None 让调用方重跑该区间，配合 _保证最小时长 自愈。
    可用, 标注 = _区间可用(轮次, 数据.get("开始秒"), 数据.get("结束秒"))
    if not 可用:
        return None
