...
def 聚类区间(疑难段: list[dict], 间隔秒: int = 默认间隔秒) -> list[dict]:
    """疑难段聚类成复核区间，返回 {"区间": [...]}。

    相邻疑难段间隔严格小于阈值才并入同一区间；间隔恰好等于阈值算两段。
    """
    阈值 = _取数(间隔秒)
...
def 复核区间(源音频路径: str, 区间: dict, 复核目录: str, 调用能力,
    ...
    """对单个区间跑多轮独立转写并落盘，返回 状态/错误码/错误说明 齐全的字典。"""
    区间id, 开始, 结束 = _区间信息(区间)
    超时 = _取数(超时秒)

    def 失败(说明: str) -> dict:
        return {"区间id": 区间id if 区间id is not None else 0, "开始秒": 开始, "结束秒": 结束,
                "轮次": [], "状态": "失败", "错误码": "参数不合法", "错误说明": 说明, "写入路径": ""}
    ...
    轮次列表 = [_执行一轮(调用能力, 源音频路径, 超时, 模型配置, 序号)
                for 序号 in range(1, 轮数 + 1)]
    写入路径, 写盘错误 = _写复核盘(
        复核目录, 区间id, {"区间id": 区间id, "开始秒": 开始, "结束秒": 结束, "轮次": 轮次列表})
