...
def _写JSON(路径: Path, 数据: dict, 开工ID: str | None = None) -> bool:
    """经底座序列化并写 JSON（utf-8、缩进 2）；成功返回 True。

    开工ID 穿透（2026-09-25）：`原子写入` 按「活跃写租约所有者 == 开工ID」判授权
    （`公共契约/运行时/写入授权.py::校验写入授权`）；分片转写落点（缓存目录）受管时
    缺它即 `越界`。
    """
...
def _转写一片(音频路径: str, 区间: dict, 缓存: dict, 调用能力, 模型配置,
             附加术语: str, 超时秒: float, 续跑: bool,
             开工ID: str | None = None) -> tuple[dict | None, str, bool]:
    """转写一片（已落盘且区间吻合才复用）；返回 (分片转写条目, 错误说明, 是否复用)。

    开工ID 穿透（2026-09-25）：落盘走 `_写JSON` → `原子写入`，按「活跃写租约所有者
    == 开工ID」判授权，原样透传。
    """
    ...
    if 续跑:
        ...
        if isinstance(已有, dict) and isinstance(已有.get("分段"), list) and _区间吻合(已有, 区间):
            return 已有, "", 真
    参数 = {"文件路径": 音频路径, "超时秒": 超时秒,
            "附加术语": 附加术语, "返回分段": 真}
    # 「配置」在契约里声明为 字典型（必填=假，默认值=None）：**显式传 None 会被拒**
    # —— 唯一校验点 `类型规格.校验能力参数` 的判据是「类型不符即拒」，HTTP 腿与
    # 进程内腿同一把尺子（2026-09-23 进程内腿补齐校验后实测：显式 None 回
    # `参数不合法：参数 配置 必须是 字典型`，经网关调用本来也是这个结论）。
    # 未配置模型时**省略该键**，交给支持库默认值 —— 与本包 `直播逐字稿._转写文件`
    # （`if 模型配置 is not None`）及 `媒体转写.转写音频文件` 的既有口径一致。
    if 模型配置 is not None:
        参数["配置"] = 模型配置
    结果对象 = 调用能力(转写能力id, 参数)
    if not 结果对象.成功:
