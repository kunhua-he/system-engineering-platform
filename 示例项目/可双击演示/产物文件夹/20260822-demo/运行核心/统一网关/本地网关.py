...
class 本地网关服务器:
    ...
    def __init__(self, *, 网关核心实例: 网关核心, 地址: str = "127.0.0.1",
        ...
        self.安全配置 = 安全配置(
            ...
            监听地址="127.0.0.1",
            允许路径表=set(self.配置.get("允许路径表", {"/健康", "/网关/请求", "/网关/调用", "/网关/流式"})),
            要求凭证=要求凭证,
        ...
        class 处理类(BaseHTTPRequestHandler):
            ...
            def _处理网关请求(self) -> None:
                ...
                if not 读取成功:
                    ...
                    return
                操作 = 请求数据.get("操作") or ("调用能力" if 请求数据.get("目标") else "")
                if not isinstance(操作, str) or not 操作:
                    self._拒绝(400, "参数不合法", "缺少目标")
                    return
                参数 = 请求数据.get("参数") or {}
                ...
                if 超时秒 <= 0 or 超时秒 > 请求超时秒:
                    ...
                    return
                请求对象 = 网关请求(
                    操作=操作, 能力id=str(请求数据.get("能力id", "")),
                    目标=str(请求数据.get("目标", "")), 参数=参数,
                    句柄=str(请求数据.get("句柄", "")),
                    获取句柄=bool(请求数据.get("获取句柄", False)),
