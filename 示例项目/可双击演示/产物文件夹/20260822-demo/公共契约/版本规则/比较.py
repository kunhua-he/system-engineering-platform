...
def 比较版本(当前文本: str, 目标文本: str) -> 版本判定:
    ...
    if 目标 > 当前:
        ...
        if 目标.主 != 当前.主:
            说明 += "，但跨主版本，需人工确认"
        return 版本判定(当前, 目标, 可升级=True, 可回滚=False, 兼容=当前.兼容(目标), 说明=说明)
    if 目标 < 当前:
        return 版本判定(当前, 目标, 可升级=False, 可回滚=True, 兼容=当前.兼容(目标), 说明="目标低于当前：仅允许回滚")
    return 版本判定(当前, 目标, 可升级=False, 可回滚=False, 兼容=True, 说明="版本一致")
