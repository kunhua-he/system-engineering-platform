...
#: 指纹校验开关的环境变量名（唯一具名处）。
指纹开关键 = "系统底座_指纹校验"
#: 本门禁的历史接受面（显式传给 `环境开关`；**不得改用并集** `真值词表`：并集多出的
#: `真` / `on` 会把门禁的「关」静默改成「开」＝放宽接受面）。
指纹开关键词表 = frozenset({"1", "true", "yes", "是"})
...
class 包指纹面:
    ...
    def 启用包指纹校验(self) -> bool:
        ...
        **接受面逐字保持（显式传 `指纹开关键词表`）**：旧实现认 `1/true/yes/是`，并集
        `真值词表` 多出的 `真` / `on` **故意不认** —— 传并集会把这两个值从「关」判成
        「开」＝放宽本门禁接受面（指纹校验会突然开始拦人），违反「同义收口不得改变
        任何一处判定结果」。
        """
        if not 环境开关(指纹开关键, 词表=指纹开关键词表):
            return 假
