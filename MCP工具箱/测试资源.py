...
def _规范资源类型(资源类型: str) -> str:
    """统一资源类型；仅兼容已知历史别名，未知类型立即拒绝。"""
    类型 = _资源类型别名.get(str(资源类型).strip(), str(资源类型).strip())
    if 类型 not in _资源类型集合:
        raise ValueError(f"未知资源类型：{资源类型}")
    return 类型
