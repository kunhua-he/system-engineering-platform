...
class 资源协调器:
    ...
    def 提交(self, *, 事务id: str, 句柄id: str, 资源id: str,
        ...
        工作文件 = self.工作目录 / f"{事务id}.json"
        if not 工作文件.is_file():
            return 假, f"未知事务: {事务id}", ""
        # 1. 句柄校验（权限与所有者）
        句柄 = self.状态.读取句柄(句柄id)
        if 句柄 is None or 句柄["状态"] != "有效":
            return 假, f"句柄已失效（{句柄['失效原因'] if 句柄 else '不存在'}），不能自动复活", ""
        if 句柄["项目id"] and 句柄["项目id"] != 项目id:
            return 假, f"跨项目复用被拒绝: 句柄属 {句柄['项目id']}，请求 {项目id}", ""
        if 句柄["所有者"] and 句柄["所有者"] != 所有者:
            return 假, f"跨所有者复用被拒绝: 句柄属 {句柄['所有者']}，请求 {所有者}", ""
        # 2. 提交前比较版本
        ...
        if 当前版本 != 基础版本:
            ...
            self.状态.完成事务(事务id=事务id, 成功=假)
            return 假, f"版本冲突: 基础版本 {基础版本}，当前 {当前版本}（他人已提交，禁止覆盖）", ""
        ...
        try:
            ...
            新值 = 工作数据.get("值")
            # A3 同型防护（2026-09-18 主会话补，与 权威状态.提交资源 同口径）：
            # 「版本」是外部可写的账本字段（迁移库/手工写入/异常数据可能是 "v2" 或 NULL），
            # 这里的 int() 若不设防，异常会穿透错误边界并把内部文本漏给调用方。
            # 版本域失败统一用已登记中文码「版本冲突」（网关状态映射 → 409）。
            try:
                新版本 = str(int(当前版本) + 1)
            except (TypeError, ValueError):
                self.状态.失效句柄(句柄id, "版本冲突")
                self.状态.完成事务(事务id=事务id, 成功=假)
                return 假, f"版本冲突: 基础版本 {当前版本!r} 不是合法版本号（资源版本域被异常数据污染）", ""
            新数据 = {"值": 新值}
            ...
            self.状态.完成事务(事务id=事务id, 成功=真, 新版本=新版本)
            return 真, "提交成功", 新版本
        finally:
