...
class 资源账本维护循环:
    ...
    def 清扫一次(self) -> list[str]:
        ...
        with self._清扫锁:
            ...
            # ★ 2026-09-24（批P·P-1）：**顺手回收过期文件租约**（接入既有周期机制，不新建调度器）。
            #
            # ⚠ 上面那句 `扫描过期租约()` 管的是**另一套账**：`权威状态`（SQLite 里的**句柄租约**，
            #   表在 `<运行缓存根>/运行数据/`）。这里收的是**文件租约**
            #   （`<运行缓存根>/平台控制面/文件租约.json`，即 `开工即占/收工释放` 那条腿的账）
            #   —— 两者**同名不同物**，不是同一份事实，**不得合并**。
            #   「两套租约账」本身已登记为待裁项（本批未合并，只如实标注）。
            #
            # 为什么挂在这里（华哥目标第 6 条：底座已有状态机 + 线程池，要复用、不新建）：
            #   文件租约的到期是**时间的函数、不是事件** —— 回锁只在「下一次认领的懒回收」里
            #   发生，于是「租约已过期、写窗口还开着、没人来收」时，`仓库只读锁.查询()` 把这几条
            #   判成真缺口（主会话活体实证：未锁 29 / 开窗 0 / 缺口 29 ⇒ `pre-commit` 拒提交）。
            #   本循环是长驻网关里**本来就有的**周期推动点（`启动运行核心网关.py:172` 接线，
            #   每 300 秒一轮），故回收搭在它上面，与 `查询()` 的顺手回收是**同一个动作的**
            #   两个唤醒点（都在调 `写入授权.顺手回收过期租约`，判据仍只一处）。
            # 失败只留痕：维护循环不得因清理失败而中断网关（同上两个 try 的口径）。
            try:
                from 公共契约.运行时.写入授权 import 顺手回收过期租约
                from 公共契约.运行时.仓库只读锁 import 推断仓库根

                # 项目根从**租约账所在树**反推（与 `写入授权.受管相对路径` / `回锁` 同一条
                # `推断仓库根` 判据，本处不自写向上找 `.git` 的循环）。
                # ★ 边界（2026-09-24 三例实测，不猜）：`推断仓库根` 认的是「往上找 `.git`」，
                #   故它**不总是** `None`，按跑法分三种：
                #     甲、源码树跑（账在 `<仓库>/工程缓存/运行数据`）⇒ 回**仓库根**，本唤醒点照常回收；
                #     乙、跑**仓内**制品（`<仓库>/工程缓存/制品仓库/…/平台客户端`）⇒ 也回**仓库根**
                #        （制品在仓里，往上撞到的是同一个 `.git`）⇒ 回收的正是本仓那份账；
                #     丙、跑**装到仓外**的激活制品（账在 `~/Library/Caches/系统工程平台/运行缓存/`）
                #        ⇒ 那棵树里没有 `.git` ⇒ 回 `None` ⇒ **静默跳过**：不知道锁哪棵树就不锁
                #        （宁可不做，也不把内核不可变标志打到错那棵树上）。
                #   丙 那条缺口由另两个唤醒点奶上：`仓库只读锁.查询()` 的顺手回收（拿的是调用方
                #   给的**真仓库根**），与下一次 `申请文件租约` 的既有懒回收（根来自调用方 `项目根`）。
                仓库根 = 推断仓库根(self.状态账本.存储目录)
                if 仓库根 is not None:
                    回收数, 回收问题 = 顺手回收过期租约(str(仓库根))
                    if 回收数:
                        清理列表.append(f"过期文件租约回收 {回收数} 条")
                    elif 回收问题:
                        记录忽略('资源账本维护循环.回收过期文件租约', 回收问题)
            except Exception as 错误:  # 允许忽略，但留痕
                记录忽略('资源账本维护循环.回收过期文件租约', 错误)
            self.清扫次数 += 1
            self.最近清扫 = 清理列表[-20:]
            return 清理列表
