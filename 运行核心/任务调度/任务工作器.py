...
def 处理请求(请求: dict) -> dict:
    ...
    协议循环（就绪标记 / 空行跳过 / 停止即结束）由 公共契约/运行时/子进程协议 承担；
    本函数只管「这一条请求怎么答」。`停止` 也要答一条确认（协议层写完响应才结束循环）。
    """
    from 公共契约.基础类型.逻辑类型 import 假, 真

    if 请求.get("类型") == "停止":
        return {"任务id": 请求.get("任务id", ""), "成功": 真, "值": "停止确认", "错误码": ""}
    ...
    try:
        ...
        return {"任务id": 请求.get("任务id", ""), "成功": 假,
                "错误码": "内部错误", "错误说明": str(错误)}


def 主循环() -> int:
    """多请求协议主循环（常驻工作器）：READY → 逐行处理 → 收到 停止 即结束。

    协议唯一实现在 公共契约/运行时/子进程协议；本模块也作独立脚本启动
    （`python 任务工作器.py <系统根>`），模块级不能导入公共契约，故此处函数内导入。
    """
