...
class 回收落盘面:
    ...
    def _落盘波次(self) -> None:
        ...
        压在池锁里。落盘失败**fail-closed**（2026-09-25 收口）：① 先把失败说明**留痕**
        进本波次每个受影响任务的日志（`查询日志` 读得到，上层可见）；② 再把原异常
        **如实上抛**给调用方（监视轮询记入池级 `落盘错误`、排空/停止记为首个异常后
        继续走完各自的完整路径）。**绝不静默返回成功**。
        """
        with self.锁:
        ...
        try:
            self._落盘(波次)
        except BaseException as 错误:  # noqa: BLE001 —— 留痕后原样上抛，不吞不换
            # fail-closed（2026-09-25）：落盘失败**不许静默返回成功** ——
            # ① 先留痕：失败说明逐条 append 进本波次每个受影响任务的日志
            #    （`查询日志` 读得到，上层可见）；
            # ② 再把原异常**原样上抛**（不吞、不换、不降级成成功），由调用方按既有
            #    路径处置（监视轮询记入池级 `落盘错误`；排空/停止记为首个异常后
            #    继续走完各自的完整路径）。
            with self.锁:
                说明 = f"落盘失败：{type(错误).__name__}: {错误}"
                for 任务对象 in 波次:
                    任务对象.日志.append(说明)
            raise
