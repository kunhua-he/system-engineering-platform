...
#: 任务会连同它的句柄被永久引用 —— 实测约 **12 个 fd/条**（接收管道 2 个 + `取消事件`
#: 与 `进程组就绪事件` 各 4 个 POSIX 信号量 + 进程对象 sentinel 1 个；40 条实测
#: +480 个 fd）。只写不删时提交 N 个任务就永久留下约 12N 个 fd，软限 256 的网关
#: 提交数十个任务即把下一次 `Pipe()` 打成 `OSError: [Errno 24]`。
#:
#: 取值口径：**只约束 fd 与内存，不约束可查性** —— 被驱逐的任务仍按磁盘快照重建
#: （`查询` 的既有回退），网关的权威账本在 `任务系统.任务表` + 运行库，与本表无关。
#: 64 条 ≈ 768 个 fd（远低于修好的软限 65536），且**不随历史任务总数增长**。
任务表历史上限 = 64
