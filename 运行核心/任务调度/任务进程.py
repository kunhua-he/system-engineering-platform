...
class 任务进程池:
    ...
    def 排空(self, *, 超时秒: float | None = None) -> tuple[bool, str]:
        ...
        - `_拒绝等待队列` 直接写在 `已停止 = True` 之后：它内部的 `_完成失败` 一落盘
          失败就把异常放走，方法当场退出 —— 队列虽已由 `_拒绝等待队列` 自己清空
          （禁止项③ 已修），但**排空没做、监视线程没停**；
        - 收敛轮询段 `_轮询任务`（`_完成` 尾部同样要落盘）：盘满导致每次落盘都失败时，
          异常同样从轮询里放走，`_停止监视` 执行不到，监视线程照旧留着。
