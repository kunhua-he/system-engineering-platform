...
def _尝试镜像命中(提供者目录: Path, 依赖锁: dict, 目标: Path,
    ...
    - 环境结果(成功=False)：镜像尝试失败（证据类型 失败，错误码为
      镜像不可用/镜像摘要不匹配/镜像下载失败 之一），调用方回退本地构建。
    """
    输入哈希 = _输入哈希(提供者目录)
    ...
    if not 校验结果.允许命中:
        _记录证据(提供者目录, "失败", 摘要, 输入哈希,
                   错误码=镜像摘要不匹配, 错误说明=校验结果.错误说明)
        return 环境结果(False, 错误码=镜像摘要不匹配, 错误说明=校验结果.错误说明)
    ...
    if not 复校验.允许命中:
        shutil.rmtree(临时目录, ignore_errors=True)
        _记录证据(提供者目录, "失败", 摘要, 输入哈希,
                   错误码=镜像摘要不匹配, 错误说明=复校验.错误说明)
        return 环境结果(False, 错误码=镜像摘要不匹配, 错误说明=复校验.错误说明)
