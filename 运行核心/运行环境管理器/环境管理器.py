...
- 环境摘要里的「提供者id」**一律先归一到 `提供者目录.name`**（即环境目录的路径段口径）：
  指纹只该表达「环境构成」，身份已经落在路径段上；两种 id 形态（短名 / 全限定包 id）
  必须落到同一个摘要，否则同一提供者会分叉出两份环境（见 `_归一提供者id`）。
...
def _归一提供者id(提供者id: str) -> str:
    """把提供者id 归一到「环境目录路径段口径」= `提供者目录.name`。

    接受的两种写法（全仓实测只有这两种）：

    - **全限定包id**：``支持库.适配层.psycopg提供者`` → 末段 ``psycopg提供者``
    - **目录名短名**：``psycopg提供者`` → 原样返回

    取末段是**同义归一**，不是模糊匹配：包id 用点号分段、目录名不带点（包命名规则），
    故「全限定包id 的末段」恒等于「目录名」（全仓 33 份 `依赖锁.json` 实测 33/33 成立；
    目录名含点实测 0 例）。**幂等**：``归一(归一(x)) == 归一(x)``，因此调用方
    传目录名（生产唯一口径）时归一是恒等变换，既有环境摘要逐字节不变。

    为什么必须归一而不是把 提供者id 从指纹里删掉：指纹是**环境目录名的唯一依据**
    （`环境目录`），身份已经落在路径段 ``<提供者目录.name>`` 上，删掉身份字段等于
    让两个提供者在依赖锁相同时共用一个环境 —— 与决策记录 `0047`（不合并依赖锁相同
    的提供者环境）冲突，且会作废全仓既有环境（实测 21 份 / 3103.7 MB，含 3GB 级
    转写与 MLXWhisper 各一份）。
    """
    文本 = str(提供者id or "").strip()
    if not 文本:
        return ""
    return 文本.split(".")[-1]
...
def 计算环境摘要(依赖锁: dict, 提供者id: str) -> str:
    ...
    `提供者id` **先经 `_归一提供者id` 归一**再入指纹：同一提供者不论传短名
    （``psycopg提供者``）还是全限定包id（``支持库.适配层.psycopg提供者``）都得到
    同一个摘要，不会因 id 形态分叉出两份环境。
    """
    指纹 = {
