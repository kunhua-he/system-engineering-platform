...
def 确保环境(提供者目录: Path, *, 超时秒: int = 300,
    ...
    if isinstance(超时秒, bool) or not isinstance(超时秒, int) or not 1 <= 超时秒 <= 3600:
        return 环境结果(假, 错误码="参数不合法", 错误说明="环境构建超时秒必须在 1 到 3600 之间")
    _报告阶段(进度回调, "检查缓存")
    输入哈希 = _输入哈希(提供者目录)
    提供者id = 提供者目录.name
    ...
    if not (提供者目录 / "依赖锁.json").is_file():
        _记录证据(提供者目录, "命中", "", 输入哈希)
        _报告阶段(进度回调, "环境就绪")
        return 环境结果(真, 解释器路径=sys.executable,
                         错误说明="无第三方依赖，使用系统解释器")
    ...
    if 错误码:
        _记录证据(提供者目录, "失败", "", 输入哈希,
                   错误码=错误码, 错误说明=错误说明)
        return 环境结果(假, 错误码=错误码, 错误说明=错误说明)
    依赖锁 = 读取依赖锁(提供者目录)
    if not 依赖锁:
        _记录证据(提供者目录, "命中", "", 输入哈希)
        return 环境结果(真, 解释器路径=sys.executable, 错误说明="无第三方依赖，使用系统解释器")
    # 全部为外部应用/系统工具（非 pip 包）→ 使用系统解释器，不构建 venv
    # **按平台过滤**（2026-09-19 跨平台双后端）：只适用于别的平台的 pip 依赖不参与
    # 「要不要建 venv」的判定。例：只声明 mlx（macOS 专有）的提供者在 Linux 上，
    # 本平台没有任何 pip 依赖要装 → 走系统解释器，不再尝试构建一个注定失败的 venv。
    pip包表 = [包 for 包 in 依赖锁.get("包", []) if _是pip包(包) and _适用当前平台(包)]
    if not pip包表:
        _记录证据(提供者目录, "命中", "", 输入哈希)
        return 环境结果(真, 解释器路径=sys.executable,
...
def _尝试镜像命中(提供者目录: Path, 依赖锁: dict, 目标: Path,
    ...
    if not 清单结果.成功:
        ...
        return 环境结果(假, 错误码=镜像不可用, 错误说明=清单结果.错误说明)
    ...
    try:
        ...
        return 环境结果(假, 错误码=镜像下载失败, 错误说明=f"镜像落盘失败: {错误}")
...
def 校验环境(解释器: Path, 依赖锁: dict) -> bool:
    ...
    if not 解释器.is_file():
        return 假
    # **按平台过滤**（2026-09-19 跨平台双后端）：`适用平台` 不含当前平台的依赖项
    # 既不安装也不校验 —— 否则 mlx（Apple Silicon 专有）会让 Windows/Linux 恒定
    # 校验失败，把「本平台结构性没有这个依赖」误报成「环境损坏」。
    # 不写该字段的条目全平台适用，行为与改动前逐字一致（旧锁不改也能读）。
    包表 = [包 for 包 in 依赖锁.get("包", []) if _是pip包(包) and _适用当前平台(包)]
    if not 包表:
        return 真
    模块名表 = []
    for 包 in 包表:
...
#: 依赖锁条目的可选字段名：该依赖「适用的平台名列表」。
#: **不写该字段 = 全平台适用**（向后兼容：旧锁不改也能读，判定行为零变化）。
依赖项适用平台字段 = "适用平台"

#: 平台名别名归一表：接受常见原始写法，一律归到 `平台适配.当前平台()` 的口径名。
#: 为什么要容忍别名：写锁的人容易按 `sys.platform` 的习惯写 `darwin`/`win32`，
#: 而本仓平台名的**唯一权威口径**是 `平台适配.当前平台()`（`macOS`/`Windows`/`Linux`）。
#: 只做同义归一，不做模糊匹配（`unknown` 之类不猜，直接判不适用）。
平台名别名表 = {
...
def _归一平台名(名称: object) -> str:
    """把锁内写的平台名归一到 `平台适配.当前平台()` 的口径名；认不出返回空串。"""
    文本 = str(名称 or "").strip()
    if not 文本:
        return ""
    return 平台名别名表.get(文本.lower(), 文本)


def _适用当前平台(包: dict) -> bool:
    """依赖锁条目是否适用于**当前平台**（判定口径的唯一实现）。

    规则（容错优先，只有强制参数不满足才失败）：

    1. **不写 `适用平台` 字段 / 写成空值 → 全平台适用**（向后兼容：全仓旧锁不改也能读，
       判定结果与本次改动前逐字一致）。
    2. 写成**列表** → 成员（经 `平台名别名表` 归一）包含 `平台适配.当前平台()` 即适用。
       例：``["macOS"]`` 在 Linux 上不适用；``["Windows", "Linux"]`` 在 macOS 上不适用。
    3. 写成**非列表**（字符串/数字/对象）→ 按**不适用**处理（fail-closed 的保守口径）：
       写法不合法时宁可「不装、不校验」，也不把一份说不清适用面的依赖当全平台装上。
       该字段是本次新增，全仓既有锁 0 处使用（实测 `rg 适用平台` 命中 0），故无历史包袱。

    平台判定**只经收口层** `平台适配.当前平台()`：本函数不做任何 `sys.platform` /
    `os.name` 判断（跨平台收口铁律，调用点不许自带平台分叉）。

    调用点：本包 `校验环境`（import 校验的包表）、`_构建环境`（pip 安装的包表）、
    `确保环境`（「是否全是外部应用/系统工具」判定）、`环境摘要信息`（展示用依赖清单），
    以及 `强制校验` 的规则 3／规则 4（版本与闭包覆盖）。
    """
    if 依赖项适用平台字段 not in 包:
        return 真
    适用表 = 包.get(依赖项适用平台字段)
    if 适用表 is None or 适用表 == "" or 适用表 == []:
        return 真
    if not isinstance(适用表, list):
        return 假
    当前 = 平台适配.当前平台()
    return any(_归一平台名(项) == 当前 for 项 in 适用表)
...
def _构建环境(提供者目录: Path, 依赖锁: dict, 目标: Path,
    ...
    try:
        ...
        # **按平台过滤**（2026-09-19 跨平台双后端）：`适用平台` 不含当前平台的依赖项
        # 不进入安装循环。这一步是 Windows/Linux 装配能否通过的关键 —— mlx 是
        # Apple Silicon 专有（实测 Linux 上 `import mlx.core` 恒报 libmlx.so 缺失，
        # 装得上、用不了），把它从安装集合里摘掉，装配才不再被它整体阻断。
        包表 = [包 for 包 in 依赖锁.get("包", []) if _是pip包(包) and _适用当前平台(包)]
        ...
        当前阶段 = "提交环境缓存"
        _报告阶段(进度回调, 当前阶段)
        # ★ 2026-09-18 修同型缺陷（与「远程镜像.py」同一族）：原写法是
        #   `rmtree(目标)` → `os.replace(临时, 目标)` —— **先删旧环境再顶位**，
        #   顶位失败（父目录不可写、跨设备、进程被杀）时**新旧环境同时消失**，
        #   调用方拿到的是一条普通失败，磁盘上却已经没有可用的环境了。
        #   现复用同包 `原子落盘`：旧环境改名让位 → 顶位 → 成功后才清理让位件；
        #   失败回滚让位件；连回滚都失败则不删让位件并把位置写进错误说明。
        #   口径：**任一时刻磁盘上至少有一个可用环境**。
        原子落盘(临时目录, 目标)
        return 环境结果(真, 解释器路径=str(解释器), 环境摘要=摘要)
