...
def _归一化系统名(系统名: str) -> str:
    """归一化**锁内**系统名写法：去掉括号/空格后缀；macOS 与 Darwin 视为同族。

    **职责边界（#103 收口后）**：本函数只归一**依赖锁里落盘的系统名字文本**——那是不可控
    的历史输入（实测混写 ``macOS（Darwin 26.5.2）`` / ``darwin`` / ``macos`` 三态）；
    **本机侧不再经过它**，本机系统名一律取唯一口径 `公共契约.运行时.平台适配.本机系统名()`
    （归一化后稳定为 ``Darwin``/``Windows``/``Linux``/``未知``）。

    **为什么不删**：旧锁仍在库里，删掉会让历史写法判不匹配（假红）。它属**历史锁兼容补丁**，
    不是第二份「本机系统名」实现 —— 输入域与本机口径互不相交。
    """
    名 = 系统名.strip().lower()
    for 分隔 in ("（", "(", " "):
        if 分隔 in 名:
...
def 系统匹配(锁内系统: Any, 当前系统: str) -> bool:
    ...
    双边都经 `_归一化系统名`：`当前系统` 由调用方从 `平台适配.本机系统名()` 取
    （本机侧唯一口径），锁内值仍按历史写法归一 —— 故 ``Darwin`` 与 ``macOS（…）``
    仍判同族。
    """
