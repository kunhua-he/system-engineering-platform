...
运行核心 -> 公共契约和支持库公开入口
项目适配层 -> 公共契约和核心公开入口
（原 MCP工具箱 服务宿主层已于 2026-09-15 随 D 清场删除；接入面薄壳现居
  `开发工具/薄壳/`，它的第三方宿主 SDK（mcp）由 `服务宿主SDK白名单` 挂在
  `开发工具` 层显式放行，不走「一第三方一支持库一提供者」口径。）
...
允许依赖表: dict[str, set[str]] = {
    ...
    "平台控制面": {"公共契约", "支持库", "运行核心"},
    "项目适配层": {"公共契约", "运行核心"},
    # （原「服务宿主层」条目已随 MCP工具箱 删除移除：薄壳现居 开发工具/薄壳/，
    #   由 `开发工具` 的放行集合覆盖，第三方宿主 SDK 见 `服务宿主SDK白名单`。）
