...
# 路径边界判定的**唯一判据节点**（2026-09-24 批H 裁-A 收口）：本文件不再自写路径判据。
# 层向依据：`允许依赖表["运行核心"]` 明文含 `支持库`（且该包只依赖 `公共契约`，不成环）；
# 该节点是**进程内直接 import** 的公开函数（不是注册能力，见其包 `__init__::注册能力` 的撤注册说明）。
...
层名称表 = {
    ...
    "后端核心": "后端核心",
    "运行核心": "运行核心",
    # 正式根之一（`公共契约/正式根.py::正式根名表` 列有 `启动监督器`；2026-09-26 补登记）：
    # 此前 `启动监督器/**` 的顶层段不在本表 → `_确定层` 回落成「其他」，且制品态下
    # `剥制品根前缀` 因「段名须命中本层表」而拒绝剥制品包名前缀，两处叠加使包级入口
    # 导自身 `实现/` 被判「跨包导入 实现/ 目录」（实测 2 条假红，见 `允许依赖表` 该层注释）。
    "启动监督器": "启动监督器",
...
允许依赖表: dict[str, set[str]] = {
    ...
    "后端核心": {"公共契约", "运行核心"},
    "运行核心": {"公共契约", "支持库"},
    # 启动监督器 层实测导入落点（2026-09-26 全层 AST 审计；取证口径：对
    # `启动监督器/**/*.py` 做 AST import 扫描，按 `模块名.split(".")[0]` 归层）：
    # 公共契约 9 处（健康监督/__init__.py:25、健康监督/实现/提供者.py:23,29、
    # 健康监督/核心.py:49,58、容量基线.py:10,11,12）、支持库 1 处（健康监督/核心.py:50）、
    # 项目适配层 3 处（健康监督/核心.py:51,52,57）；其余顶层段只有标准库与
    # `启动监督器` 自身（同层，不需要授权）。
    # 按**最小授权**登记这三层，**不照抄「其他」的宽松集合**（那会把 模块库/技能库/
    # 平台控制面/开发工具 等零落点的层一并放行）。`公共契约` 虽在 `审计依赖` 里由
    # 「顶层 == 公共契约 → continue」全局放行，仍按各层惯例显式登记（表即口径）。
    # 三层均已被真实使用，故 `预留授权表` 不登记（该表只登记「备案未使用」的边）。
    "启动监督器": {"公共契约", "支持库", "项目适配层"},
...
文件级豁免表: dict[str, dict[str, frozenset[str]]] = {
    ...
    # （`模块库/项目文档/实现/文档生成器.py::_跑生成器`），子进程从不载入租约能力面
    # ⇒ 事实源恒为空。没有这条边里的懒加载，「人工改 md 的唯一合法通道」结构性永远过不了
    # 这道门（实测：前 3 份 md 落盘成功、之后 4 份全部退出码 1）。
    "公共契约": {
        "公共契约/运行时/写入授权.py": frozenset({"平台控制面"}),
    },
    # `支持库/后端/组件规范支持库/实现/组件规范.py` → 开发工具（2026-09-24，批R R-31，
    # 华哥裁决「有统一节点就切统一节点」收口 O-12 第三腿）：
    # 该文件是 `完整性摘要.json` 的**写者之一**，此前自写一份序列化公式
    # （`json.dumps(摘要, ensure_ascii=False, indent=2) + "\n"`），与唯一权威
    # `开发工具/全量重算摘要.完整性摘要文本` 并存 ⇒ 同一产物两条腿（口径已分叉）。
    # 收口＝转调权威，而权威住 `开发工具` 层 —— `允许依赖表["支持库"]` 只有 `公共契约`，
    # 这条边没有别的合法走法。**只放行这一个文件、不开放整层**：不把「支持库 可依赖
    # 开发工具」推广成整层边（那是扩大授权），与上面 2026-09-23 的先例逐字同形。
    "支持库": {
        "支持库/后端/组件规范支持库/实现/组件规范.py": frozenset({"开发工具"}),
    },
}
...
def _审计根(目标目录: Path | None) -> Path:
    ...
    **反斜杠字面量必须与正斜杠同结果**（本函数与 `归一相对路径` 同一口径，2026-09-18 实测）：
    ...
    ① 相对写法（**唯一判据节点** `校验相对路径文本` 判为项目根内合法相对路径；2026-09-24 批H 裁-A
    收口前是自写的「不以 `/`、`\\` 开头也不带盘符」）→ 拼 `系统根`（`运行核心/…` 与 `运行核心\\…` 同支）；
    ...
    # ★ 2026-09-24（批H 裁-A 收口）：此处原有的自写判据（带盘符 + 前导 `/`/`\`）已**全删**，
    # 「这是不是项目根内合法相对路径」一律转调**唯一判据节点** `校验相对路径文本`
    # （它先 URL 解码一次，再判绝对写法含盘符 / `~` / `..` 段 / `.` 段）。
    # 传**原始入参**（未解码）：节点自己解一次码，传已解码值就是二次解码漏洞。
    # 被节点拒的写法（绝对、盘符、`~`、含 `..`/`.` 段）一律**不进 ①** —— 与旧行为一致（旧判据
    # 也是「不像相对写法就不拼根」），落回下面「三条都落空则原样返回」的 fail-closed 口径。
    文本判定 = 校验相对路径文本(文本)
    判定值 = 文本判定.值 if isinstance(文本判定.值, dict) else {}
    if 段表 and 文本判定.成功 and 判定值.get("通过"):
        # ① 相对写法（`运行核心/…` 与 `运行核心\…` 走同一支）：一律按 `系统根` 解析。
        #    「相对 → 绝对」转调全平台唯一那条解析腿（本文件不自带绝对写法分支、也不自己拼根）。
