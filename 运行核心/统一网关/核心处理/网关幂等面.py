...
#: 本进程内存幂等表的条目上限（FIFO 淘汰最老条目）—— 有界留痕，无界增长会吃内存。
幂等表上限 = 500

#: 「进行中」表条目上限（与 `幂等表上限` 同口径）。为什么也必须要有界：执行挂死
#: （`_执行` 不返回）时条目**永不清**，正常路径的 `pop` 到不了 ⇒ 无界增长。
#: 淘汰是兜底，淘汰时放行其等待者（`完成事件.set()`），语义退化为「可能重复执行一次」，
#: 优于内存被吃光。
幂等进行中上限 = 500
...
class 幂等记忆面:
    ...
    def _修剪进行中(self) -> None:
        """把「进行中」表压回上限：FIFO 淘汰最老条目并放行其等待者。

        **调用点必须已持有 `_幂等锁`**（本方法不自取锁，避免与调用点重复加锁）。

        为什么必须做：`_执行` 挂死不返回时条目**永不清**（正常路径的两处 `pop`
        到不了）⇒ `_幂等进行中` 无界增长会吃进程内存。淘汰是兜底，语义退化为
        「该幂等键可能重复执行一次」，优于内存被吃光；正常情况下表远小于上限，
        本方法零开销。
        """
        while len(self._幂等进行中) > 幂等进行中上限:
            键 = next(iter(self._幂等进行中))
            _摘要, 事件 = self._幂等进行中.pop(键)
            事件.set()
