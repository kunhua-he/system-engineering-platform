...
from 运行核心.统一网关.运行态.调用耗时日志 import 记录耗时
...
class 请求处理面:
    def 处理(self, 请求: 网关请求) -> 网关响应:
        ...
        # 已随批次0-3 登记为公开码，不再回落「请求处理失败」+500）。
            ...
            # 慢调用留痕（2026-09-21 华哥裁决：html 要有调用耗时日志，超 10 秒必须优化、
            # 超 3 秒考虑优化）。**只在达阈值时才落盘**，亚秒级调用零开销；
            # 本调用绝不抛异常（日志是观测面，不能把请求搞失败）。
            记录耗时(
                能力id=请求.能力id or 请求.目标, 操作=请求.操作,
                耗时毫秒=响应.耗时毫秒, 成功=响应.成功, 错误码=响应.错误码,
            )
            self.请求数 += 1
            if not 响应.成功:
                self.失败数 += 1
