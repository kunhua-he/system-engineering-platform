...
class 流式请求冲突(ValueError):
    """流式请求id 与既有通道冲突（同键重复到达）。

    **带公开错误码的专用异常**：上层按「异常类型 + 公开错误码」判定状态码（409），
    不解析异常文案 —— 文案改字（含中文化）时结论必须不变（P1-6）。
    """
        ...
        class 处理器(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:
                ...
                try:
                    ...
                    # 抛出点是 流式请求冲突（错误码 幂等键冲突）；文案改字不影响结论。
