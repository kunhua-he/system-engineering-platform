...
# ── 职责块装配（导入即再导出，调用方零改动）──────────────────────────────
from 运行核心.统一网关.协议.错误说明表 import 公开错误说明表  # noqa: F401
from 运行核心.统一网关.运行态.幂等存储 import 幂等存储, 幂等默认保留秒  # noqa: F401
from 运行核心.统一网关.协议.网关信封 import 网关请求, 网关响应  # noqa: F401
from 运行核心.统一网关.协议.调用点缺参判定 import 操作不存在错误, _是调用点缺参错误  # noqa: F401
from 运行核心.统一网关.协议.参数别名表 import 能力参数别名表, _应用参数别名  # noqa: F401
from 运行核心.统一网关.协议.操作协议表 import 允许操作表, 操作权限表  # noqa: F401
from 运行核心.统一网关.协议.幂等协议常量 import 幂等库文件名, 纯查询操作, 幂等响应字段  # noqa: F401
from 运行核心.统一网关.协议.类型规格 import (  # noqa: F401 —— 2026-09-18 拆出时的再导出，本行原样保留
    类型短名映射, 类型匹配表, 数值类型名, _类型表自检, 校验能力参数,
)
from 运行核心.统一网关.核心处理.网关幂等面 import 幂等记忆面
from 运行核心.统一网关.核心处理.网关参数校验面 import 参数校验面
from 运行核心.统一网关.核心处理.网关执行面 import 执行面
from 运行核心.统一网关.核心处理.网关处理面 import 请求处理面
