...
class 网关核心(幂等记忆面, 参数校验面, 执行面, 请求处理面):
    ...
    def __init__(self, 后端核心: Any = None, 任务系统: Any = None,
                 ...
                 *,
                 ...
                 幂等保留秒: float = 幂等默认保留秒,
                 参数账实例: 调用参数账 | None = None) -> None:
        self.后端核心 = 后端核心
        self.任务系统 = 任务系统
        self.限流器 = 限流器实例 or 限流器()
        self.审计 = 审计实例 or 安全审计()
        # 调用参数账（入参按形状自增计数，供「常用参数组合」排序返回）。
        # 与审计**分账**：审计是逐条证据、参数账是聚合计数；两者的量级与用途都不同
        # （入参可以是几十 KB 的补丁文本，塞进审计会让只读证据膨胀）。
        # 口径/表结构/落点唯一实现见 `公共契约.诊断.调用账本`。
        self.参数账 = 参数账实例 or 调用参数账()
        self.请求数 = 0
        self.失败数 = 0
