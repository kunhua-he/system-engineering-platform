...
class 安全配置:
    ...
    要求凭证: bool = 真
    # ★ 阶段一（危险能力授权门）：含「管理员」⇒ 行为不变（`操作协议表.能力权限表`
    # 里的危险档位照旧放行，现有调用方一个都不受影响）。阶段二：从本集合删掉「管理员」，
    # 调用方就必须显式声明才能调危险能力（一行之差，可随时回滚）。
    默认权限范围: set[str] = field(default_factory=lambda: {"查询", "调用", "任务", "管理员"})
...
def 异常说明(错误: BaseException, 前缀: str) -> str:
    """统一的异常说明：`前缀: 异常类型: 脱敏消息`（缺陷 #157，2026-09-20）。

    为什么放在本模块（而非调用点）：网关多处 `except Exception:` 原先只落固定文案
    （如「网关处理失败」），把编程 bug（`TypeError`/`AttributeError`）与真实运行故障
    一并归为「内部错误」，且**丢弃异常类型与消息** —— 事后无法定位到底哪里坏了。
    本模块是安全的公共边界（已被各传输面导入、且已有 `脱敏错误信息`），
    放这里可被所有调用点复用**且不产生循环导入**（曾放 `本地网关` 导致
    `本地网关 ↔ 网关边界面` 循环导入，实测报 partially initialized module）。

    保证三件事：
      ① 异常类型必留（编程 bug 一眼可辨）；
      ② 消息经 `脱敏错误信息` 处理（不把请求体里的凭证泄进错误响应与审计）；
      ③ 截断到有界长度（异常消息本身不得成为新的资源风险）。
    """
    类型名 = type(错误).__name__
    try:
        消息 = 脱敏错误信息(str(错误))[:200]
    except Exception:  # 脱敏自身失败也不能让错误路径再抛
        消息 = ""
    return f"{前缀}: {类型名}: {消息}" if 消息 else f"{前缀}: {类型名}"


def 脱敏参数(参数: dict[str, Any]) -> dict[str, Any]:
    """入参的**参数级脱敏**：命中敏感键名的键，其值不可逆替换为 `已脱敏`。

    返回的是**副本**，调用方拿到的原 `参数` 一字不改 —— 真实值仍照常派发给能力，
    本函数只服务「要落进只读证据/持久账本的入参副本」这一类消费点。

    ## 敏感键名判定（2026-09-23 收口）
    判定走唯一腿 `公共契约.基础类型.字段名册.是敏感键名`（忽略大小写、含子串即命中）。
    收口前本函数收调用方自带的 `敏感键表` 做**全等**匹配，且**全仓零调用点**
    （「有实现零调用 + 判定表自备」那一类的旁证；原债务行已随 2026-09-23 收口销账删除）
    ——「有实现零调用」+「判定表自备」本身就是缺陷，
    两条一并修掉：判定表不再由调用方传，统一由唯一腿给。

    非敏感键的值仍走 `脱敏值`（按值特征掩掉 `sk-`/`gh*_` 等密钥片段、超长文本只留摘要），
    故「键名不敏感但值是密钥」这一形态也有兜底。
    """
    return {键: ("已脱敏" if 是敏感键名(键) else 脱敏值(值, 键))
