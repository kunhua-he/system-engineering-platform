...
文件注释）：`/网关/流式` 的实体是 `运行核心.统一网关.传输.流式HTTP.HTTP流式管理器`；
...
from 运行核心.统一网关.安全.有界服务器 import 有界线程HTTP服务器  # 网关域唯一有界实现（429 结构化拒绝），勿改用 公共契约.运行时.有界HTTP 那份（满载不回响应）
    ...
    def _处理流式网关请求(self) -> None:
        ...
        try:
            ...
            def 事件生成函数(_停止事件):
                ...
                # ★ 本特例的裁决与到期条件见 开发文档/决策记录/0056（2026-09-20，销 #178）：
                #   契约侧尚无「可流式返回」正式类型，改走注册表需先做架构级改造，故登记在案
                #   接受该单腿特例；**一旦契约具备可流式类型，本进口立即改走注册表**，不得并存。
            ...
            from 运行核心.统一网关.传输.流式HTTP import 流式请求冲突
