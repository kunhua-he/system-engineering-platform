...
def 发现全部(
    ...
    """发现全部支持库、模块与技能库，检查包 id 与能力 id 唯一性。

    技能库根目录（顶层 `技能库/`）为可选第三根：技能库包的类型是「支持库」，
    单独成根只是让它在目录上与支持库/模块库平级；该根不存在时行为与原来完全一致。
    """
    结果 = 发现结果()
    声明列表 = 扫描目录(支持库根目录, "支持库")
    if 技能库根目录 is not None:
        声明列表 = 声明列表 + 扫描目录(技能库根目录, "支持库")
    声明列表 = 声明列表 + 扫描目录(模块根目录, "模块")
