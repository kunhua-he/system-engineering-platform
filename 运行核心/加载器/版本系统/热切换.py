...
class 热切换管理器:
    ...
    # 观察期的指标来源**只有一个**：`灰度指标.py` 的 `灰度指标库`（落运行库、可持久化、
    # 与 `真实热切换` 第 7 步同一份实现）。本类不再保留进程内的第二套观测入口 ——
    # 原 `记录灰度观测` + `灰度指标表`（全仓 0 调用方、只写进程内字典、不落盘）
    # 已于 2026-09-20（待办 #70）删除，理由是「同一目的不保留第二套实现」（哲学 1.2）：
    #   * `真实热切换` 第 7 步用的是 `指标库.观测(...)`（持久化），不是进程内表；
    #   * 进程内表**没有任何写入者**，其 超过阈值() 对零样本恒返回空列表，
    #     而 `热切换` 第 9 步曾把它直接读成「指标正常」—— 恒真结论（未接线却报正常）。
    #     该恒真已被 `样本充足()` 门槛挡住；现在把喂数据的死入口一并删掉，
    #     避免后人误以为「再调一次记录灰度观测 就能让指标活起来」。
    # 要判观察期是否正常：问 `灰度指标库.聚合(...)` 拿样本量，样本不足一律「未做结论」。
