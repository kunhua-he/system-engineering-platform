...
class 安全审计:
    ...
    def 记录(self, *, 操作: str, 用户id: str = "", 项目id: str = "",
        ...
        记录 = {
            ...
            # ★ 2026-09-24（批R·R-23）返回体量维：**只记字符数，绝不记正文** ——
            # 记正文＝审计文件自己就爆，且只读证据里躺业务数据。取数走 `返回体量报告()`。
            "返回字符数": max(0, int(返回字符数 or 0)),
    ...
    def 返回体量报告(self, *, 最大记录数: int = 默认JSONL读取上限记录,
                     上限: int = 20, 大返回字符数: int = 20000) -> dict[str, Any]:
        """从**权威审计账本**派生「哪些能力返回过大」——与 `慢调用报告` 同源、同形制。

        华哥 2026-09-24（批R·R-22）：「我的目的是知道哪一个速度比较慢，哪一些是返回过于大，
        并不是实际需要的……方便优化底座而已」。速度维已由 `耗时毫秒` 覆盖（`慢调用报告`）；
        体量维此前**无权威落点**（`MCP与能力网关升级总方案` 的 F1 第二步未做），故在此补上：
        `记录()` 逐条写 `返回字符数`，本方法只**派生**，不另存第二份（哲学 1.2）。

        **只统计字符数，一个字正文都不进账本**：否则审计文件自己就爆，且只读证据里会躺业务数据。
        `大返回字符数` 是「判大」的分级线（默认 20,000 字符），只用于筛选与排序、**不是裁决** ——
        「该不该瘦」由调用方按薄壳 `返回上限字符` / `值裁剪` 现场判。
        """
        阈值 = max(0, int(大返回字符数))
        记录表 = self.查询(最大记录数=最大记录数)
        大条目: list[dict[str, Any]] = []
        for 记录 in 记录表:
            try:
                字符数 = int(记录.get("返回字符数") or 0)
        ...
        for 条目 in 大条目:
            键 = 条目["能力id"]
            项 = 聚合.setdefault(键, {"能力id": 键, "条数": 0,
                                     "最大字符数": 0, "合计字符数": 0})
            项["条数"] += 1
            项["合计字符数"] += 条目["返回字符数"]
            项["最大字符数"] = max(项["最大字符数"], 条目["返回字符数"])
        聚合表 = sorted(聚合.values(), key=lambda 项: (-项["最大字符数"], 项["能力id"]))
        for 项 in 聚合表:
            项["平均字符数"] = (round(项["合计字符数"] / 项["条数"], 1)
                                if 项["条数"] else 0.0)
            del 项["合计字符数"]
        最大明细 = sorted(大条目, key=lambda 条目: (-条目["返回字符数"], 条目["能力id"]))
        return {
            "阈值字符数": 阈值,
            "读取记录数": len(记录表),
            "大返回条数": len(大条目),
            "按能力聚合": 聚合表[:max(1, int(上限))],
            "最大明细": 最大明细[:max(1, int(上限))],
            "说明": ("来自权威审计账本的 `返回字符数`（逐条，只记字符数不记正文）；"
                    "阈值只作筛选，裁决按薄壳 `返回上限字符`/`值裁剪` 现场判。"),
        }
