...
def _项目根检查(参数: dict) -> tuple[bool, str]:
    ...
    if not 传入:
        return False, ("未传 项目根：本工具只服务系统工程平台仓库。"
                       "请传仓库绝对路径（含 AGENTS.md 与 开发文档/项目说明.md）后再调用；"
                       "非本仓库会话不要调用本工具，也不要据此载入平台上下文。")
    根 = Path(传入).expanduser()
    if not 根.is_dir():
        return False, f"项目根不是目录：{传入}。本工具只服务系统工程平台仓库，已拒绝（未载入任何平台上下文）。"
    if not ((根 / "AGENTS.md").is_file() and (根 / "开发文档" / "项目说明.md").is_file()):
        return False, (f"项目根缺少平台锚文件（AGENTS.md 与 开发文档/项目说明.md）：{根}。"
...
def _查询能力(参数: dict) -> dict[str, Any]:
    ...
    通过, 原因 = _项目根检查(参数)
    if not 通过:
        return {"成功": False, "错误码": "非本平台项目根", "错误说明": 原因}
    关键词 = str(参数.get("关键词", "") or "").strip()
    if not 关键词:
        return {"成功": False, "错误码": "参数不合法", "错误说明": "关键词必填"}
    try:
        限制 = int(参数.get("限制", 20))
    except (TypeError, ValueError):
        return {"成功": False, "错误码": "参数不合法", "错误说明": "限制必须是整数"}
...
def _调用能力(参数: dict) -> dict[str, Any]:
    ...
    if not 能力id:
        return {"成功": False, "错误码": "参数不合法", "错误说明": "能力id必填"}
    入参 = 参数.get("参数")
    if 入参 is None:
        入参 = {}
    if not isinstance(入参, dict):
        return {"成功": False, "错误码": "参数不合法", "错误说明": "参数必须是对象"}
...
async def 调用工具(名称: str, 参数: dict[str, Any]) -> list:
    ...
    处理 = 分发表.get(协议名)
    if 处理 is None:
        数据: dict[str, Any] = {"成功": False, "错误码": "未知工具",
