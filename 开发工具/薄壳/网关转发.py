...
#:
#: ★ **`调用方超时秒` 只对同步腿注入**（2026-09-24 批R R-25 收口）：该字段的语义是
#:   「调用方愿意**等**多久」。异步腿（`任务提交`/`任务查询`/`任务取消`）调用方**不等**
#:   （提交立即回 `任务id`），后台任务的寿命由**能力契约的 `超时秒`** 有界
#:   （如 `自修复工具.静态编译` 缺省 600 秒），不归连接寿命管。
#:   不分家的实测后果（R-19）：网关把 120 当任务预算传给 `任务系统.提交(超时秒=…)`
#:   ⇒ 大模块（30 文件）编译**跑到 120 秒被硬杀**，读数取不到，多会话互相撞死。
#:   谁要改这条：别把「薄壳等多久」重新当成「任务能跑多久」—— 那是两个不同主体的寿命。
同步腿操作 = frozenset({"调用能力"})
...
def 转发(请求体: dict) -> dict:
    ...
    **注入范围只限同步腿**（2026-09-24 批R R-25）：判据是模块常量 `同步腿操作`。
    异步腿调用方不等，后台任务寿命归能力契约的 `超时秒` 管（理由见 `同步腿操作`）。

    收发实现走 `发送()`（全平台唯一一条 HTTP 腿）；凭证缺失的 fail-closed 留在这里：
    薄壳是受控接入面，凭证缺失时必须**明确失败且不发起转发**。
    """
    载荷 = dict(请求体)
    if str(载荷.get("操作") or "调用能力") in 同步腿操作:
