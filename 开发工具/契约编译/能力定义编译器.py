...
**本表只是简表**；「生成物 → 权威生成器 → 比对粒度」的**唯一权威表**在
`开发工具/契约编译/生成物一致性检查.py:17-27`（另含 `说明/使用说明.md`、`依赖锁.json`，
以及 8 份「无生成器、只登记机器不可重算」的契约件）。改本表时**同批**改那张。
★ 2026-09-22（批D D1）实测更正：原审计件写「该清单被复制 4 处」**不准确** ——
`生成物一致性检查.py:301-304` 与 `漂移判定.py:439-442` 是 **import 块**（用到哪个就导哪个），
**不是**清单副本；真正的清单只有本简表 + 上面那张权威表两处。
...
# 历史短名归一表**不在本文件**（2026-09-24 批R R-1 收口）：唯一真源是
# `公共契约/基础类型/历史短名.py::历史短名到正式名`，本文件只转调
# （原 `易语言类型别名` 10 键是同一判据的第二份表，已删）。
...
def 生成完整性摘要(定义: dict[str, Any], 包id: str, 实现摘要: str,
                包目录: Path) -> str:
    """生成 完整性摘要.json 的文本 —— **只委托唯一生成器**（形状唯一，序列化也唯一）。

    历史：此处曾追加 能力数/能力清单/实现摘要/生成时间，造成与唯一生成器形状不一致；
    现统一由 `支持库/后端/组件规范支持库/实现/完整性摘要.py::生成完整性摘要` 产出全部字段。

    ★ 2026-09-22 修真缺陷：本函数此前自己 `json.dumps(..., indent=1)`（且不加尾换行），
    而本文件**也在写**同一个 `完整性摘要.json` 的另一条腿（`开发工具.全量重算摘要`）用
    `indent=2` + 尾换行 ⇒ 同一产物两种字节。危害：跑一次能力定义编译就把该包的摘要刷成
    与已提交内容不符的字节（产生与本次改动无关的 diff），而摘要门禁只解析后逐键比、
    对字节差异完全不可见（漂移静默）。现改为调 `全量重算摘要.完整性摘要文本`，
    **序列化口径全仓只剩一处**。
    """
    from 开发工具.全量重算摘要 import 完整性摘要文本

    return 完整性摘要文本(包目录, 包id=包id, 版本=str(定义.get("版本", "1.0.0")))
