...
def _注解可接受(形状: str, 期望集: set[str]) -> bool:
    ...
    ① **中文别名**：实现写 `文本`（历史短名，唯一短名表 `历史短名到正式名` 已定义
       `文本 → 文本型`），判据未走同一短名表 → 归一后再比。
    ...
    **不放过真差异**：集合是**白名单**，只有明确等价才放行；
    ...
    if not 文本:
        return 假
    # ① 中文别名：走唯一短名表（唯一源，不另列）——2026-09-24 批R R-1 起真源是
    # `公共契约/基础类型/历史短名.py::历史短名到正式名`（编译器侧已改为转调它，
    # 此处直接指向真源，不再绕经 `能力定义编译器`）
    try:
        from 公共契约.基础类型.历史短名 import 历史短名到正式名
        文本 = 历史短名到正式名.get(文本, 文本)
        ...
        # 短名表是唯一源，取不到就必须响亮失败，不能拿一份陈旧副本兜着。
        raise RuntimeError(
            f"取 历史短名到正式名 失败（唯一源：公共契约.基础类型.历史短名）：{错误}"
        ) from 错误
    候选: set[str] = {文本}
    # ② 联合类型：逐分量展开
    for 段 in 文本.split("|"):
...
def 检测说明书一致(契约: dict[str, Any], 说明书文件: Path) -> str | None:
    ...
    **判据唯一真源（2026-09-24 批Q · Q-i 收口）**：本函数原先自持一份**全文子串**判据
    ——「版本号 / 参数名 / 错误码前 3 个是否出现在说明书任意位置」。它粗到会漏：参数写错
    类型/必填/默认值、返回类型写错、错误码只写了第 4 个之后，它一条都拦不住（同一判据的
    第二份实现，哲学 1.3 结果唯一即收口）。现**本函数不持有任何判据**，只做适配：转调
    `开发工具/公开调用完整性门禁_说明书解析.py::检查说明书字段`（那份按说明书结构化载位
    逐能力对账 版本/参数/必填/默认值/错误码/返回），把包级违规清单收敛成**本能力**那一条。

    包目录按真源的唯一约定 `说明书相对路径`（`说明/使用说明.md`）反推；路径不符约定或包
    声明缺失/不可读时**不猜、不静默放过**，如实回报「不可判」。包级违规（能力id 为 `*`）
    不并入本函数：那是包级根因，按「同一根因只计一条」由门禁的包级环负责。
    """
    from 开发工具.公开调用完整性门禁_说明书解析 import 检查说明书字段, 说明书相对路径

    if not 说明书文件.is_file():
        return f"说明书缺失: {说明书文件}"
    能力id = str(契约.get("能力id") or "")
    if not 能力id:
        # 无能力id 就没有「本能力那一条」可比（全仓枚举侧同样跳过），不猜、不出条目。
        return None
    包目录 = 说明书文件
    for _ in 说明书相对路径:
        包目录 = 包目录.parent
    声明文件 = 包目录 / "包声明.json"
    if not 声明文件.is_file():
        return f"说明书不可判（缺包声明.json）: {说明书文件}"
    try:
        声明 = json.loads(声明文件.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as 错误:
        return f"说明书不可判（包声明不可读）: {说明书文件}: {错误}"
    命中 = [条 for 条 in 检查说明书字段(包目录, 声明)
            if 条.get("能力id") == 能力id]
    if not 命中:
        return None
    首条 = 命中[0]
    详情 = f"（{首条['详情']}）" if 首条.get("详情") else ""
    return (f"说明书与入口不一致: {首条['缺口类型']} {能力id}{详情}"
            f"（本能力共 {len(命中)} 条）")
