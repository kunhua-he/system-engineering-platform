...
# 存量基线「分桶 + 收敛」的唯一节点（零自举中立件）：本处原先自写循环（形状正确），
# 与 `开发工具/公开调用完整性门禁_检查与判定.应用存量基线` 是同职责的第二份实现——
# 那一份漏了「遍历基线独有桶」，收敛恒为 0（2026-09-24 收口为一份）。
...
def _应用全面漂移基线(条目列表: list[dict],
    ...
    # 分桶消费收成唯一节点（`开发工具/存量基线消费.分桶与收敛`）：本处原先自写循环
    # （形状正确），与 `开发工具/公开调用完整性门禁_检查与判定.应用存量基线` 是同职责的
    # 第二份实现 —— 那一份漏了「遍历基线独有桶」，收敛恒为 0（2026-09-24 收口为一份）。
    # 本处只提供**桶键构造**（包|缺口类型，不做路径归一），判定口径不再自持第二份。
    return 分桶与收敛(条目列表, 上限表, lambda 条: f"{条['包']}|{条['缺口类型']}")
...
def _枚举正式包目录与契约(系统根: Path, 顶层顺序: tuple[str, ...]
                       ) -> list[tuple[Path, list[dict[str, Any]]]]:
    """枚举正式包目录及其契约条目：**包清单转调唯一腿**
    （`公共契约/正式根.py::枚举包目录`）。

    为什么转调（2026-09-23）：原实现逐根 `glob("**/能力契约/参数契约.json")` 再自己
    `json.loads` 读契约，与唯一腿是同一个「读契约」动作的两条路（哲学 1.3「结果唯一
    即收口」）。`公共契约/正式根.py` 是「正式根下哪些目录算包声明面」的唯一事实源
    （哲学 12.1），明文规定任何调用方要一份包清单都必须走 `枚举包目录`。本函数只做
    两件事：按 `顶层顺序` 保持原枚举次序、只留确有 `能力契约/参数契约.json` 的包
    （与逐条 `sorted(glob)` 的文件面逐字一致）。

    **为什么不调 `模块库/能力目录.读取能力`**（2026-09-23 按 MCP `能力详情` 核过真实
    契约；缺口如实登记于此，不另造第二条腿）：它的入参只有 `能力id`（必填、单次一条，
    无批量/无全量枚举）、项目根由 `__file__` 现场解析、返回是「一条能力的契约」，
    不含本扫描要的 `包目录`／`实现目录`／`说明/使用说明.md` 路径与逐包相对路径；
    它的发现面还排除 `支持库.适配层.*`（本仓 21 包／74 条），而本扫描要全量。
    故「包清单」收口到唯一腿，契约条目仍按原语义就地读（缺文件/非法一律空表）。

    实测等价（2026-09-23，本仓）：三根下包集合与逐条 `sorted(glob)` 完全一致
    （125 个包），故 `包数/能力数/缺口计数/只报计数` 逐字不变。
    """
    from 公共契约.正式根 import 扫描口径, 枚举包目录

    全部包目录 = 枚举包目录(系统根, 扫描口径(根名=顶层顺序, 含聚合视图包=真))
    表: list[tuple[Path, list[dict[str, Any]]]] = []
    for 顶层 in 顶层顺序:
        根目录 = 系统根 / 顶层
        if not 根目录.is_dir():
            continue
        for 包目录 in 全部包目录:
            if not 包目录.is_relative_to(根目录):
                continue
            契约文件 = 包目录 / "能力契约" / "参数契约.json"
            if not 契约文件.is_file():
                continue
            try:
                条目表 = (json.loads(契约文件.read_text(encoding="utf-8"))
                        .get("能力契约") or [])
...
def 全面漂移扫描(系统根: Path, *,
    ...
    声明面枚举**转调唯一腿**（`公共契约/正式根.py::枚举包目录`，哲学 12.1）：原实现逐根
    `glob("**/能力契约/参数契约.json")` 再自己 `json.loads`，与唯一腿是同一个「读契约」
    动作的两条路（哲学 1.3「结果唯一即收口」）。枚举与契约条目读取口径由
    `_枚举正式包目录与契约` 一处承担（缺口与理由见该函数 docstring）。
    ...
    只报列表: list[dict] = []
    包数 = 能力数 = 0
    for 包目录, 契约条目表 in _枚举正式包目录与契约(系统根, ("支持库", "模块库", "技能库")):
...
def 生成全面漂移基线(系统根: Path) -> dict[str, Any]:
    ...
    条目列表: list[dict] = []
    包数 = 能力数 = 0
    # 与 `全面漂移扫描` 同一条腿：枚举 + 读契约一律转调 `_枚举正式包目录与契约`
    # （包清单唯一腿 = `公共契约/正式根.py::枚举包目录`），本处不再自己 glob/读盘。
    for 包目录, 契约条目表 in _枚举正式包目录与契约(系统根, ("支持库", "模块库", "技能库")):
        包相对路径 = (str(包目录.relative_to(系统根)) if 包目录.is_relative_to(系统根)
                    else str(包目录))
        包数 += 1
