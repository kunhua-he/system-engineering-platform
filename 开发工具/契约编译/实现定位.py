...
顶层目录名 = 正式根名表
"""正式包根名表（唯一事实源 `公共契约/正式根.正式根名表`）：点分模块名首段 → 文件路径的解析起点。

原为本地自写三根 `("支持库", "模块库", "技能库")` —— 会漏掉 `平台控制面`/`运行核心`/
`启动监督器` 这些同样放着正式包的顶层目录（同类「三根白名单」缺陷已由
`开发工具/契约编译/口径比对.py::_枚举全仓正式包目录` 的 `#201` 实测证实），
且属「同一件事两套实现」；改由唯一事实源出清单。
"""
内部实现文件名前缀 = ("子进程",)
...
def 模块替换标记(文件: Path) -> dict[str, Any] | None:
    ...
    ```
    （2026-09-23 批 3 R1：兜底载入块已从 30 份垫片收进 `公共契约/运行时/导入前缀.载入唯一实现`，
    垫片只留一行调用；那句 `sys.modules[__name__] =` 收口仍留在垫片里 —— 别把它也搬进共享函数，
    搬了垫片就不再被本判据认出来。）
