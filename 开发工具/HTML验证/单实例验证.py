...
def 验证全部(
    ...
    if 直连地址:
        ...
        if 端口 == 动态端口:
            try:
                ...
                # #168：原先硬编码「已回收: True」—— 启动助手确实调了回收，但
                # `_回收进程组` 返回的 `已回收` 可能是 False（进程组残留 / 未死透）。
                # 现读它回传的真实结论；拿不到（旧版助手 / 异常不可挂属性）才如实标未知。
                回收 = getattr(错误, "启动回收结果", None)
                报告.资源回收 = (dict(回收, 原因="启动助手已回收") if isinstance(回收, dict)
                                 else {"已回收": None, "原因": "启动助手已回收但未回传回收结论"})
                return 报告, None, None
        else:
            可用, 消息 = _检查端口可用(端口)
            if not 可用:
            ...
            try:
                ...
                回收 = getattr(错误, "启动回收结果", None)   # #168：同分支1，如实回传
                报告.资源回收 = (dict(回收, 原因="启动助手已回收") if isinstance(回收, dict)
                                 else {"已回收": None, "原因": "启动助手已回收但未回传回收结论"})
                return 报告, None, None
    ...
    if isinstance(场景列表, 验证场景束):
        ...
        if 直连地址:
            # #168：#168 核验口径 —— 直连模式**从未启动制品进程**，没有进程资源可回收。
            # 原写法 `已回收: True` 会被读成「回收动作成功执行过」，属语义失真；
            # 改为如实声明「本模式无进程资源」，字段名保持兼容（下游读 已回收 仍得真）。
            场景报告.资源回收 = {"已回收": True, "模式": "直连", "说明": "直连模式未启动制品进程，无进程资源"}
        return 场景报告, 实际端口, 进程
    ...
    报告.并发峰值 = 峰值
    if 直连地址:
        # #168：同上，直连模式无进程资源（但读取侧 `已回收` 语义保持"无残留"=真）
        报告.资源回收 = {"已回收": True, "模式": "直连", "说明": "直连模式未启动制品进程，无进程资源"}
        报告.制品摘要后 = _制品全文件摘要(制品目录)
        _校验制品前后绑定(报告)
    return 报告, 实际端口, 进程
