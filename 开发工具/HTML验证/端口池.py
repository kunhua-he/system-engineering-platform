...
def _场景资源键(场景: 多步骤验证场景) -> tuple[str, ...]:
    ...
    if any(能力.startswith(("开工编排.", "平台控制面.能力目录.")) for 能力 in 能力表):
        键表.add("文件租约")
    # 提供者注册表场景独占：`登记提供者` 写的是**进程内注册表**（不是磁盘状态），
    # `调用提供者` / `查询提供者` / `卸载提供者` 必须落在**同一个制品实例**上才看得见。
    # 默认多实例并发跑时，登记落在实例 A、查询落在实例 B ⇒ `已登记=False` + 状态码 404
    # （2026-09-18 实测：进程内串行三步全成功，HTTP 并发报 404）。
    if any(能力.startswith("平台控制面.提供者.注册表.") for 能力 in 能力表):
        键表.add("提供者注册表")
    # 语音转写类场景独占：`转写支持库.转写.*` / `媒体转写.*` / `直播逐字稿.*` 都要加载
    # **同一份 MLXWhisper 模型权重（large-v3，约 3GB）** 到各自进程；默认多实例并发跑时
    # 多份同时加载互抢资源，实测表现是「统一返回缺字段 ['值','请求id','耗时毫秒']」——
    # **现象像返回契约缺陷，真因是并发资源竞争**（2026-09-18 实测：单跑该能力 2.9 秒成功、
    # 并发 1 跑全量验证器退出码 0；并发 8 固定报 8 项红）。
    # 加资源键后同键场景固定同实例并按键串行。
    if any(能力.startswith(("转写支持库.转写.", "媒体转写.", "直播逐字稿.")) for 能力 in 能力表):
        键表.add("语音转写模型")
