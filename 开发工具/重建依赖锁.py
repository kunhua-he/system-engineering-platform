...
语义边界（只刷新平台指纹，不做依赖升级）
    ...
    **就地重算**那些登记了该锁的摘要（`刷新摘要`）：重算只走唯一序列化腿
    `开发工具.全量重算摘要.完整性摘要文本`，扫描面仍限于「登记了本锁的摘要」，不扫别的包。
...
def 刷新摘要(锁路径: Path, 旧sha: str, 新sha: str) -> tuple[list[str], list[str]]:
    """重算登记了该锁的那些完整性摘要（序列化走唯一权威，故整包摘要同轮闭合）。

    与旧写的差别（2026-09-24 批O O-5）：旧写是「只改那一条 sha256」+ 自写 `json.dumps`
    公式（且**无尾换行**，与权威口径 `indent=2` + 尾换行不同 ⇒ 跑一次就把该包摘要刷成
    与已提交内容不符的字节）；现在改为按 `完整性摘要文本` 整包重算 ⇒ 顺带闭合该包里
    其它已漂移的条目。这是**同一口径**的必然结果：序列化只有一条腿，就不能再只补单个字段。
    """
    已刷新: list[str] = []
    未刷新: list[str] = []
    根 = 系统根()
    for 摘要路径 in 枚举摘要文件():
        ...
        # `json.dumps` 公式（2026-09-24 批O O-5）。包id/版本按**唯一权威的同一取舍**从
        # 包声明.json 取（缺项回落 提供者id／目录名、1.0.0），保证重算字节与 `全量重算摘要` 一致。
        from 开发工具.全量重算摘要 import 完整性摘要文本
        声明路径 = 摘要路径.parent / "包声明.json"
        try:
        ...
        try:
            新文本 = 完整性摘要文本(
                摘要路径.parent,
                包id=str(声明.get("包id") or 声明.get("提供者id") or 摘要路径.parent.name),
                版本=str(声明.get("版本") or "1.0.0"))
        except (OSError, ValueError) as 错误:
            # 重算不出来（包已删／包内无正式文件等）如实登记为未刷新，不静默放过
            未刷新.append(f"{摘要路径.relative_to(根)}: 摘要重算失败（{type(错误).__name__}: {错误}）")
            continue
        if 新文本.strip() == 原文本.strip():
            未刷新.append(f"{摘要路径.relative_to(根)}: 摘要需改但重算后文本无差异（异常）")
            continue
