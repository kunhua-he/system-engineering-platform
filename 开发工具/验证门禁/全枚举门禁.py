...
#: 本检查器在 `验证门禁/存量基线.json` 里的段名（与兄弟检查器同形）。
检查器名 = "全枚举门禁"
系统根 = Path(__file__).resolve().parents[2]
#: 扫描面 = 全部源码层，唯一事实源 `公共契约/正式根.源码层名表`（`.py` 搜集口径）。
#: 原先自写 8 层名单（缺 支持库/项目适配层/开发文档/测试中心/示例项目/运维脚本）——
#: 正是本门禁要防的「同一件事两套实现」；改由唯一事实源出清单，不再本地另列。
扫描层名表 = 源码层名表
...
def 构建结论(_根: Path) -> 检查结论:
    """`门禁公共.跑标准主流程` 要的结论构建函数（本检查器的判据面 = `检查()`）。

    **为什么改成这个形状**（2026-09-24 批P 收口）：本文件原来自建 `主函数`，直接
    `return 1 if 结论.判定红 else 0` —— 它**不消费** `验证门禁/存量基线.json`（该文件
    只有 4 段，没有本门禁的段），**不走** `门禁公共` 的 `跑标准主流程` / `写基线段`。
    后果：其余四个检查器都有「债务冻结 / 只减不增 / 可用门禁自己的出口重建基线」这套
    机制，唯独本腿缺位 —— 一旦真有命中，既没有存量基线可冻结，也没法走统一出口重建。
    这正是哲学 1.2「同一件事两套实现即缺陷」+ 13.3 四环链缺第 ④ 环（门禁）的形状。
    """
    return 检查()
