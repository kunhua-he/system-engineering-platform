...
用法：
    ...
    python3.14 开发工具/验证场景体检.py --并发 8               # 体检线程池并发数（默认 32；只影响速度，判据不变）
"""
...
def 主() -> int:
    ...
    解析.add_argument("--并发", type=int, default=32,
                    help="体检线程池并发数（默认 32，与发布门禁冻结口径同值；"
                         "只影响扫描速度，判据与结果不变 —— 显式可配，不再写死）")
    参数 = 解析.parse_args()
    根 = Path(参数.根).resolve()
    场景加载 = _载入判据(根)
