...
def 检查停止入口(目录: Path) -> list[str]:
    ...
    if 契约路径.is_file():
        ...
        释放策略 = 契约.get("释放策略")
        释放文本 = str(释放策略 or "")
        # 【2026-09-24 批O·O-10 清死条目】原表第 6 词 `"无跨调用状态"` 已删：
        # 现场实测 50 份带 `释放策略` 文本的契约里，该词**命中 0 份** ⇒ 永远匹配不到
        # （同表其余 5 词各命中 11-44 份），留着只会让人以为「无跨调用状态」这种写法
        # 被认过。它若真该算释放证据，正确做法是补进判据的**行为面**而非词表（另议）。
        释放证据词 = ("finally", "关闭", "释放", "终止", "回收")
