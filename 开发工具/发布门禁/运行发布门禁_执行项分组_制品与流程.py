...
def _检查制品与结构(检查, *, 包目录: Path | None, 制品目录: Path | None
    ...
    if 包目录列表:
        ...
        for 当前包目录 in 包目录列表:
            ...
            try:
                ...
                声明问题.append(str(当前包目录.relative_to(系统根)))
        检查("包结构完整", not 结构问题,
             f"校验 {len(包目录列表)} 包；结构问题: {结构问题 or '无'}")
        检查("包声明合法", not 声明问题,
             f"校验 {len(包目录列表)} 包；声明问题: {声明问题 or '无'}")
    ...
    try:
        ...
        依赖结果 = 审计依赖(系统根)
        依赖详情 = "无" if 依赖结果.成功 else "；".join(审计结果转清单(依赖结果))
        检查("依赖防火墙", 依赖结果.成功,
    ...
    try:
        ...
        公开调用详情 = "无" if not 公开调用违规 else "；".join(
            f"{项.get('缺口类型')}:{项.get('包')}:{项.get('能力id')}" for 项 in 公开调用违规)
...
def _检查环境残留(检查) -> None:
    ...
    try:
        ...
        检查("工程缓存无Python源码", not 缓存源码表,
             f"工程缓存源码文件数: {len(缓存源码表)}（应为0）{缓存源码表}")
    except Exception as 错误:
        检查("工程缓存无Python源码", False, f"异常: {错误}")


    # 【2026-09-24 批O·O-10：原第 24 项「旧过渡目录不存在」已删除 —— 恒真条目，零鉴别力】
    # 原判据 = 「24 个旧过渡顶层目录名在系统根下是否还存在」（`残留表` 非空即红）。
    # **现场实测这 24 个名字在本树里全部不存在** ⇒ `残留表` 恒为空 ⇒ 该行**恒真**、
    # 检不出任何东西（本条注释在 2026-09-18 就写明了这件事，只是一直没人处置）。
    # 附注两点，避免误读：
    #   ① 留着一个恒真的检查项，比没有更坏 —— 它让人以为「旧顶层目录复活」有人管。
    #   ② 「反向破坏」一词曾只以**包名清单字符串**出现在本项里，**不是调用**
    #      （哲学总账 1.4 实测结论：字面 ≠ 调用）；真正的调用接线在「反向破坏（空转即杀）」
    #      门禁项（见 `校验反向破坏`），删除本项**不影响**那条接线。
    # 若将来要恢复这类约束，正确形状是**新判据**「系统根下出现不在 正式源码目录名表 里的
    # 顶层目录」（`运行发布门禁_源码边界与待检清单.py::正式源码目录名表` 是现成真源），
    # 那需要配反向试验与存量基线，不属于「清死条目」。

    # 26 测试伪装门禁接入（2026-09-17 修 B2）：全仓唯一能识别「测试自证」的门禁。
    # 口径「只报告 + 硬限制不得增加」——存量违约（取值见 `测试伪装门禁.存量违约上限`）不因历史债打死发版，但**新增即红**，
...
def 校验测试伪装(根: Path, *, 存量上限: int | None = None) -> tuple[bool, str]:
    ...
    于是「测试是不是真的在测」对发版完全不可见。但它的存量违约不为零
    （取值见 `测试伪装门禁.存量违约上限`），直接按「零违约才通过」接进来会当场
    把发版打死。因此接入口径取「只报告 + 硬限制不得增加」：

    - 判据是 `计数 = 违规数 + 清单豁免数 <= 存量违约上限`（默认取门禁自己的
      `存量违约上限`，**只降不升**，当前值见 `开发工具/测试伪装门禁.py`）；
