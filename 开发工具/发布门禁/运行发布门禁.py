...
def 校验第三方访问声明(制品目录: Path) -> tuple[bool, str]:
    ...
    if 提供者数 == 0 and not 问题表:
        ...
        支持库根 = Path(制品目录) / "支持库"
        if not 支持库根.is_dir():
            # 制品整体不含 支持库/：本项对本制品为空跑，必须写明“没东西可查”，
            # 否则日志里的“无需第三方访问声明”会被读成“已核验且无问题”。
            return True, f"制品不含 支持库 目录（{支持库根.relative_to(制品目录)} 不存在）；本项声明核验对本制品为空"
        外置锁数 = len([路径 for 路径 in 支持库根.rglob("依赖锁.json")
                       if 适配层根 not in 路径.parents])
        return True, (f"支持库/适配层 下未发现第三方提供者（标准提供者 0、依赖锁 0）；"
