...
组件·契约·资源·平台行为类检查。**不含「测试全部通过」**：全仓 unittest 从不
在本门禁执行，`开发工具/测试体系门禁.py` 与 `开发工具/测试伪装门禁.py` 也不在
本门禁调用链内（二者需单独手工执行）。故本门禁判「通过」只覆盖上列检查项，
不构成任何测试用例已执行的证据。
...
空集不能出具“通过/无需声明”这类看上去已验证的说法（哲学第 1 条 4 项：空转即杀）：
...
def 主函数(argv: list[str] | None = None) -> int:
    ...
    解析器 = argparse.ArgumentParser(
        ...
        epilog="判定口径（华哥 2026-08-29 裁决）：编译产物 + HTML GET/POST 并发黑盒"
               "（HTML黑盒验证／HTML契约真实执行矩阵／待验证制品字节不变）+ 语法编译；"
               "本门禁不执行全量 unittest（测试体系门禁／测试伪装门禁需单独手工执行）。",
    )
