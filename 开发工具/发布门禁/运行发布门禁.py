...
_错误码说明表变量 = "公开错误说明表"
_视图登记源 = ("测试中心", "平台控制面", "测试_视图错误码键一致.py")
_视图登记变量表 = ("英文中文对应", "无中文对应的英文键")
_视图表源 = (
...
def 校验英文错误码防回潮(根: Path | None = None) -> tuple[bool | None, str]:
    ...
    判据分两面，两面共用**既有实现**，不重写：
      ...
      ② 视图面 —— ``开发工具/统一能力入口/视图`` 三张「错误码 → 建议」表里的英文键，
         必须在测试侧登记的**向后兼容英文键**清单内（登记真源 = ``测试中心/平台控制面/
    ...
    登记英文键: set[str] = set()
    for 变量 in _视图登记变量表:
        值 = _读模块字面量(扫描根.joinpath(*_视图登记源), 变量)
        if isinstance(值, dict):
            登记英文键 |= {str(键) for 键 in 值}
        elif isinstance(值, (list, tuple, set, frozenset)):
            登记英文键 |= {str(项) for 项 in 值}
    视图英文键: list[str] = []
    for 路径段 in _视图表源:
        表 = _读模块字面量(扫描根.joinpath(*路径段[:4]), 路径段[4])
        if not isinstance(表, dict):
            continue
        for 键 in 表:
            面计数 += 1
...
def 执行门禁(*, 包目录: Path | None = None, 制品目录: Path | None = None, 运行测试: bool = True,
    ...
    # `检测能力定义漂移`（测试中心.测试_漂移门禁 在跑）只比「能力定义 ↔ 派生物」，
