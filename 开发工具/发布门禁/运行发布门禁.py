...
错误码·占位代称·能力调用图）/提供者可启动可停止/最小能力调用成功/热切换成功/自动回滚成功/失败日志
...
组件·契约·资源·平台行为类检查。**仍不含「测试全部通过」**：全仓 unittest 不在
本门禁执行，`开发工具/测试体系门禁.py`（含其 2026-09-17 新增的「真跑」项）
仍需单独手工执行。故本门禁判「通过」只覆盖上列检查项，不构成任何测试用例
已执行的证据。
...
且恒真真对照通过。**实测耗时 0.37 秒**（装配已暖；独立冷跑 2.6–3.1 秒），
占整轮门禁（实测 111–132 秒）约 0.3%，故取全量常驻、不抽样、不删项。
...
防回潮阻断配置: dict[str, dict[str, Any]] = {
    ...
    # 哲学 5.1 的判据此前只存在于单测 `测试中心/开发工具/测试_契约版本唯一.py`，
...
def 执行门禁(*, 包目录: Path | None = None, 制品目录: Path | None = None, 运行测试: bool = True,
             ...
             测试并行数: int = 32) -> 门禁结果:
    """执行全部发布门禁检查（口径见模块 docstring：不含全量 unittest）。"""
    结果 = 门禁结果()
    门禁项列表 = 结果.门禁项列表
...
def 主函数(argv: list[str] | None = None) -> int:
    ...
    解析器 = argparse.ArgumentParser(
        ...
        epilog="判定口径（华哥 2026-08-29 裁决）：编译产物 + HTML GET/POST 并发黑盒"
               ...
               "它含 2026-09-17 新增的「真跑」项）。测试伪装门禁已接入，"
