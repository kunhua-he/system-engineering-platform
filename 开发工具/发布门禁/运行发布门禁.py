...
完整性摘要一致/第三方权限声明/中文边界/防回潮三项（裸布尔·非正式类型名·英文
错误码）/提供者可启动可停止/最小能力调用成功/热切换成功/自动回滚成功/失败日志
...
def 执行门禁(*, 包目录: Path | None = None, 制品目录: Path | None = None, 运行测试: bool = True,
    ...
    # 27 防回潮四项（2026-09-18 接线，B-10）：裸布尔 / 非正式类型名 / 英文错误码 / 占位代称。
    # 为什么接在这里：三项判据此前只存在于文档与别的工具里，**不在必经路径上**——
