...
def 按位置推系统根(起始路径: Path) -> Path:
    """从 `起始路径` 向上找「同时含 `平台控制面/` 与 `开发工具/`」的祖先目录。

    **抽成函数（2026-09-22，批D D3）**：原为模块级内联循环，无法单独验证
    「根是按自身位置推的、不是硬编码」。测试改为直接调本函数并传隔离路径 ⇒
    真实验证推导逻辑，而不是在同一进程里加载隔离副本（那条路走不通：`sys.modules`
    已缓存真模块，隔离副本里的 `from …_底座 import 系统根` 仍解析到真仓库）。
    找不到匹配祖先时**原样返回起始路径**（不回落硬编码路径）。
    """
    for _祖先 in 起始路径.parents:
        if (_祖先 / "平台控制面").is_dir() and (_祖先 / "开发工具").is_dir():
            return _祖先
    return 起始路径


系统根 = 按位置推系统根(Path(__file__).resolve())
