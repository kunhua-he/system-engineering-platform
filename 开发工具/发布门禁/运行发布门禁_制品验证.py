...
def _读取契约能力(制品目录: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    声明面枚举**转调唯一腿**：`公共契约/正式根.py::枚举包目录`（哲学 12.1
    「正式根下有哪些包」的唯一实现 —— 该文件明文规定：任何调用方要一份包清单都
    必须走它，不许再写 `rglob("包声明.json")` + 自定过滤）。原实现自己
    `rglob("能力契约/参数契约.json")` 扫声明面，与唯一腿是同一个「读契约」动作的
    两条路（哲学 1.3「结果唯一即收口」）。

    **为什么不是 `模块库/能力目录.读取能力`**（2026-09-23 按 MCP `能力详情` 逐字
    核过它的真实契约；缺口如实登记于此，不另造第二条腿）：
    ① 它的入参只有 `能力id` 一个（必填、单次一条，无批量、无「全量枚举」入口）
       ⇒ 给不出「制品里有哪些包」这份清单；
    ② 它的项目根由 `__file__` 现场解析（永远指仓库，指不到 `制品目录`）
       ⇒ 读不了制品的契约面；
    ③ 它的返回是「一条能力的契约」（参数/返回结构/错误码/…），不含本门禁要的
       `包目录`／`实现目录`／`说明/使用说明.md` 路径与逐包相对路径；
    ④ 它的发现面排除 `支持库.适配层.*`（本仓 21 包／74 条），而本门禁要的是制品
       **全量**正式能力（实测 142 包／752 条，含适配层）。
    故本处只把「包清单」这一件事收口到唯一腿；契约字段仍由唯一聚合契约解析器
    （`解析聚合契约`）读，字段口径与判定结论逐字不变。

    制品根与代码根不同（代码在 `制品目录/平台客户端/`，`运行入口/`、`前端/`
    在 `制品目录` 根），故先经 `_定位制品层根` 取层根再枚举。
    """
    ...
    已见: set[str] = set()
    层根 = _定位制品层根(制品目录)
    for 包目录 in (枚举包目录(层根, 全仓口径) if 层根 is not None else []):
