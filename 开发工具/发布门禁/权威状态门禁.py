...
def _陈旧写者拒绝() -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="门禁_陈旧写者_") as 临时目录:
        ...
        状态.初始化资源("资源", {"值": 0})
        成功1, _, 令牌1 = 状态.获取锁(
            "资源", 事务id="事务1", 进程身份键="进程1", 项目id="门禁", 所有者="门禁")
        状态.释放锁("资源", 事务id="事务1", 进程身份键="进程1", 令牌=令牌1)
        成功2, _, 令牌2 = 状态.获取锁(
            "资源", 事务id="事务2", 进程身份键="进程2", 项目id="门禁", 所有者="门禁")
        旧成功, 旧说明 = 状态.提交资源(
            资源id="资源", 期望版本="0", 期望令牌=str(令牌1),
            事务id="事务1", 进程身份键="进程1", 新值={"值": 1}, 新摘要="摘要1")
        新成功, 新说明 = 状态.提交资源(
            资源id="资源", 期望版本="0", 期望令牌=str(令牌2),
            事务id="事务2", 进程身份键="进程2", 项目id="门禁", 所有者="门禁",
            新值={"值": 2}, 新摘要="摘要2")
        状态.关闭()
        if not (成功1 and 成功2 and 令牌2 > 令牌1 and not 旧成功 and 新成功):
            raise AssertionError(f"陈旧写者门禁异常: {旧说明} / {新说明}")
        return f"旧令牌 {令牌1} 被拒，新令牌 {令牌2} 成功"
