...
# 「显式零能力面」的**唯一节点**（2026-09-24 批R 收尾下沉）：`能力定义.json` 与
# `包声明.json` 的 能力 两侧同时为空才算。2026-09-25 件3 起，本门禁用它把
# **包级版本载位**的对账射程从「能力面非空」扩到「显式零能力面」—— 转调同一节点，
# 不另写第二套判据（哲学 1.2）。与生成腿 `MD文档生成/说明书/载位同步.py` 同口径。
from 开发工具.契约编译.聚合契约解析 import 包声明显式零能力  # noqa: E402
...
def 检查说明书字段(包目录: Path, 声明: dict) -> list[dict]:
    ...
    # ★ 2026-09-25 件3：**包级版本载位与能力面无关**，故这条对账挪到 `if not 期望表` **之前**。
    #   旧序（先 `if not 期望表: return []`）把「显式零能力面」与「聚合父包」的包级版本
    #   一起免检 ⇒ 实测 Git提供者 说明书 `- 版本：1.2.1` 对 包声明.json `"版本": "2.0.0"`，
    #   HEAD 上就躺红而**任何门禁都不报**（假绿：刷不动 + 刷错了也没人报）。
    #   射程只放行到「显式零能力面或能力面非空」：聚合父包（不持 能力定义.json）保持原状
    #   —— 它们是不是该有包级版本载位是另一件待定的事，不借本次顺手改判据（不加假红）。
    包级版本 = _说明书包级版本(头部行, 全文行)
    声明包版本 = 声明.get("版本")
    if (期望表 or 包声明显式零能力(包目录)) \
            and isinstance(声明包版本, str) and 声明包版本:
        if 包级版本 is None:
            _记("说明书-缺包级版本载位", "*")
        elif 包级版本 != 声明包版本:
