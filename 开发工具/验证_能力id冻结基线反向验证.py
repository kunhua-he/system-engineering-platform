...
def 造夹具根(父目录: Path) -> Path:
    夹具 = 父目录 / "夹具根"
    写包(夹具 / "支持库" / "后端" / "夹具包1", "支持库.后端.夹具包1", [
        {"能力id": 夹具能力一, "版本": "1.0.0",
         "参数": [{"名称": "输入", "类型": "文本型", "必填": 真}],
         "返回": {"类型": "结果型", "值结构": {"值": "文本"}},
         "错误码": ["参数不合法"]},
    ])
    写包(夹具 / "支持库" / "后端" / "夹具包2", "支持库.后端.夹具包2", [
        {"能力id": 夹具能力二, "版本": "1.0.0",
...
def main() -> int:
    ...
    try:
        ...
        # ---- ① 删一条基线里的 id → 必红 ----
        # 破坏点 = 真的把能力一从 夹具包1/能力定义.json 里删掉（等价于「改/删了 id」）。
        示例包1 = 夹具 / "支持库" / "后端" / "夹具包1"
        定义 = json.loads((示例包1 / "能力定义.json").read_text(encoding="utf-8"))
        assert any(条["能力id"] == 夹具能力一 for 条 in 定义["能力列表"]), "夹具里应有能力一"
        定义["能力列表"] = [条 for 条 in 定义["能力列表"] if 条["能力id"] != 夹具能力一]
        (示例包1 / "能力定义.json").write_text(json.dumps(定义, ensure_ascii=False, indent=1), encoding="utf-8")
        声明 = json.loads((示例包1 / "包声明.json").read_text(encoding="utf-8"))
        声明["能力"] = [条 for 条 in 声明.get("能力", []) if 条.get("能力id") != 夹具能力一]
        (示例包1 / "包声明.json").write_text(json.dumps(声明, ensure_ascii=False, indent=1), encoding="utf-8")
        码, 出 = 跑(夹具, 基线)
        ...
        定义["能力列表"] = [{"能力id": "夹具.包1.能力一改名", "版本": "1.0.0",
                          "参数": [], "返回": {"类型": "结果型", "值结构": {}}, "错误码": []}]
        (示例包1 / "能力定义.json").write_text(json.dumps(定义, ensure_ascii=False, indent=1), encoding="utf-8")
        声明["能力"] = [{"能力id": "夹具.包1.能力一改名", "名称": "能力一改名"}]
        (示例包1 / "包声明.json").write_text(json.dumps(声明, ensure_ascii=False, indent=1), encoding="utf-8")
        码, 出 = 跑(夹具, 基线)
