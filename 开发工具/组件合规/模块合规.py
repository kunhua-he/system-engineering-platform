...
from 运行核心.依赖防火墙 import 同包实现导入, 核心层表  # noqa: E402 —— 须在 系统根 入 sys.path 之后
...
# 核心层集合**不本地复刻**：唯一事实源 = `运行核心/依赖防火墙.py::核心层表`（逐元素相同）。
# 依赖方向核对：允许依赖表["开发工具"] 含 "运行核心"（开发工具 → 运行核心 合法，反向无引用）。
核心根目录名 = 核心层表
