    ...
    def _场景依赖(self) -> tuple[bool, str]:
        ...
        for 依赖 in 依赖列表:
            if not isinstance(依赖, dict):
                ...
                无法核验项.append(依赖)
        if 无法核验项:
            return 假, f"依赖项未声明 能力/包id/模块名，无从核验: {无法核验项}"
        问题列表: list[str] = []
        if 包id项:
    ...
    def _场景真实返回值(self) -> tuple[bool, str]:
        ...
        class _合规调用器:
            ...
            def 调用能力(self, 能力id: str, 参数: dict | None = None, **选项) -> Any:
                ...
                # 参数校验**转调唯一校验点**（与 HTTP 腿 `网关参数校验面._能力参数错误`、
                # 进程内唯一能力调用腿 `唯一能力调用.调用能力` 是**同一个函数、同一份声明**
                # `实现.参数`）——本脚手架不得另养第二套类型表/必填判据。
                #
                # 缺陷（2026-09-23 实测）：改前这里**直调** `实现.调用`（原第 689 行），
                # 绕过唯一校验点 ⇒ 缺必填参数时解释器在调用点抛 `TypeError`，**异常逸出**
                # （调用方连统一结果都拿不到）；而同一条入参在两条正式腿上都回 `参数不合法`。
                # 回归锁见 `测试中心/开发工具/测试_合规调用器参数校验.py`。
                ...
                if 实现 is None:
                    return 结果.失败("提供者不可用", f"能力未注册: {能力id}",
                                      来源="组件合规", 可重试=True)
                参数表 = 参数 or {}
                参数错误 = 校验能力参数(能力id, 实现.参数, 参数表)
                if 参数错误:
                    return 结果.失败("参数不合法", 参数错误, 来源="组件合规")
                返回值 = 实现.调用(**参数表)
