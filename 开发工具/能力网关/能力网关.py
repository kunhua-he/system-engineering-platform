...
class 能力网关请求处理器(BaseHTTPRequestHandler):
    ...
    def _处理搜索(self, 查询: dict) -> None:
        ...
        if not 1 <= 限制 <= 100:
            ...
            return
        # ★ 2026-09-23：本路由是「开发者拿一条就能用」的发现面，每条命中必须带 `参数`。
        # 唯一腿 `能力目录.搜索能力` 的默认档已收敛为「名称」（只回指针，P-9 前缀缓存取舍，
        # 见 `模块库/能力目录/说明/设计说明.md`），故这里**显式要最高档**（消费方兼容走法
        # 原话：「传 细节级别=完整契约」）。不显式要时本路由每条命中缺 `参数`，本路由的
        # 验证场景 `测试中心/开发工具/测试_能力网关.py::test_搜索能力返回结果` 当场判红。
        结果表 = 搜索能力(关键词=关键词, 限制=限制, 细节级别="完整契约")
