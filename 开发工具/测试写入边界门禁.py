...
| 一 写动作**未解析**（fail-closed） | 「解析不出」被当放行 ⇒ 判据一全盲（**实测 356 处未解析**，2026-09-23 复测） | `测试写入-写动作未解析` | 基线内 **271 键 / 356 处**（只减不增；2026-09-23 复测订正，原写 854/1151 与基线文件差 3 倍） |
| 二 只断言「成功」 | 未装配空转 ⇒ `成功=True` 而**什么都没测** | `测试断言-只断言成功` | 基线内 **17 条**（只减不增；2026-09-23 复测订正，原写 18） |
