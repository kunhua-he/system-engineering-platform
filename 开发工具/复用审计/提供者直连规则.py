...
**解析失败口径（2026-09-16 A-2 修复）**：源码无法 ast 解析时**产出「解析失败」
命中并恒判定为阻断**，与同仓 ``开发工具/复用审计/能力调用图审计.py`` 对同情形
记「解析失败」违规一致。原实现 ``except SyntaxError: return []`` 是 fail-open：
一份语法坏掉的提供者源码在门禁里等于「零直连」，比不检更危险（改坏源码反而放行）。
"""
...
# 解析失败是**保留模式**：不可登记、不可豁免，恒判定为阻断（A-2 口径统一）。
解析失败模式 = "解析失败"
...
class 直连规则注册表:
    ...
    def 登记规则(self, 提供者id: str, 直连模式: str, 规则: dict) -> None:
        ...
        if 直连模式 == 解析失败模式:
            raise ValueError(f"{解析失败模式}是保留模式，不可登记（否则可借登记绕过解析失败阻断）")
        self.已登记规则[直连模式] = {
            "提供者id": 提供者id,
            "模块": list(规则.get("模块", [])),
            "函数名": list(规则.get("函数名", [])),
    ...
    def 审计源码(self, 源码文本: str) -> list[dict]:
        ...
        返回命中列表，每项含 模式、模块或函数、行号；**源码无法解析时返回一条
        「解析失败」命中**（模式=解析失败模式，行号取异常行号），与
        ``能力调用图审计.py`` 对同情形记「解析失败」违规同口径。
        """
        try:
            树 = ast.parse(源码文本)
        except (SyntaxError, ValueError) as 错误:
            行号 = getattr(错误, "lineno", 0) or 0
            return [{"模式": 解析失败模式, "模块或函数": f"<{type(错误).__name__}>",
                     "行号": 行号, "详情": str(错误)}]
        特征表 = self._检测特征表()
        命中: list[dict] = []
    ...
    def 门禁判定(self, 源码文本: str) -> tuple[bool, list[dict]]:
        ...
        解析失败恒判定为阻断（不可登记、不可豁免）：解析失败意味着这份源码的
        直连情况**未知**，而不是「没有直连」。
        """
        未声明 = [命中 for 命中 in self.审计源码(源码文本)
                 if 命中["模式"] == 解析失败模式 or 命中["模式"] not in self.已登记规则]
        return (bool(未声明), 未声明)
