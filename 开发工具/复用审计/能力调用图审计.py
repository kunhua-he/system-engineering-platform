...
from 运行核心.依赖防火墙 import 第三方前缀表, 同包实现导入  # noqa: E402 —— 须在 系统根 入 sys.path 之后
...
#: `from urllib.parse import unquote`，被判「原子旁路-网络」，而该项门禁口径是
#: 「干净通过、存量 0、新增即红」⇒ **真阻断**（编译口定向测试因此判红）。
#:
...
审计剪枝目录表 = ("pycache", "__pycache__", "工程缓存", "测试中心", "示例项目", "开发文档")
