...
def 主流程(题数: int, 期望条数: int, 种子: int, 输出: str) -> int:
    ...
    # ★ 统一检索（RRF 融合）单独量一路：与单路用**同一批题、同一口径**，
    # 这样「融合到底有没有增益」是实测数字而不是感觉。融合较慢（每题的 2~3 秒），
    # 故只取前 8 题。
    融合题数 = min(8, len(能力题))
    if 融合题数:
        融合结果 = []
        for 题 in 能力题[:融合题数]:
            信封 = 调能力("检索支持库.统一检索.统一检索",
                          {"关键词": 题["查询"], "期望条数": 1}, 凭证)
            命中 = [str(项.get("指向", ""))
                    for 项 in ((信封.get("值") or {}).get("命中列表") or [])]
            排名 = 命中.index(题["标准答案"]) + 1 if 题["标准答案"] in 命中 else 0
            融合结果.append({"查询": 题["查询"][:40], "标准答案": 题["标准答案"],
                             "排名": 排名, "命中": 排名 > 0, "返回数": len(命中)})
        报告["路"].append(汇总("统一检索·RRF融合", 融合结果, 1))
