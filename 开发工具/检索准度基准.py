"""检索准度基准：用平台自身数据造「带标准答案」的查询，量各路检索的命中率与 Top1 率。
...
用法：
    python3.14 开发工具/检索准度基准.py              # 每路 30 题
    python3.14 开发工具/检索准度基准.py --题数 50
...
def 判能力题(题: dict, 凭证: str, 顶部: int) -> dict:
    信封 = 调能力("能力目录.搜索能力", {"关键词": 题["查询"], "限制": 顶部}, 凭证)
...
def 判符号题(题: dict, 凭证: str, 顶部: int) -> dict:
    信封 = 调能力("代码解析支持库.代码地图.查询节点",
...
def 汇总(名称: str, 结果表: list[dict], 顶部: int) -> dict:
    题数 = len(结果表)
    命中数 = sum(1 for 项 in 结果表 if 项["命中"])
    首位 = sum(1 for 项 in 结果表 if 项["排名"] == 1)
    倒数 = [1 / 项["排名"] for 项 in 结果表 if 项["排名"] > 0]
    return {"路": 名称, "题数": 题数, "顶部": 顶部,
            "命中率": round(命中数 / 题数, 4) if 题数 else 0.0,
            "Top1率": round(首位 / 题数, 4) if 题数 else 0.0,
            "MRR": round(statistics.fmean(倒数), 4) if 倒数 else 0.0}


def 主流程(题数: int, 顶部: int, 种子: int, 输出: str) -> int:
    凭证 = 取凭证()
    print(f"系统根 {系统根}\n代码地图 {代码地图}（存在={代码地图.is_file()}）\n"
          f"每路 {题数} 题 / 顶部 {顶部} / 种子 {种子}\n")
    报告: dict = {"题数": 题数, "顶部": 顶部, "种子": 种子, "路": []}

    能力题 = 造能力题(凭证, 题数, 种子)
    if 能力题:
        报告["路"].append(汇总("能力层·搜索能力",
                              [判能力题(题, 凭证, 顶部) for 题 in 能力题], 顶部))
    else:
    ...
    if 符号题:
        报告["路"].append(汇总("符号层·代码地图",
                              [判符号题(题, 凭证, 顶部) for 题 in 符号题], 顶部))
        报告["路"].append(汇总("文本层·正则搜索",
                              [判文本题(题, 凭证) for 题 in 符号题], 顶部))
    else:
        print("⚠ 符号层造题失败（代码地图无节点），如实跳过，不伪造数字")

    print(f"{'路':<18}{'题数':>6}{'命中率':>9}{'Top1率':>9}{'MRR':>8}")
    for 行 in 报告["路"]:
        print(f"{行['路']:<18}{行['题数']:>6}{行['命中率']:>9}{行['Top1率']:>9}{行['MRR']:>8}")
    if 输出:
