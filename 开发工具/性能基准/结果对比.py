...
def 对比结果(基线: dict[str, Any], 当前: dict[str, Any], *,
    ...
    for 键 in sorted(set(基线表) & set(当前表)):
        基线条目, 当前条目 = 基线表[键], 当前表[键]
        明细.append({
            "场景": 键[0], "测量项": 键[1],
            "基线p50毫秒": 基线条目.get("p50毫秒"), "当前p50毫秒": 当前条目.get("p50毫秒"),
            "p50变化率": _变化率(基线条目.get("p50毫秒"), 当前条目.get("p50毫秒")),
            "基线p99毫秒": 基线条目.get("p99毫秒"), "当前p99毫秒": 当前条目.get("p99毫秒"),
            "p99变化率": _变化率(基线条目.get("p99毫秒"), 当前条目.get("p99毫秒")),
            "基线每秒次数": 基线条目.get("每秒次数"), "当前每秒次数": 当前条目.get("每秒次数"),
            "每秒次数变化率": _变化率(基线条目.get("每秒次数"), 当前条目.get("每秒次数")),
        })
    return {
        "基线文件": 基线文件,
        "基线生成时间": 基线.get("生成时间"),
