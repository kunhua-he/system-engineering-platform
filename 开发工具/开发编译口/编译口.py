...
def 主函数() -> int:
    ...
    for 门禁 in 门禁清单:
        ...
        if 门禁["模块"] == "开发工具.全量重算摘要" and 摘要已核对:
            # 阶段乙 已同一口径跑过（--只报 与写入版内容一致，只差落盘）：
            # 结论已按乙的退出码归并，此处不再跑第二遍（修前版本同轮重复跑两遍）。
            print(f"  [{角色标注}] {门禁['名字']} 已在阶段乙 核对，跳过（避免同轮重复执行）")
            continue
        退出码, 输出, 耗时 = _跑(
    ...
    # 阶段丁（自动修复）属审计件 §8.7 第二批，本版未实现。
    # ★ 必须显式声明「跳过」：静默从丙跳到戊会让读者以为漏跑了一个阶段（P3）。
    print("── 阶段丁 生成物自动修复（本版未实现，跳过；见审计件 §8.7 第二批）──")
