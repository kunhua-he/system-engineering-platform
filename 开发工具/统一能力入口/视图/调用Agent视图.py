...
class 调用Agent视图:
    ...
    def 搜索能力(self, *, 令牌: str, 关键词: str = "", 限制: int = 10) -> dict[str, Any]:
        ...
        if not 结果.get("成功"):
            # 产出者 `统一入口._搜索能力` 恒返回成功；失败只可能来自 执行操作 的三处返回点
            # （未知操作/授权拒绝/内部错误），均带非空中文错误码，故此处兜底值对齐
            #「对外错误只走中文」（决策 0003），不再回落英文码。
            失败码 = 结果.get("错误码") or "内部错误"
            return self._统一错误(错误码=失败码, 消息=错误说明表.get(失败码, "搜索失败"))
        摘要表 = []
