...
class 查询入口:
    ...
    def 搜索能力(self, *, 关键词: str = "") -> 查询结果:
        """搜索能力：转调唯一检索能力 `能力目录.搜索能力`（唯一实现腿 = 模块库.能力目录）。

        本类不再自行发现包声明：检索规则的唯一来源是已注册能力 `能力目录.搜索能力`。
        旧 `声明列表` 入参随收口删除——检索范围固定为项目根下全部公开能力声明，
        不再接受调用方传入一份可能与唯一能力腿口径不同的列表（避免第二套口径）。
        """
        from 开发工具.能力搜索.能力搜索器 import 搜索带原因

        结果列表, 原因 = 搜索带原因(关键词=关键词)
        if 原因:
