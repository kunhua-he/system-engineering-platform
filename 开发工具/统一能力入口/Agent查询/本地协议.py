...
def _子结果失败(子结果: Any) -> tuple[bool, str, str]:
    """从子结果提取 (是否失败, 错误码, 错误说明)。

    协议层要同时认两种子结果：查询入口的 查询结果(成功/数据/说明)，
    以及各控制操作返回的原始字典（成功 / 错误码 两套键）。不认就会把
    子操作失败当成本次操作「完成」静默返回（退出码 0）。
    """
    if isinstance(子结果, dict):
        成功 = 子结果.get("成功")
...
class 本地协议服务器:
    ...
    def 处理(self, 请求: dict) -> 协议响应:
        ...
        try:
            if 操作 == "搜索包":
                子结果 = self.入口.搜索能力(关键词=参数.get("关键词", ""))
                响应.结果, 响应.错误说明 = 子结果.数据, 子结果.说明
            elif 操作 == "查看版本":
                子结果 = self.入口.查看包版本(参数.get("包id", ""))
                响应.结果 = 子结果.数据
            elif 操作 == "查看能力详情":
                子结果 = self.入口.查看能力版本(参数.get("能力id", ""))
                响应.结果 = 子结果.数据 if 子结果.成功 else None
            elif 操作 == "查看激活版本":
                子结果 = self.入口.查看能力版本(参数.get("能力id", ""))
                响应.结果 = 子结果.数据.get("当前激活版本") if 子结果.成功 else None
            elif 操作 == "查看回滚版本":
                子结果 = self.入口.查看能力版本(参数.get("能力id", ""))
                响应.结果 = 子结果.数据.get("可回滚版本") if 子结果.成功 else None
            elif 操作 == "查询失败记录":
                子结果 = self.入口.查询失败记录(包id=参数.get("包id", ""), 错误码=参数.get("错误码", ""))
                响应.结果, 响应.错误说明 = 子结果.数据, 子结果.说明
            elif 操作 == "查看诊断结论":
                子结果 = self.入口.查看诊断详情(参数.get("记录id", ""))
                响应.结果, 响应.错误说明 = 子结果.数据, 子结果.说明
            elif 操作 == "执行诊断复现":
                子结果 = self.执行复现(参数)
                响应.结果 = 子结果
            elif 操作 == "执行升级前检查":
                子结果 = self.入口.执行升级前检查(参数.get("能力id", ""), 参数.get("新版本", ""))
                响应.结果, 响应.错误说明 = 子结果.数据, 子结果.说明
            elif 操作 == "执行热切换":
                子结果 = self.执行热切换(参数)
                响应.结果 = 子结果
                ...
                响应.错误码 = "未实现"
                响应.错误说明 = f"操作未实现: {操作}"
            if 响应.状态 != "失败" and 子结果 is not None:
                失败, 错误码, 错误说明 = _子结果失败(子结果)
                if 失败:
                    响应.状态 = "失败"
                    响应.错误码 = 响应.错误码 or 错误码
                    响应.错误说明 = 响应.错误说明 or 错误说明
