"""备份恢复演练的核对引擎（B7）：副本构建、三类数据一致性核对、反向验证。
...
def 核对一致性(源根: Path, 目标根: Path) -> dict:
    """按备份契约三类数据逐类核对（权威状态/证据账本/包仓库）；空集不判绿。"""
    差异: list[str] = []
    类结果: list[dict] = []
...
def 建源副本(生产根: Path, 副本根: Path, 制品夹具: Path) -> dict:
    ...
    **生产根已由调用方改为「逐类权威宿主」建成的合并副本**（2026-09-20 修 #63）：
    三类数据本来就不共用一个目录——状态/证据在 `平台控制面/`、包仓库在
    `制品仓库/`。旧版把 `解析运行数据根()` 当成同一个生产根去找三类，于是
    「包仓库缺 `制品/`」这条**报告**其实是**取错根**的症状，
    不是生产真的不产出（真实制品仓库实测 15633 个文件、活状态库证据 54 条）。

    本函数仍按「三类各就各位」逐类取：`生产根` 是调用方已经合并好的副本目录，
    其内三类的相对位置与夹具形态一致（`权威状态.db` / `制品/`）。
    某类在合并副本里确实没有时，才补**真实文件夹具**并逐类标注来源——
    让报告能分清「恢复了生产真数据」与「恢复的是演练夹具」，不标注等于把夹具伪装成生产数据。
    """
    ...
    if (生产根 / "制品").is_dir():
        ...
        来源["包仓库"] = f"演练夹具（真实文件副本：{制品夹具.name}）"
    return 来源
