...
def 建源副本(生产根: Path, 副本根: Path, 制品夹具: Path) -> dict:
    ...
    **生产根已由调用方改为「逐类权威宿主」建成的合并副本**（2026-09-20 修 #63）：
    四类数据本来就不共用一个目录——状态/证据/锁在 `平台控制面/`、包仓库在
    `制品仓库/`。旧版把 `解析运行数据根()` 当成同一个生产根去找四类，于是
    「包仓库缺 `制品/`、项目锁缺 `项目锁.json`」这条**报告**其实是**取错根**的症状，
    不是生产真的不产出（真实制品仓库实测 15633 个文件、活状态库证据 54 条）。

    本函数仍按「四类各就各位」逐类取：`生产根` 是调用方已经合并好的副本目录，
    其内四类的相对位置与夹具形态一致（`权威状态.db` / `制品/` / `项目锁.json`）。
    ...
    if (生产根 / "项目锁.json").is_file():
        ...
        来源["项目锁"] = "演练自建（生产侧无 项目锁.json 产出方，见债务 #63）"
    return 来源
