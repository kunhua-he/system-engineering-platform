...
from 开发工具.公开调用完整性门禁_说明书解析 import (
    说明书相对路径, 表头行, 标题行表, 版本行, 参数列表行, 必填参数行, 错误码行,
...
from 开发工具.公开调用完整性门禁_检查与判定 import (
    ...
    内部层页面缺口类型, 内部层可绑定集缺口类型, 收集内部层能力, 页面引用能力id集,
    页面可绑定能力集, 检查内部层可绑定,
)
...
def 检查错误码登记(根: Path) -> list[dict]:
    ...
    违规: list[dict] = []
    # ★早退口径（2026-09-20 修 #182）：两张表读不到时**只跳过依赖它们的前两条判据**
    #   （声明码 ⊆ 状态映射码、状态映射码 == 说明表码），**第④条实现侧判据恒执行**。
    #   原实现 `return []` 整体早退 —— 实现侧判据只依赖 `实现/**.py` 与扫到的码、
    #   根本不需要两张表，实测后果是单测夹具（根非仓库根）下实现侧三条判据
    #   **一行都没执行**、3 个用例恒空，判据在测试面上等于不存在（正是本门禁自己在
    #   「扫描面为空必须判红」处要防的那种自证恒绿）。真仓库路径下两表可读，生产侧未见异常。
    两表可读 = isinstance(状态映射, dict) and isinstance(说明表, dict)
    if not 两表可读 and 根.resolve() == 仓库根.resolve():
        # 真仓库读不到 = 源文件被移走/改名（fail-closed 出条目，不许静默放过）。
        if not isinstance(状态映射, dict):
            违规.append(_错误码条目("-", "错误码-状态映射源不可读"))
        if not isinstance(说明表, dict):
            违规.append(_错误码条目("-", "错误码-错误说明源不可读"))
    # 非仓库根（临时夹具）如实跳过前两条，继续走实现侧判据。
    if 两表可读:
...
def 检查门禁原始项(根: Path) -> list[dict]:
    ...
    违规.extend(检查错误码登记(根))
    # 内部层收口「第二面」闸（2026-09-24 批R·R-9）：第一面（包声明 `内部层=真`）早就是
    # 机器可读字段，第二面（页面/可绑定集不再引用该 `能力id`）此前没有任何机器闸 ——
    # 撤了一面、漏了另一面无人发现。判据本体在 `公开调用完整性门禁_检查与判定`。
    违规.extend(检查内部层可绑定(根))
    return 违规
