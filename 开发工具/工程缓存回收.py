...
清理工程缓存中的无回收残留：
  ...
  5. 边界保护：禁止触碰 提供者运行环境/、制品仓库/、发布证据/、核心快照/、
     ...
     唯一例外（债务 #142）：制品仓库里的 `当前.json.统一schema前_*` 一次性迁移备份，
     按「留最近 3 份」窄口径清理（全仓无引用，见 `清理制品迁移残留`）。
...
#: 「统一 schema 前」迁移备份的保留份数（债务 #142）：最近 N 份留作可回溯，
#: 更早的按回收器策略清掉 —— 策略本身是「留最近 N 份」，不是「永不清理」。
制品迁移残留保留份数 = 3
#: 迁移备份的文件名标记：`当前.json.统一schema前_<时间戳>`。
制品迁移残留标记 = ".统一schema前_"
...
def 清理制品迁移残留(试运行: bool, 统计: 清理统计) -> None:
    """制品仓库里的「统一 schema 前」迁移备份：按「留最近 N 份」策略清理（债务 #142）。

    为什么单列一条、且要绕过 `边界保护目录`：制品仓库整体在边界保护内（防误删活跃
    制品与激活指针），但 `当前.json.统一schema前_<时间戳>` 是 `平台控制面/包仓库/
    激活指针面.py` 在把旧格式 `当前.json` 归一前留的**一次性回溯备份**，全仓无任何
    代码读它（只有精确名 `当前.json` 被读）——实测积压 36 份，而通用 `删除()` 因
    边界保护一律拒绝，`工程缓存回收` 因此恒报「无需清理」。

    本条是**窄口径例外**：只认 `当前.json` 开头且含 `.统一schema前_` 的文件、
    只删超出保留份数的那些（按文件名时间戳倒序保留最近 `制品迁移残留保留份数` 份），
    每一份仍逐条打印路径；`--试运行` 只打印不删。
    """
    制品根 = 工程缓存根 / "制品仓库"
    if not 制品根.is_dir():
        return
    for 制品目录 in sorted(路径 for 路径 in 制品根.rglob("*") if 路径.is_dir()):
        try:
            残留表 = sorted(
                (项 for 项 in 制品目录.iterdir()
                 if 项.is_file() and 项.name.startswith("当前.json")
                 and 制品迁移残留标记 in 项.name),
                key=lambda 项: 项.name,
            )
        except OSError:
            continue
        if len(残留表) <= 制品迁移残留保留份数:
            continue
        待删 = 残留表[:-制品迁移残留保留份数]
        print(f"制品迁移残留：{制品目录} 共 {len(残留表)} 份，"
              f"保留最近 {制品迁移残留保留份数} 份（可回溯），待清 {len(待删)} 份")
        for 路径 in 待删:
            _删除制品残留(路径, 试运行, 统计)


def _删除制品残留(路径: Path, 试运行: bool, 统计: 清理统计) -> None:
    """窄口径删除：仅供「统一 schema 前」迁移备份使用（文件名模式已在上层限定）。"""
    大小 = 路径大小(路径)
    if 试运行:
        print(f"[试运行] 将删除 {路径}（{格式大小(大小)}）")
    else:
