...
def 构建索引(系统根: Path) -> dict[str, Any]:
    ...
    # 其余正式根（平台控制面/运行核心 等）按「支持库」口径并入同一索引，
    # 不再要求调用方逐个加根；空根（无包声明）不产生任何变化。
    for 根目录名 in 存在根名(系统根):
        if 根目录名 in ("支持库", "技能库", "模块库"):
            continue
        正式, 排除 = _扫描包(系统根, 根目录名)
        支持库.update(正式)
        支持库排除.update(排除)
    ...
    # 页面可绑定集（2026-09-24 批R·R-20，审计 T8 的生成侧过滤）：`能力所有者` 是**全量 owner**
    # —— 调用通道、依赖闭包（`项目编译器.能力归属`）与制品装配（`HTML验证/制品事实`）都要它，
    # 内部层能力必须留在里面，否则那些消费方会「找不到能力所属包」；页面绑定只看**可见性**，
    # 故本键 = `能力所有者` 去掉 `包声明.json` 为 `内部层=真` 的包所拥有的能力。
    # 口径唯一源 = `公共契约/包声明/声明.解析包声明`（`内部层=bool(数据.get(...))`），
    # 与 `公开调用完整性门禁.收集内部层能力` 同读法（不自持第二份判据）。
    页面可绑定能力: dict[str, str] = {}
    能力排除: dict[str, str] = {}
    能力冲突: dict[str, list[str]] = {}
    for 包id, (包路径, 声明) in sorted(所有包.items()):
        是内部层 = bool(声明.get("内部层", 假))
        能力表 = 声明.get("能力", [])
        if not isinstance(能力表, list):
            raise ValueError(f"包 {包id} 的能力必须是列表")
        # 聚合父包只作视图，不占用子包 owner；真实叶子包即使没有
        # 能力定义.json（如前端描述包）仍必须进入唯一可调用索引。
        # 判据与运行时发现器一致：只有「无能力定义.json 且含子包」的目录
        # 才是聚合视图；自带 能力定义.json 的包即便含子包（如 代码解析
        # 支持库 含 语法索引）仍是真实能力 owner，其能力必须登记。
        ...
        # 正式根清单是唯一事实源（公共契约.正式根）：新增正式层
        # （平台控制面/运行核心）无需改这里；命令行模块根回退为 模块库。
        类型目录 = "模块库"
        for 根目录名 in 存在根名(系统根):
            if 包路径.is_relative_to(系统根 / 根目录名):
                类型目录 = 根目录名
                break
        ...
        for 能力 in 能力表:
            ...
            if not 是内部层:
                页面可绑定能力[能力id] = 依赖能力所有者[能力id]
    for 包id, 原因 in {**支持库排除, **技能库排除, **模块库排除}.items():
        # 排除包的能力只用于给显式引用提供清晰阻断原因，绝不进入 owner。
        for 根 in (系统根 / "支持库", 系统根 / "技能库", 系统根 / "模块库"):
...
def 包所属根(包id: str) -> str:
    """包所属正式根：模块库 / 支持库，无法识别返回空串。

    技能库是本平台的第三根正式包根，与支持库、模块库平级，包声明类型同为
    「支持库」（见 开发文档/项目说明.md），且 构建索引 已把它并进 索引["支持库"]。
    故归类必须走这一个入口：此前 解析依赖闭包/项目编译器/快速编译 各自按前缀
    分支，技能库包被漏在分支外——闭包报「依赖包类型非法」、快速编译则两集合
    皆空导致依赖闭包检查空跑。
    """
    ...
    if 包id.startswith("模块库."):
        return "模块库"
    if 包id.startswith("支持库.") or 包id.startswith("技能库."):
        return "支持库"
    return ""
    ...
    *,
    ...
    def 加入(包id: str, 来源: str) -> None:
        ...
        if 所属根 == "模块库":
            ...
            raise ValueError(f"依赖包所属根无法识别: {包id}")
        访问栈.append(包id)
        声明 = 全部包[包id][1]
        直接依赖 = 声明.get("依赖", []) or []
...
__all__ = ["构建索引", "正式包表", "能力所有者表", "排除包表", "包所属根", "校验显式包引用", "校验能力引用", "解析依赖闭包"]
