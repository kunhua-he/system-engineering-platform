...
from 公共契约.正式根 import 源码层名表  # noqa: E402
...
#: `支持库/适配层/` 下**必须随制品复制**的目录（运行核心/启动闭包直接引用它们）。
#: #96 包化后追加 `系统探针提供者`（根下平铺件 系统探针.py 现在转发到它）与
#: `模型服务提供者`（根下平铺件 模型服务.py 现在转发到它）—— 不复制会让平铺件在
#: 制品里 `ModuleNotFoundError`，启动即崩。
适配层固定复制目录 = ("提供者注册表", "密码签名提供者", "数据库适配器", "HTTP服务适配器",
                    "本地进程适配器", "动态库适配器", "配置契约",
                    "系统探针提供者", "模型服务提供者")
#: 只复制 `__init__.py` + `实现/` 的目录：它们是**启动闭包内部依赖**，不是制品公开
#: Provider —— 留下 包声明.json/依赖锁.json 会让独立项目的包发现把运行时内部件当成
#: 可装配支持库、误生成环境缓存（口径同 `密码签名提供者`，见下方 `_复制目录` 处注释）。
适配层只复制实现目录 = {"密码签名提供者", "系统探针提供者", "模型服务提供者"}
依赖锁文件名 = "依赖锁.json"
编译器版本 = "1.4.0"
# 关键源码目录**不本地复刻**：唯一事实源 = `公共契约/正式根.py::源码层名表`（全部源码层）
# ∪ {".git"}（生成式/隐藏目录，见 `生成式目录表`；此处单列只为拒绝把输出写进 .git）。
# 原手抄 15 项缺 `技能库`/`运维脚本`（二者都在 `源码层名表` 里）—— 属第二事实源。
# 注：**不得并入 `生成式目录表` 整表** —— 其中 `工程缓存` 是本函数显式放行的输出根
# （见 `校验输出目录` 的「源码树内只允许工程缓存子目录」），并进来会把它自己判红。
关键源码目录 = set(源码层名表) | {".git"}
...
def 读取工作区字节指纹() -> dict[str, str]:
    """公开唯一工作区字节指纹入口。"""
    结果 = 计算工作区字节指纹(系统根)
    return {键: str(结果[键]) for 键 in ("提交", "工作区字节指纹", "工作区状态")}
...
def 编译项目(
    ...
    for 目录名 in 适配层固定复制目录:
        ...
        # #96 后 系统探针提供者 / 模型服务提供者 同口径：它们只被根下平铺件
        # （系统探针.py / 模型服务.py）在启动闭包内转发使用。
        if 目录名 in 适配层只复制实现目录:
    ...
    for 包id in sorted(选中支持库):
        ...
        源, _ = 支持库表[包id]; _复制目录(源, 输出目录 / 源.relative_to(系统根))
    for 包id in sorted(选中模块):
