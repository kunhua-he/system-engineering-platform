...
def _读取能力目录() -> set[str]:
    """读取**页面可绑定**能力集：页面绑定只能引用已登记、且非 `内部层=真` 的唯一能力。

    2026-09-24 批R·R-20（审计 T8 生成侧过滤）：改读
    `正式包索引.构建索引(...)["页面可绑定能力"]` —— 同一份索引里过滤掉 `包声明.json`
    为 `内部层=真` 的包所拥有的能力（内部层＝装配可用、网关 0 暴露、页面不可绑）；
    `能力所有者` 仍是全量 owner，调用通道与依赖闭包不受影响。
    """
    return set(构建索引(根目录)["页面可绑定能力"])
