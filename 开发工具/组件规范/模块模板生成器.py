...
允许类型 = {"基础模块", "功能模块"}
允许参数类型 = {"文本", "整数", "浮点数", "布尔", "字典", "列表", "资源引用", "空"}
契约版本 = "1.0.0"
占位值表 = {"文本": '""', "整数": "0", "浮点数": "0.0", "布尔": "False",
           "字典": "{}", "列表": "[]", "资源引用": "''", "空": "None"}
...
def 实现函数块(模块名: str, 条目: dict, 依赖清单: list[dict]) -> str:
    ...
    名称 = 条目["名称"]
    参数表 = [{"名称": p["名称"], "类型": p.get("类型", "文本"), "必填": bool(p.get("必填", True))}
             for p in 条目.get("参数", [])]
...
def 模块包内容(模块名: str, 类型: str, 能力清单: list[dict], 依赖清单: list[dict]) -> dict[str, str]:
    ...
    包声明 = {"包id": 包id, "名称": 模块名, "类型": 类型, "版本": "1.0.0",
              "说明": f"{模块名} 模块（{类型}）：组合支持库公开能力。", "入口": "__init__.py",
              "依赖": 依赖声明, "能力": 能力声明}
    聚合契约 = {"契约版本": 契约版本, "能力契约": [
        {"能力id": 能力id, "版本": 契约版本, "说明": e.get("说明", ""),
         "参数": [{"名称": p["名称"], "类型": p.get("类型", "文本"), "必填": bool(p.get("必填", True)),
