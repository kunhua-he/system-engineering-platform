...
def _规范能力(能力: dict) -> dict:
    ...
    if not any(p.get("名称") == "超时秒" for p in 参数表):
        参数表.append({"名称": "超时秒", "类型": "数字", "必填": False, "默认值": 60.0,
                      "说明": "子进程超时"})
...
def _数据表(包id: str, 名称: str, 说明: str, 能力清单: list[dict],
    ...
    return {
        "能力定义.json": _JSON({"包id": 包id, "版本": "1.0.0", "说明": 说明,
                               "能力列表": 定义列表}),
        "包声明.json": _JSON({"包id": 包id, "名称": 名称, "类型": "支持库",
                             "版本": "1.0.0", "说明": 说明, "入口": "__init__.py",
                             "依赖": [], "能力": 声明列表}),
        "能力契约/参数契约.json": _JSON({"能力契约": 契约列表}),
        "依赖锁.json": _JSON({"包": [], "提供者id": 包id, "直接依赖": [], "依赖闭包": [],
                             "第三方说明": 第三方说明,
                             "环境": {"Python": "", "操作系统": "", "CPU": ""},
                             "生成时间": "",
                             "说明": "第三方不得直接进入模板；只能声明为外部工具/受管，第三方必须单独支持库"}),
        "配置契约.json": _JSON({"配置需求": [], "说明": "骨架无配置需求，新增后由配置契约校验"}),
        "权限契约.json": _JSON({"权限需求": [], "说明": "骨架无权限需求"}),
        "验证场景引用.json": _JSON({"验证场景引用": [
