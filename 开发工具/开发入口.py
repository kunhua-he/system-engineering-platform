...
def 创建模块模板(名称: str, 目录: Path | None = None) -> str:
    ...
    目录 = 目录 or 系统根 / "模块库" / 名称
    目录.mkdir(parents=True, exist_ok=True)
    for 子目录 in ("实现", "能力契约", "依赖契约", "配置契约", "权限契约", "说明", "验证场景"):
        (目录 / 子目录).mkdir(exist_ok=True)
    (目录 / "包声明.json").write_text(json.dumps({
        "包id": f"模块库.{名称}", "名称": 名称, "类型": "基础模块",
        "版本": "1.0.0", "说明": f"{名称} 模块", "入口": "__init__.py", "依赖": [],
        "能力": [{"能力id": f"{名称}.示例流程", "名称": "示例流程",
                  "参数": [{"名称": "输入", "类型": "文本"}], "说明": "示例流程"}],
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    (目录 / "__init__.py").write_text(
        f'"""模块库.{名称} 包级中文入口。"""\n\nfrom 公共契约.能力契约.契约 import 能力实现\n\n\ndef 注册能力(注册表) -> None:\n    注册表.注册(能力实现(\n        能力id="{名称}.示例流程", 包id="模块库.{名称}",\n        实现函数=示例流程, 参数=[{{"名称": "输入", "类型": "文本"}}],\n        返回="结果", 说明="示例流程",\n    ))\n\n\ndef 示例流程(输入: str) -> dict:\n    return {{"成功": True, "值": {{"处理": 输入}}}}\n',
        encoding="utf-8")
    (目录 / "能力契约" / "参数契约.json").write_text(json.dumps({
        "能力契约": [{"能力id": f"{名称}.示例流程", "参数": [{"名称": "输入", "类型": "文本"}], "返回": "结果"}],
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    (目录 / "说明" / f"{名称}说明.md").write_text(f"# {名称} 模块\n\n示例流程。\n", encoding="utf-8")
    (目录 / "验证场景引用.json").write_text(json.dumps({
        f"{名称}.示例流程": {"能力": "示例流程", "验证场景": ["组件合规"]},
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    return f"模块模板已创建: {目录}"
