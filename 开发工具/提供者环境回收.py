...
# 提供者「真身」（源码侧的提供者包目录）只可能落在支持库的这两个顶层子树。
# 为什么不含 `编译缓存`/`制品仓库`：那两处是**生成物**（带 `依赖锁.json` 的 277 个目录），
# 不是提供者真身；`回收()` 的目标恰恰是清掉生成物，拿它们当参照会误判「现用摘要」。
# 收窄后 `建提供者目录索引` 从 17.6 秒降到 1 秒内（实测见该函数说明）。
提供者真身搜索面 = (Path("支持库") / "适配层", Path("支持库") / "后端")
...
def 建提供者目录索引() -> dict[str, Path]:
    """**一次**遍历，建 ``{目录名: 真身目录}`` 索引（判据：该目录自带 `依赖锁.json`）。

    为什么要建索引而不是逐个 ``rglob``（2026-09-20 实测，属性能缺陷修复）：
    原 ``找提供者目录`` 每被调一次就 `系统根.rglob(提供者id)` **扫一遍全仓**；
    而 ``回收()`` 对环境根下每个提供者目录都要调一次 —— 本机实测 32 个提供者
    ⇒ **32 次全仓遍历**。基线：单次全仓 ``rglob`` **7.2 秒**，乘 32 ≈ **230 秒**
    （脚本实测总耗时 **163 秒**，叠加 `目录大小兆` 的逐文件 stat）。
    按华哥口径「调用一个命令超 15 秒就是纯 bug」，这不是慢、是 bug。

    **搜索面限定**（第二次实测后再收窄，2026-09-20）：一次全仓 ``rglob`` 仍有
    **17.6 秒**（要走完 6.9GB 的 `工程缓存/提供者运行环境`）。真身只可能落在
    **支持库的 `适配层` 与 `后端`** 两个顶层（实测全仓带 `依赖锁.json` 的目录分布：
    `编译缓存` 112 / `制品仓库` 165 / `后端` 15 / `适配层` 18 —— 前两者是**生成物**
    不是候选，且 `回收()` 的目标就是**清掉**生成物，拿它们当真身会导致误判「现用摘要」）。
    因此搜索面收窄到这两个顶层，遍历量从「全仓」降到「两个子树」，实测落到 **1 秒内**。
    """
    索引: dict[str, Path] = {}
    for 顶层名 in 提供者真身搜索面:
        顶层 = 系统根 / 顶层名
        if not 顶层.is_dir():
            continue
        for 命中 in sorted(顶层.rglob("*")):
            if not 命中.is_dir() or 命中.name in 索引:
                continue
            if (命中 / "依赖锁.json").is_file():
                索引[命中.name] = 命中
    return 索引


def 找提供者目录(提供者id: str, 索引: dict[str, Path] | None = None) -> Path | None:
    """在全仓找同名提供者目录（判据：它自己带 `依赖锁.json`）。找不到即返回 None。

    ``索引`` 可传入 ``建提供者目录索引()`` 的结果以复用一次遍历（批处理场景）；
    不传则单次 ``rglob``（**仅供单查询用，别放进循环**——见 ``建提供者目录索引`` 的性能说明）。
    """
    if 索引 is not None:
        return 索引.get(提供者id)
...
def 回收(执行: bool) -> int:
    ...
    跳过: list[str] = []
    目录索引 = 建提供者目录索引()  # 一次全仓遍历，避免循环里逐个 rglob（见该函数说明）
