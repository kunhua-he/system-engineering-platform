...
def _固定排除目录名() -> tuple[str, ...]:
    """转调唯一事实源 `工作区指纹.固定排除目录`，只留**按目录名**比对的条目。

    事实源里那条 `开发文档/项目证据` 是**带路径**的，本表按目录名比对（`d not in 剪枝目录`），
    路径条目永不命中，留着只会误导读者。
    """
    from 开发工具.项目编译.工作区指纹 import 固定排除目录
    return tuple(名 for 名 in 固定排除目录 if "/" not in 名)


#: 目录遍历时要剪掉的目录名（`rglob` 不支持剪枝，故手写 `os.walk`）。
#: 共享部分**转调唯一事实源**，本模块原先自维护一份 —— 2026-09-23 实测漏了 `.tmp`
#: （平台自己的**仓库内临时根**：`进程管理` 的 `临时目录 = _工作目录/".tmp"`，以仓库根为
#: 工作目录时落这里）⇒ 运行时残留（`反向破坏_*` / `注册口径漂移测试_*`）里的 md 被当成
#: 正式文档参与查重，`md查重` 于是把 `.tmp/注册口径漂移测试_*/…/使用说明.md` 报成
#: 「★高度疑似重复 1.00」，**这是假红**。同批 `测试写入边界门禁` 早已转调同一事实源
#: （`_固定排除目录表()`），本表是漏改的那个兄弟。
#: 只做一处加工：补本模块特有的**文档类**目录（参考资料/归档 属文档分类口径，
#: 与工作区指纹无关，故不进事实源）。
剪枝目录 = _固定排除目录名() + ("参考资料", "归档")
