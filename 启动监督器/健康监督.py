...
class 系统提供者健康监督:
    ...
    def _裁剪证据(self, 时间戳: str | None = None) -> None:
        ...
        保护行（任何情况下不裁剪）：失败证据行（成功=False 或 健康=False，
        字段缺失按成功/健康处理）与本轮刚写入的当前任务行（时间戳等于
        本次检查时间戳）；TTL 只对非保护行生效，条数只对非保护行计数并
        保留最近 N 条非保护行，因此文件总行数可能超过 保留条数；读取/
        重写失败静默（不拖垮主进程）；时间戳无法解析的行保守保留。
        """
        策略 = self._证据保留策略
        if not 策略:
            return
