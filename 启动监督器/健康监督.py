...
结果写入诊断证据（`启动监督器/健康证据.jsonl`，位于**运行缓存根**下：
源码态 = `<系统根>/工程缓存/启动监督器`，制品/受管缓存态经唯一解析器
`公共契约/运行时/运行缓存.解析运行缓存根` 落制品外；字段：时间/提供者/
...
# `<系统根>/工程缓存`（与旧写法**逐字相同**），制品态改落平台受管缓存。
# 裸拼在制品态就是把诊断证据写进不可变制品——2026-09-17 伪制品布局实测：
# 旧写法得到 `<制品>/平台客户端/工程缓存/启动监督器/健康证据.jsonl`，
# 并且构造默认实例时就 mkdir + 真实追加证据行（真写进制品）。
默认证据文件 = 解析运行缓存根(系统根) / "启动监督器" / "健康证据.jsonl"
时间格式 = "%Y-%m-%d %H:%M:%S"
标准错误摘要最大长度 = 300
