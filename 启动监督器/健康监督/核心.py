...
# sys.path 自举（唯一一处判定）：本文件是 `启动监督器/健康监督/核心.py`，系统根 = parents[2]。
# 为什么必须有：本模块会 `import 支持库.* / 项目适配层.* / 公共契约.*`，按**文件路径**
# 直接运行（`python3.14 启动监督器/健康监督/核心.py`）时那三个顶层包不在 sys.path 上，
# 会以 `ModuleNotFoundError` 失败。自举必须在这些 import 之前执行（旧平铺版本把
# 系统根放在 import 之后，是靠调用方已注入 sys.path 才侥幸跑得起来）。
