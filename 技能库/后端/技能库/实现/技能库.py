...
网关地址环境变量 = "技能库_网关地址"
网关凭证环境变量 = "系统库网关凭证"
#: 写入凭证（开工ID）透传环境变量名：技能脚本经合成模块调底座能力时，本变量里的
#: 开工ID 会被并入「参数」里的 `开工ID` 位 ⇒ 受管路径写入腿（文件操作/文本补丁/资源管理）
#: 能拿到调用方凭证（判据唯一在 `公共契约/运行时/写入授权.校验写入授权`）。
开工ID环境变量 = "系统平台_开工ID"
默认网关超时秒 = 20.0
...
def 调用底座能力(能力id, 参数=None, 超时秒=None) -> dict:
    ...
    参数对象 = dict(参数) if isinstance(参数, dict) else {}
    开工ID = str(os.environ.get(开工ID变量名) or "").strip()
    if 开工ID and not 参数对象.get("开工ID"):
        参数对象["开工ID"] = 开工ID
    载荷 = json.dumps(
        {"操作": "调用能力", "能力id": 名称, "参数": 参数对象},
        ensure_ascii=False,
    ).encode("utf-8")
    地址 = _拆地址(网关地址)
...
def 组装合成模块源码(白名单: tuple[str, ...], 网关地址: str,
    ...
    return (
        ...
        .replace("__注入开工ID变量名__", str(开工ID环境变量))
...
def 创建能力注入(白名单: tuple[str, ...], 凭证: str, 超时秒: int = 默认超时秒) -> 能力注入:
    ...
    try:
        # 落盘走**唯一原子写腿**（2026-09-25 核心批·写腿下沉）：本层按依赖防火墙只可依赖
        # 公共契约，不得转调 `支持库.后端.文件系统支持库.文件操作.写入文件` ⇒ 转调
        # `公共契约.运行时.写文件.原子写文件`（合成模块写在系统临时区，仓库外非受管，
        # 不涉租约判据）。本层不再自建第二条写实现（哲学 1.2：一类事情只有一条腿）。
...
def 进程组终止(进程: subprocess.Popen) -> None:
    """向进程组发强杀信号并等待回收；子进程与孙进程一并清理。

    唯一实现在 公共契约.运行时.进程终止.结束并留痕（终止 → 宽限 → 强杀 → **整组**收敛
    复查，组长已回收但同组子孙仍在 ⇒ 未收敛）；回收失败不静默，收口层会登记一条回收失败留痕。
    """
    进程终止.结束并留痕(进程, 位置="技能库.进程组终止", 宽限秒=5.0, 等待秒=5.0)
...
def 运行受控脚本(
    ...
    *,
    ...
    `开工ID`（写入凭证，**只做形态归一、不自行判拒**）：非空时以 `开工ID环境变量`
    注入子进程环境 ⇒ 技能脚本经合成模块调底座能力时，该凭证并入「参数」里的 `开工ID`
    位，受管路径写入腿据它核对「是不是你认领的」（判据唯一在
    `公共契约/运行时/写入授权.校验写入授权`，本层不自行判拒）。
    """
    ...
    try:
        注入环境 = dict(能力注入.环境附加) if 能力注入 else {}
        if 开工ID:
            注入环境[开工ID环境变量] = str(开工ID)
        环境 = 构造白名单环境(
            根,
            环境变量,
...
def 校验技能包(技能根目录: str = None, 技能标识或路径: str = None) -> 结果:
    ...
    # `技能根目录` 是 `基准路径字段名` 成员（规范写法＝绝对），绝对原样、相对按生效根
    # （显式根 → 环境变量 系统平台_项目根 → 进程 cwd）；**本函数不自带绝对性分支、不自己拼根。**
...
def 生成技能索引(技能根目录: str = None, 写回索引: bool = 真,
               开工ID: str | None = None) -> 结果:
    """能力 技能库.技能索引.生成索引：扫描全部技能包，从契约派生索引并（可选）写回。

    开工ID 穿透（2026-09-25）：本层按依赖防火墙只可依赖 公共契约，不能转调
    `文件系统支持库.文件操作.写入文件`，故落盘转调**唯一原子写腿**
    `公共契约.运行时.写文件.原子写文件`，并把调用方的开工ID 一路传给它 —— 它与写腿
    共用同一处判据（活跃写租约所有者 == 开工ID）。

    写回失败如实回失败（2026-09-25 收口，假绿）：写回前**前置**判据
    `公共契约/运行时/写入授权.授权拒绝结果`（受管路径必须被调用方认领的活跃写租约覆盖），
    不过即原样回 `越界`；写腿再抛 `OSError` 时同样回失败、**不再吞成成功**。
    """
    if not 技能根目录:
        return 结果.失败("参数不合法", "技能根目录 必填", 来源=来源标识)
    根 = Path(技能根目录).resolve()
    ...
    if 写回索引:
        索引路径 = 根 / "索引.json"
        # 写腿第二判据（写入授权）**前置**：受管路径必须被「调用方认领的」活跃写租约覆盖。
        # 判据本体与失败说明的唯一来源是 `公共契约/运行时/写入授权.授权拒绝结果`
        # —— 与 `文件系统支持库.文件操作._授权拒绝` 同序同口径，本层不另写授权语义、
        # 不另写「越界」说明文本（哲学 1.2：同一道口只有一份判据）。
        # ★ 为什么必须前置（2026-09-25 收口，假绿）：此前这条腿把落盘失败吞成
        #   `结果值["写回失败"]` 字段却**仍返回 `结果.成功结果`** ⇒ 调用方看到
        #   「成功=True」而索引实际没写回（上层据此继续往下走）。失败即失败。
        from 公共契约.运行时.写入授权 import 授权拒绝结果
        授权问题 = 授权拒绝结果(索引路径, 来源标识, str(开工ID or "").strip())
        if 授权问题 is not None:
            return 授权问题
        try:
        ...
        新索引["能力列表"] = 条目列表
        # 落盘走**唯一原子写腿**（2026-09-25 核心批·写腿下沉）：本层按依赖防火墙只可依赖
        # 公共契约，不得转调 `支持库.后端.文件系统支持库.文件操作.写入文件`；写腿已下沉到
        # `公共契约/运行时/` ⇒ 本层够得到，不再自建「解锁 → 写 → 收口锁态」骨架
        # （哲学 1.2：一类事情只有一条腿）。`开工ID` 一路穿透（受管路径 + 空凭证 ⇒ 腿自己就拒）。
        from 公共契约.运行时.写文件 import 原子写文件
        try:
            原子写文件(索引路径, json.dumps(新索引, ensure_ascii=False, indent=2) + "\n", "utf-8", str(开工ID or ""))
            结果值["已写回"] = str(索引路径)
        except OSError as 错误:
            # 授权已在上面前置判过，走到这里的是**真落盘错误**（磁盘/内核锁残留/窗口竞态）——
            # 同样不许吞成成功。`结果值["写回失败"]` 字段随之删除：失败不再走成功信封，
            # 留着就是一个永不出现的死字段（失败真因改由 错误码 + 错误说明 承载）。
            return 结果.失败("写入失败", f"索引写回失败: {错误}", 来源=来源标识)
    return 结果.成功结果(结果值)
