...
#: （`支持库/适配层/Git提供者/实现/提交回滚.py::提交开工ID句式`、`开发工具/git钩子/commit-msg`）
#: 同一句式。改前本件钉「16 位裸十六进制」（旧 `MCP工具箱/项目服务.py` 的 `work_id` 口径，
#: 该文件已删），与发放侧互不兼容且没有任何地方报出来 ⇒ 真开工ID 进不了本门。
...
#: 本脚本的开工id 格式门=全仓唯一口径（见上方 `开工id模式`）；★ 改本口径必须同批改
#: `技能库/技能/交付收尾/SKILL.md`、`契约.json`、`工作流.json` 三处说明面，
#: 否则技能说明仍教调用方传 16 位十六进制（本批已如实点名，未在认领清单内不动）。
...
def _解析路径(文本: str, 项目根: Path) -> Path:
    ...
    边界项（批G L3 类1，**保留**）：本脚本**够不到**唯一腿
    `公共契约.运行时.平台适配.解析路径` —— 技能脚本受导入审计约束
    （`技能库.受控执行.运行技能包` → `审计脚本源码`：只放行标准库 + 注入的合成模块
    `技能底座能力`；实测给 `from 公共契约…` 判 `禁止导入非标准库模块`）。
    且本函数解析的 `反馈文件` / `验证历史文件` / `worktree路径` **不在**
    `公共契约/基础类型/字段名册.py::路径族字段名` 里，网关的
    `_归一化路径族参数`（唯一消费点）不碰它们 ⇒ 本函数是它们**唯一**的解析腿，删不得。
    收口路子（本批不做，交批H）：把这三个入参名登记进 `路径族字段名`（或新增一个
    「解析路径」底座能力），届时本函数整段删除、改走网关唯一消费点。
    """
...
def 主流程(参数: dict) -> dict:
    ...
    # 边界项（批G L5 自制生效根，**保留**）：同上 —— 本脚本不能 import 唯一腿。
    # `项目根目录` 是 `字段名册.基准路径字段名` 成员（规范写法＝绝对路径），网关已按
    # 唯一腿归一过；本行只是对它的再解析（缺省 `.` → cwd），保留到批H 与上面同批收口。
