...
#: 唯一发布命令的**规范写法**（展示用，仍写优先项 3.14）。识别**不按解释器判** ——
#: 见 `主流程` 按**入口**比对（U-5，2026-09-24 华哥裁决「不强制 3.14」）。
...
def _绝对路径(文本: str, 项目根: Path) -> Path:
    ...
    # 且 `反馈路径` / `账本路径` **不在** `字段名册.路径族字段名` 里，网关的
    # `_归一化路径族参数`（唯一消费点）不碰它们 ⇒ 本函数是它们**唯一**的解析腿，删不得。
    # 收口路子（本批不做，交批H）：把这两个入参名登记进 `路径族字段名`（或新增一个
...
def 主流程(参数: dict) -> dict:
    ...
    # 唯一腿归一过；本行只是对它的再解析，保留到批H 与上面同批收口。
    ...
    # 与 `测试支持库.验证命令白名单`、`发布治理` 同一口径；回落解释器跑同一入口仍是正式发布。
    是唯一发布命令 = list(命令)[1:] == list(唯一发布命令)[1:]
    值 = {
        "技能标识": 技能标识,
        "名称": 名称,
