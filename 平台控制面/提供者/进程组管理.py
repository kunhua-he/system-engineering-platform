...
- 启动进程树(工作目录=None, 模式=None) -> 进程组信息：启动真实 3 层
  ...
  验证整组回收不依赖组长存活。
- 强杀进程组(进程组信息) -> 回收报告：按跨平台收口 `进程终止.发进程组信号`
  （先按组长身份整组强杀，第一发未送达时按组号补发）强杀整个进程组，等待全部 PID 从系统中消失，清理临时工作目录，验证端口
  可重绑；重复调用安全（幂等）。
...
def 启动进程树(工作目录: str | Path | None = None, 模式: str | None = None) -> dict:
    ...
    if len(节点pid表) != 3:
        # 整组强杀只走 公共契约.运行时.进程终止.发进程组信号（组长被 wait 回收时按组号补发）。
        进程终止.发进程组信号(组长.pid, 信号="强杀")
        try:
            组长.wait(timeout=5)
