...
def _受限通信(进程: subprocess.Popen, 超时秒: float,
    ...
    返回第 5 项是**回收失败留痕**（本次调用里 ``进程终止.结束并留痕`` 未确认收敛的条目），
        ...
        try:
            while True:
                ...
                if len(目标) >= 输出上限 and len(块) > 输出上限 - len(目标):
                    超限[名称] = 真
                    if not 进程终止.结束并留痕(
                            进程, 位置="资源硬限制.受限通信输出超限", 宽限秒=2.0, 等待秒=2.0):
    ...
    try:
        ...
        if not 进程终止.结束并留痕(
                进程, 位置="资源硬限制.受限通信超时", 宽限秒=2.0, 等待秒=2.0):
...
def 在独立进程组中运行(命令列表: list[str], 预算: dict, 超时秒: float = 30) -> dict:
    ...
    try:
        ...
        if not 进程终止.结束并留痕(
                进程, 位置="资源硬限制.受限读取异常", 宽限秒=2.0, 等待秒=2.0):
    ...
    if 输出超限:
        if not 进程终止.结束并留痕(
                进程, 位置="资源硬限制.输出超限路径", 宽限秒=2.0, 等待秒=2.0):
