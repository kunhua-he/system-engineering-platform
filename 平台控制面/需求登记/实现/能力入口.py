...
稳定错误码口径（本批收口「死表」）：
- 能力面只自报**中文域错误码**（`结果.失败` 的第一参数）；平台稳定码一律经
  `需求错误码.稳定错误码映射`（唯一映射表）取，`详情.稳定错误码` 由 `_失败` 统一注入；
- 修前本文件 import 了那张表却 0 处使用、稳定码全写死在分支里，两张事实源可各自漂移
  （发布门禁按能力面自报的 `详情.稳定错误码` 建映射，故漂移会直接改变门禁判定）；
- 新增失败分支必须在 `需求错误码.py` 补表，分支内不得再出现稳定码字面量。
"""
...
from 平台控制面.需求登记.需求服务 import 需求登记, 取登记快照, 已登记且已确认, 需求不存在错误
from 平台控制面.需求登记.需求错误码 import 稳定错误码映射
from 公共契约.基础类型.逻辑类型 import 真, 假
from 公共契约.运行时.运行缓存 import 解析运行缓存根

# 存储目录缺省值：经唯一解析器取运行态存储根（源码态 = `<系统根>/工程缓存/平台控制面`，
# 与旧的裸相对值同义；制品态 = 平台受管缓存）。裸相对路径会绕过解析器、相对制品根落盘，
# 2026-09-17 实测验证时 `<制品>/工程缓存/平台控制面/权威状态.db` 因此被写进制品。
...
def _稳定码(域错误码: str) -> str:
    """按 `需求错误码.稳定错误码映射`（唯一映射表）取平台稳定码。

    查不到即抛 `KeyError`：宁可当场炸，也不给任何分支自造未登记稳定码的机会。
    修前本文件把稳定码写死在字面量里，那张自称唯一映射表的表全程无人读，
    两张表可各自漂移；本函数是「全包稳定码只此一处来源」的执行点。
    """
    try:
        return 稳定错误码映射[域错误码]
    except KeyError:
        raise KeyError(
            f"未登记的平台稳定码：中文域错误码 {域错误码!r} 不在 "
            "`平台控制面/需求登记/需求错误码.py` 的 稳定错误码映射 内"
            "（新增失败分支必须先补表，不得在分支里写死稳定码）") from None


def _失败(域错误码: str, 错误说明: str, *, 来源: str = 来源名称,
          详情: dict[str, Any] | None = None, **附加: Any) -> Any:
    """统一的失败结果出口：能力面只自报中文域错误码，平台稳定码一律查表。

    `详情.稳定错误码` 由 `_稳定码(域错误码)` 生成（键序与收敛前一致：稳定码在前）。
    调用方传入的 详情 里再写 `稳定错误码` 即当场抛错（第二事实源一律拒绝）。
    """
    from 公共契约.基础类型.结果类型 import 结果

    if 详情 and "稳定错误码" in 详情:
        raise ValueError("详情.稳定错误码 一律由 需求错误码.稳定错误码映射 生成，"
                         "调用方不得传入（唯一事实源不得有第二个出口）")
    合并详情 = {"稳定错误码": _稳定码(域错误码), **(详情 or {})}
    return 结果.失败(域错误码, 错误说明, 来源=来源, 详情=合并详情, **附加)
...
def _取服务或失败(存储目录: str) -> tuple[Any, Any]:
    ...
    try:
        ...
        return None, _失败("参数不合法", f"存储目录不合法：{错误}", 来源=来源名称)
...
def 登记需求(目标: str, 非目标: str = "", 输入: dict[str, Any] | None = None,
    ...
    try:
        ...
        return _失败("需求登记失败", f"需求快照写入失败：{错误}", 来源=来源名称)
    except (TypeError, ValueError) as 错误:
        return _失败("参数不合法", f"需求快照参数不合法：{错误}", 来源=来源名称)
    return 结果.成功结果(快照)
...
def 确认需求(需求id: str, 调用者: str = "", 角色: str = "", 存储目录: str = "") -> Any:
    ...
    if not isinstance(需求id, str) or not 需求id.strip():
        return _失败("需求id不能为空",
                     f"需求id不能为空（平台稳定码 {_稳定码('需求id不能为空')}：空需求id一律阻断）",
                     来源=来源名称)
    ...
    try:
        ...
        return _失败("需求未确认", f"需求确认写入失败：{错误}", 来源=来源名称,
                     详情={"需求id": 需求id})
    if not 成功:
        return _失败("需求不存在", f"{消息}（平台稳定码 {_稳定码('需求不存在')}："
                     "需求快照查不到，不能确认）",
                     来源=来源名称,
                     详情={"需求id": 需求id})
    return 结果.成功结果({"需求id": 需求id, "是否已确认": 真, "消息": 消息})


def 查询需求(需求id: str = "", 限制: int = 50, 包含工作包: bool = 假,
            存储目录: str = "") -> Any:
    """查询需求（只读）：给 需求id 单查确认状态，留空按创建顺序列表，可带该需求的工作包。"""
    from 公共契约.基础类型.结果类型 import 结果

    if 需求id is None or not isinstance(需求id, str):
        return _失败("参数不合法", "需求id 必须是文本（留空表示列表查询）", 来源=来源名称)
    if not isinstance(限制, int) or isinstance(限制, bool) or not 1 <= 限制 <= 1000:
        return _失败("参数不合法", "限制必须是 1 到 1000 的整数", 来源=来源名称)
    if not isinstance(包含工作包, bool):
        return _失败("参数不合法", "包含工作包 必须是布尔", 来源=来源名称)
    服务, 失败 = _取服务或失败(存储目录)
    if 服务 is None:
        return 失败
    标识 = 需求id.strip()
    try:
        ...
        return _失败("需求查询失败", f"需求查询失败：{错误}", 来源=来源名称)
    ...
    return 结果.成功结果({
        ...
        "需求": 单条,
        "是否命中": 单条 is not None,
        # 与 需求服务.已登记且已确认 同口径：登记过（有快照）且已确认才算 true，
        # 否则脏行（有确认状态、无快照）会经本字段把「已确认」这条权威事实放出去。
        "是否已确认": bool(单条 is not None and 已登记且已确认(单条)),
        "工作包表": 工作包表,
    })
...
def 需求已确认(需求id: str, 存储目录: str = "") -> Any:
    ...
    if 命中 is None and not 读取失败:
        # 干净读、无记录 → 目标快照查不到（稳定码取 需求错误码.稳定错误码映射['需求不存在']，
        # 与 统一入口.py::_装配计划 的「需求不存在」逐字同词）。
        return _失败("需求不存在",
                     f"{需求id} 查不到需求快照（平台稳定码 {_稳定码('需求不存在')}："
                     "不存在一律阻断开发与发布放行）",
                     来源=来源名称,
                     详情={"需求id": 需求id})
    if 命中 is not None and 取登记快照(命中) is None:
        # 行存在但没有 `登记需求` 写入的快照（历史脏行）→ 与 确认需求 同口径按「需求不存在」阻断。
        # 修前只比 确认状态，这类脏行若已被确认过，闸门会真的放行 —— 权威事实就不再唯一来自 登记需求。
        return _失败("需求不存在",
                     f"{需求id} 只有需求行、没有 登记需求 写入的需求快照"
                     f"（平台稳定码 {_稳定码('需求不存在')}：未登记一律阻断开发与发布放行）",
                     来源=来源名称,
                     详情={"需求id": 需求id})
    if 读取失败 or (命中.get("确认状态") if 命中 else "") != "已确认":
        # 需求存在但未确认（或读取失败 fail-safe）→ 稳定码取 需求错误码.稳定错误码映射['需求未确认']，
        # 与 统一入口.py::_装配计划/::_创建组件/::_提交候选包 的「需求未确认」逐字同词。
        return _失败("需求未确认",
                     f"{需求id} 未确认（平台稳定码 {_稳定码('需求未确认')}："
                     "未确认一律阻断开发与发布放行）",
                     来源=来源名称,
                     详情={"需求id": 需求id})
    return 结果.成功结果({"需求id": 需求id, "是否已确认": 真, "消息": "需求已确认"})
...
def 保存工作包(需求id: str, 波次: int, 说明: str, 输入快照: dict[str, Any],
    ...
    if not isinstance(波次, int) or isinstance(波次, bool):
        return _失败("参数不合法", "波次 必须是整数", 来源=来源名称)
    ...
    for 名称, 值 in (("允许修改路径", 允许修改路径), ("能力占用", 能力占用)):
        if not isinstance(值, list):
            return _失败("参数不合法", f"{名称} 必须是列表", 来源=来源名称)
    服务, 失败 = _取服务或失败(存储目录)
    if 服务 is None:
        return 失败
    try:
        ...
        return _失败("工作包登记失败", f"工作包写入失败：{错误}", 来源=来源名称)
    except (TypeError, ValueError) as 错误:
        return _失败("参数不合法", f"工作包参数不合法：{错误}", 来源=来源名称)
...
def 查询工作包(需求id: str = "", 限制: int = 50, 存储目录: str = "") -> Any:
    ...
    if 需求id is None or not isinstance(需求id, str):
        return _失败("参数不合法", "需求id 必须是文本（留空表示列全部）", 来源=来源名称)
    if not isinstance(限制, int) or isinstance(限制, bool) or not 1 <= 限制 <= 1000:
        return _失败("参数不合法", "限制必须是 1 到 1000 的整数", 来源=来源名称)
    服务, 失败 = _取服务或失败(存储目录)
    if 服务 is None:
...
def 登记装配计划(需求id: str, 能力搜索结果: list[Any] | None = None,
    ...
    if not isinstance(候选, list):
        return _失败("参数不合法", "能力搜索结果 必须是列表", 来源=来源名称)
    if not isinstance(包表, list):
        return _失败("参数不合法", "工作包表 必须是列表", 来源=来源名称)
    if not isinstance(落工作包, bool):
        return _失败("参数不合法", "落工作包 必须是布尔", 来源=来源名称)
    ...
    for 序号, 包 in enumerate(包表):
        if not isinstance(包, dict):
            return _失败("参数不合法", f"工作包表第 {序号 + 1} 项必须是 JSON 对象",
                         来源=来源名称)
        ...
        if "波次" in 包 and (not isinstance(包["波次"], int) or isinstance(包["波次"], bool)):
            return _失败("参数不合法", f"工作包表第 {序号 + 1} 项的 波次 必须是整数",
                         来源=来源名称)
        if "能力占用" in 包 and not isinstance(包["能力占用"], list):
            return _失败("参数不合法", f"工作包表第 {序号 + 1} 项的 能力占用 必须是列表",
                         来源=来源名称)
    if 落工作包:
    ...
    except 需求不存在错误 as 错误:
        # 需求快照查不到 → 中文域错误码 `需求不存在`（修前这里会平白建出一行需求行，
        # 于是「已确认」被谁写都能成立；本分支是「未登记需求不得生成装配计划」的执行点）。
        return _失败("需求不存在",
                     f"{需求id} 查不到需求快照（平台稳定码 {_稳定码('需求不存在')}："
                     "未登记需求一律不得生成装配计划，也不得凭写动作建出需求行）",
                     来源=来源名称,
                     详情={"需求id": 需求id, "原因": str(错误)})
    except ValueError as 错误:
        return _失败("装配计划生成失败", str(错误), 来源=来源名称)
    except sqlite3.Error as 错误:
        return _失败("装配计划生成失败", f"装配计划落盘失败：{错误}", 来源=来源名称)
    已落工作包数 = 0
    if 落工作包:
        try:
            for 包 in 包表:
            ...
            return _失败("工作包登记失败", f"装配计划已落盘、工作包落盘失败：{错误}",
                         来源=来源名称)
