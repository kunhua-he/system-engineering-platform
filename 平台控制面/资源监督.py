...
from 公共契约.运行时.资源水位 import (
    内存采样未知,
    整机内存水位阈值,
    读取整机内存水位 as _读整机内存水位,
)


#: 整机内存水位阈值与采样口径的**唯一来源**已下沉 `公共契约/运行时/资源水位.py`
#: （2026-09-21）：判据既要被本监督器用，也要被**每次能力调用的准入**用
#: （`运行核心/能力调用/超时执行/准入闸门.py`），而运行核心不得依赖平台控制面
#: —— 判据只能住在谁都能依赖的下层。此处按名再导出，旧调用点与旧文档口径零改动。
__all__ = ["资源监督器", "整机内存水位阈值", "内存采样未知"]


def _整机内存采样() -> tuple[float, float]:
    """整机内存采样 ``(占用率 0..1, 占用MB)``；采不到返回 ``(内存采样未知, 内存采样未知)``。

    **采样与口径的唯一实现在 `公共契约.运行时.资源水位`**（再往下一层是
    `平台适配.内存容量信息`）。本函数只做单位换算，不再自采一次、也不复制口径
    —— 判据与采样各只有一处（哲学 1.3 结果唯一）。

    **读的是定时器快照，不是每次现采**（华哥 2026-09-21 定）：采样由 `资源水位` 里
    一条 30 秒周期的定时线程负责，监控器只读那份快照（一次字典读取）。本函数因此
    是纯读，放在高频路径上也零系统调用。

    **为什么不用调用方传参**：闸门要在每次请求上判定，若要求调用方传内存，
    等于要求每个调用点都记得传 —— 2026-09-21 实测全仓**无任何调用点**传该参数，
    判据恒为 0、永不触发。
    """
    快照 = _读整机内存水位()
    if not 快照.get("支持") or 快照.get("快照陈旧"):
        return 内存采样未知, 内存采样未知
    return float(快照["占用率"]), float(快照["已用字节"]) / 1024 / 1024
...
class 资源监督器:
    ...
    def 采样(self, 单元id: str, *, 内存MB: float | None = None) -> dict[str, Any]:
        """运行中采样：峰值来自实际运行状态；**整机内存自动采，不用调用方传**。

        `内存MB` 是**调用方自报的进程级内存**（可选，与预算 `内存上限` 同口径）；
        整机占用是**监督器自己采**的独立字段，两者**不混用**。
        整机采不到时只计失败次数、**不写 0**（哲学第 3 条 2 项）。
        """
        整机率, 整机MB = _整机内存采样()
        with self._锁:
            报告 = self._报告表.setdefault(单元id, {"线程峰值": 0, "并发峰值": 0, "队列峰值": 0,
                                                "内存峰值": 0, "整机内存占用MB": 0.0,
                                                "整机内存占用率": 0.0, "内存采样失败": 0,
                                                "超限次数": 0, "动作": [],
                                                "任务总数": 0, "成功": 0, "失败": 0, "取消": 0})
            ...
            if 内存MB is not None:
                报告["内存峰值"] = max(报告["内存峰值"], float(内存MB))
            if 整机MB >= 0:
                报告["整机内存占用MB"] = max(float(报告.get("整机内存占用MB", 0.0)), 整机MB)
                报告["整机内存占用率"] = 整机率
            else:
                报告["内存采样失败"] = int(报告.get("内存采样失败", 0)) + 1
            return dict(报告)

    def 检查超限(self, 单元id: str, 预算: dict[str, Any]) -> tuple[bool, str]:
        """超限检查：线程/并发/队列/**整机内存水位**任一超限即失败（由调用方处置）。

        **包级 `内存上限` 不在此判（2026-09-21 实测口径混用）**：预算里的
        `内存上限`（如 64 MB）是**单个执行单元**的额度，而整机占用是 GB 量级；
        拿整机值比包级额度会**永远超限**（实测整机 29457 MB 对包级 64 MB）。
        包级内存由 `提供者/资源硬限制.py` 的 setrlimit 在子进程内强制并验证
        （`验证内存()`：超限映射被拒才算生效），本处不重复判、不误判。

        整机水位：整机占用率 > `整机内存水位阈值`。采样不可用时取哨兵
        `内存采样未知`（-1.0），比较恒为假，**不误判放行**。
        """
        报告 = self.采样(单元id)
        活跃数 = len(self._活跃表.get(单元id, {}))
        整机率 = float(报告.get("整机内存占用率", 内存采样未知))
        检查表 = [
            (报告["线程峰值"] > int(预算.get("线程上限", 10**9)), "线程超限"),
            (活跃数 > int(预算.get("并发调用上限", 10**9)), "并发超限"),
            (活跃数 > int(预算.get("队列长度", 10**9)), "队列超限"),
            (整机率 > 整机内存水位阈值,
             f"整机内存水位超限（{整机率:.1%} > {整机内存水位阈值:.0%}）"),
        ]
        for 超限, 原因 in 检查表:
            if 超限:
                self._记录超限(单元id, 原因)
