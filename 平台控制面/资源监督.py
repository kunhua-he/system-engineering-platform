    ...
    def 提交任务(self, 单元id: str, 任务: Callable[[], Any], *参数,
        ...
        if 超时秒 > 0:
            try:
                ...
                # 只取消：完成回调统一负责释放信号量+移除活跃表（防止双重释放）
                未来.cancel()
                self._记录超限(单元id, f"单次调用超时({超时秒}s)")
                return False, f"单次调用超时({超时秒}s)，任务已取消且资源将释放", None
