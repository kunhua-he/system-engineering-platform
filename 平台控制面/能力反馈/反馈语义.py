...
反向验证见 `开发文档/未完成事项.md` 2026-09-23 敏感键名表收口的销账证据（§四 那一行已随收口删除）。`摘要`/`内容摘要` 的入参形状
...
# ★ 本件 `脱敏` 与 `运行核心/运行诊断/运行事件/脱敏工具.脱敏值` **有意不合并**（T2 复核，2026-09-24）：
# 掩码面互不为超集 —— 本件独有 Bearer 与 xai-（`脱敏工具` 的词表里没有），
# 对方独有 eyJ/AKIA/-----BEGIN/gho_/ghu_/ghs_。且本件的截断/递归上限/返回形状
# （2048 截断、深度 6、容器前 100 项、键 str()[:128]）是**能力反馈返回值的行为冻结契约**，
# 合并即改返回值（漂移）。故保留双实现；改任一处前必须证明掩码面不缩小。
