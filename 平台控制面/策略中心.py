...
class 策略中心:
    ...
    # 已实现的策略类型（与 策略闸门.判定键表 同口径）；未知类型一律拒绝，绝不默认放行。
    已知策略类型 = ("复用", "依赖", "签名", "发布", "资源")
    ...
    def _执行判定(self, 类型: str, 主题: str, 请求: dict[str, Any]) -> dict[str, Any]:
        """按策略类型执行判定（可被测试扩展）。"""
        # fail-closed：未知/拼写错误/未实现的类型一律拒绝。
        # 为什么不能默认放行：判定类型是调用方给的字符串，拼错一个类型（如「复用」写成
        # 「复用性」）就等于整条策略门禁静默失效；同层 `授权.操作最小角色.get(操作, 99)`
        # 对未知操作取最高权限要求，此处口径必须一致（决策：未知即拒绝）。
        if 类型 not in self.已知策略类型:
            return {"允许": 假, "理由": f"未知策略类型，禁止放行: {类型!r}",
                    "错误码": "策略类型未知"}
        if 类型 == "复用":
    ...
    def _复用判定(self, 能力id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 记录 is None:
            return {"允许": 真, "理由": "能力不存在，允许新增"}
        if 记录["裁决状态"] == "待裁决":
            return {"允许": 假, "理由": "疑似重叠能力未裁决，禁止发布", "错误码": "待裁决"}
        if 记录["复用裁决"] == "拒绝":
            return {"允许": 假, "理由": f"维护者裁决拒绝: {记录['复用裁决']}", "错误码": "复用被拒"}
        if 记录["复用裁决"] in ("复用", "合并", "允许并存"):
            return {"允许": 真, "理由": f"复用裁决: {记录['复用裁决']}"}
        return {"允许": 真, "理由": "已登记能力"}
    ...
    def _依赖判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        缺失 = 声明 - 已存在
        if 缺失:
            return {"允许": 假, "理由": f"依赖能力未登记: {缺失}", "错误码": "依赖缺失"}
        return {"允许": 真, "理由": "依赖全部已登记"}

    def _签名判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        if not 请求.get("已签名"):
            return {"允许": 假, "理由": "未签名包禁止安装/激活", "错误码": "未签名"}
        if 请求.get("签名失效"):
            return {"允许": 假, "理由": "签名失效（内容/权限/预算变化）", "错误码": "签名失效"}
        发布者 = 请求.get("签名者", "")
        信任 = self.状态.读取记录("信任", "发布者", 发布者)
        if 信任 is None or 信任["状态"] != "有效":
            return {"允许": 假, "理由": f"发布者不在信任目录: {发布者}", "错误码": "发布者不受信"}
        return {"允许": 真, "理由": f"签名有效，发布者受信: {发布者}"}
    ...
    def _发布判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 需求id:
            ...
            if 需求 is None or 需求["确认状态"] != "已确认":
                return {"允许": 假, "理由": "需求未确认，禁止发布", "错误码": "需求未确认"}
        候选 = 请求.get("候选版本", "")
        已有 = [记录 for 记录 in self.状态.查询记录("发布", "包id=?", (包id,))
                if 记录["期望版本"] == 候选 and 记录["状态"] in ("激活", "灰度")]
        if 已有:
            return {"允许": 假, "理由": f"候选版本已发布: {候选}", "错误码": "版本已存在"}
        return {"允许": 真, "理由": "发布条件满足"}
    ...
    def _资源判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 缺失:
            return {"允许": 假, "理由": f"资源预算缺少必需项: {缺失}", "错误码": "预算不完整"}
        return {"允许": 真, "理由": "资源预算完整"}
