...
class 策略中心:
    ...
    def _复用判定(self, 能力id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 记录["裁决状态"] == "待裁决":
            return {"允许": False, "理由": "疑似重叠能力未裁决，禁止发布", "错误码": "PENDING_RULING"}
        if 记录["复用裁决"] == "拒绝":
            return {"允许": False, "理由": f"维护者裁决拒绝: {记录['复用裁决']}", "错误码": "REUSE_DENIED"}
    ...
    def _依赖判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        缺失 = 声明 - 已存在
        if 缺失:
            return {"允许": False, "理由": f"依赖能力未登记: {缺失}", "错误码": "DEPENDENCY_MISSING"}
        return {"允许": True, "理由": "依赖全部已登记"}

    def _签名判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        if not 请求.get("已签名"):
            return {"允许": False, "理由": "未签名包禁止安装/激活", "错误码": "NOT_SIGNED"}
        if 请求.get("签名失效"):
            return {"允许": False, "理由": "签名失效（内容/权限/预算变化）", "错误码": "SIGNATURE_INVALID"}
        发布者 = 请求.get("签名者", "")
        信任 = self.状态.读取记录("信任", "发布者", 发布者)
        if 信任 is None or 信任["状态"] != "有效":
            return {"允许": False, "理由": f"发布者不在信任目录: {发布者}", "错误码": "UNTRUSTED_PUBLISHER"}
    ...
    def _发布判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 需求id:
            ...
            if 需求 is None or 需求["确认状态"] != "已确认":
                return {"允许": False, "理由": "需求未确认，禁止发布", "错误码": "REQUIREMENT_UNCONFIRMED"}
        候选 = 请求.get("候选版本", "")
        已有 = [记录 for 记录 in self.状态.查询记录("发布", "包id=?", (包id,))
                if 记录["期望版本"] == 候选 and 记录["状态"] in ("激活", "灰度")]
        if 已有:
            return {"允许": False, "理由": f"候选版本已发布: {候选}", "错误码": "VERSION_EXISTS"}
    ...
    def _资源判定(self, 包id: str, 请求: dict[str, Any]) -> dict[str, Any]:
        ...
        if 缺失:
            return {"允许": False, "理由": f"资源预算缺少必需项: {缺失}", "错误码": "BUDGET_INCOMPLETE"}
