...
class 平台客户端制品接入:
    ...
    （键表见 `激活指针.指针键表`），**本模块是制品根那一份的唯一写入点**，落盘前
    一律经 `规范化指针文件` 归一到同一 schema（历史键 `路径` 原位归一为 `制品目录`，
    ...
    # ---- 回收（唯一实现见 制品保留.py；本模块是「必经路径」的接线点） ----
    ...
    def _身份目录名表(self, 摘要表: tuple[str, ...] | list[str]) -> list[str]:
        ...
        见 `入库` 的制品名组装）。只保护摘要本体，身份目录照样被回收删掉，表现为
        「安装成功但紧接着 `指向的制品目录不存在` / `制品目录缺失（半成品）`」。
        查不到记录的摘要（如伪造摘要）不产出名字，不掩盖，也不瞎猜。
        """
        名字表: list[str] = []
        for 摘要 in 摘要表:
            摘要 = str(摘要 or "").strip()
            if not 摘要:
                continue
            记录 = self.状态.读取记录("制品", "制品摘要", 摘要)
            身份 = str((记录 or {}).get("版本", "") or "")
            if 身份:
                名字表.append(f"{self.包id}-{身份}")
        return 名字表

    def 回收过期制品(self, *, 触发点: str = "手动", 写证据: bool = True,
        ...
        **任何一次入库/安装/清点动作都会经过同一个回收入口，且有上限**（`回收上限`）。
        ...
        保留项怎么算（**三路并集，每路都要按两种名称形态展开，缺一路就会删掉手上的件**）：
        ...
        第 1 路的当前摘要也要经 `_身份目录名表` 换算：环境的 `制品目录` 字段一旦
        被写错（现场测试与真实故障都会发生），身份目录就失去唯一的名字来源，
        只剩摘要可追 —— 不换算等于「只要指针坏过一次，制品就被回收吃掉」。
    ...
    def 规范化指针文件(self, 指针文件: Path, 触发点: str = "手动") -> tuple[bool, str]:
        ...
        **不重写文件、不备份**（避免每次必经路径都产生一份无用备份）。
        ...
        形态 = 指针形态(指针文件)
        if not 形态["存在"]:
            return False, f"指针文件不存在（无历史写法可归一）: {指针文件}"
        if not 形态["可读"]:
            return False, f"指针文件不可读，拒绝覆盖（请人工确认）: {指针文件}"
        if 形态["已统一"]:
            return True, f"指针已是统一 schema（无需归一）: {指针文件}"
        ...
        try:
            ...
            return False, f"归一前备份失败，已放弃改写（原文件未动）: {错误}"
        try:
            写激活指针(指针文件, 原始)
        except OSError as 错误:
            return False, f"归一写入失败（备份留在 {备份}）: {错误}"
        归一 = 归一激活指针(原始)
        return True, (f"指针已归一到统一 schema（触发点 {触发点}）："
    ...
    def _安装到环境主体(self, 制品摘要: str, 制品名: str | None = None) -> tuple[bool, str, Path | None]:
        ...
        if not 制品目录.is_dir():
            return False, (f"制品目录缺失（半成品）: {制品目录.name}；"
    ...
    # ---- 校验（陈旧指针拒绝 / 外部调用方稳定路径） ----
    def 诊断激活指针(self, *, 自动收尾: bool = True) -> dict[str, Any]:
