...
class 包仓库(签名能力, 安装能力):
    ...
    def 可复现校验(self, *, 包id: str, 版本: str, 文件表: dict[str, str],
        ...
        """同源码同输入连续构建摘要一致；输入变化摘要变化。"""
        _, _, 摘要1 = self.构建制品(包id=包id, 版本=版本, 文件表=文件表, 构建输入=构建输入)
        _, _, 摘要2 = self.构建制品(包id=包id, 版本=版本, 文件表=文件表, 构建输入=构建输入)
        if 摘要1 != 摘要2:
            return False, f"同输入构建摘要不一致: {摘要1} vs {摘要2}"
        文件表2 = dict(文件表)
        文件表2["新增文件.txt"] = "x"
        _, _, 摘要3 = self.构建制品(包id=包id, 版本=版本 + "b", 文件表=文件表2, 构建输入=构建输入)
        if 摘要3 == 摘要1:
