...
def 落盘同步目录(目录: Path) -> None:
    """发布前落盘屏障：逐文件 fsync + 逐层目录 fsync。

    为什么必须有（未完成事项 7.9.1 #162 / #163）：`os.replace` 只保证「改名这个动作」原子，
    不保证被改名目录里的**文件内容与目录项已经落盘** —— 掉电/内核崩溃后可能出现
    「内容寻址目录已存在、内容却不全」的半成品制品，而本包判「已有一份可用的同版制品」
    挂的判据正是 `.is_dir()`，于是半成品会被后续构建当成成品**幂等复用**。
    故发布（改名）前必须先落盘，再改名。

    本函数是包内唯一**整树落盘屏障**实现（`服务.py` 与 `版本仓库.py` 共用），不另立第二套；
    口径与 `平台客户端制品._原子写入安装目录` 的「逐文件 fsync + 目录 fsync」一致，
    并补齐子目录层（子目录的目录项同样要落盘）。**目录项那一段不自建**：转调唯一腿
    `公共契约/运行时/同步目录.py::同步目录项`（`留痕位置` 传本模块原口径）；本层只留
    「递归遍历 + 逐文件 fsync」这一件本层才有的事 —— 整函数不能做成一行转发（那会丢
    逐文件 fsync，正是 #162/#163 的缺陷本体）。

    best-effort：平台不支持目录 fsync（Windows）时如实记忽略，不假装成功、也不炸掉发布。
    """
    路径表 = sorted(目录.rglob("*"))
    for 路径 in 路径表:
        if not 路径.is_file():
    ...
    # 本函数自己只留「整树落盘屏障」这一件**本层才有**的事（递归遍历 + 逐文件 fsync）。
    from 公共契约.运行时.同步目录 import 同步目录项
    for 子目录 in [路径 for 路径 in 路径表 if 路径.is_dir()] + [目录]:
        同步目录项(子目录, 留痕位置="包仓库.落盘同步目录")
...
class 包仓库(签名能力, 安装能力):
    ...
    def 构建制品(self, *, 包id: str, 版本: str, 文件表: dict[str, str],
        ...
        try:
            ...
            if not 最终目录.exists():
                # 落盘屏障（未完成事项 7.9.1 #162）：改名**之前**先把文件内容与目录项 fsync，
                # 否则「已发布的目录」可能内容不全，而 `.is_dir()` 判据会把它当成品复用。
                落盘同步目录(临时目录)
                try:
                    os.replace(临时目录, 最终目录)
                except OSError:
                    # check-then-act 竞态兜容：并发构建同一摘要时对方可能已抢先发布。
                    # 改名原子 ⇒ 最终目录一旦存在就是**一份完整制品**（临时目录先写满再改名），
                    # 故此时按「对方已发布」收敛，只清掉自己的临时目录。
                    # 诚实标注：POSIX 上 os.replace 与 os.rename 是同一个 rename(2)，对**已存在
                    # 非空目录**同样报 ENOTEMPTY —— 真正的兜容来自改名失败后的这次复查，
                    # 不是 os.replace 本身；换成加锁也可，本处选无锁兜容以避免多引入一把跨进程锁。
                    if not 最终目录.exists():
                        raise
                    清只读后删除树(临时目录, 忽略失败=真)
            else:
