...
恢复结论三态（`恢复` 返回值的 `状态` 键）：`通过` / `未核验` / `失败`。
**空表 = 没有可核验对象 = 未核验** —— 既不算「通过」也不算「失败」（哲学 1.4
空集 ≠ 通过 + 平台五态结论：干净通过 / 债务冻结 / 只报告 / 未核验 / 失败）。
`成功` 键在未核验时仍为 假（未核验绝不能被读成通过），但它与「失败」靠
`状态`/`是否未核验`/`结果条数`/`状态说明` 四键可当场分辨，不再被笼统报成失败。
"""
...
class 恢复执行能力:
    ...
    def 恢复(self, 备份目录: Path | str, 目标目录: Path | str) -> dict:
        ...
        if 占用原因:
            ...
            return {"成功": 假, "状态": 状态_失败, "是否未核验": 假, "结果条数": 1,
                    "状态说明": 占用原因, "恢复顺序": 顺序类表,
        ...
        结果条数 = len(结果表)
        if 结果条数 == 0:
            # 空集 ≠ 通过（哲学 1.4）：没有可核验对象 ⇒ 未核验态。
            状态 = 状态_未核验
            是否未核验 = 真
            状态说明 = ("空表 = 没有可核验对象，属未核验，不是失败也不是通过"
                      "（哲学 1.4 空集 ≠ 通过）")
        elif all(项["成功"] for 项 in 结果表):
            状态 = 状态_通过
            是否未核验 = 假
            状态说明 = ""
        else:
            ...
            是否未核验 = 假
            状态说明 = ""
        # `成功` 在未核验时为 假：任何只看布尔的调用方都不可能把空集读成通过；
        # 要分辨「未核验」与「失败」必须读 状态/是否未核验/结果条数/状态说明。
        return {"成功": 结果条数 > 0 and all(项["成功"] for 项 in 结果表),
                "状态": 状态, "是否未核验": 是否未核验, "结果条数": 结果条数,
                "状态说明": 状态说明,
