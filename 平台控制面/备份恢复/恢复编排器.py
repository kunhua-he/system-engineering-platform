...
class 新机器恢复编排器:
    ...
    def 编排恢复(self, 恢复输入: dict) -> dict:
        ...
        步骤结果["校验依赖锁定"] = {"成功": 真, "包数": len(锁对象["包列表"])}
        清空目录(空目录)
        ...
        步骤结果["写入依赖锁定"] = {"成功": 真, "相对路径": 依赖锁定文件名}
        步骤结果["恢复备份"] = self._恢复备份(备份目录, 空目录) if 备份目录 is not None \
            else {"成功": 真, "是否执行": 假, "文件数": 0, "失败原因": []}
    ...
    def _恢复备份(self, 备份目录: Path, 恢复目录: Path) -> dict:
        ...
        清单路径 = 备份目录 / 备份清单文件名
        if not 清单路径.is_file():
            return {"成功": 假, "是否执行": 真, "文件数": 0, "失败原因": [f"备份缺少清单文件: {备份清单文件名}"]}
        try:
            条目表 = json.loads(清单路径.read_text(encoding="utf-8")).get("条目", [])
        except json.JSONDecodeError as 错误:
            return {"成功": 假, "是否执行": 真, "文件数": 0, "失败原因": [f"备份清单不是合法 JSON: {错误}"]}
        文件数 = 0
        失败原因: list[str] = []
        for 条目 in 条目表:
            相对 = 条目["相对路径"]
            ...
            文件数 += 1
        return {"成功": not 失败原因, "是否执行": 真, "文件数": 文件数,
                "失败原因": 失败原因}
