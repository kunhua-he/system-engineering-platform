"""备份执行能力：三类权威数据的备份写入与快照落盘（权威数据备份契约 mixin）。"""
...
class 备份执行能力:
    ...
    def _备份一类(self, 类名: str, 快照目录: Path) -> dict:
        ...
        # 源目录逐类取（见 类根解析.解析类根）：显式传根=由该根推出；未传=各类权威宿主。
        # 包仓库那一类的返回值**已经是源目录本身**（真实形态下制品仓库根之下没有
        # `制品/` 层），故不再拼 `文件名表["包仓库"]`；其余三类是「根 + 文件名」。
        源根 = self._类根(类名)
        源 = 源根 if 类名 == "包仓库" else 源根 / (
            "权威状态.db" if 类名 == "证据账本" else self.文件名表[类名])
