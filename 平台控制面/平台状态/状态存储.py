...
# ---- 占用租约的「盘上活跃」口径（唯一具名定义；表由本模块建，故 SQL 形态定在这里）----
# 唯一索引 `索引_占用活跃` 约束的是「盘上还标着活跃的行同能力唯一」这一**结构事实**，
# 所以它的 WHERE 子句只能看 `状态`、**不看过期时间**：
# - 看 `过期时间` 是错的：已过期仍写活跃的行会因此不再占唯一槽位，而它们恰恰要靠
#   「盘上活跃 + 应用层 CAS 回收」这对组合收掉（回收动作判据见 `能力目录/服务.已失效`，
#   入口门见同处 `回收能力过期租约`）；
# - 也做不到：SQLite 部分索引的 WHERE 必须是**确定性表达式**，不能引用会走的 now。
# Python 形态的同一口径（全仓唯一）：`平台控制面/能力目录/文件租约存储.盘上活跃`。
# **占用判定**（这条租约还挡不挡路：状态活跃 **且** 未过期）是另一个问题，唯一实现是同文件
# `是活跃` —— 结构层不做业务判定，也不复制它。
占用租约状态_活跃 = "活跃"
占用租约盘上活跃条件 = f"状态='{占用租约状态_活跃}'"
...
class 平台状态(权威状态):
    ...
    def _迁移到130(self, 连接) -> None:
        ...
        `索引_占用活跃` 的 WHERE 子句取自模块常量 `占用租约盘上活跃条件`（唯一具名定义，
        即「盘上活跃」口径：只看 `状态`，**不看过期时间**，理由见模块头部那段注释）。
        """
        连接.executescript(f"""
