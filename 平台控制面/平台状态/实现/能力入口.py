...
def 追加证据(存储目录: str = "", 证据类型: str = "", 主题: str = "", 内容: dict | None = None,
            操作id: str = "", 调用者: str = "", 角色: str = "", 结果: str = "",
            错误码: str = "") -> Any:
    """向平台证据账本追加一条证据（仅追加，旧证据不可覆盖）。"""
    from 公共契约.基础类型.结果类型 import 结果 as 结果类型

    if not 证据类型 or not 主题 or not isinstance(内容, dict):
        return 结果类型.失败("参数不合法", "证据类型/主题/内容必须给齐，内容必须是 JSON 对象",
                            来源=来源名称)
    状态, 失败 = _取状态(存储目录, "")
    if 状态 is None:
        return 失败
    try:
        证据id = 状态.追加证据(类型=证据类型, 主题=主题, 内容=内容, 操作id=操作id,
                            调用者=调用者, 角色=角色, 结果=结果, 错误码=错误码)
        证据表 = 状态.查询证据(主题=主题, 类型=证据类型, 限制=10)
    except sqlite3.Error as 错误:
...
def 查询证据(存储目录: str = "", 主题: str = "", 证据类型: str = "", 限制: int = 50,
            项目id: str = "") -> Any:
    ...
    if 状态 is None:
        return 失败
    try:
        证据表 = 状态.查询证据(主题=主题, 类型=证据类型, 限制=限制)
    except sqlite3.Error as 错误:
