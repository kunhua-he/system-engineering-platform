...
锁内重读磁盘全落本模块；**落盘动作一律转调唯一写腿**（`公共契约/运行时/写文件.py::
原子写文件`，唯一实现 = 同目录唯一临时名 + fsync + `os.replace` + 权限位恢复 + 目录项
fsync + 内核锁解锁窗口）；子类只管自己的数据语义
...
写侧原子性：**只转调唯一写腿**（`公共契约/运行时/写文件.py::原子写文件`，唯一实现 =
同目录唯一临时名 + `fsync` + `os.replace` + 权限位恢复 + 目录项 fsync + 内核锁解锁窗口）。
本模块原先自建过第二套「唯一临时名 + fsync + `os.replace`」骨架（固定临时名会让两个写者
往同一个临时文件里交错写内容，再把交错结果 replace 成正式存储——实测 120/120 轮把 JSON
写坏 `Extra data: line N column 1`），已随本次收口整段删除，不留第二套实现。
...
class 单文件互斥存储:
    ...
    def 原子写文本(self, 正文: str, 权限: int = 0o600) -> None:
        """原子替换：**转调唯一写腿**（`公共契约/运行时/写文件.py::原子写文件`）。

        原实现是本层自建的「唯一临时名 + fsync + os.replace」第二套落盘骨架（哲学 1.2：
        同一条腿两处实现即缺陷），且不含内核只读锁的解锁窗口。唯一写腿自带「解锁窗口 +
        同目录临时件 + fsync + os.replace + 权限位恢复 + 目录项 fsync + 收口锁态」。

        **锁闸门留在本层**（这是本层**才有**的判据，唯一写腿不做租约/锁判定）：
        锁不可用（`锁问题` 非空）时**拒绝落盘** —— 无锁写入正是丢更新的入口，宁可失败。
        仍抛 `OSError`，与改前逐字同型。

        **行为差异（就地补，不静默改权限）**：唯一写腿的权限位语义是「目标已存在则保持原
        权限位；新建文件由临时件决定 0600」，而原实现是 `os.open(..., 权限)`（受 umask
        作用、replace 时**无条件**把目标置成该权限）。故写完就地 `chmod` 到 `权限`，恢复
        「结果恒为 `权限`」的原语义（缺省 0600，存储文件只由本方法创建 ⇒ 新建档逐字等价）。
        残余差异（未自证）：`chmod` 在 `os.replace` 之后，失败时目标已是新内容。
        """
        锁问题 = self.锁问题()
        if 锁问题:
            raise OSError(锁问题)
        原子写文件(self.存储文件, 正文, 编码="utf-8")
        if 支持chmod():
            os.chmod(self.存储文件, 权限)
