...
当前轻代码前端编辑器已经有组件拖放、属性编辑、项目类型、自动保存和父容器字段，但还没有完成：
...
这五项是下一轮 IDE 升级的明确落点，不应继续增加与用户日常操作无关的高级配置。
...
当前轻代码编辑器的数据模型仍把 `组件id` 当身份，把 `组件列表` 当事实，并在浏览器内直接修改对象。这可以作为单人演示，但不满足“代码/窗体/模块多源修改不乱”的底座要求。

正式升级顺序应为：

1. 新增不可变 `对象id`，保留 `组件id` 作为显示兼容字段。
2. 把页面、程序集、成员、符号、引用收口为统一工程模型。
3. 增加统一编辑操作和修订号，所有视图只提交操作。
4. 将代码视图、窗体视图、属性表改成工程模型投影。
5. 增加冲突拒绝、撤销/重做、自动保存恢复和操作日志。
6. 最后再做界面像素级调整和组件箱扩充。

这部分完成前，不应继续扩充大量组件；否则只是把重复和冲突复制到更多组件上。
...
当前轻代码编辑器唯一写入口是 `POST /工程/请求`，请求以 `操作=提交变更` 携带 `工程id`、`基准修订号`、`请求id`、`来源` 和 `操作列表`。服务端补齐对象身份，使用修订号 CAS 原子提交，成功返回 `新修订号`，旧修订返回 HTTP 409 `编辑冲突`，并写入同目录 `编辑记录.jsonl`。读取、预览、提交和编译均使用这一入口，不保留第二套保存语义。
