...
结果.append(报("F-01 新语义：实际错误码=文本 \"123\"（合法文本）→ 应 200 且不命中",
             lambda st, d, v: st == 200 and v.get("是否重试") is False, st, d, t))

# 文本型参数收到非文本必须明确拒绝（网关唯一校验点；整数型传文本是反向对照，见下方第 5 条）
st, d, t = 调用("大语言模型支持库.模型连接器.重试调度",
              {"重试模式": "1", "错误码白名单": ["A"], "实际错误码": 123})
结果.append(报("F-01 类型铁律：实际错误码传整数 123（非文本）→ 应 400",
             lambda st, d, v: st == 400 and d.get("错误码") == "参数不合法", st, d, t))
