...
| 项目 | 现场事实 |
|---|---|
| 项目根 | `~/Documents/Agent/github 源码参考/90_历史与专项/非Git源码/Claude-Code` |
...
本轮按任务要求先调用 `project_context`，但专属 MCP 返回的绑定项目是 **`华世王镞_v3`**，根目录为 `~/Documents/Agent/PHP/华世王镞_v3`，MCP 实例标识为 `project_toolkit`，不是本任务目标。随后调用 `codegraph_explore` 时显式传入目标根，返回：目标目录不存在 `.codegraph/`，未建立代码图，不能查询符号或调用图。
...
| `git -C '~/Documents/Agent/github 源码参考/90_历史与专项/非Git源码/Claude-Code' status --short --branch` | 命令返回 Git fatal；目录确认为非 Git，不能记录 commit/branch |
...
| `project_context(task=...)` | 工具调用成功，但错绑 `~/Documents/Agent/PHP/华世王镞_v3`；不作为本项目代码图证据 |
