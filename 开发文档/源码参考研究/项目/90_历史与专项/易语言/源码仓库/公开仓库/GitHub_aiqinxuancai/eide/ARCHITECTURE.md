...
本节是第三轮补充，不是把 eide 宣称成完整 IDE 平台。映射规则是：先记录源码已有边界，再判断它可归入开发工具支持库、开发工具模块、运行核心或网关；没有源码证据的项目模型、编译器、终端、任务调度和 HTTP/CLI 网关明确标为“缺失/待核”。本轮只修改本文件；此前 `project_context` 错绑到 `/Users/hekunhua/Documents/Agent/PHP/华世王镞_v3`，代码图结果不作为 eide 证据，改用目标仓库本地静态取证。
