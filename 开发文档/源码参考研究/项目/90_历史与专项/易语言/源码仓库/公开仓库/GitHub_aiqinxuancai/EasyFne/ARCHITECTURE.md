...
> 本文件是 EasyFne 当前源码的唯一架构事实源。本次只新增本文件；未修改源码、依赖、测试、配置或 Git 历史，未删除任何历史研究文件。
...
7. 未发现 `历史研究-*.md`；因此没有历史研究可吸收或删除。

## 11. 当前裁决：FNE/DLL ABI、加载、注册与生命周期

> 本节是基于当前仓库源码的当前裁决增量，不把平台的 L0-L4 当成 EasyFne 已有实现。源码没有 HTTP/RPC、句柄注册表、受管子进程或统一网关；下文的底座归属是映射与裁决。`project_context` 当前审计错误绑定到 `/Users/hekunhua/Documents/Agent/PHP/华世王镞_v3`，该环境问题不作为 EasyFne 证据；以下证据全部重新取自本目录静态源码，因此验证强度为“源码存在/未运行”。
...
当前审计结论只能写成“静态证据充分、运行验证缺失”：成功注册链和失败/释放缺口均来自源码；没有把未构建、未加载、未崩溃注入的场景记为通过。
...
仓库生产代码集中于 `EasyFne.cpp`、`main.cpp` 和 `elib/` 头/源文件；工程文件约束 Win32 配置、导出定义与预编译宏。`EasyFne.cpp:63-230` 负责注册、通知和命令表装配，`main.cpp:1-16` 提供 DLL 初始化薄入口，`EasyFne.def:1-3` 固定导出符号。

`elib/fnshare.cpp:1-71` 处理宿主共享内存和回调辅助，`elib/lib2.h:1-1598` 定义 `LIB_INFO`、`MDATA_INF` 等 ABI 结构；任何字段顺序、对齐、指针宽度变化都会导致易语言宿主加载后崩溃。`PublicIDEFunctions.h` 是 IDE 通知协议边界，必须与目标 IDE 版本配套。

构建矩阵包含 `fne_debug/release` 与 `static_lib_debug/release`，均为 Win32；当前仓库未提供跨平台实现和自动化宿主测试。静态证据能确认文件、导出和调用约定，不能证明真实注册握手、内存释放、卸载顺序或异常隔离。

## 14. 失败与复用启示

空回调、错误命令索引、重复注册、分配失败、宿主提前卸载和通知线程竞态是主要失败面；应在宿主测试中记录返回值、分配/释放配对、线程状态和 DLL 引用计数。平台复用时只能吸收“版本化 ABI + 受管生命周期 + 结果复制”模式，不能直接把裸指针、全局回调或 Win32 DLL 作为公共能力。
