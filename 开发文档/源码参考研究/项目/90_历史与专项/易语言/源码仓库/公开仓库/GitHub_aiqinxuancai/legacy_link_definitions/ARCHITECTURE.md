...
- 项目根：`/Users/hekunhua/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/GitHub_aiqinxuancai/legacy_link_definitions`
...
源码没有显式写 `__cdecl`、结构体打包、位宽别名、`noexcept`、版本号或能力标识。Win32 下默认调用约定和符号前缀、x64 下统一调用约定等细节不能仅凭 README 视为已兼容；当前审计没有 `dumpbin /symbols`、实际 `.lib` 或消费者链接结果，相关判断均标记为未验证。
...
|---|---|---|---|---|---|
| MSBuild 编译输入 | 编译两个 `.cpp`，归档为静态库 | 头文件、Windows SDK、工具集或架构不匹配则构建失败 | 非运行时操作；取消由构建工具负责 | 构建工具负责临时文件；源码没有清理钩子 | `.vcxproj` L2；当前审计未构建 |
...
## 14. L0-L4 证据等级与当前审计结论

当前审计采用以下等级，避免把声明当实现或把静态存在当运行通过：
...
**当前裁决可吸收结论**：实现性质上属于 ABI 支持库中的最小静态兼容 shim；`.sln`/`.vcxproj` 属于开发工具的生成输入；它不构成运行核心，也没有运行时加载契约。**待核结论**：能否准确满足目标 VC6 静态库的实际外部引用、x86/x64 下的符号拼写、`long&` 返回 ABI、各 CRT/工具集组合以及 `_get_timezone` 失败语义，必须在 Windows/MSVC 与真实消费者上升到 L3/L4 后才能裁决。
...
开工 `project_context` 返回了不相关的 `/Users/hekunhua/Documents/Agent/PHP/华世王镞_v3` 项目上下文；该错绑结果已按要求**不作为本仓库证据**，当前审计改以目标目录的本地源码、工程文件和 Git 现场静态取证，并将链接/运行结论标为 L0-L2 或待核。目标仓库当前审计唯一修改文件仍是根目录 `ARCHITECTURE.md`。
