...
| 项目 | 事实 |
|---|---|
| 本地路径 | `~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/script` |
| 分支 | `master` |
...
仓库内没有 `README`、`AGENTS.md`、测试文件、CI 配置或旧 `细探*.md`；也没有既有 `ARCHITECTURE.md`。本轮不删除任何旧细探文件。
...
本轮仅做了只读人工源码、工程和 Git 取证，并确认：文件清单、项目配置、命令/属性元数据、入口导出、通知链和占位实现。未在 macOS 上运行 Visual Studio/MSBuild、易语言 IDE、`.fne` 装载、脚本引擎或 Windows 宿主测试；因此以下均为**未验证**：
...
本轮没有修改源码、工程、依赖、测试、配置或 Git；唯一新增文件是本项目根目录的 `ARCHITECTURE.md`。后续架构更新应继续维护本文件，旧细探如未来出现，须先逐份核对源码后再收口，不能形成第二事实源。
