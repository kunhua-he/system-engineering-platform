...
> 本文件是 `iconv` 仓库的唯一架构事实源。源码审计建档依据当前工作树中的源码、工程文件、构建日志和 Git 元数据；未运行或未能由源码直接证明的内容明确标为“未验证”。旧研究材料文件当前验证未发现，因此无可吸收/删除项。后续深挖只更新本文件，不复制出平行架构文档。
...
仓库没有 `README`、`LICENSE`、测试目录、测试脚本、CI 配置或旧 `研究材料-*.md` 文件。
...
`iconv.log` 记录了一次 Windows Visual Studio Release 构建：编译了 `fnshare.cpp`、5 个根/桥接源文件、4 个 `cppCode` 文件、`iconv.c`、`localcharset.c`，并记录输出 `E:\支持库\功能库\iconv\Release\iconv.fne`。日志包含大量 C4819（代码页字符）、C4018（有符号/无符号比较）和 C4090（const 限定符）警告；日志没有显示构建失败，但这是历史日志，不是当前验证在当前环境重新执行的结果。
...
- 当前验证未在 macOS 上执行 Visual Studio/MSBuild，也未加载 Windows 二进制；因此未验证 DLL ABI、易语言 IDE 加载、命令实际分派、中文/GBK 文件转换、全部编码兼容性和 x64 构建。
...
- 仓库：`~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/iconv`
- 分支：`master`
...
- cppCode/iconv_0_iconv.cpp 至 iconv_3_iconv_close.cpp 和 libiconv/iconv.c 是当前仓的主要实现证据。
- iconv_dllMain.cpp、Source_iconv.def 和 iconv.vcxproj 定义 DLL 生命周期、导出边界和 Visual Studio 构建；cmdDef/cmdInfo/dtType 文件分别承载执行、元数据和类型表。
...
- iconv 的调用依赖 Windows、易语言运行时、正确位数和调用约定；当前环境未执行 Windows 构建或宿主装载。
...
- 当前 HEAD 为 f9702c446345f56b7cb3cf43cb0680cc4102a824；本仓此次只更新根 ARCHITECTURE.md，保留源码事实，不把历史快照或二进制当作运行验证。
...
- ./Project6.dll
- ./Source_iconv.def
- ./cppCode/iconv_0_iconv.cpp
- ./cppCode/iconv_1_iconv_open.cpp
- ./cppCode/iconv_2_iconv_iconv.cpp
- ./cppCode/iconv_3_iconv_close.cpp
...
- ./iconv_vc6.e
- ./iconv_vc6.exe
- ./include_iconv_header.h
- ./libiconv/Makefile.in
- ./libiconv/aliases.gperf
- ./libiconv/aliases.h
- ./libiconv/aliases2.h
- ./libiconv/aliases_aix.h
- ./libiconv/aliases_aix_sysaix.h
- ./libiconv/aliases_dos.h
- ./libiconv/aliases_extra.h
- ./libiconv/aliases_osf1.h
- ./libiconv/aliases_osf1_sysosf1.h
- ./libiconv/aliases_sysaix.gperf
- ./libiconv/aliases_sysaix.h
- ./libiconv/aliases_syshpux.gperf
- ./libiconv/aliases_syshpux.h
- ./libiconv/aliases_sysosf1.gperf
- ./libiconv/aliases_sysosf1.h
- ./libiconv/aliases_syssolaris.gperf
- ./libiconv/aliases_syssolaris.h
- ./libiconv/aliases_zos.h
- ./libiconv/armscii_8.h
- ./libiconv/ascii.h
- ./libiconv/atarist.h
- ./libiconv/big5.h
- ./libiconv/big5_2003.h
- ./libiconv/big5hkscs1999.h
- ./libiconv/big5hkscs2001.h
- ./libiconv/big5hkscs2004.h
- ./libiconv/big5hkscs2008.h
- ./libiconv/c99.h
- ./libiconv/canonical.h
- ./libiconv/canonical_aix.h
- ./libiconv/canonical_aix_sysaix.h
