...
> 本文件是 `ethread` 项目的唯一架构事实源。本轮为首轮全量只读建档；后续细探只增量维护本文件，不再建立平行架构报告。
...
| 项目 | 现场事实 |
|---|---|
| 本地根目录 | `/Users/hekunhua/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/ethread` |
...
因此，本文件的“已确认”是源码/工程/元数据静态证据，“运行正确”“构建通过”“兼容 x64/Linux”均尚未得到本轮执行证据。
...
5. 若需要代码图，将由用户决定是否在该外部仓库初始化 `.codegraph/`；本轮不创建索引目录，避免把研究副产物写入源码参考库。
...
> 已吸收本轮所有旧细探线索（目标根未发现旧细探文件）；后续只维护本文件。
