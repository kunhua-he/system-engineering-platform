...
- **边界**：本文件是项目根目录唯一架构事实源。当前核对只新增/更新本文件，未修改源码、工程、依赖、测试、配置或 Git 历史；未删除任何旧细探文件（现场未发现旧细探文件）。
...
5. **x64 预处理器风险（重要）**：两个 x64 配置没有定义 `__E_FNENAME`，会触发 `elib/lib2.h:23-25` 的硬错误；必须在目标 Windows 工具链中确认或修正后才能宣称 x64 支持。当前核对按边界未修改工程。
