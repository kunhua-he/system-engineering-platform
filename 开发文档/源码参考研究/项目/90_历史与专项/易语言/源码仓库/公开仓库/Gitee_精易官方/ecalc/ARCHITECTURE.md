...
当前验证遵守“不安装、不构建、不改源码、不提交 Git”的边界，仅执行了现场盘点和静态读取：
...
目标项目根目录未发现 `研究材料-*.md` 或其他旧架构文档；在源码参考库范围按 `ecalc` 文件名检索也未发现与本项目对应的旧研究材料文件。本文件因此直接以当前源码、工程文件和 Git 远程基线为唯一事实源。
...
| 工作树 | 建档前为 clean；当前验证只新增本文件，未修改已有源码、工程或 Git 提交 |

本文件是 ecalc 根目录唯一架构事实源。后续深挖应直接增量更新本文件，不再新建平行“研究材料”架构文档。
## 14. 当前源码级收口

### 14.1 入口与 ABI

- ecalc_cmdDef.cpp、cmdInfo.cpp、include_ecalc_header.h 是当前仓的主要实现证据。
- ecalc_dllMain.cpp、Source_ecalc.def 和 ecalc.vcxproj 定义 DLL 生命周期、导出边界和 Visual Studio 构建；cmdDef/cmdInfo/dtType 文件分别承载执行、元数据和类型表。
...
- ecalc 的调用依赖 Windows、易语言运行时、正确位数和调用约定；当前环境未执行 Windows 构建或宿主装载。
...
- 当前 HEAD 为 492603fce0104282cc96e5e54fed7a9a54574f82；本仓此次只更新根 ARCHITECTURE.md，保留源码事实，不把历史快照或二进制当作运行验证。
...
- ./ecalc_dllMain.cpp
- ./ecalc_dtType.cpp
- ./ecalc_static/ecalc_static.vcxproj
- ./ecalc_static/ecalc_static.vcxproj.filters
- ./ecalc_static/ecalc_static.vcxproj.user
