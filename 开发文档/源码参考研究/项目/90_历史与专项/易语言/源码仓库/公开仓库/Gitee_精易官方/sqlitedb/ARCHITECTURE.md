...
- 仓库：`~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/sqlitedb`
...
- 提交者：`精易科技`
- 提交说明：`初始化仓库`
- 本轮开始时工作树：干净；此前不存在 `ARCHITECTURE.md`。
- 本轮允许变更：仅新增根目录 `ARCHITECTURE.md`；不删除旧细探（现场未发现旧细探文件）。
...
> 本轮没有旧 `ARCHITECTURE.md` 或 `细探-*.md` 可吸收；后续细探应增量维护本文件，不另建平行架构事实源。
