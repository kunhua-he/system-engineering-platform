...
本轮只做只读取证，没有安装依赖、没有启动服务、没有生成构建产物，也没有修改源码、工程、依赖、测试、配置或 Git 历史。已完成：
...
5. 搜索端口实现、`port95nt`、TODO、测试和既有 `细探-*.md`。
...
7. **无测试与样例**：没有自动化回归、硬件模拟、加载探针或 ABI 断言。任何后续实现都应至少增加隔离的参数/返回值测试、导出检查和无硬件失败路径验证，但本轮不修改测试目录。
...
- 本地仓库：`~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/portio`
...
- 文档材料：建档前工作树干净；本轮未发现已有 `ARCHITECTURE.md` 或 `细探-*.md`，因此不存在旧细探可吸收或删除。本文件是本轮唯一新增文件。
...
本文件已完成首轮全量架构建档。后续如继续研究，只能在本文件内增量补证，不另建平行事实源：

- 第二轮：在 Windows 隔离环境中沿 `GetNewInf`、通知回调和命令函数做真实加载/调用取证，补返回值、错误码和资源释放；
- 第三轮：核查 `port95nt.exe` 或替代驱动的真实接口、权限、并发、故障恢复和硬件安全边界；
- 第四轮：分别验证 Win32/x64 动态导出、静态链接、易语言 IDE 登记和版本兼容；
- 第五轮：如有跨项目共性，只将裁决写入源码参考库的 `00_catalog/` 专题文档，单仓事实仍保留在本文件。
