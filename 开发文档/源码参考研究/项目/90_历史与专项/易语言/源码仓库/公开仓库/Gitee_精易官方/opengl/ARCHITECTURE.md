...
本文件是仓库根目录唯一的架构事实文档，依据当前工作树源码、Visual Studio 工程文件和 Git 元数据建立。源码参考库按只读研究处理；本轮只新增本文件，不修改源码、工程、依赖、测试、配置或 Git 历史。
...
- **仓库根目录**：`/Users/hekunhua/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/opengl`
...
- **建档前工作树**：干净，仅显示 `## master...origin/master`；本轮仅新增/更新根目录 `ARCHITECTURE.md`。
...
> 维护规则：本文件只记录当前源码可验证的架构事实；“命令说明想做什么”与“命令函数实际做了什么”必须分栏表达。旧细探文件不存在，本轮没有删除任何旧研究材料。
