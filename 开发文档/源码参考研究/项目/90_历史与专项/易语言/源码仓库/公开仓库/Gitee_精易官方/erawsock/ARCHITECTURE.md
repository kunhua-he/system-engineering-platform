...
> 本文件是 `erawsock` 项目根目录唯一的架构归档文件。本文档依据本地源码、Visual Studio 工程文件和 Git 元数据建立；当前验证仅做只读取证，没有修改源码、依赖、测试、配置，没有安装、构建或提交。
...
| 项目 | 现场事实 |
|---|---|
| 本地根目录 | `~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/erawsock` |
...
本地没有发现 `README.md`、`AGENTS.md`、`CLAUDE.md` 或旧 `研究材料-*.md`；因此本档案没有可吸收的项目级旧研究材料内容。远程版本只通过 `git ls-remote` 复核，未执行 fetch、pull 或构建。
...
- 当前验证未安装 Visual Studio/Windows SDK/WinPcap，未执行构建、链接、加载 DLL、易语言 IDE 验证或真实网络收发。
...
- erawsock_dtType.cpp:9-191 的 RawSocket、ARP 和协议头对象 是当前仓的主要实现证据。
- erawsock_dllMain.cpp、Source_erawsock.def 和 erawsock.vcxproj 定义 DLL 生命周期、导出边界和 Visual Studio 构建；cmdDef/cmdInfo/dtType 文件分别承载执行、元数据和类型表。
...
- erawsock 的调用依赖 Windows、易语言运行时、正确位数和调用约定；当前环境未执行 Windows 构建或宿主装载。
...
- 当前 HEAD 为 e0b7f5f1b9c896145d0da97d1eadfc3e71fef4bd；本仓此次只更新根 ARCHITECTURE.md，保留源码事实，不把历史快照或二进制当作运行验证。
...
- ./erawsock.sln
- ./erawsock.vcxproj
- ./erawsock.vcxproj.filters
- ./erawsock.vcxproj.user
- ./erawsock_cmdDef.cpp
- ./erawsock_cmdInfo.cpp
- ./erawsock_cmd_typedef.h
- ./erawsock_const.cpp
- ./erawsock_dllMain.cpp
- ./erawsock_dtType.cpp
- ./erawsock_static/erawsock_static.vcxproj
- ./erawsock_static/erawsock_static.vcxproj.filters
- ./erawsock_static/erawsock_static.vcxproj.user
- ./include_erawsock_header.h
