...
本节是**跨项目映射建议，不是 Cognee 的项目事实**。前文带有源码路径、函数名、测试路径的内容属于 `[项目事实]`；本节“建议进入支持库/模块库/运行核心/网关”属于 `[平台建议]`。平台建议不能反向证明 Cognee 已经接入目标平台，也不能作为平台现有能力、契约或已验收实现的证明。除本文件外，当前核对没有修改 Cognee 源码、依赖、配置、测试或 README，也没有把建议写入平台生产目录。
...
| ontology、truth_subspace、memify/self-improvement、skills、Slack、前端、部署模板 | `cognee/modules/{ontology,truth_subspace,memify}`、`cognee-frontend/`、`distributed/deploy/` | 仅研究参考 | 暂不进入底座 | 需要独立需求、契约和真实外部依赖验收；当前核对没有足够证据证明它们是平台公共原子能力 |
...
| `search()` / `recall()` | `recall` 规范化 scope，合并 session/trace/session_context/graph/tools，并用 source discriminator；graph 路径先授权 dataset 再设 ContextVar 和 retriever | scope、授权、provider 不可用、LLM 失败的最终统一错误形状需按各入口分别验证；当前核对没有 API smoke、真实 graph/vector、MCP handshake，因此不宣称端到端返回稳定 | 源码事实；未执行 |
| `serve()` / API / MCP | SDK 可切换 direct/cloud；FastAPI 注册 `/api/v1/*`；MCP 支持 stdio/SSE/HTTP，direct/API 双模式 | remote 下部分本地能力显式不支持；网络断开、连接超时、客户端断开后的请求/流式清理没有由当前核对统一测试证明；各入口错误映射不能直接当平台网关错误码 | 源码事实；未执行 |
...
| `add` 入口 | 可解析输入、授权 dataset、系统 setup | 建 `resolve_data_directories` 与 `ingest_data` 任务，执行 add pipeline | 失败由 pipeline 状态/异常路径承担；当前核对未执行真实 loader | 本文 §2.3；`cognee/api/v1/add/add.py` |
...
| API shutdown | FastAPI lifespan 正常退出 | cache-clear graph/vector engine，意图是让存储 WAL 在退出前完成收尾 | SIGKILL 等非正常退出不经过 lifespan；当前核对没有强杀后 WAL/残留实测 | `cognee/api/client.py:66-107` |
...
| graph/vector engine | dataset ContextVar 选择；统一 engine 持有 graph/vector adapter | API lifespan 对缓存工厂 `cache_clear()`；正常 run 在 Completed 前调用 push | graph 删除失败保留关系回滚元数据；非正常进程退出的 WAL/adapter 清理未由当前核对执行证明 | 不能把 adapter 接口存在写成后端已安装/可用 |
...
| 临时/生成文件 | loader、技能 materialize、enola snapshot、数据库后端各自创建 | 由各子模块决定；当前核对只确认技能目录按 dataset hash 稳定命名、enola 产物目录存在 | 没有全仓统一“异常后残留扫描”证据；崩溃可能留下 snapshot、锁或 WAL | 不声称零临时文件、零锁、零进程残留 |
...
| provider 缺失/后端不可用 | optional extras 和 adapter 在运行时决定；部分入口会抛配置/系统错误 | 未在当前核对安装或探测真实 provider | 需要缺库、空 key、断网和 backend unsupported 的非 skip 测试；“导入成功”不是 provider 可用 |
...
| 流/客户端断开 | MCP/API 有多种 transport；当前核对没有统一客户端断开/半关闭测试 | 仅源码证据 | 需真实连接断开后确认服务端 task、socket、队列和临时资源收敛 |

#### L0-L4 验证等级

| 等级 | 含义 | 当前核对可给 Cognee 的判定 |
|---|---|---|
...
| L2 | 定向测试源码存在，且测试断言对应真实生产接口 | remember background anchoring、recovery、rollback、cognify routing 等达到“有 L2 测试源码”；当前核对未执行，不能写“L2 已通过” |
| L3 | 当前核对真实执行，含真实数据库/图/向量/HTTP/MCP/外部进程，并读回状态 | 当前核对未执行；所有项目能力均不得标 L3 |
| L4 | 独立进程强杀/取消/超时/重启，读回无半成品、无进程/锁/临时残留，并有可重复验收命令 | 当前核对未执行；崩溃恢复、取消和残留治理全部未达 L4 |

当前核对严格结论是：**源码和测试源码证明了若干设计意图，不等于当前 checkout 已通过真实运行验证**。旧细探和本文件中的“测试覆盖”章节只能说明测试源码存在；本次没有安装依赖、启动 API/MCP、连接真实 graph/vector/LLM、执行 pytest、模拟超时/取消、强杀恢复或做残留扫描。
...
| 归属 | 只允许接收的 Cognee 研究产物 | 当前核对是否进入目标平台 |
|---|---|---|
...
这份唯一归属清单完成后续裁决，但不构成平台改造授权。若后续要落地，必须另行登记能力需求、复用裁决、消费者契约、资源/验收契约和装配计划；在此之前，Cognee 只能作为研究输入。

## 12. 后续架构收口：真实契约、调用链与生命周期

本节是当前核对对真实源码的收口补充。它不把函数注释、README 或测试名称当成已运行证据；路径后的行号是当前核对读取的源码定位，验证等级仍按 §11.6 的 L0-L4 解释。凡标注“未运行”的内容，都只能说明静态行为已定位，不能说明当前环境可用。
...
| graph/vector adapter | dataset ContextVar 选择；`UnifiedStoreEngine` 持有 adapter | API lifespan 正常 shutdown `cache_clear()`；任务完成前 push | pipeline rollback 尝试 source-ref/pipeline-run 删除；并行写可能部分完成 | 仅局部 provider 可能有 timeout；顶层无 deadline/排空协议 | SIGKILL 不走 lifespan；WAL、锁、embedded engine 残留未做当前核对扫描 |
...
| enola 子进程/pipe | `create_subprocess_exec`，stdout/stderr PIPE | `communicate()` 收完，自然退出 | 非零码或缺 `facts.jsonl` 抛 `EnolaSnapshotError` | `wait_for` 超时后 `kill()` + `wait()`；只覆盖该子流程 | 父进程崩溃时当前核对没有证据证明子进程一定被回收 |

### 12.5 失败、超时、取消、崩溃的真实矩阵

| 场景 | 当前源码事实 | 当前核对验证等级 | 收口结论 |
|---|---|---:|---|
...
| 客户端断开/MCP 连接断开 | transport 支持 stdio/SSE/HTTP，但当前核对没有断连后 server task/socket/pipe 清理实测 | L0-L1 | 必须独立测试，不能由 transport 枚举推定安全 |

### 12.6 当前核对验证等级和禁止误读

| 证据项 | 现场事实 | 等级 |
|---|---|---:|
| 生产源码路径、函数签名、异常/资源分支 | 当前核对直接读取入口、pipeline、retriever、session、rollback、recovery、enola 源码 | L1 |
...
| 当前核对真实执行 | 未安装依赖，未启动 API/MCP，未连接真实 LLM/graph/vector/cache，未运行 pytest/ruff | 未达 L3 |
| 独立进程故障验证 | 未执行 SIGKILL、Task.cancel、provider timeout、客户端断连、重启后数据库/WAL/锁/临时目录扫描 | 未达 L4 |

因此当前核对收口后的最高诚实表述是：**Cognee 的入口契约、标准/DLT/code/temporal 摄取分支、graph/vector 写入边界、scope→授权→retriever→结果链和有限的 rollback/startup recovery 已达到源码级 L1；若对应测试文件的断言确实覆盖该路径，可标“存在 L2 测试源码”，但整个 checkout 仍未获得 L3/L4 运行验证。** 特别不能把 `PipelineRunErrored`、日志中的 rollback、`RememberResult.done` 或 API 已返回 Started 解读为“已清理、已成功或可取消”。
...
## 13. 后续源码复核增补：入口分支、任务持有和外部进程

本节是对上一节已记录事实的再次逐符号核对，不删除或改写 `细探-Cognee.md`。新增内容仍按“源码事实”和“未运行”分开：当前核对只读了本机 checkout，没有安装依赖、启动服务或执行测试。
...
完成事件前会先尝试 graph/vector 与 relational 的 `push_to_s3()`，push 失败仍归入失败路径；这强化了“Completed 不是在内存 task 完成时写入”的事实，但不改变跨存储非原子性。证据：`cognee/modules/pipelines/operations/run_tasks.py:63-225`。这部分已有测试源码线索，但当前核对仍未运行，最高只能记为 L1，或“存在 L2 测试源码”，不能记为 L2 通过。
...
- **L2（仅测试源码存在）**：后台任务 anchoring、cognify routing、rollback/recovery、session 和 provenance 相关定向测试仍可定位；当前核对没有执行，因此不写“通过”。
...
当前核对未验证项仍包括：依赖安装与版本解析、真实 provider 可用性、网络/密钥、跨 graph/vector/relational 一致性、取消后的终态和 sibling 排空、跨进程 lease、重复请求幂等、MCP/API 断连清理，以及完整测试套件结果。

## 14. 当前核对深度源码研究收口（2026-08-21）

本节是当前核对实际读取后的增补。它只修改本文件；`细探-Cognee.md` 保留不动。研究目标是从 `add/remember` 进入，经 `cognify`、pipeline、graph/vector/relational/cache，再到 `recall/authorized_search`，逐节点核对真实参数、返回、状态、并发、取消、超时、异常和资源 owner。所有等级均按 §11.6 定义；当前核对未安装依赖、未启动服务、未运行测试，所以没有 L3/L4。
...
| L2 | cognify 单 logical run、rollback、recall dataset precedence、search 多 dataset、remember 后台 task anchoring 等测试源码存在；当前核对未运行，不能写“通过”。 |
...
- README、AGENTS 与 `pyproject.toml` 的 Python 上限表述存在漂移：README写 3.10–3.14，AGENTS 写 `<3.14`，而 `pyproject.toml` 和锁文件约束为 `>=3.10,<3.15`。运行环境事实未在当前核对安装或解析验证，故以配置文件记录为静态事实，不能宣称支持矩阵已运行确认。

## 15. 当前核对深度源码审计补充（2026-08-21）

本节记录当前核对在目标仓库本地完成的进一步源码研究。没有调用 MCP/Hermes，没有安装依赖、启动服务、连接外部 provider 或运行测试；因此本节仍然只提供源码级事实和未验证边界，不把测试文件存在写成测试通过。
...
| 位置 | 发现 | 影响 | 本文裁决 |
|---|---|---|---|
| `README.md` 性能说明 | 把 `CACHING=false` 概括为 session memory 完全停止，但普通 session remember 会返回 `session_stored`，typed entry 才硬失败 | 使用者无法区分“未写入 cache”“结果状态仍成功样式”和 typed entry 硬失败 | 以源码分支为准，README 该表述属于待修正文档漂移；当前核对不改 README |
| `AGENTS.md` / `README.md` / `pyproject.toml` | Python 支持上限分别出现 `<3.14`、3.10–3.14、`>=3.10,<3.15` | 安装与 CI 支持矩阵可能被错误理解 | `pyproject.toml`/锁文件是静态配置事实；运行兼容性仍未验证；当前核对不改 AGENTS/README |
...
| MCP README / `server.py` / hardening tests | README 侧重 memory API；server 仍保留大量 V1、workspace、legacy 工具，tools/list 又受 `COGNEE_MCP_TOOL_MODE` 和 BM25 transform 影响 | “只有三个工具”与真实可调用/可发现面不等价 | 本文区分注册、advertised、searchable、direct-call 四个面；当前核对不改 MCP 文档 |
| 根 ARCHITECTURE.md | 历史章节、第二/后续收口和当前核对补充均保留，内容详尽但重复较多 | 便于追踪证据演进，但降低快速阅读和视觉扫描效率 | 不删除旧章节；本节用证据等级和事实/建议边界避免再次复制完整主链 |
...
| graph/vector adapter | engine cache / provider | API lifespan `cache_clear()`；queue 释放时 TTL touch 或 force close | rollback 尝试 source-ref 或 pipeline-run 删除 | SIGKILL 不经 lifespan；WAL、文件锁、跨进程 worker 未在当前核对实测 |
...
| skill materialization | `remember(content_type="skills")` | `finally` 中 `shutil.rmtree(materialize_root)` | add_skills 失败仍进入清理 | 进程崩溃可能留下稳定 temp root；当前核对未做残留扫描 |
...
- `DatasetQueue` 的 task-done callback 是 legacy slot 的安全网；scoped context 会显式释放。其独立 daemon reaper 每 5 至 60 秒扫描 idle subprocess engines；reaper 异常被记录后继续循环，但当前核对没有验证 event-loop 关闭、线程退出或进程崩溃下的行为。
...
- 代表性测试使用 pytest/pytest-asyncio 和大量 mock/fixture；源码读取能确认断言与生产符号对应，但当前核对没有安装依赖或执行 pytest。因此证据等级为“测试源码存在”，不是 L2 通过，更不是 L3/L4。
- 当前核对没有验证：真实 LLM key/provider、数据库迁移、Ladybug/Kuzu/LanceDB/PGVector/Neo4j/Redis、API/MCP handshake、远端 CloudClient、Git/enola、客户端断连、Task.cancel、顶层 deadline、SIGKILL 重启、进程组、WAL/锁/temp/.enola 残留、跨 graph/vector/relational 一致性、重复请求幂等。
...
- 默认 `BaseConfig` 将数据、系统、cache、日志路径映射到本地目录或用户 home；`STORAGE_BACKEND=s3` 时可自动把 cache root 设为 `s3://...`，但当前核对未验证对应文件 API 和权限行为。
- 多租户隔离要求 graph/vector handler 都支持 dataset isolation；关系库仍共享保存用户、ACL、registry 等元数据。任何一侧不支持时应硬失败，而不是静默回退 shared backend；当前核对未真实探测各 provider。
...
4. **文件边界**：当前核对只更新平台侧 `ARCHITECTURE.md`；`细探-Cognee.md`、README、源码、测试、配置均未修改。目标 checkout 的 `.codegraph` 已建立并核对，但仅用于定位；异常、取消、provider 和残留项均以“未运行/待核”保守表达。

## 16. 本轮现场收口记录（2026-08-22）

| 项目 | 现场证据 | 结果 |
|---|---|---|
| 远程版本 | `git fetch origin --prune`、`git pull --ff-only` | `Already up to date` |
| 当前提交 | `git rev-parse HEAD` | `fd5045f6b60522c1953fc1ae258e041ba53602d8` |
| CodeGraph | `codegraph status`、`codegraph sync` | 2,574 files / 35,080 nodes / 92,786 edges；Python 2,168、TSX 167、TS 141、YAML 88、JS 10 |
| 文档唯一性 | 平台项目目录定向 `find` | 仅维护本项目唯一 `ARCHITECTURE.md` |
