...
|---|---|---|---|
| L0 静态事实与边界 | 源码路径、条目字段、窗口/检索/反思调用链、现有存储和明确缺口；支持库/模块唯一 owner 不冲突；禁止直接把声明当实现 | `ARCHITECTURE.md` 第 2-15 节；对 `models/`、`core/`、`database/`、`utils/` 的逐路径复核；平台 `项目说明.md`、`支持库总览.md`、`模块库总览.md` | **当前核对完成**；TriMem 根目录存在独立 `.codegraph/`，CLI 状态可读；图谱仅作定位辅助，未伪造运行验证 |
