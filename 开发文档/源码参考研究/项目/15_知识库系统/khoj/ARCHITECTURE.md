...
当前审计按要求先调用专属 `system_engineering_toolkit` 的 `project_context`，再调用 `codegraph_explore`。该 MCP 当前返回的项目根是 `/Users/hekunhua/Documents/Agent/PHP/系统工程平台`，其 CodeGraph 也只覆盖系统工程平台，而不是本目标 `khoj`；因此 MCP 返回的证据可信度、最近成功验证和代码地图不能作为 Khoj 的源码证据。Khoj 本文事实以目标目录的只读文件、Git 版本信息、远程只读快照与现有 `历史研究-khoj.md` 为准。该 MCP 项目根错配是当前审计必须保留的集成风险。
