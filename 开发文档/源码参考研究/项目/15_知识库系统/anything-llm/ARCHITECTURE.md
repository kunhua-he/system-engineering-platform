...
- 本地根目录：`/Users/hekunhua/Documents/Agent/github 源码参考/15_知识库系统/anything-llm`
...
专属 `codegraph_explore` 已按要求调用，但目标目录未发现 `.codegraph/`，工具明确返回“未建立索引，无法查询”；未使用其他仓库代码地图或验证证据。首个 `project_context` 返回的活动项目却是 `华世王镞_v3`，根目录为 `/Users/hekunhua/Documents/Agent/PHP/华世王镞_v3`，与本项目不一致，因此该上下文不能作为本项目证据。
