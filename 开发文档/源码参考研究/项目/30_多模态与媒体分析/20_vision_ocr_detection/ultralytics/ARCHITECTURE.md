...
>
> **目标项目**：`~/Documents/Agent/github 源码参考/30_多模态与媒体分析/20_vision_ocr_detection/Ultralytics`
>
...
本轮不把 Ultralytics 源码搬入平台，而是把已确认的 `Model`/`task_map`、四类 engine 骨架、`AutoBackend`、数据源加载器、tracker 回调和导出格式表映射为四层公共边界。以下内容是**底座装配输入**；“建议能力”不是已经存在的平台实现，不能当作已落地能力。
...
**第三轮结论**：吸收的是“任务装配、四种运行骨架、流式数据和后端适配的边界模式”；升级的是资源、制品和生命周期治理；新建的是 L4 统一监督/注册/证据契约；废弃的是直嵌闭源业务、隐式下载、跨服务暴露内部对象和全局 tracker。当前项目仍只能作为“视觉模型运行时/检测分割底座候选”，不是平台生产实现。
...
## 15. 第三轮源码映射增补：Model/task_map/Results 到受治理工作流
...
本轮最终裁决：复用 `Model/task_map` 的装配思想、Trainer/Validator/Predictor 的任务差异分层、`Results` 的内部结果模型、source/batch 的流式边界以及 AutoBackend 的 provider 隔离思想；不复用 AGPL 实现，不把内部对象作为平台协议，不允许隐式下载、无界批处理、全局 tracker 或未监督的训练/导出进程。只有 L0-L3 定向验收和 L4 运行核心证据同时存在时，才能将视觉能力标记为生产可用。
