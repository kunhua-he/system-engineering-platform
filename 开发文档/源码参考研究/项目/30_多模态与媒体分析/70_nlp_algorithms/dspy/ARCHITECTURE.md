...
本轮只做底座映射和裁决，不把 DSPy 源码复制进生产底座，也不修改 DSPy 源码、依赖、配置或测试。此前调用平台 `project_context` 时返回的是另一个项目（`/Users/hekunhua/Documents/Agent/PHP/华世王镞_v3`），与本目标根不一致；该结果仅记录为环境错绑，不作为 DSPy 的代码地图、验证或架构证据。本轮结论均回到目标仓库本地源码静态取证。
