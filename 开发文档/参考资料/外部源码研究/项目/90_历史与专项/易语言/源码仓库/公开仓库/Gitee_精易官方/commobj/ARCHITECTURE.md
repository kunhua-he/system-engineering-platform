...
| 项目 | 当前事实 |
|---|---|
| 本地根目录 | `~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/commobj` |
...
建议后续在隔离 Windows + Visual Studio 环境中按以下顺序验证，不把建议命令当作当前核对已执行事实：
...
- [ ] 若后续发现旧 `细探-*.md`，逐条和源码核对后吸收到本文件；当前核对未发现旧细探，未执行删除。

## 10. 后续：原生对象支持库、运行核心与宿主边界

当前核对只做底座映射，不把 `commobj` 的接口声明改造成生产实现。以下结论分为“源码事实”和“底座候选裁决”两类；候选裁决不是本仓库已经存在的代码。另：开工上下文工具曾误绑定到另一个 V3 项目，该结果未作为 `commobj` 证据；当前核对证据全部重新来自本项目本地源码静态读取。
...
这里的 L0-L4 是当前核对底座验收分层，不是 `commobj` 已实现的能力等级：

| 等级 | 验收目标 | 当前状态 | 证据等级 |
|---|---|---|---|
| L0 | 静态 ABI：入口、结构、命令索引、类型索引、消息号、函数表数量一致 | 已完成静态盘点：140 命令、2 类型、各 60 槽位、`GetNewInf`/`.def` 存在 | **源码存在；当前核对未编译** |
...
当前核对实际完成的是源码静态归属与验收契约补全，不是上述实现或真实运行验证。
...
后续进一步裁决：ABI/类型注册归原生对象支持库；宿主回调、消息、跨边界内存/数组/复合释放、位宽与生命周期治理归运行核心；COM `IUnknown`/引用计数当前没有源码证据，只能作为需求确认后的原生对象适配能力，不能由 `NRS_FREE_COMOBJECT` 或对象析构标志推导出来。
