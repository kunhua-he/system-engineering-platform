...
> 本文是 `iext6` 当前本地源码快照的唯一架构说明。本文只记录源码和工程文件已经给出的事实；“已实现”“仅声明”“未验证”严格区分。源码参考仓库按只读研究处理，当前取证只新增本文件，未修改源码、工程、依赖、测试、配置或 Git 历史。
...
```text
iext6/
├── ARCHITECTURE.md                 # 当前取证新增；项目架构唯一说明
...
- 仓库：`~/Documents/Agent/github 源码参考/90_历史与专项/易语言/源码仓库/公开仓库/Gitee_精易官方/iext6`
- 当前分支：`master`
...
- 建档前工作树：干净；当前取证之后预期仅新增 `ARCHITECTURE.md`
...
仓库共 23 个跟踪文件，属于 `SuperBar` 控件支持库模板。命令、属性和事件的声明集中在 `iext6_cmdInfo.cpp`、`iext6_cmdDef.cpp`、`iext6_dtType.cpp` 与 `iext6_cmd_typedef.h`；真正的窗口创建、属性读写和事件派发尚未在这些文件中形成完整实现。

动态工程 `iext6.vcxproj` 与静态工程 `iext6_static/iext6_static.vcxproj` 共享 `elib/*` 宿主 ABI；`Source_iext6.def` 和 `iext6_dllMain.cpp` 决定 DLL 导出及 `GetNewInf` 入口。没有测试、资源包或 Windows 构建产物，当前无法确认导出可加载、句柄释放、异常路径、x86/x64 布局或易语言 IDE 注册。文档规模按小型元数据仓保持，未以重复段落制造虚假实现证据。
