...
专属 `system_engineering_toolkit` MCP 的真实开工上下文返回的是项目 `系统工程平台`（根目录 `~/Documents/Agent/PHP/系统工程平台`，MCP 实例 `system_engineering_toolkit`，开工 id `e8d1ecb90f4444d6`），其代码图也固定指向该平台，不是本项目；`codegraph_explore` 因此只能提供平台映射基线，不能作为 Elephant-Agent 源码证据。本节涉及 Elephant-Agent 的事实均来自目标根目录现场读取；验证结果也必须按目标仓库本地命令单独判定。
...
当前核对只读取目标仓库当前磁盘源码、SQLite schema、测试和设计文档，不使用 MCP/Hermes，不启动服务、不调用真实 provider、不修改源码和测试。目标仓库根目录当前存在独立 `.codegraph/`，本轮仅用目标目录内 CLI 作定位；源码事实仍以现场目录、全文检索和逐段源码读取为准。唯一落盘文件仍是本根 `ARCHITECTURE.md`。
