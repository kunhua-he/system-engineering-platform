...
文中早期“目标项目没有 `.codegraph`”记录对应旧快照，现已过期。当前目标根 `~/Documents/Agent/github 源码参考/30_多模态与媒体分析/10_scene_segmentation/PySceneDetect` 存在独立 `.codegraph/`；`codegraph status` 退出码为 0，返回 98 files、1,820 nodes、4,731 edges。该状态只证明源码导航索引可用，不提升真实媒体运行、取消或资源回收验证等级。
