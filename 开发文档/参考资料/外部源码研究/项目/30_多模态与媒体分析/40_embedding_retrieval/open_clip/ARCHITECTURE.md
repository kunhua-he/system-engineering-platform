...
>
> 目标项目：`~/Documents/Agent/github 源码参考/30_多模态与媒体分析/40_embedding_retrieval/open_clip`
>
...
- 当前核对只允许并只修改：`~/Documents/Agent/github 源码参考/30_多模态与媒体分析/40_embedding_retrieval/open_clip/ARCHITECTURE.md`。
...
| 现有 `支持库/后端/资源管理` 的原子写入、临时登记、CAS/摘要模式 | 平台 `开发文档/项目说明.md` 的资源治理边界 | **复用/升级候选**：用于 checkpoint 制品、临时目录、摘要和幂等释放；不承载模型推理 |
...
- 源码根目录：`~/Documents/Agent/github 源码参考/30_多模态与媒体分析/40_embedding_retrieval/open_clip`。
