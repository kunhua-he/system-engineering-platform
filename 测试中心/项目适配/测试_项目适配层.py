...
class Test项目声明(unittest.TestCase):
    def test_读写往返(self):
        ...
        声明.支持库绑定 = [{"包id": "支持库.后端.文件系统", "版本约束": ">=1.0.0"}]
        写入项目声明(声明, 项目根 / "项目声明.json")
        重读 = 加载项目声明(项目根 / "项目声明.json")
        self.assertEqual(重读.支持库绑定[0]["包id"], "支持库.后端.文件系统")
...
class Test支持库绑定(unittest.TestCase):
    def test_有效绑定(self):
        结果 = 校验支持库("支持库.后端.文件系统", ">=1.0.0", 系统根 / "支持库")
    ...
    def test_版本不满足失败(self):
        结果 = 校验支持库("支持库.后端.文件系统", ">=2.0.0", 系统根 / "支持库")
        self.assertFalse(结果.成功)
...
class Test依赖锁定(unittest.TestCase):
    ...
    def test_锁定包含绑定包(self):
        ...
        self.assertIn("支持库.后端.文件系统", 包id集合)
        self.assertIn("支持库.后端.文本处理", 包id集合)
