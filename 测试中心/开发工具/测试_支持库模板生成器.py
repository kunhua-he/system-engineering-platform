    ...
    return [
        {
            ...
            "说明": "骨架占位能力：返回占位结果",
            "参数": [
                {"名称": "输入", "类型": "文本", "必填": True, "说明": "任意输入"},
                {"名称": "数量", "类型": "整数", "必填": False, "默认值": 1, "说明": "次数"},
            ],
            "返回": "结果",
        ...
        },
        {
            ...
            "参数": [{"名称": "输入", "类型": "文本", "必填": True, "说明": "任意输入"}],
            "返回": "结果",
        },
    ]
...
class Test支持库模板生成器(unittest.TestCase):
    ...
    def test_生成九要素齐全(self):
        ...
        必要文件 = [
            "能力定义.json", "包声明.json", "能力契约/参数契约.json", "依赖锁.json",
            "配置契约.json", "权限契约.json", "验证场景引用.json", "说明/使用说明.md",
        ...
        # 第三方向导：依赖锁.json 声明外部工具
        依赖锁 = json.loads((self.包目录 / "依赖锁.json").read_text(encoding="utf-8"))
        self.assertEqual(依赖锁["包"], [])
    ...
    def test_路径逃逸拒绝(self):
        ...
        self.assertTrue(self.包目录.exists() is False or not self.包目录.is_dir())
