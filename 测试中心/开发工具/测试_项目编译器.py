...
class 测试项目编译器(unittest.TestCase):
    ...
    def test_项目引用模板_fail_closed(self):
        根 = Path(tempfile.mkdtemp(prefix="项目编译器模板阻断_", dir=受管临时根))
    ...
    def test_支持库能力依赖递归进入闭包(self):
        支持库 = {
            "支持库.包1": _包("支持库.包1", [{"能力": "能力.示例2"}], [{"能力id": "能力.示例1"}]),
            "支持库.包2": _包("支持库.包2", [], [{"能力id": "能力.示例2"}]),
        }
        支持库选中, 模块选中, 锁 = _解析依赖闭包(
            支持库, {}, {"能力.示例1": "支持库.包1", "能力.示例2": "支持库.包2"},
            {"支持库.包1"}, set())
        self.assertEqual(支持库选中, {"支持库.包1", "支持库.包2"})
        self.assertEqual(模块选中, set())
        self.assertEqual(锁[0]["目标包"], "支持库.包2")

    def test_循环和缺依赖_fail_closed(self):
        表 = {
            "支持库.包1": _包("支持库.包1", [{"包id": "支持库.包2"}]),
            "支持库.包2": _包("支持库.包2", [{"包id": "支持库.包1"}]),
        }
        with self.assertRaisesRegex(ValueError, "依赖循环"):
            _解析依赖闭包(表, {}, {}, {"支持库.包1"}, set())
        with self.assertRaisesRegex(ValueError, "不存在"):
            _解析依赖闭包({"支持库.包1": _包("支持库.包1", [{"包id": "支持库.不存在"}])}, {}, {}, {"支持库.包1"}, set())

    def test_新鲜制品启动入口不写pyc(self):
        根 = Path(tempfile.mkdtemp(prefix="项目编译器工作包_", dir=受管临时根))
        self.addCleanup(清只读后删除树, 根, 忽略失败=真)
        制品 = 根 / "制品"
