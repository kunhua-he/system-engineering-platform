...
class Test能力调用图接线反向验证(unittest.TestCase):
    ...
    # URL 解码归一化 import 了 `urllib.parse`，被判「原子旁路-网络」⇒ 该项「存量 0、新增即红」
    # 当场真阻断。★ 2026-09-24（批H-H2）：那处 import 已随判据收口删除，**豁免不随之撤销**
    # —— 它按精确子模块名生效，全仓仍有多处 `urllib.parse` 使用者（纯字符串处理，无 I/O）。
