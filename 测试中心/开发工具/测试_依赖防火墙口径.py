...
class 依赖防火墙口径测试(unittest.TestCase):
    ...
    def test_制品态模块名剥前缀后同包实现导入仍成立(self) -> None:
        ...
        依赖防火墙.制品根前缀 = "平台客户端"
        try:
            self.assertEqual(剥制品根前缀("平台客户端.运行核心.包A.实现.提供者"),
                             "运行核心.包A.实现.提供者")
            self.assertEqual(剥制品根前缀("运行核心.包A.实现.提供者"),
                             "运行核心.包A.实现.提供者", "源码态无前缀，必须原样返回")
            self.assertEqual(剥制品根前缀("平台客户端.第三方名.模块"),
                             "平台客户端.第三方名.模块", "前缀后不是层名时不得误剥")
            self.assertTrue(同包实现导入("运行核心/包A/__init__.py",
                                        "平台客户端.运行核心.包A.实现.提供者"),
                            "同包导自身 实现/ 必须放行")
            self.assertFalse(同包实现导入("运行核心/包A/__init__.py",
                                         "平台客户端.运行核心.包B.实现.提供者"),
                             "真跨包导 实现/ 必须照旧报红")
        finally:
            依赖防火墙.制品根前缀 = 原前缀
