...
class 提示词与实现同源测试(unittest.TestCase):
    ...
    def test_描述必须提异步腿(self):
        ...
        这条腿此前只写在 `开工准备` 返回的通用纪律里，而**工具面是 Agent 选工具时唯一看得见的东西**
        ⇒ 新会话必然只用同步的 `执行命令` 硬扛 15 秒前台硬上限（超了被杀、白付一轮）。
        故钉在工具面上：四步的名字一个都不能少（少了哪步，那条腿就走不通）。
        """
        描述 = _取工具("capability_call").description or ""
        for 关键词 in ("异步腿", "启动进程", "查询进程状态", "等待进程结束", "释放句柄"):
...
class 自换新壳测试(unittest.TestCase):
    ...
    def test_指纹串只认内容不认键序(self):
        ...
        """
        甲 = {"工具清单.py": "a", "薄壳服务.py": "b"}
        乙 = {"薄壳服务.py": "b", "工具清单.py": "a"}
        self.assertEqual(清单.指纹串(甲), 清单.指纹串(乙))
        self.assertNotEqual(清单.指纹串(甲), 清单.指纹串({**甲, "薄壳服务.py": "c"}))
        self.assertNotEqual(清单.指纹串(甲), 清单.指纹串({**甲, "多出来.py": "c"}),
