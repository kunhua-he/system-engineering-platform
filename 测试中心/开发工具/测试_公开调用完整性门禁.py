...
class Test存量基线只减不增(unittest.TestCase):
    ...
    def test_真仓库基线覆盖全部存量(self):
        """真仓库：基线必须与现场逐桶一致 —— 新增空、收敛空、基线「条目」只装内部层环的存量。

        2026-09-24 批P·P-7（开工-20260924-094428-1577）把该基线「条目」清空（现场原始违规 0 条，
        原 7 桶 20 条全部收敛 ⇒ 判据自报 [可收敛] 7 桶）。**2026-09-24 批R·R-9
        （开工-20260924-111036-45b3）新增「内部层收口第二面闸」**：新判据上线即带 16 桶 / 69 条
        **真实存量**（16 个 `内部层=真` 的包，其能力仍落在「页面可绑定集」内），按铁律「判据变更
        同批处理存量」登记进基线 ⇒ 原「条目真空」断言随之失效（它锁的是「现场原始违规 0 条」这个
        **历史时点**，与 P·P-7 时点的 `assertGreater(存量, 0)` 同一种失效）。

        现锁三条（比「真空」更有分辨力）：
        ① 新增空、收敛空；
        ② 基线条目**逐桶等于**现场「内部层-」桶的计数（不许多、不许少）；
        ③ 基线条目**只许**装「内部层-」开头的桶 —— 其它桶一旦被回填（把旧缺口偷偷洗白）即报红。
        """
        ...
        基线条目 = json.loads(默认存量基线路径().read_text(encoding="utf-8")).get("条目")
        现场内部层桶: dict[str, int] = {}
        for 条 in 检查门禁原始项(仓库根):
            if not str(条["缺口类型"]).startswith("内部层-"):
                continue
            键 = 分桶(条, 仓库根)
            现场内部层桶[键] = 现场内部层桶.get(键, 0) + 1
        self.assertEqual(基线条目, 现场内部层桶,
                         "基线「条目」必须逐桶等于现场「内部层-」桶（批R·R-9 新增环的存量）")
        self.assertEqual(len(存量), sum(现场内部层桶.values()),
                         "存量条数必须等于基线内条目数（存量只报不拦，不进新增）")
        self.assertTrue(all(键.split("|")[-1].startswith("内部层-") for 键 in (基线条目 or {})),
                        "基线「条目」只许装「内部层-」桶；其它桶被回填即报红")
...
class Test内部层第二面闸(unittest.TestCase):
    """内部层收口「第二面」机器闸（2026-09-24 批R·R-9，审计 T8）。

    口径：`开发文档/项目说明.md` 哲学 7.1 段「收口面有两处，必须一起撤」——第二面＝
    页面/可绑定集不再引用该 `能力id`。两载体：①页面 JSON 真引用 ⇒ 判红；②页面可绑定集
    仍含 ⇒ 存量（只报不拦）。
    """
    ...
    def setUp(self):
        self.临时 = Path(tempfile.mkdtemp(dir=受管临时根))
        self.addCleanup(清只读后删除树, self.临时, 忽略失败=真)
        self.包目录 = 合法包(self.临时)
        # 夹具包声明成内部层（第一面）；能力 id 不变，六环其余部分仍是合法包
        写(self.包目录 / "包声明.json", json.dumps(
            {"包id": "支持库.后端.示例包", "内部层": True,
             "能力": [{"能力id": "示例.测试能力", "名称": "测试能力"}]}, ensure_ascii=False))
        self.页面文件 = self.临时 / "示例项目" / "演示" / "前端" / "页面" / "主页.json"
        self.基线目录 = Path(tempfile.mkdtemp(dir=受管临时根))
        self.addCleanup(清只读后删除树, self.基线目录, 忽略失败=真)
        self.基线文件 = self.基线目录 / "基线.json"

    def _写页面(self, 能力id: str = "示例.测试能力") -> None:
        """写一张引用（或空引用）该能力 id 的页面（页面里承载引用的字段＝属性.能力id）。"""
        写(self.页面文件, json.dumps({
            "页面id": "主页", "标题": "夹具页", "路由": "/",
            "组件列表": [{"组件id": "按钮1", "对象id": "对象1", "类型": "按钮",
                         "属性": {"能力id": 能力id}, "事件": []}],
        }, ensure_ascii=False))

    def _冻结载体二(self) -> None:
        """拍 0：按本门禁既有体例落夹具基线（走判据自己的 `分桶`，不手写数值）。

        夹具根不吃仓库那份基线（`运行门禁并分诊` 只在根＝仓库时自动取），故显式传本夹具基线。
        """
        条目: dict[str, int] = {}
        for 条 in 检查门禁原始项(self.临时):
            键 = 分桶(条, self.临时)
            条目[键] = 条目.get(键, 0) + 1
        self.基线文件.write_text(json.dumps({"版本": 1, "条目": 条目}, ensure_ascii=False),
                                encoding="utf-8")

    def test_内部层能力在可绑定集内出存量条目(self):
        """载体②：`内部层=真` 的能力仍在「页面可绑定集」（构建索引.能力所有者）内 ⇒ 出条目。"""
        类型表 = [条["缺口类型"] for 条 in 检查内部层可绑定(self.临时)]
        self.assertIn("内部层-可绑定集仍含", 类型表)

    def test_非内部层包不进本闸(self):
        """对照：同一夹具把 `内部层` 撤掉 ⇒ 本闸零条目（不是恒红）。"""
        写(self.包目录 / "包声明.json", json.dumps(
            {"包id": "支持库.后端.示例包",
             "能力": [{"能力id": "示例.测试能力", "名称": "测试能力"}]}, ensure_ascii=False))
        self._写页面()
        self.assertEqual(检查内部层可绑定(self.临时), [])

    def test_反向样本三拍_判红转绿复红(self):
        """反向样本三拍：造「内部层=真 且 被页面引用」⇒ 判红；撤掉引用 ⇒ 转绿；恢复 ⇒ 复红。

        拍 0 先冻结载体②（夹具的 `内部层` 包照样进 `构建索引` ⇒ 载体② 在夹具下同样命中，
        那是平台现状的真实存量，不是夹具缺陷），读数因此只反映载体①（页面引用）。
        """
        self._冻结载体二()
        新增, 存量, _ = 运行门禁并分诊(self.临时, self.基线文件)
        self.assertEqual(新增, [], f"拍 0（未引用）应无新增: {新增[:3]}")
        self.assertEqual([条["缺口类型"] for 条 in 存量], ["内部层-可绑定集仍含"])

        self._写页面()                                    # 拍 1：造「内部层=真 且 被页面引用」
        新增, _, _ = 运行门禁并分诊(self.临时, self.基线文件)
        self.assertEqual([条["缺口类型"] for 条 in 新增], ["内部层-页面仍引用"], "拍 1 必须判红")
        self.assertEqual(新增[0]["能力id"], "示例.测试能力")
        self.assertIn("主页.json", 新增[0]["详情"])

        self._写页面("")                                  # 拍 2：撤掉页面引用
        新增, _, _ = 运行门禁并分诊(self.临时, self.基线文件)
        self.assertEqual(新增, [], f"拍 2 应转绿: {新增[:3]}")

        self._写页面()                                    # 拍 3：恢复引用
        新增, _, _ = 运行门禁并分诊(self.临时, self.基线文件)
        self.assertEqual([条["缺口类型"] for 条 in 新增], ["内部层-页面仍引用"], "拍 3 必须复红")

    def test_页面生成物不被当页面(self):
        """`编译制品.json`/`编辑记录.json` 是编译产物与编辑留痕，不算「页面引用」。

        夹具里只放编译制品（无设计态页面）：若把编译输出当页面，就会拿**制品里的旧引用**
        判红 —— 那是把编译产物当契约，本闸刻意跳过这两个名字。
        """
        写(self.页面文件.parent / "编译制品.json", json.dumps({
            "制品类型": "轻代码前端制品",
            "页面": {"页面id": "主页", "路由": "/", "组件列表": [
                {"组件id": "按钮1", "类型": "按钮", "属性": {"能力id": "示例.测试能力"}}]},
        }, ensure_ascii=False))
        self.assertNotIn("内部层-页面仍引用",
                         [条["缺口类型"] for 条 in 检查内部层可绑定(self.临时)])
        self._写页面()
        self.assertIn("内部层-页面仍引用",
                      [条["缺口类型"] for 条 in 检查内部层可绑定(self.临时)])
