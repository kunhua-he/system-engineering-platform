...
class Test存量基线只减不增(unittest.TestCase):
    ...
    def test_真仓库基线覆盖全部存量(self):
        """真仓库：基线必须与现场逐桶一致 —— 新增空、收敛空、基线「条目」为空。

        2026-09-24 批P·P-7（开工-20260924-094428-1577）把该基线「条目」清空（现场原始违规 0 条，
        原 7 桶 20 条全部收敛 ⇒ 判据自报 [可收敛] 7 桶）。**原断言 `assertGreater(len(存量), 0)`
        随之失效**：它锁的是「E-2/E-3 上线时点有存量」这个**历史时点**，不是判据口径；基线一旦
        合法清零，存量必然是 0，该断言会把「已收敛」误判成回归。现改为锁「**双空 + 条目真空**」
        —— 后者才是对「基线被偷偷回填」有分辨力的那一条（存量/新增在空基线下恒空，锁不住回填）。
        """
        新增, 存量, 收敛 = 运行门禁并分诊(仓库根)
        self.assertEqual(新增, [], f"真仓库不得有基线外新增: {新增[:3]}")
        self.assertEqual(存量, [], f"基线已清空 ⇒ 存量恒空: {存量[:3]}")
        ...
        self.assertEqual(基线条目, {}, "基线「条目」应为空（2026-09-24 批P·P-7 清空）；被回填即报红")
