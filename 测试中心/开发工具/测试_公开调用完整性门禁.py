...
'''
    ...
    def test_真实扫描如实输出(self):
        ...
        修红线（本轮实测的真缺口）：实现侧有 2 个码直接写进代码、既未登记状态映射、
        也不在任何 能力定义.json 里 —— `写回压缩结果失败`（大语言模型支持库.上下文压缩）
        与 `已存在文件`（组件规范支持库.技能模板生成器）。它们的修法在**生产包**（补登记），
        不在本测试的修改域，因此这里断言「门禁如实报出这 2 条、且没有其它类别缺口」：
        修好之后本断言要跟着改（与 `测试_真实OCR审计清零确认` 同一种诚实写法）。
        """
        违规 = 运行门禁(仓库根)
        缺口类型表 = sorted({条["缺口类型"] for 条 in 违规})
        self.assertEqual(缺口类型表, ["错误码-实现侧产生码未登记状态映射"],
                         f"真实仓库只应残留实现侧未登记码这一个类别: {违规}")
        self.assertEqual(sorted(条["错误码"] for 条 in 违规),
                         ["写回压缩结果失败", "已存在文件"], f"真缺口清单变了: {违规}")
        结果 = subprocess.run([sys.executable, "开发工具/公开调用完整性门禁.py"],
                             cwd=仓库根, capture_output=True, text=True)
        self.assertIn("公开调用完整性门禁失败", 结果.stdout)
        self.assertEqual(结果.returncode, 1, "存在真缺口时退出码必须是 1，不得恒绿")
