...
class Test公开调用完整性门禁(unittest.TestCase):
    ...
    def test_模板与适配层系统适配器不进入正式扫描(self):
        ...
        (self.临时 / "支持库" / "适配层" / "提供者配置读取").mkdir(parents=True)
        写(self.临时 / "支持库" / "适配层" / "提供者配置读取" / "配置读取.py", "值 = 1\n")
        self.assertEqual(找包目录(self.临时), [])
    ...
    def test_系统适配器名录用包声明自证豁免时照六环检查(self):
        ...
        """
        包目录 = self.临时 / "支持库" / "适配层" / "提供者配置读取"
        写(包目录 / "包声明.json", json.dumps(
            {"包id": "支持库.适配层.提供者配置读取", "版本": "1.0.0",
             "能力": [{"能力id": "支持库.适配层.提供者配置读取.读取", "名称": "读取"}]},
            ensure_ascii=False))
    ...
    def test_扫描面为空判红(self):
        ...
        **两条都报才对**（2026-09-20 修 #182 时华哥裁决「改成 B：空根下实现侧判据也如实报」）：
        空根下**两个扫描面都是空的** —— 六环啥都没扫到、实现侧也一个文件都没扫到，
        各自如实出一条才是事实。原期望只列六环，实际是靠「实现侧判据被错误码环的
        `return []` 早退静默跳过」才凑出来的 —— 那个早退正是 #182 要修的缺陷
...
class Test存量基线只减不增(unittest.TestCase):
    ...
    def test_接线模块的判据链真被挂到对外导出面(self):
        """接线判据链：门禁主文件**按名再导出**拆出去的全部簇成员，私有件也在内。

        为什么必须钉这一条（2026-09-19 拆分后补）：本门禁对外面既有 51 个顶层函数
        （含被 `运行发布门禁` 直接 import 的私有件 `_是聚合父包`），也有 `python3.14
        开发工具/公开调用完整性门禁.py` 这条**按文件名直跑**的用法。原文件按职责拆成
        三个同目录模块后，「再导出」这一步如果漏一个名字，功能回归可能一条都不红
        （例如 `_是保留目录` 全仓无活跃调用点，漏导它不会有任何用例变红）——
        门面漏导就是对外契约变化，故落成静态断言，不靠人眼。
        """
        门禁模块 = importlib.import_module("开发工具.公开调用完整性门禁")
        模块文件 = Path(门禁模块.__file__ or "")
        self.assertFalse(模块文件.parent.joinpath("__init__.py").exists(),
                         "开发工具/ 是命名空间包，本模块的 import 路径不得依赖包 __init__")
        树 = ast.parse(模块文件.read_text(encoding="utf-8"))
        再导出 = {别名.asname or 别名.name
                 for 节点 in 树.body if isinstance(节点, ast.ImportFrom)
                 and (节点.module or "").startswith("开发工具.公开调用完整性门禁_")
                 for 别名 in 节点.names}
        ...
        拆分前对外面 = (
            ...
            "_段内载位", "_清单形", "_清单载位", "_表格行", "_解析参数散文", "_解析说明书", "_说明书包级版本", "_返回类型名",
            ...
            "错误码字面量键", "错误码扫描排除根", "错误码状态映射变量", "错误码状态映射源", "错误码行", "错误码说明表变量", "错误码说明表源",
            "默认存量基线路径",
        )
        self.assertEqual(len(拆分前对外面), 85)
        缺失 = sorted(名 for 名 in 拆分前对外面 if not hasattr(门禁模块, 名))
        self.assertEqual(缺失, [], f"拆分后对外面缺名（漏再导出即对外契约变化）: {缺失}")
        # 拆出去的名字必须真来自再导出那一行；留在主文件的必须真在主文件里定义，
        # 两者都不得「在别处又定义一份」（第二份实现）。
        主文件定义 = {节点.name for 节点 in 树.body
                  if isinstance(节点, (ast.FunctionDef, ast.AsyncFunctionDef))}
        for 名 in 拆分前对外面:
            值 = getattr(门禁模块, 名)
            if not hasattr(值, "__code__"):
                continue      # 正则/常量/模块对象无常住模块归属（re.Pattern.__module__ 恒为 "re"）
            定义模块 = 值.__module__
            if 名 in 再导出:
                self.assertTrue(定义模块.endswith("公开调用完整性门禁_包发现")
                                or 定义模块.endswith("公开调用完整性门禁_说明书解析")
                                or 定义模块.endswith("公开调用完整性门禁_检查与判定"),
                                f"{名} 应在三个职责簇模块之一，实际 {定义模块}")
            elif 名 in 主文件定义:
                self.assertTrue(定义模块.endswith("开发工具.公开调用完整性门禁"),
                                f"{名} 应留在门禁主文件，实际 {定义模块}")
