...
class 搜索14字段测试(unittest.TestCase):
    ...
    def test_公开面契约字段完整且内部层不暴露(self) -> None:
        """公开搜索面的契约字段完整性 + 内部层不得出现在公开面（2026-09-17 口径收口）。

        口径依据（唯一实现腿 `能力目录.搜索能力` 的「公开能力面」语义）：
        - `包声明.内部层=true` 的第三方能力内部支持库（如 `图像处理支持库.图像解码.*`）
          **装配可用但网关 0 暴露** → 不出现在公开搜索面（旧实现静态扫全量声明，会把内部层一并曝出）；
        - `调用示例` 一律为**契约声明原值**，不得合成文本——与
          `测试中心/模块库/测试_能力搜索覆盖率.py` 冻结的口径一致（两边必须同口径，否则必有一边假红）。
        """
        结果 = 搜索公开能力(项目根, "解码图像", 10)
        self.assertTrue(结果, "解码图像 应有公开面结果")
        编号表 = [能力["能力id"] for 能力 in 结果]
        self.assertNotIn("图像处理支持库.图像解码.解码图像", 编号表,
                         "内部层能力不得出现在公开搜索面（内部层语义：装配可用、网关 0 暴露）")
