...
必填A = {"名称": "A", "类型": "文本型", "必填": True}
必填B = {"名称": "B", "类型": "文本型", "必填": True}
...
class 契约升级判定(unittest.TestCase):
    ...
    def test_非破坏变更不判红(self):
        旧 = _契约("1.0.0", [必填A])
        新 = _契约("1.0.0", [必填A], 错误码=["参数不合法", "超出限制"])
        self.assertIsNone(检测契约升级(旧, 新))

    def test_缺版本键走零兜底不抛异常(self):
        """版本键缺失/为空时按 0.0.0 兜底（旧判据口径），不得抛异常。"""
        self.assertIsNotNone(检测契约升级({"参数": [必填A]}, {"参数": [必填A, 必填B]}))
        self.assertIsNone(检测契约升级({"参数": [必填A]},
                                  {"参数": [必填A, 必填B], "版本": "1.0.0"}))
