...
# --------------------------------------------------------------------------- #
class 假环境根夹具:
    """在 `self.临时`（tempfile 临时目录）下建一个自足假仓库根。

    2026-09-23 从「模块级函数 + 夹具根当形参」改为**方法**：夹具根必须从 `self.临时` 取，
    否则「夹具根」这个外部形参在 `测试写入边界门禁` 判据一的静态解析器里解析不出 ——
    `假根 / "支持库"` 一类**合法临时夹具写**会被整片计入「未解析」违规（判据 fail-closed）。

    假仓库根里只有 `工程缓存/提供者运行环境` 与提供者真身；
    软链 `运行核心`/`公共契约`/`模块库` 到真仓库：回收脚本要 import
    平台的 `环境管理器`（摘要算法是唯一判据，不许在测试里另写一套）。

    **提供者真身必须落在脚本的搜索面里**（2026-09-20 搜索面收窄为
    `支持库/{适配层,后端}`，见 `提供者环境回收.提供者真身搜索面`）：脚本的
    `建提供者目录索引` 用 `rglob`，而 **rglob 不跟随符号链接目录** ⇒ 整层软链的
    `支持库` 里的提供者真身一个都扫不到（实测症状：期望「保守跳过 2 个」实得 3 个）。
    故 `支持库` 落真目录，`适配层` 软链、`后端` 落真目录放夹具真身；原实现的
    `假提供者库/` 在假根顶层，正好落在搜索面**外**。
    """

    def 建假环境根(self, 回收源: Path, 名字: str) -> tuple[Path, dict]:
        根 = self.临时
        假根 = 根 / 名字
        (假根 / "开发工具").mkdir(parents=True)
        脚本 = 假根 / "开发工具" / "回收.py"
        shutil.copy2(回收源, 脚本)
        for 层名 in ("运行核心", "公共契约", "模块库"):
            (假根 / 层名).symlink_to(系统根 / 层名)
        # `支持库` **不能**整层软链：提供者真身搜索面是 `支持库/{适配层,后端}`
        # （见 提供者环境回收.提供者真身搜索面），而 `建提供者目录索引` 用 `rglob`
        # —— rglob 不跟随符号链接目录 ⇒ 软链层里的提供者真身一个都扫不到，三个提供者
        # 会全落「保守跳过」（实测：期望「保守跳过 2 个」实得 3 个，三条用例全红）。
        # 故 `支持库` 落**真目录**：`适配层` 仍软链（回收脚本的 import 链要
        # `支持库.适配层.系统探针`/`密码签名提供者`），`后端` 落真目录并放夹具真身
        # —— 正好在搜索面内，且不碰真仓库任何文件。
        (假根 / "支持库").mkdir()
        (假根 / "支持库" / "适配层").symlink_to(系统根 / "支持库" / "适配层")
        真身库 = 假根 / "支持库" / "后端" / "假提供者库"
...
class 提供者环境回收退出码测试(假环境根夹具, unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.脚本, self.摘要表 = self.建假环境根(回收脚本, "假仓库根")
    ...
    def test_反向_修前等价副本必须假绿(self) -> None:
        ...
        场景脚本, _ = self.建假环境根(回收脚本, "假根")
        探针 = self.临时 / "假根" / "开发工具/回收_探针.py"
        探针.write_text(
