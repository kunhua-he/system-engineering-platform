...
class 真仓库夹具反向验证(unittest.TestCase):
    ...
    def test_本地残留不改变字节指纹(self):
        ...
        self._造残留("x.db")
        甲 = 计算工作区字节指纹(self.根)["工作区字节指纹"]
        (self.根 / "x.db").write_text("乙" * 100, encoding="utf-8")
        self._造残留("y.log", "z.tmp")
        乙 = 计算工作区字节指纹(self.根)["工作区字节指纹"]
        self.assertEqual(基线指纹, 甲, "新增残留改变了字节指纹（跨机不可复现）")
        self.assertEqual(甲, 乙, "残留内容变化改变了字节指纹（跨机不可复现）")
