...
class 拒绝手写判据两问测试(unittest.TestCase):
    ...
    def test_改了md的写入腿都接了(self) -> None:
        """**每一条会改 `.md` 内容的写腿**都必须转调唯一节点（2026-09-24 裁决：根推断与登记
        只留一处实现）。

        改前这里断言的是两条腿**各自实现**一份私有登记函数（`_登记md写入凭据(路径, 内容)`
        / `_登记md写入凭据(目标, 正文)`）—— 那正是「同一件事两套实现」（哲学 1.2），
        且两份根推断必然走偏。现在唯一节点是 `公共契约.诊断.写入流水.登记md写入凭据`，
        每条腿只许转调它（含 `.md` 判据与根推断，腿里一个字都不许再有）。

        ★ 2026-09-26 扩到第三条（华哥「MCP 错误的，就先修 mcp，想办法治根先」）：本条原只钉
        `写入文件` ＋ 文本补丁，**漏了 `追加写入`** —— 而它恰恰是唯一一条改了 md 却不登记的
        写腿，后果是 `pre-commit` 的「拒绝手写」判据把**工具写的文件判成手写**并拒提交
        （实测：`开发文档/未完成事项.md` 追加后提交被拒，判据报「印记 1 份在／写入凭据 0 份
        命中」）。判据只认「转调了唯一节点」这个**事实**，不认注释里写没写「我也登记」——
        漏接即红。方法名一并从 `test_三条写入腿都接了` 改成 `test_改了md的写入腿都接了`：
        原名的「三」既与正文的「两条」对不上，也把判据说成了「数条数」——它管的是**覆盖面**。
        """
        唯一节点 = "写入流水.登记md写入凭据("
        ...
        补充 = (系统根 / "支持库" / "后端" / "文件系统支持库" / "文件操作"
              / "实现" / "文件系统补充.py").read_text(encoding="utf-8")
        self.assertIn(唯一节点, 补充, "追加写入这条腿必须转调唯一登记节点（它改 md 内容）")
        self.assertNotIn("def _登记md写入凭据", 补充, "腿里不得再留第二份登记实现")
