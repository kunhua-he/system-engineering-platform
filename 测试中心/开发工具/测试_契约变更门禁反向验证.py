...
class 契约口径接线反向验证(unittest.TestCase):
    ...
    def test_真仓库契约变更判绿且报出判据面(self) -> None:
        ...
        """
        通过, 详情 = 校验对外契约变更()
        # 不写死包数：分包/新增包会让计数自然增长，写死即「假红」（正文明文可复核，
        # 断言只要求「判据真的扫到了面」，数字本身由详情原样报出）。
        判据面 = int(详情.split("判据面 ")[1].split(" 包")[0])
        self.assertGreater(判据面, 100, f"判据面异常过小，判据可能塌缩：{详情}")
        ...
        if 要求递增 == 0:
            ...
            # ★ 不断言「全部逐字未变」（2026-09-22 修）：工作树里改了任何包的**文档性字段**
            #   （如 `返回.值结构`、`参数.说明`）都会让该包进入「仅文档性变化」桶 —— 那是
            #   合法状态、不是违约。断言改成**分桶覆盖判据面**（逐字未变 + 仅文档性变化 +
            #   已递增承载 == 判据面），这才是不变量；写死「全部未变」会让任何一次契约文档
            #   修订把测试判红，而测试红无人处理（AGENTS.md「判据变更必须同批清断言」）。
            逐字未变 = int(详情.split("契约面逐字未变 ")[1].split(" 包")[0])
            仅文档性 = int(详情.split("仅文档性变化（不要求递增）")[1].split(" 包")[0])
            已递增 = int(详情.split("契约面变且已由版本递增承载 ")[1].split(" 包")[0])
            self.assertEqual(
                逐字未变 + 仅文档性 + 已递增, 判据面,
                f"分桶未覆盖判据面（判据可能漏桶）：{详情}")
            self.assertGreater(逐字未变, 100, f"逐字未变包数异常偏少，判据可能塌缩：{详情}")
            return
