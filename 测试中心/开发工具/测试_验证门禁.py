...
class 真实仓库测试(unittest.TestCase):
    """真实仓库：表内各检查器在基线内应为绿（存量只报不拦，只减不增）。"""
...
class 平台判断越界判据精度测试(unittest.TestCase):
    ...
    def test_白名单外裸读平台标志判红(self) -> None:
        ...
        try:
            for 相对, 内容 in (
                ...
                # 2026-09-23：`平台适配` 已拆成**同名包**，白名单也改成「精确 ∪ 前缀」。
                # 这条样本钉住「**不保留旧腿**」：平铺的 `平台适配.py` 不再在白名单内
                # —— 谁要是把它当薄入口加回来，这条样本会立刻判红（与 `编译口`
                # 实测的「保留平铺件 ⇒ 包 `__init__` 收不到 import 边」同一处置）。
                ("公共契约/运行时/平台适配.py", "import sys\n\n\n夹具值 = sys.platform\n"),
            ):
                路径 = 夹具 / 相对
    ...
    def test_收口层本体不判(self) -> None:
        """白名单内（平台适配**包** / 进程终止）自带平台判定是它的职责，不得判红。

        收口层白名单是**双形态**（2026-09-23）：`进程终止.py` 走精确路径，
        `公共契约/运行时/平台适配/` 走前缀（该文件已按职责拆成同名包，包内每个子文件
        都是收口层本体的一部分）。本条同时钉住两种形态 —— 只钉一种会让另一种的
        形态变更悄悄失效。
        """
        夹具 = Path(tempfile.mkdtemp(prefix="平台判断越界_收口层_"))
        try:
            for 相对 in (
                "公共契约/运行时/平台适配/判定.py",      # 前缀形态（包内子文件）
                "公共契约/运行时/平台适配/硬件画像.py",  # 前缀形态（包内子文件）
                "公共契约/运行时/进程终止.py",            # 精确形态（单文件收口层）
            ):
                路径 = 夹具 / 相对
                路径.parent.mkdir(parents=True, exist_ok=True)
                路径.write_text(
