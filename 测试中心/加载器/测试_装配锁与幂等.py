...
class Test装配顺序与失败回滚(unittest.TestCase):
    """装配顺序严格按拓扑（提供者在先）；单包级问题只跳过该包，跨包真冲突
    整体失败并恢复注册表（架构项A：半成品包不阻断全平台，但绝不放行半装配）。"""
    ...
    def test_单包入口缺失只跳过该包无半装配残留(self):
        """模块2 入口文件不存在（实现缺失）：只跳过模块2 并告警，模块1 照常装配。

        新规（架构项A，2026-09-15）：实现缺失是「这个包自己的半成品问题」，
        不再全平台阻断；但该包的能力绝不允许以半装配形态留在注册表里。
        """
        注册表 = 能力注册表()
        ...
        结果 = 装配系统(self.临时 / "支持库", self.临时 / "模块库", 注册表)
        self.assertTrue(结果.成功, str(结果.问题列表))
        self.assertEqual(结果.问题列表, [], "单包问题不得整体阻断")
        self.assertEqual(len(结果.跳过包列表), 1, str(结果.跳过包列表))
        self.assertIn("回滚.模块2", 结果.跳过包列表[0])
        self.assertIn("实现缺失", 结果.跳过包列表[0])
        # 该包能力未注册（对调用方不可见），模块1 与既有能力保留
        self.assertIsNone(注册表.获取("回滚.模块2能力"), "被跳过的包不得留下半装配能力")
        self.assertEqual(注册表.获取("回滚.模块1能力").包id, "回滚.模块1")
        self.assertEqual(注册表.获取("既有.能力").包id, "既有包")
        self.assertEqual(结果.声明能力数, 1, "被跳过包的声明能力不计入本次闭环")
        self.assertEqual(结果.已注册能力数, 2, "既有能力 + 模块1 能力")
        # 生命周期留痕：跳过包必须有回滚记录（证据不丢）
        回滚记录 = [记录 for 记录 in 结果.生命周期记录 if 记录.操作名称 == "回滚"]
        self.assertTrue(回滚记录, "单包级跳过必须产生回滚记录")
        self.assertIn("回滚.模块2", [记录.包id for 记录 in 回滚记录])

    def test_跨包能力重复注册整体失败并恢复注册表(self):
        """两个包注册同一能力 id（跨包真冲突）→ 必须整体失败并恢复注册表。

        这类冲突不是单包自己的半成品问题，属能力多提供者冲突：隔离机制不得
        把它降级成「跳过某个包」，必须保持 fail-closed 整体阻断。
        """
        注册表 = 能力注册表()
        注册表.注册(能力实现(能力id="既有.能力", 包id="既有包", 实现函数=lambda: "旧"))
        for 序号 in (1, 2):
            写临时包(self.临时 / "支持库", f"冲突包{序号}", {
                "包id": f"冲突.模块{序号}", "名称": f"冲突包{序号}", "类型": "支持库",
                "版本": "1.0.0", "入口": "入口.py",
                "能力": [{"能力id": f"冲突.模块{序号}能力"}],
            }, (
                "from 公共契约.能力契约.契约 import 能力实现\n"
                "def 注册能力(注册表):\n"
                "    注册表.注册(能力实现(\n"
