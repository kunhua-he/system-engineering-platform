...
class TestPillow提供者(unittest.TestCase):
    ...
    def test_子进程自举激活指针解析(self):
        ...
        try:
            ...
            # 后端腿「子进程入口.py」在 D-1 收口后是**转调门面**，唯一实现是本适配层腿的
            # 同名文件；真·部署制品里两件都在（实测 `find 平台客户端环境 -path '*适配层/Pillow提供者*'`），
            # 故夹具必须同批补上这一件，否则门面按文件路径载入唯一实现时读不到（2026-09-20 实测真红）。
            shutil.copy2(适配层源实现 / "子进程入口.py", 适配层目录 / "实现" / "子进程入口.py")
            (客户端目录 / "__init__.py").write_text("", encoding="utf-8")
            # 真·部署制品里 `公共契约` 是平台客户端的一部分：后端腿入口/解析自 #218
            # （提交 804dcc74「前缀/取根推导收口为一处」）起改调
            # `公共契约.运行时.导入前缀`（取根前缀/取系统根的唯一推导落点）。夹具必须同批
            # 带上整棵 `公共契约`，否则子进程在 `from 公共契约.运行时.导入前缀 import 取根前缀`
            # 处 `ModuleNotFoundError: 公共契约`（2026-09-23 实测真红：夹具停在 #218 之前，
            # 只造了 支持库 两条腿，没造 公共契约）。
            仓库根 = Path(__file__).resolve().parents[2]
            shutil.copytree(仓库根 / "公共契约", 客户端目录 / "公共契约",
