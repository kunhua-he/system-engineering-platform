...
class 测试展平结构条目(unittest.TestCase):
    def test_嵌套结构与列表索引(self):
        ...
        self.assertEqual(结果.值["条目列表"], [
            {"路径": "a.b", "值": 1, "超深": False},
            {"路径": "a.c[0]", "值": 1, "超深": False},
            {"路径": "a.c[1].d", "值": 2, "超深": False},
            {"路径": "e", "值": "x", "超深": False},
        ])
    ...
    def test_根标量与空容器(self):
        标量 = 展平结构条目(42)
        self.assertEqual(标量.值["条目列表"], [{"路径": "$", "值": 42, "超深": False}])
        根数组 = 展平结构条目([1, [2, 3]])
        self.assertEqual([x["路径"] for x in 根数组.值["条目列表"]], ["[0]", "[1][0]", "[1][1]"])
        空 = 展平结构条目({})
        self.assertEqual(空.值, {"条目列表": [], "总条目数": 0, "已截断": False})

    def test_深度上限产出超深标记(self):
        结果 = 展平结构条目({"a": {"b": {"c": {"d": 1}}}}, 最大深度=1)
        self.assertEqual(结果.值["条目列表"], [{"路径": "a.b", "值": "(已超最大深度)", "超深": True}])
    ...
    def test_可定制分隔符与索引格式(self):
        ...
        self.assertEqual(结果.值["条目列表"], [{"路径": "a<0>", "值": 1, "超深": False}])
