...
class 测试_选择模型智能模式(用量测试基类):
    ...
    def test_四个维度都必须真的参与打分(self):
        ...
        """
        # 甲：慢、贵、全成功；乙：快、免费、成功率一般
        for _ in range(3):
            记录模型调用(模型="甲", 成功=True, 任务类型="策划", 耗时毫秒=10000, 成本=1.0, 数据库路径=self.库)
        for i in range(3):
            记录模型调用(模型="乙", 成功=(i < 2), 任务类型="策划", 耗时毫秒=100, 成本=0.0, 数据库路径=self.库)

        只看成功率 = 选择模型(候选清单=["甲", "乙"], 任务类型="策划", 启用智能路由=True,
                          权重={"成功率": 1.0, "效率": 0, "性价比": 0, "匹配度": 0},
                          数据库路径=self.库)
        self.assertEqual(只看成功率.值["建议"][0]["模型"], "甲", "成功率权重全开时应选甲")

        只看效率性价比 = 选择模型(候选清单=["甲", "乙"], 任务类型="策划", 启用智能路由=True,
                            权重={"成功率": 0, "效率": 0.5, "性价比": 0.5, "匹配度": 0},
                            数据库路径=self.库)
        self.assertEqual(只看效率性价比.值["建议"][0]["模型"], "乙",
                         "效率+性价比权重全开时应选乙（若基准取错，两维恒0会选错）")
