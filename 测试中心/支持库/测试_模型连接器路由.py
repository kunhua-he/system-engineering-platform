...
class 用量测试基类(unittest.TestCase):
    ...
    def 造数据(self):
        ...
        for i in range(4):
            记录模型调用(模型="快稳云模型", 成功=True, 任务类型="策划",
                       耗时毫秒=1000 + i * 10, 成本=0.01, 数据库路径=self.库)
        for i in range(4):
            记录模型调用(模型="慢贵本地模型", 成功=(i < 1), 任务类型="策划",
                       耗时毫秒=5000, 成本=0.10, 数据库路径=self.库)


class 测试_记录模型调用(用量测试基类):
    def test_正常记录(self):
        r = 记录模型调用(模型="快稳云模型", 成功=True, 任务类型="策划", 耗时毫秒=900,
    ...
    def test_不给任务类型记为未分类(self):
        记录模型调用(模型="快稳云模型", 成功=True, 数据库路径=self.库)
        r = 汇总模型表现(候选清单=["快稳云模型"], 任务类型="未分类", 数据库路径=self.库)
        self.assertEqual(r.值["数据条数"], 1)

    def test_同记录id重复写为更新语义(self):
        r1 = 记录模型调用(模型="快稳云模型", 成功=True, 数据库路径=self.库)
        rid = r1.值["记录id"]
        r2 = 记录模型调用(模型="快稳云模型", 成功=False, 记录id=rid, 数据库路径=self.库)
        self.assertTrue(r2.成功)
        self.assertEqual(r2.值["累计条数"], 1, "同 id 重复写不应增行")

    def test_耗时毫秒必须是整数(self):
        """★ 契约是整数型；传浮点由网关拦，这里验实现层的宽松度（整数通过）。"""
        self.assertTrue(记录模型调用(模型="快稳云模型", 成功=True, 耗时毫秒=900, 数据库路径=self.库).成功)

    def test_反向_缺必填(self):
        self.assertEqual(记录模型调用(成功=True, 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="快稳云模型", 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="", 成功=True, 数据库路径=self.库).错误码, "参数不合法")

    def test_反向_类型不符(self):
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功="是", 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功=True, 耗时毫秒="abc", 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功=True, 耗时毫秒=-1, 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功=True, 成本=-1, 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功=True, 输入令牌=1.5, 数据库路径=self.库).错误码, "参数不合法")
        self.assertEqual(记录模型调用(模型=123, 成功=True, 数据库路径=self.库).错误码, "参数不合法")

    def test_反向_布尔不算整数(self):
        self.assertEqual(记录模型调用(模型="快稳云模型", 成功=True, 耗时毫秒=True, 数据库路径=self.库).错误码,
                        "参数不合法")
...
class 测试_汇总模型表现(用量测试基类):
    def test_统计正确(self):
        self.造数据()
        r = 汇总模型表现(候选清单=["快稳云模型", "慢贵本地模型"], 任务类型="策划", 数据库路径=self.库)
        self.assertTrue(r.成功, r.错误说明)
        汇总 = {项["模型"]: 项 for 项 in r.值["汇总"]}
        self.assertEqual(汇总["快稳云模型"]["样本数"], 4)
        self.assertEqual(汇总["快稳云模型"]["成功率"], 1.0)
        self.assertEqual(汇总["慢贵本地模型"]["成功率"], 0.25)
        self.assertTrue(汇总["快稳云模型"]["是否样本充足"])
    ...
    def test_样本不足按门槛标注(self):
        for _ in range(2):
            记录模型调用(模型="快稳云模型", 成功=True, 数据库路径=self.库)
        r = 汇总模型表现(候选清单=["快稳云模型"], 最少样本数=3, 数据库路径=self.库)
        self.assertFalse(r.值["汇总"][0]["是否样本充足"])
        r2 = 汇总模型表现(候选清单=["快稳云模型"], 最少样本数=2, 数据库路径=self.库)
        self.assertTrue(r2.值["汇总"][0]["是否样本充足"])

    def test_不给任务类型汇总全部(self):
        self.造数据()
        记录模型调用(模型="快稳云模型", 成功=True, 任务类型="日常", 数据库路径=self.库)
        r = 汇总模型表现(候选清单=["快稳云模型"], 数据库路径=self.库)
    ...
    def test_反向(self):
        ...
        self.assertEqual(汇总模型表现(候选清单=["快稳云模型"], 最少样本数=0).错误码, "参数不合法")
        self.assertEqual(汇总模型表现(候选清单=["快稳云模型"], 最少样本数=-1).错误码, "参数不合法")


class 测试_选择模型保守模式(用量测试基类):
    def test_默认是保守模式(self):
        r = 选择模型(候选清单=["快稳云模型", "慢贵本地模型"], 数据库路径=self.库)
        self.assertTrue(r.成功, r.错误说明)
    ...
    def test_保守模式额度说明写明未消耗(self):
        r = 选择模型(候选清单=["快稳云模型"], 数据库路径=self.库)
        self.assertIn("未消耗", r.值["额度说明"])

    def test_额度说明默认开启_显式假才清空(self):
        """★ 实测坑：写成 `if not 启用额度标注` 会让默认 None 判成假、说明恒为空。"""
        self.assertNotEqual(选择模型(候选清单=["快稳云模型"], 数据库路径=self.库).值["额度说明"], "")
        self.assertNotEqual(选择模型(候选清单=["快稳云模型"], 启用额度标注=True, 数据库路径=self.库).值["额度说明"], "")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 启用额度标注=False, 数据库路径=self.库).值["额度说明"], "")


class 测试_选择模型智能模式(用量测试基类):
    def test_按表现排序(self):
        self.造数据()
        r = 选择模型(候选清单=["快稳云模型", "慢贵本地模型"], 任务类型="策划", 启用智能路由=True, 数据库路径=self.库)
        self.assertEqual(r.值["模式"], "智能")
        self.assertEqual(r.值["建议"][0]["模型"], "快稳云模型", f"快稳云模型应排第一：{r.值['建议']}")
        self.assertGreater(r.值["建议"][0]["分数"], r.值["建议"][1]["分数"])

    def test_样本不足者不给分(self):
        self.造数据()
        r = 选择模型(候选清单=["快稳云模型", "没见过的"], 任务类型="策划",
    ...
    def test_可复现(self):
        self.造数据()
        参数 = {"候选清单": ["快稳云模型", "慢贵本地模型"], "任务类型": "策划", "启用智能路由": True, "数据库路径": self.库}
        第一次 = 选择模型(**参数).值["建议"]
        第二次 = 选择模型(**参数).值["建议"]
        self.assertEqual(第一次, 第二次, "同一输入两次选择必须给出同一建议（可复现）")

    def test_智能模式仍零额度(self):
        self.造数据()
        r = 选择模型(候选清单=["快稳云模型"], 任务类型="策划", 启用智能路由=True, 数据库路径=self.库)
        self.assertIn("未消耗", r.值["额度说明"])

    def test_权重可覆盖且影响排序(self):
        self.造数据()
        r = 选择模型(候选清单=["快稳云模型", "慢贵本地模型"], 任务类型="策划", 启用智能路由=True,
                   权重={"成功率": 1.0, "效率": 0, "性价比": 0, "匹配度": 0}, 数据库路径=self.库)
        self.assertEqual(r.值["建议"][0]["模型"], "快稳云模型")

    def test_四个维度都必须真的参与打分(self):
        """★ 回归：实测坑 —— 基准若取「该模型自己的中位数」，效率与性价比恒为 0（两维死掉）。

        用「成功率低但极快极便宜」的候选对比「成功率高但慢且贵」的候选：
        两个方向的权重配置应把赢家分别推给不同模型，才算四维真的都在起作用。
        """
        # 快稳云模型：慢、贵、全成功；慢贵本地模型：快、免费、成功率一般
        for _ in range(3):
            记录模型调用(模型="快稳云模型", 成功=True, 任务类型="策划", 耗时毫秒=10000, 成本=1.0, 数据库路径=self.库)
        for i in range(3):
            记录模型调用(模型="慢贵本地模型", 成功=(i < 2), 任务类型="策划", 耗时毫秒=100, 成本=0.0, 数据库路径=self.库)

        只看成功率 = 选择模型(候选清单=["快稳云模型", "慢贵本地模型"], 任务类型="策划", 启用智能路由=True,
                          权重={"成功率": 1.0, "效率": 0, "性价比": 0, "匹配度": 0},
                          数据库路径=self.库)
        self.assertEqual(只看成功率.值["建议"][0]["模型"], "快稳云模型", "成功率权重全开时应选快稳云模型")

        只看效率性价比 = 选择模型(候选清单=["快稳云模型", "慢贵本地模型"], 任务类型="策划", 启用智能路由=True,
                            权重={"成功率": 0, "效率": 0.5, "性价比": 0.5, "匹配度": 0},
                            数据库路径=self.库)
        self.assertEqual(只看效率性价比.值["建议"][0]["模型"], "慢贵本地模型",
                         "效率+性价比权重全开时应选慢贵本地模型（若基准取错，两维恒0会选错）")
...
class 测试_参考路径(用量测试基类):
    ...
    def test_不给参考路径就不涉及(self):
        r = 选择模型(候选清单=["快稳云模型"], 任务描述="处理内部项目的资料", 数据库路径=self.库)
    ...
    def test_路径不存在报参数不合法(self):
        r = 选择模型(候选清单=["快稳云模型"], 参考路径="/绝对不存在的路径/x.txt", 数据库路径=self.库)
...
class 测试_选择模型反向(用量测试基类):
    def test_参数不合法(self):
        ...
        self.assertEqual(选择模型(候选清单=[{"模型": "快稳云模型", "上下文长度": 0}]).错误码, "参数不合法")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 上下文长度=0).错误码, "参数不合法")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 启用智能路由="是").错误码, "参数不合法")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 权重={"乱写": 1}).错误码, "参数不合法")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 权重="不是字典").错误码, "参数不合法")
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 最少样本数=0).错误码, "参数不合法")

    def test_权重给负值被拒(self):
        self.assertEqual(选择模型(候选清单=["快稳云模型"], 权重={"成功率": -1}).错误码, "参数不合法")
