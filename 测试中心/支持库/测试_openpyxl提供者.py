...
class TestOpenpyxl提供者(unittest.TestCase):
    ...
    def test_注册能力(self):
        ...
        注册表 = 能力注册表()
        注册能力(注册表)
        # 解析实现只注册内部能力 id；公开 owner 由 后端/表格文档 持有。
        self.assertEqual(注册表.能力id列表, ["内部.表格文档.解析", "办公文档支持库.表格文档.生成表格文档"])
        实现 = 注册表.获取("内部.表格文档.解析")
        self.assertEqual([参数["名称"] for 参数 in 实现.参数],
                         ["文件路径", "数据模式", "最大工作表数", "最大行数", "最大字节数"])
        调用结果 = 实现.调用(文件路径="/不存在的路径/文件.xlsx")
