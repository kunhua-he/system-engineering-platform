"""psycopg 提供者测试：参数不合法/驱动缺失/真实连接/连接生命周期受管/注册与摘要一致。

无数据库服务时必须明确失败（连接失败/超时），绝不假装连接成功；
连接对象与游标调用后必然关闭（mock 驱动验证释放，psycopg3 游标上下文管理器）。
"""
...
from 支持库.适配层.psycopg提供者 import 关闭, 连接, 查询, 注册能力, 事务执行
...
class Testpsycopg提供者(unittest.TestCase):
    ...
    def test_连接生命周期受管调用后关闭(self):
        ...
        with mock.patch.object(模块.psycopg, "connect",
    ...
    def test_事务执行生命周期受管(self):
        ...
        with mock.patch.object(模块.psycopg, "connect",
            ...
            结果 = 事务执行("postgresql://u@127.0.0.1:54320/db",
                           ["SELECT 1"], 超时秒=2)
        self.assertTrue(结果.成功, 结果.错误说明)
        self.assertTrue(假调用.return_value.已关闭, "事务连接未关闭（泄漏）")

    @unittest.skipUnless(_驱动可用(), "psycopg 未安装")
    ...
    def test_完整性摘要一致(self):
        ...
        self.assertEqual(摘要数据["包id"], "支持库.适配层.psycopg提供者")
