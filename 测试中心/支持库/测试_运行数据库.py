"""底座运行库回归：七个域表初始化、写入查询闭环、并发写入、参数口径、验收标识护栏。
...
域清单：`检查点索引`/`缓存索引`/`发布` 三张恒空表已从 `提供者.运行库表定义` 删除
（写入侧分别在各自专用库、以及平台控制面权威状态库）；`会话` 原同在删域之列，后依
`开发文档/临时文档/20260915_MCP收敛/00_收口方案与依赖.md`「B4 会话态入库」**补齐为第 7 域**、
写入侧已接线（`提供者.运行库表定义` 已登记 `会话` 与主键 `开工id`），故不再是删域。
...
七个域 = ["任务", "作业", "协作状态", "能力占用", "灰度观测", "灰度状态", "会话"]
删掉的三个域 = ["检查点索引", "缓存索引", "发布"]
...
class 运行数据库回归(unittest.TestCase):
    ...
    def test_删掉的三个域按未知域报参数不合法(self):
