...
class Test发布门禁与四示例残留综合审计(unittest.TestCase):
    ...
    def _安装临时目录快照(self) -> set[Path]:
        ...
        集合: set[Path] = set()
        # 工程缓存可能包含数 GB 的制品/提供者环境；残留目录只允许出现在
        # 正式源码树中，遍历时剪枝缓存、git 和字节码目录，避免全盘 rglob。
        跳过目录 = {"工程缓存", ".git", "__pycache__"}
        try:
            for 当前根, 目录名表, _ in os.walk(系统根):
                目录名表[:] = [名称 for 名称 in 目录名表 if 名称 not in 跳过目录]
                当前路径 = Path(当前根)
                for 名称 in 目录名表:
                    if 名称.startswith(".临时_") or 名称.startswith(".安装临时_"):
                        集合.add(当前路径 / 名称)
        except OSError:
            # 扫描异常不能被当成“无残留”，保留已发现项并让后续证据暴露。
            pass
        return 集合
    ...
    def test_发布门禁四示例与残留资源综合审计(self):
        ...
        if os.environ.get("系统底座_发布门禁外层") == "1":
            # 外层发布门禁已经在当前测试事务中运行并负责判定全部测试；
            # 这里不再递归启动第二个门禁进程，避免重复执行数百项检查。
            门禁输出 = "复用外层发布门禁事务"
        else:
            门禁环境 = dict(os.environ)
            门禁环境[递归防护环境变量] = "1"
            # 发布门禁由本场景直接启动时，明确声明正在执行的外层慢速事务，
            # 让门禁复用当前慢速证据，禁止再递归启动第二个慢速层。
            门禁环境["系统底座_慢速验证事务"] = "残留审计外层"
            门禁环境["系统底座_慢速验证进程"] = str(os.getpid())
            门禁退出码, 门禁输出 = self._运行子进程(
                [sys.executable, "开发工具/发布门禁/运行发布门禁.py"], self.最长门禁秒, 门禁环境)
            门禁失败项 = "\n".join(
                行 for 行 in 门禁输出.splitlines()
                if "✗" in 行 or "发布状态: 失败" in 行
            ) or "未提取到明确失败项"
        ...
        审计产物根 = Path(tempfile.mkdtemp(prefix="可双击演示_审计_"))
        示例表 = [
            ("适配层示例", "示例项目/适配层示例/运行入口/运行.py",
             ["适配层示例运行成功"]),
            ("前后端核心示例", "示例项目/前后端核心示例/运行示例.py",
             ["前后端核心示例", "本地网关与后端核心已优雅关闭"]),
            # 旧的独立真实浏览器示例已删除；浏览器能力和 HTTP 交互由
            # 当前唯一保留的可双击演示项目编译链验证，避免审计引用死目录。
            ("可双击演示编译", "示例项目/可双击演示/开发文件夹/编译.py",
             ["编译完成：", "支持库 ", "模块 "]),
        ...
        try:
            for 名称, 入口, 关键输出 in 示例表:
                命令 = [sys.executable, 入口]
                if 名称 == "可双击演示编译":
                    命令.extend(["--名称", "残留审计", "--输出", str(审计产物根 / "产物")])
                退出码, 输出 = self._运行子进程(
                    命令, self.最长示例秒)
                示例输出表[名称] = 输出
            ...
            shutil.rmtree(审计产物根, ignore_errors=True)
