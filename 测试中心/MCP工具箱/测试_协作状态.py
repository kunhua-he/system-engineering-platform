...
def _写验证(路径: Path, 开工id: str, 内容指纹: str, *, 提交: str = "abc123",
            名称: str = "测试验证", 工作区指纹: str | None = None) -> None:
    """写一条验证证据，默认按**真实生产者**（`项目服务.py::_运行验证`）的记录形状落盘。

    `工作区指纹` 是 `sha256(git status)` 的 64 位十六进制（发布证据路径用），
    `指纹` 才是 `计算代码指纹` 的目录内容摘要前 16 位（收口登记读的是它）。
    此前本辅助只写 `工作区指纹` 并塞进内容摘要 —— 等于按消费者口径自造证据，
    掩盖了生产者/消费者算法不一致（旁路问题 B8），故改为照实形状。
    """
    路径.parent.mkdir(parents=True, exist_ok=True)
    记录 = {"名称": 名称, "命令": ["python3.14", "测试"],
            "工作区指纹": 工作区指纹 if 工作区指纹 is not None else "a" * 64,
            "指纹": 内容指纹,
...
class 协作状态测试(unittest.TestCase):
    ...
    def test_生产者形状证据可收口且旧形状被阻断(self) -> None:
        """B8 回归：证据必须携带目录内容摘要 `指纹` 才能收口，不许再按消费者口径自造。

        - 旧形状（只有 `工作区指纹` = sha256(git status) 64 位）：收口必须被阻断，
          这正是修复前恒失败的现场。
        - 生产者形状（`工作区指纹` 64 位 + `指纹` 16 位内容摘要）：收口必须成功。
        """
        self.assertTrue(self._登记父()["成功"])
        _写反馈(self.反馈文件, 父id)
        真实内容指纹 = 计算代码指纹(self.工作区)["指纹"]

        # 旧形状：只写 `工作区指纹` 的 64 位 git 状态摘要，没有目录内容摘要，手写落盘。
        旧路径 = self.验证历史文件.with_name("旧形状验证.jsonl")
        with 旧路径.open("w", encoding="utf-8") as 文件:
            文件.write(json.dumps(
                {"名称": "旧形状", "命令": [], "开工id": 父id, "提交": "abc123",
                 "工作区指纹": "c" * 64, "时间": "2026-08-03T00:00:00+00:00", "退出码": 0},
                ensure_ascii=False) + "\n")
        旧收口 = 收口登记(父id, 五件套路径="回信.md", 结论="完成",
                       状态目录=self.状态目录, 反馈文件=self.反馈文件,
                       验证历史文件=旧路径)
        self.assertFalse(旧收口["成功"], "旧形状证据（缺目录内容摘要）不应能收口")
        self.assertEqual(旧收口["错误码"], "EVIDENCE_MISMATCH")

        # 生产者形状：两种指纹都带，收口必须成功。
        _写验证(self.验证历史文件, 父id, 真实内容指纹)
        新收口 = 收口登记(父id, 五件套路径="回信.md", 结论="完成",
                       状态目录=self.状态目录, 反馈文件=self.反馈文件,
                       验证历史文件=self.验证历史文件)
        self.assertTrue(新收口["成功"], f"生产者形状证据应能收口: {新收口}")
        self.assertEqual(新收口["生命周期"], "完成")
