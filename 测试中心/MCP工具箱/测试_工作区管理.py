...
class 工作区管理测试(unittest.TestCase):
    ...
    def test_创建工作区真实成功(self) -> None:
        结果 = 创建工作区(self.项目根, self.工作区根, 任务id="任务甲")
        ...
        分支 = _运行(self.项目根, ["git", "branch", "--list", "codex/任务-任务甲"])
        self.assertIn("codex/任务-任务甲", 分支.stdout)

    def test_脏改动保护拒绝关闭且强制可关闭(self) -> None:
        创建 = 创建工作区(self.项目根, self.工作区根, 任务id="任务乙")
    ...
    def test_合并分支成功包含来源文件(self) -> None:
        _运行(self.项目根, ["git", "checkout", "-b", "甲"])
        (self.项目根 / "甲.txt").write_text("甲\n", encoding="utf-8")
        _运行(self.项目根, ["git", "add", "甲.txt"])
        提交甲 = _运行(self.项目根, ["git", "commit", "-m", "甲提交"])
        self.assertEqual(提交甲.returncode, 0, 提交甲.stderr)
        _运行(self.项目根, ["git", "checkout", "-b", "乙", "甲"])
        (self.项目根 / "乙.txt").write_text("乙\n", encoding="utf-8")
        _运行(self.项目根, ["git", "add", "乙.txt"])
        提交乙 = _运行(self.项目根, ["git", "commit", "-m", "乙提交"])
        self.assertEqual(提交乙.returncode, 0, 提交乙.stderr)
        _运行(self.项目根, ["git", "checkout", "甲"])
        结果 = 合并分支(self.项目根, "甲", "乙")
        self.assertTrue(结果["成功"], 结果)
        self.assertEqual(结果["冲突列表"], [])
        文件列表 = _运行(self.项目根, ["git", "-c", "core.quotePath=false", "ls-tree", "-r", "--name-only", "甲"])
        self.assertIn("乙.txt", 文件列表.stdout)
    ...
    def test_合并冲突返回失败且冲突列表非空(self) -> None:
        ...
        _运行(self.项目根, ["git", "checkout", "-b", "甲"])
        (self.项目根 / "冲突.txt").write_text("甲修改\n", encoding="utf-8")
        _运行(self.项目根, ["git", "add", "冲突.txt"])
        _运行(self.项目根, ["git", "commit", "-m", "甲改冲突"])
        _运行(self.项目根, ["git", "checkout", "-b", "乙", "HEAD~1"])
        (self.项目根 / "冲突.txt").write_text("乙修改\n", encoding="utf-8")
        _运行(self.项目根, ["git", "add", "冲突.txt"])
        _运行(self.项目根, ["git", "commit", "-m", "乙改冲突"])
        _运行(self.项目根, ["git", "checkout", "甲"])
        结果 = 合并分支(self.项目根, "甲", "乙")
    ...
    def test_非法任务标识抛错(self) -> None:
        with self.assertRaises(ValueError):
            创建工作区(self.项目根, self.工作区根, 任务id="任务甲!!!")
        with self.assertRaises(ValueError):
