...
class 任务观测测试(unittest.TestCase):
    def test_任务时间线可查询并聚合(self) -> None:
        with tempfile.TemporaryDirectory() as 临时目录:
            ...
            任务开始(路径, 任务id="任务1", 开工id="开工1", 角色="平台维护者")
            工具事件(路径, 任务id="任务1", 开工id="开工1", 工具="代码地图", 开始单调=0)
            任务结束(路径, 任务id="任务1", 开工id="开工1", 成功=True)
            结果 = 查询任务(路径, "任务1")
    ...
    def test_阶段报告区分分类与未分类时间(self) -> None:
        with tempfile.TemporaryDirectory() as 临时目录:
            路径 = Path(临时目录) / "事件.jsonl"
            任务开始(路径, 任务id="任务1", 开工id="开工1", 角色="平台维护者")
            开始 = 阶段记录(路径, 任务id="任务1", 开工id="开工1", 阶段="探索", 状态="开始")
            阶段记录(路径, 任务id="任务1", 开工id="开工1", 阶段="探索", 状态="结束", 阶段id=开始["阶段id"])
            任务结束(路径, 任务id="任务1", 开工id="开工1", 成功=True)
            报告 = 生成效率报告(路径, "任务1")
    ...
    def test_未知阶段拒绝(self) -> None:
        with tempfile.TemporaryDirectory() as 临时目录:
            with self.assertRaises(ValueError):
                阶段记录(Path(临时目录) / "事件.jsonl", 任务id="未登记任务", 开工id="未登记开工", 阶段="发呆", 状态="开始")
