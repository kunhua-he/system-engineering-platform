...
class Test失败记录与诊断(unittest.TestCase):
    ...
    def test_失败记录状态流转(self):
        记录 = self.失败库.登记失败(
            追踪id="追踪1", 包id="支持库.后端.文件系统", 错误码="版本不兼容",
            错误说明="契约漂移", 能力id="文件系统.读取文件", 版本="1.0.0",
        )
...
class Test热切换(unittest.TestCase):
    ...
    def test_有状态排空切换(self):
        ...
        self.assertFalse(判断有无状态("文本处理.分割文本"))
