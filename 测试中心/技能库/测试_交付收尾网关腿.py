...
class 交付收尾网关腿测试(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.夹具 = 夹具目标
        #: 真夹具进测试前的摘要基线（tearDownClass 复算比对，见那里的判据）。
        cls.真实夹具摘要 = cls._夹具摘要表()
    ...
    def _重建文件租约存储(cls) -> None:
        ...
        # ★ 2026-09-25 口径统一：夹具里的**开工id / 所有者**必须合全仓唯一句式
        #   `开工-YYYYMMDD-HHMMSS-短随机`（改前是 16 位裸十六进制 `b2c10de0…`）。
        #   为什么必须改夹具而不是放宽门禁：门禁句式是与提交强制点共用的**消费侧判据**，
        #   为让测试过而放宽它，等于把判据往回改（同一件事两套口径）。
        #   注意 `b2c10de0-文件-0001` 这类是**租约id**（另一维标识），不在本次口径内。
        存储 = {
            "文件::技能库/技能/交付收尾/SKILL.md": 租约(
                "b2c10de0-文件-0001", "技能库/技能/交付收尾/SKILL.md", "开工-20260925-101010-0a01", "批次B2"),
            "文件::技能库/技能/验证编排/SKILL.md": 租约(
                "b2c10de0-他人-文件", "技能库/技能/验证编排/SKILL.md", "开工-20260925-101010-0b01", "别人的包"),
        }
        (cls.夹具 / "文件租约.json").write_text(
            json.dumps(存储, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")

    @staticmethod
    def _夹具摘要表() -> dict[str, str]:
        """真实夹具目录（`技能库/技能/交付收尾/验证夹具/`）里每个文件的 sha256。

        为什么要有它（2026-09-25 实测）：本类 docstring 写着「夹具复制到临时目录，
        **绝不碰真实库与真实租约**」，但那句话此前**只是声明、没有判据**。
        实测（2026-09-25 会话）：真实夹具的 4 个文件确实变 modified，一度被读成
        「测试跑动弄脏了真夹具」。后经 A/B 实证（跑测试前后逐一复算摘要，四值全同）：
        **本测试腿没碰真夹具** —— 真凶是同一次会话里按编排授权、用 `事务执行` 与
        `文本补丁` 归档那批 16-hex 旧夹具 ID 的正常编辑。故补上这条判据：
        它不指责测试，只把「绝不碰真夹具」从声明变成**机器可判**。
        """
        import hashlib

        表: dict[str, str] = {}
        for 路径 in sorted(技能包目录.joinpath("验证夹具").rglob("*")):
            if 路径.is_file():
                相对 = str(路径.relative_to(技能包目录))
                表[相对] = hashlib.sha256(路径.read_bytes()).hexdigest()
        return 表

    @classmethod
    def tearDownClass(cls) -> None:
        # ★ 判据（2026-09-25）：本类全程只复制真夹具到临时目录，**不得**改动真夹具。
        #   跑完逐一复算摘要：任一变化即判红并点名是哪几个文件 —— 读只读夹具时写坏了，
        #   症状是「测试全绿、真夹具却脏了」，靠人看 `git status` 才发现（本会话亲历）。
        #   比对放在 `rmtree` 之前，临时目录随测试类销毁，不影响本条判据。
        现摘要 = cls._夹具摘要表()
        变了的 = sorted(
            相对 for 相对, 摘要 in cls.真实夹具摘要.items() if 现摘要.get(相对) != 摘要)
        cls.真实夹具摘要 = 现摘要
        shutil.rmtree(cls.临时目录, ignore_errors=True)
        if 变了的:
            raise AssertionError(
                f"本测试腿改动了**真实夹具**（应只读；它只该复制到临时目录）：{变了的}。"
                "真实夹具是受版本控制的仓库文件，改它属越界副作用 —— "
                "要改夹具请显式改 `技能库/技能/交付收尾/验证夹具/` 下被跟踪的文件"
                "（经 开工编排.开工即占 认领 + 文本补丁/事务执行，留提交记录），"
                "不要在测试腿里顺手写进去。")
    ...
    def test_正向_写终态并释放文件租约(self):
        ...
        回执 = self._跑技能({"开工id": "开工-20260925-101010-0a01",
                     "五件套路径": "工程缓存/交付证据/开工-20260925-101010-0a01",
                     "结论": "交付收尾网关腿正向验收"})
        内层 = self._内层(回执)
        self._断言成功(内层)
        ...
        记录 = self._查协作状态("开工-20260925-101010-0a01")
    ...
    def test_未反馈阻断不写终态(self):
        内层 = self._内层(self._跑技能({"开工id": "开工-20260925-101010-0a02",
                          "五件套路径": "工程缓存/交付证据/开工-20260925-101010-0a02",
                          "结论": "未反馈阻断验收"}))
        self.assertFalse(内层.get("成功"))
        self.assertEqual(内层.get("错误码"), "未反馈阻断")
        self.assertEqual(内层.get("阻断标记"), ["未反馈"])
        self.assertEqual(self._查协作状态("开工-20260925-101010-0a02")["生命周期"], "创建")

    def test_证据指纹不匹配阻断(self):
        内层 = self._内层(self._跑技能({"开工id": "开工-20260925-101010-0a03",
                          "五件套路径": "工程缓存/交付证据/开工-20260925-101010-0a03",
                          "结论": "证据不匹配验收"}))
        self.assertFalse(内层.get("成功"))
        self.assertEqual(内层.get("错误码"), "证据不匹配")
        self.assertEqual(内层.get("证据指纹"), "deadbeefdeadbeef")
        self.assertEqual(self._查协作状态("开工-20260925-101010-0a03")["生命周期"], "创建")

    def test_未登记阻断(self):
        内层 = self._内层(self._跑技能({"开工id": "开工-20260925-101010-0a04",
                          "五件套路径": "工程缓存/交付证据/开工-20260925-101010-0a04",
                          "结论": "未登记验收"}))
        self.assertFalse(内层.get("成功"))
    ...
    def test_释放开关关闭只写终态(self):
        内层 = self._内层(self._跑技能({"开工id": "开工-20260925-101010-0a05",
                          "五件套路径": "工程缓存/交付证据/开工-20260925-101010-0a05",
                          "结论": "释放开关关闭验收", "释放资源": False}))
        self._断言成功(内层)
        self.assertEqual(内层.get("生命周期"), "完成")
        self.assertEqual(内层["资源释放"].get("跳过"), "释放资源=false，未释放占用租约")
        self.assertEqual(self._查协作状态("开工-20260925-101010-0a05")["生命周期"], "完成")

    def test_非法开工id读库前拒绝(self):
        """非法开工id = 不合唯一句式（改前断言的是「不是 16 位十六进制」，两种都属不合句式）。"""
    ...
    def test_白名单缺项反向验证_只剩网关一条腿(self):
        ...
        内层 = self._内层(self._跑技能(
            {"开工id": "开工-20260925-101010-0a01", "五件套路径": "工程缓存/交付证据/白名单缺项反向验证",
             "结论": "白名单缺项反向验证"},
