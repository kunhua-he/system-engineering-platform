...
class 搜索能力测试(能力目录测试基类):
    ...
    def test_适配层按同名孪生判据进出发现面(self):
        """适配层能力**只在发现面里存在同名孪生时**才排除（2026-09-23 改判）。

        原口径「适配层整包排除」的理由是「同一实现的第二条腿」，但那个理由只对
        **真有孪生**的能力成立。实测被排除的 74 条里 12 条在发现面里没有任何同名能力
        ——`适配层.YAML提供者.检查可用性`、`浏览器自动化.检查可用` 经网关调用**成功**，
        `密码签名.签名` 回「缺少必填参数 私钥PEM」（＝已注册），而经
        `能力目录.搜索能力` 搜 `YAML提供者` 一条适配层都搜不到 ⇒ 又一次
        「可调但搜不到」（与 2026-09-20 内部层那次同一病根，已复发三次）。

        故判据从**包前缀**下沉到**能力级**（末段能力名同名即同一实现）：
        - 零孪生的进发现面，并带 `适配层=真` 标记供调用方取舍；
        - 有孪生的仍排除，防同一实现出两条重复记录（检索精确率是华哥定的主指标）。
        """
        from 模块库.能力目录.实现.能力索引 import 构建能力索引

        记录表, 问题 = 构建能力索引(系统根)
        self.assertFalse(问题, f"索引构建有扫描问题: {问题[:3]}")
        名集: dict[str, list[str]] = {}
        for 记录 in 记录表:
            名集.setdefault(记录["能力id"].rsplit(".", 1)[-1], []).append(记录["能力id"])

        # 方向一：**进了**发现面的适配层记录，必须没有同名孪生（否则就是重复记录）。
        进面 = [记录 for 记录 in 记录表 if 记录.get("适配层")]
        self.assertTrue(进面, "适配层零孪生样本一条都不在发现面 —— 判据前提已变，须复核")
        for 记录 in 进面:
            同名 = 名集[记录["能力id"].rsplit(".", 1)[-1]]
            self.assertEqual(
                [记录["能力id"]], 同名,
                f"适配层记录 {记录['能力id']} 与 {同名} 同名 ⇒ 是第二条腿，不该进发现面")

        # 方向二：**有**同名孪生的经典样本必须仍然缺席（`适配层.图像解码.解码图像`
        # ↔ `图像处理支持库.图像解码.解码图像`）。
        出 = self.搜索("解码图像", 20)
        self.assertTrue(出.成功, 出.错误说明)
        self.assertFalse(
            [记录 for 记录 in 出.值["能力列表"] if 记录["包id"].startswith("支持库.适配层.")],
            "有同名孪生的适配层能力不得进发现面（同一实现不许出两条记录）")

        # 方向三：零孪生的样本必须真能搜到 —— 「可调就必须可搜」。
        零孪生 = self.搜索("密码签名", 20)
        self.assertTrue(零孪生.成功, 零孪生.错误说明)
        命中 = {记录["能力id"] for 记录 in 零孪生.值["能力列表"]}
        self.assertIn("密码签名.签名", 命中,
                      "零孪生的适配层能力可调却搜不到 ⇒ 违反「Agent 只记搜索→调用两个动作」")
        样本 = next(记录 for 记录 in 零孪生.值["能力列表"]
                  if 记录["能力id"] == "密码签名.签名")
        self.assertTrue(样本["适配层"], "进了发现面的适配层记录必须如实带 `适配层` 标记")

    def test_兜底占比超上限仍返回有界候选且标注低置信(self):
        """多词查询占比超上限时**不再判零命中**，改为有界低置信返回（2026-09-23 改判）。

        治的是「越想精确表达越一无所获」：实测 `执行命令 进程 沙箱 运行` 候选 232/690
        = 34% > 上限 30% ⇒ 原口径返回 **0 条**，而按单词搜 `执行命令` 有 21 条命中。
        子代理据此判「平台没有这个能力」，退回 `rg` 枚举源码 —— 正是要根治的那条路。

        判据分两半：① 不再零命中，且条数**有界**（≤ 兜底低置信条数上限，绝不给全表）；
        ② 如实标注低置信（顶层 `低置信=真` + 判据里的 `低置信`/`是否无效`/`已回条数`），
        不许把低置信伪装成精确命中。
        """
        from 模块库.能力目录.实现.能力目录 import 兜底低置信条数上限

        调用结果 = self.搜索("执行命令 进程 沙箱 运行", 50)
        self.assertTrue(调用结果.成功, 调用结果.错误说明)
        值 = 调用结果.值
        self.assertGreater(值["总数"], 0, "占比超上限不得再判零命中")
        self.assertLessEqual(值["总数"], 兜底低置信条数上限,
                             "低置信返回必须有界（不得把全表充数）")
        self.assertTrue(值["低置信"], "占比超上限必须如实标注低置信")
        self.assertTrue(值["是否扩词"])
        self.assertTrue(值["兜底判据"]["低置信"])
        self.assertTrue(值["兜底判据"]["是否无效"])
        self.assertEqual(值["兜底判据"]["已回条数"], 值["总数"])
        self.assertIn("低置信", 值["兜底说明"])
        # 正确答案必须真在前列（否则「有界返回」只是换个姿势的噪声）
        前列 = [记录["能力id"] for 记录 in 值["能力列表"][:5]]
        self.assertIn("系统核心支持库.进程管理.沙箱执行命令", 前列,
                      f"低置信前列未含正确答案，返回无分辨力: {前列}")

    def test_兜底低置信不得影响精确命中路径(self):
        """反向：精确命中（整串子串匹配）的查询一行都不受低置信改动影响。"""
        调用结果 = self.搜索("执行命令", 5)
        self.assertTrue(调用结果.成功, 调用结果.错误说明)
        self.assertFalse(调用结果.值["低置信"], "精确命中路径不得被标低置信")
        self.assertFalse(调用结果.值["是否扩词"], "精确命中不得走兜底")
        self.assertEqual({}, 调用结果.值["兜底判据"])
...
class 装配口径与搜索面差额测试(能力目录测试基类):
    ...
    适配层「有同名孪生」的第二条腿，不许有第二种成因，也不许留下零孪生条目。

    为什么要有它：这个差额的成因已复发三次（2026-09-20 内部层 / 2026-09-21 未纳入
    顶层包根 / 适配层），每次都是「数字过期了、没人发现」才复发。判据把三条同时钉住：
    ① 搜索面 ⊆ 装配口径（能搜到的必须能调）；② 差额 ⊆ 适配层（无第二种成因）；
    ③ **差额里每一条都能在发现面找到同名能力**（零孪生一条都不许留在差额里）。

    ③ 是 2026-09-23 新加的硬判据，补的是「历史记 `72/72 零漏网`、实际无判据支撑」
    那个洞：现场按末段同名核，74 条里 **12 条零孪生**（`密码签名.签名`、
    `适配层.YAML提供者.检查可用性` 等）—— 它们经网关调得动（`适配层.YAML提供者.检查可用性`
    实测返回 `{可用: 真, 版本: 6.0.3}`）、经搜索 0 命中，即「可调但搜不到」。
    改成能力级判据后差额 74 → 62，且 ③ 保证它不会再长回去。
    """
    ...
    def test_装配口径与搜索面差额全部来自适配层有意排除(self):
        ...
        # 方向三（2026-09-23 新增）：差额里**每一条**都必须在发现面里存在同名能力
        # —— 零孪生一条都不许留在差额里（留下的就是「可调但搜不到」）。
        搜索名集 = {i.rsplit(".", 1)[-1] for i in 搜索ids}
        零孪生 = sorted(i for i in 差 if i.rsplit(".", 1)[-1] not in 搜索名集)
        self.assertFalse(
            零孪生,
            f"装配口径−搜索面 里出现**零孪生**条目（可调却搜不到，"
            f"即 2026-09-20/2026-09-21 那两轮同一病根）: {零孪生[:5]}")
