...
class 收工释放转发项目根测试(unittest.TestCase):
    ...
    def _释放参数(self, **收工入参) -> dict:
        ...
        def 假调用(能力id, 参数):
            捕获.setdefault(能力id, []).append(参数)
            return 结果.成功结果({"释放数": 1, "释放清单": [], "幂等清单": [], "说明": ""})

        with mock.patch.object(开工实现, "_调用", 假调用):
            出 = 开工实现.收工释放(租约id清单=["abc123"], **收工入参)
        self.assertTrue(出.成功, 出.错误说明)
        参数表 = 捕获.get(能力_释放文件租约) or []
        self.assertEqual(len(参数表), 1, "必须恰好发起一次释放")
        return 参数表[0]
    ...
    def test_转发项目根真能改解锁指纹(self) -> None:
        """端到端：把 `_调用` 换成**真转发**到 `释放文件租约`，验证转发的根真改了解锁现场指纹。
        ...
        with tempfile.TemporaryDirectory(prefix="收工释放转发") as 存储目录, \
            ...
            self.assertEqual(self._解锁现场指纹(开工实现, 释放文件租约, 查询改动历史,
                                        目标, 租约id, 存储目录, 错根, 现场),
                             "不存在",
    ...
    def test_项目根留空时解锁指纹落环境变量(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="收工释放留空") as 存储目录, \
            ...
            with mock.patch.dict(os.environ, {"系统平台_项目根": 错根}):
                实际 = self._解锁现场指纹(开工实现, 释放文件租约, 查询改动历史,
    ...
    def _解锁现场指纹(self, 开工实现模块, 释放文件租约, 查询改动历史,
                 目标: str, 租约id: str, 存储目录: str, 收工项目根, 现场: str) -> str:
        """经 `开工编排.收工释放` 释放（`_调用` 换成真转发到解锁腿），回带解锁事件的现场指纹。"""

        def 真转发调用(能力id, 参数):
            if 能力id == 能力_释放文件租约:
                return 释放文件租约(**参数)
            raise AssertionError(f"本用例只该调释放腿，实际调了 {能力id}")

        with mock.patch.object(开工实现模块, "_调用", 真转发调用):
            出 = 开工实现模块.收工释放(租约id清单=[租约id], 存储目录=存储目录,
                                项目根=收工项目根)
        self.assertTrue(出.成功, 出.错误说明)
