    ...
    def test_网关按注册参数拦下缺必填参数(self):
        ...
        全给 = 校验能力参数(
            ...
            {"源文件路径": "/abs/录制.mp4", "导出路径": "/abs/出稿.md",
             ...
             # 2026-09-25：开工ID 已是变更类能力的必填入参（写入授权判据按
             # 「活跃写租约所有者 == 开工ID」比对），齐全参数集必须带上它。
             "开工ID": "测试凭证"})
        self.assertEqual(全给, "", f"参数齐全却被判不合法: {全给}")

    def test_唯一判据对改乱的名序报红(self):
        """唯一判据必须真的拦：注册实现名序 ≠ 声明名序即报红（不静默漏参）。

        2026-09-23 判据唯一化③：本包原先那份同文件自比 `_校验参数名序` 已删除，
        验证改到唯一判据侧 —— 拿本包**真注册实现**，按真声明对一次（必须绿），
        再把声明的参数名序改乱（必须红）。一红一绿的反向对照保证判据既不恒红、
        也不恒绿（判据在 ≠ 判据覆盖）。
        """
        ...
        注册能力(注册表)
        能力id = max(注册表.能力id列表,
                   key=lambda k: len(注册表.获取(k).参数))  # type: ignore[union-attr]
        实现 = 注册表.获取(能力id)
        self.assertIsNotNone(实现)
        assert 实现 is not None
        参数 = list(实现.参数)
        self.assertGreater(len(参数), 1, "本包没有多参数能力，名序判据无从验证")

        def 声明表(换掉: list[dict] | None = None) -> list:
            """本包**全部**能力的声明。

            判据还查反向（注册了却没声明也算不一致），故必须整包给全，
            只给一条会把其余能力报成「已注册实现但未在声明中列出」。
            """
            return [能力声明(
                能力id=键,
                参数=(换掉 if 换掉 is not None and 键 == 能力id
                     else list(注册表.获取(键).参数)),  # type: ignore[union-attr]
                返回=注册表.获取(键).返回)  # type: ignore[union-attr]
                for 键 in 注册表.能力id列表]

        self.assertEqual(
            注册表.校验声明一致(实现.包id, 声明表()), [],
            "同序同返回口径必须判绿（反向对照：判据不是恒红）")
        改乱 = [dict(项) for 项 in 参数]
        改乱[0], 改乱[1] = 改乱[1], 改乱[0]
        问题 = 注册表.校验声明一致(实现.包id, 声明表(改乱))
        self.assertTrue(问题, "名序被改乱却判绿：唯一判据没拦住（静默漏参）")
        self.assertIn("不一致", 问题[0])
