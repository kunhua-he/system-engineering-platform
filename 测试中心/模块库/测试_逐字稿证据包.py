...
class Test证据包结构(unittest.TestCase):
    ...
    def test_术语非空逐条列出(self):
        文本 = 生成证据包([], [], [], "华世王镞、合作店；美团")
        术语段 = 文本.split("## 已确认术语", 1)[1].split("## 死循环区间", 1)[0]
        self.assertIn("- 华世王镞", 术语段)
        self.assertIn("- 合作店", 术语段)
        self.assertIn("- 美团", 术语段)
    ...
    def test_死循环区间渲染(self):
        死循环 = [{"开始秒": 600.0, "结束秒": 900.0, "重复次数": 18, "样例文本": "免费敷面膜"}]
        文本 = 生成证据包([], [], 死循环, "")
        段 = 文本.split("## 死循环区间", 1)[1]
        self.assertIn("- 10:00.0-15:00.0 重复 18 次 样例：免费敷面膜", 段)
