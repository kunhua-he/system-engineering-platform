...
class 归因唯一腿测试(unittest.TestCase):
    ...
    def test_每一条缺件都自带修法(self) -> None:
        """★ 2026-09-25 华哥现场裁定：「当出现的报错，必须是显示解决问题的办法。这个是漏洞」

        判据：取值表里**每一条**（含兜底的 `缺乏_未归因`）都必须自带 `→ 修法：` ——
        只写「缺什么」等于把定位与复现成本整包丢回调用方；兜底也必须能照着做
        （此前兜底写「须人看一次结论原文」，等于没给动作）。
        """
        条目 = [值 for 名, 值 in vars(编译公共).items()
               if 名.startswith("缺乏_") and isinstance(值, str) and 值]
        self.assertTrue(条目, "一条取值都没扫到 ⇒ 判据本身失效")
        for 说明 in 条目:
            self.assertIn("→ 修法：", 说明, f"这条缺件没有修法：{说明}")
        self.assertIn("→ 修法：", 编译公共.归因("某种没人见过的怪错误 abc"),
                      "兜底（未归因）也必须给可执行的修法")

    def test_写入边界归到专属缺件(self) -> None:
        """实测形态（2026-09-25 本仓 `开发工具/薄壳` 真跑）：未解析档计入违规即判红。

        改前这一条落到「未归因」，调用方拿不到任何动作；现归到 `缺乏_写入边界` 并带修法。
        """
        原文 = ("[阻断] 测试写入边界 退出码=1 耗时=5.25s："
                "  [判据一 写仓库内相对路径] 扫描 282 个 .py；写动作 1352 处"
                "（仓库内写 0／受管目录内 320／解析得出但不涉仓库 686／未解析（计入违规，fail-closed）346）")
        self.assertEqual(编译公共.缺乏_写入边界, 编译公共.归因(原文))
        self.assertIn("→ 修法：", 编译公共.归因(原文))
...
class 两能力同构测试(unittest.TestCase):
    ...
    def test_参数名与形制两能力一致(self) -> None:
        ...
        # ★ 2026-09-25 修（本件首次被真正跑起来时才暴露）：这条硬钉的期望表停在
        #   批R·R-12 **加 `明细级别` 之前**（那次是「只增参数」），于是本件**躺红无人知** ——
        #   根因是本件**没接进 `定向测试`**（`模块库.自修复工具` 的短名匹配到的是
        #   `测试_自修复工具`，本件文件名不含该短名，闭包又不覆盖包内文件）。
        #   已把「本件没被点名」登记为待办；在那之前，这条期望表必须跟着参数面走。
        self.assertEqual(["类型", "模块ID", "超时秒", "源码根", "明细级别"], 静)
