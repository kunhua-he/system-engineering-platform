...
class Test查询即懒回收_过期窗口第三态(锁夹具):
    ...
    **判据本体一个字不改** —— 本组第 1、4 条专门钉这一点（过期租约**仍不进**豁免面）。
    ...
    夹具：租约账**跑真身**（真认领、真过期、真回锁），只把它的存储目录指到夹具根之外的
    临时目录（`租约边界.默认存储目录`）—— 桩掉账本就没有第三态可验了；放夹具根之内则会被
    `上锁全仓()` 锁死，连第一个租约文件都写不进去。
    """
