...
class 锁夹具(unittest.TestCase):
    ...
    def 桩租约账(self, 活跃相对路径集) -> None:
        ...
        旧 = 授权.写租约事实源
        # ★ 2026-09-25：写租约事实源的值从**路径集**改成**映射**（路径 → 所有者，见
        #   `写入授权.写租约事实源` 的注释）：写入判据要问「这条路径的写窗口是不是**你**
        #   开的」，路径集答不出。本文件的用例只关心**开窗面**（路径），故所有者统一填夹具名
        #   —— 夹具桩的是事实源，被测判据（`活跃租约开窗集`／`未锁分档`）照旧跑真身。
        所有者表 = {str(路径): "夹具所有者" for 路径 in 活跃相对路径集}
        授权.设写租约事实源(lambda: (所有者表, ""))
        self.addCleanup(lambda: 授权.设写租约事实源(旧))

    def 认领路径(self, *相对路径: str) -> None:
        """声明这些路径**已被认领**（= 有一条活跃写租约在身），供写腿的**第二判据**读。

        为什么夹具必须有这一步（2026-09-24 批R-R24）：写腿的第一判据只判「是不是经 MCP」，
        第二判据（`公共契约/运行时/写入授权.校验写入授权`，华哥 2026-09-23 裁决）还要求
        **受管**路径被一条活跃写租约覆盖。真实链路上那一半由 `申请文件租约` 建立
        （认领成功即 `解锁供写入(目标 + 父链)` 开窗，两半一次做完）；本文件的用例在
        `tempfile` 临时根里造树，只调了 `解锁供写入`（窗口那一半）⇒ 租约那一半空着，
        写腿当场回 `越界`（16 条用例判红即由此而来，见批R-R24 单据）。

        走 `桩租约账` —— 本文件既有的**租约账桩**（`Test未锁分档_开窗与缺口` 同一条腿）：
        桩的是**外部事实源**，判据本体照旧跑真身，不桩被测判据；也不碰真仓库的租约账。
        """
        self.桩租约账(frozenset(相对路径))

    def 收工释放(self, *路径) -> None:
        """**租约终结**动作：把认领期开着的写窗口收回锁态（`回锁(连父链=真)`）。

        与 `认领路径` 配对 —— 认领开窗、收工收回，这才是生产的完整循环
        （`开工编排.收工释放` / `回收过期文件租约` 收口时做的就是这一步）。

        为什么夹具必须显式走它（2026-09-24 批R-R24，口径见 `未完成事项` O-17）：
        批N 裁决「**不许把活跃写租约的窗口锁回去**」—— `对齐目标锁态()` 对落在
        `活跃租约开窗集()` 里的条目直接 `continue`。故「认领的路径写完就自动锁上」
        **不再成立**：租约在身期间它**本来就该是未锁**（否则持有方下一次写、尤其
        不经写腿的写，当场 `PermissionError`），锁只在**租约终结**时收回。
        判据据此分两读，缺一不可：窗口期内**缺口数 0**（不得留真洞）＋ 收工后
        **未锁数 0 且目标锁着**。
        """
        for 项 in 路径:
            锁.回锁(项, 连父链=True)
...
class Test活跃租约窗口写腿要对齐锁态(锁夹具):
    ...
    **活跃租约集合逐条重合**。机制是两条既有腿的口径在租约窗口里对不上：
    ...
    ① 认领后、写之前：目标与父目录**确实未锁** —— 少了这一读，② 无法与「压根没解锁」区分
       （把窗口整段删掉也能让 ② 绿）；
    ② 经写腿写完（**租约仍活跃**）：整树 `查询()["缺口数"] == 0` —— 目标与其父目录那几条是
       **设计内的开窗**（`开窗数 >= 1`），不是洞；
    ③ `收工释放()`（租约终结）之后：目标与父目录**都回到锁态**，且整树 `未锁数 == 0`。

    ★ 2026-09-24 批R-R24 改口径（`未完成事项` O-17 的裁决落地，冲突原文见 N-8）：
    本类原先断言「**写完**即回到锁态、`未锁数 == 0`」，与批N 的 N-7 修法**正面冲突** ——
    `对齐目标锁态()` 对落在 `活跃租约开窗集()` 里的条目直接 `continue`（批N 原话：
    「不许把活跃写租约的窗口锁回去」，否则持有方下一次写、尤其不经写腿的写，当场
    `PermissionError`）。而授权集 ⊆ 开窗面 ⇒ 只要写腿过了 `写入授权` 那道门，目标就必然
    在开窗面里 ⇒「写完即锁」**结构性不可达**（N-8 已定：三族 16 条里 13 条卡在此处）。
    故本类改为「窗口期内缺口 0 ＋ 收工释放后未锁 0」两读，**一条断言都没删**。

    ★ 反向样本去哪了（如实声明，不包装）：批N 之后「写完即锁」这一读**不再可达**，
    故它也不再是 `对齐目标锁态` 的反向样本。该判据的反向样本由**未被认领的子条目**承担 ——
    见 `Test复核八处窗口缺口.test_解压到不存在的目标根在锁树里能建`（解压产物 `甲.txt`
    不在开窗面里，必须锁上）与 `test_压缩覆盖既有目标不留未锁残留`（备份目录必须被清掉）。
    """
    ...
    def _认领(self, 待建: Path) -> None:
        ...
        ★ 2026-09-24 批R-R24 补：认领其实是**两半** —— 窗口（`解锁供写入`）＋ **租约登记**
        （`申请文件租约` 把那条活跃租约落盘）。写腿的第二判据读的正是后一半，故夹具必须
        把两半都摆上（`认领路径`）；只摆窗口那一半就是「夹具与生产口径不符」，
        写腿当场回 `越界`（本类 3 条用例判红即由此而来）。
        """
        锁.解锁供写入(待建)
        self.认领路径(待建.relative_to(self.根).as_posix())
    ...
    def test_写入文件在活跃租约窗口里写完要收回锁(self) -> None:
        ...
        self.assertTrue(结果对象.成功, f"{结果对象.错误码} {结果对象.错误说明}")
        self.assertEqual("改过\n", 目标.read_text(encoding="utf-8"))
        # ② 租约仍活跃：目标与其父目录都是**设计内的开窗**（批N），判红只看缺口数
        窗口读 = 锁.查询(self.根)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 租约窗口期内多出真缺口 —— 每认领并改一次，整仓锁就多一个洞")
        # ③ 收工释放（租约终结）：窗口收回，整树回到未锁数 0
        self.收工释放(目标)
        self.assertTrue(锁.是锁着的(目标), "★ 收工释放后目标没回到锁态 —— 洞没收回")
        self.assertTrue(锁.是锁着的(目录),
                        "★ 收工释放后父目录没收回：目录开着就能凭空新增条目（锁要挡的最典型漂移）")
        self.assertEqual(0, 锁.查询(self.根)["未锁数"], "收工释放后整树必须回到未锁数 0")
    ...
    def test_应用精确替换在活跃租约窗口里写完要收回锁(self) -> None:
        ...
        窗口读 = 锁.查询(self.根)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"], "★ 补丁腿在租约窗口里写出真缺口")
        self.收工释放(目标)
        self.assertTrue(锁.是锁着的(目标), "★ 收工释放后目标没回到锁态")
        self.assertTrue(锁.是锁着的(目录), "★ 收工释放后父目录也必须收回")
        self.assertEqual(0, 锁.查询(self.根)["未锁数"], "收工释放后整树必须回到未锁数 0")
    ...
    def test_追加写入在活跃租约窗口里写完要收回锁(self) -> None:
        ...
        窗口读 = 锁.查询(self.根)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"], "★ 追加腿在租约窗口里写出真缺口")
        self.收工释放(目标)
        self.assertTrue(锁.是锁着的(目标), "★ 收工释放后目标没回到锁态")
        self.assertTrue(锁.是锁着的(目录), "★ 收工释放后父目录也必须收回")
        self.assertEqual(0, 锁.查询(self.根)["未锁数"], "收工释放后整树必须回到未锁数 0")
...
class Test新增条目腿要对齐锁态(锁夹具):
    ...
    **目标位置的 flags 与能否被终端改**，不看返回值。
    ...
    def test_复制进锁树副本要上锁(self) -> None:
        ...
        落点 = 锁树 / "复制进来.txt"
        self.认领路径("复制进来.txt")
        结果对象 = 复制文件(str(外部源), str(落点), 开工ID="夹具所有者")
        self.assertTrue(结果对象.成功, f"复制被挡：{结果对象.错误码} {结果对象.错误说明}")
        窗口读 = 锁.查询(锁树)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 复制进锁树的副本成了**真缺口** —— 锁树里凭空多一个洞（洞 ④）")
        self.收工释放(落点)
        self.assertTrue(锁.是锁着的(落点),
                        "★ 收工释放后副本没上锁 —— 锁树里凭空多一个洞（洞 ④）")
        self.assertEqual(0, 锁.查询(锁树)["未锁数"], "收工释放后整树必须回到未锁数 0")
    ...
    def test_锁树里建目录要上锁(self) -> None:
        ...
        新目录 = 锁树 / "新目录"
        self.认领路径("新目录")
        结果对象 = 创建目录(str(新目录), 开工ID="夹具所有者")
        self.assertTrue(结果对象.成功, f"建目录被挡：{结果对象.错误码} {结果对象.错误说明}")
        窗口读 = 锁.查询(锁树)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 新建目录成了**真缺口** —— 那是「锁树里的一块未锁区域」")
        self.收工释放(新目录)
        self.assertTrue(锁.是锁着的(新目录),
                        "★ 收工释放后新建目录没上锁 —— 那是「锁树里的一块未锁区域」")
        self.assertEqual(0, 锁.查询(锁树)["未锁数"], "收工释放后整树必须回到未锁数 0")
    ...
    def test_移动进锁树要上锁(self) -> None:
        ...
        落点 = 锁树 / "搬进来.txt"
        self.认领路径("搬进来.txt")
        结果对象 = 移动文件(str(外部源), str(落点), 开工ID="夹具所有者")
        self.assertTrue(结果对象.成功, f"移动被挡：{结果对象.错误码} {结果对象.错误说明}")
        窗口读 = 锁.查询(锁树)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 搬进锁树的条目成了**真缺口** —— 移动腿与复制腿同族，必须一起对齐")
        self.收工释放(落点)
        self.assertTrue(锁.是锁着的(落点),
                        "★ 收工释放后搬进来的条目没上锁 —— 移动腿与复制腿同族，必须一起对齐")
        self.assertEqual(0, 锁.查询(锁树)["未锁数"], "收工释放后整树必须回到未锁数 0")
    ...
    def test_压缩产物要上锁(self) -> None:
        ...
        成品 = 锁树 / "成品.zip"
        self.认领路径("成品.zip")
        结果对象 = 压缩文件(str(成员), str(成品), 开工ID="夹具所有者")
        self.assertTrue(结果对象.成功, f"压缩被挡：{结果对象.错误码} {结果对象.错误说明}")
        窗口读 = 锁.查询(锁树)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 压缩成品成了**真缺口** —— `os.replace` 会把临时件的未锁状态带到成品上（洞 ④）")
        self.收工释放(成品)
        self.assertTrue(锁.是锁着的(成品),
                        "★ 收工释放后成品没上锁 —— `os.replace` 把临时件的未锁状态带到了成品上（洞 ④）")
        self.assertEqual(0, 锁.查询(锁树)["未锁数"], "收工释放后整树必须回到未锁数 0")
...
class Test复核八处窗口缺口(锁夹具):
    ...
    **静默留残或静默失效**（比报错难查得多）。
    ...
    def test_临时解锁树覆盖多层新建(self) -> None:
        ...
        深 = self.根 / "甲" / "乙" / "丙"
        self.认领路径("甲/乙/丙")
        # ★ 2026-09-25：`临时解锁树` 是写入授权判据的第三个调用面（内核锁窗口）——
        #   受管路径必须带**与活跃写租约所有者一致**的 开工ID，否则抛 PermissionError。
        #   本夹具的租约桩（`认领路径`）把「甲/乙/丙」记在 `夹具所有者` 名下，窗口用同一凭证。
        with 锁.临时解锁树(深, 开工ID="夹具所有者"):
            try:
    ...
    def test_解压失败回滚不留未锁残留(self) -> None:
        ...
        锁树 = self._锁树()
        self.认领路径("解压到")
        结果对象 = 解压文件(str(源), str(目标根), 开工ID="夹具所有者")
    ...
    def test_解压到不存在的目标根在锁树里能建(self) -> None:
        ...
        # 目标根本身是认领过的（设计内开窗）；但**产物** 甲.txt 不在开窗面里 ⇒ 必须锁上
        窗口读 = 锁.查询(锁树)
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"], "★ 解压写出真缺口")
        self.收工释放(目标根)
        self.assertEqual(0, 锁.查询(锁树)["未锁数"], "收工释放后整树必须回到未锁数 0")
...
class Test失败路径落点也要对齐锁态(锁夹具):
    ...
    def test_复制失败半截落点也必须上锁(self) -> None:
        ...
        with mock.patch("shutil.copy2", autospec=True, side_effect=_写半截后抛):
            结果对象 = 复制文件(str(源), str(落点), 开工ID="夹具所有者")
        ...
        self.assertGreaterEqual(窗口读["开窗数"], 1, "★ 认领的窗口不在开窗面上 —— 夹具没造出现场")
        self.assertEqual(0, 窗口读["缺口数"],
                         "★ 半截落点成了**真缺口** —— 它进入窗口时不存在，不在「原状态」表里，"
    ...
    def test_移动失败半截落点也必须上锁(self) -> None:
        ...
        with mock.patch("shutil.move", autospec=True, side_effect=_写半截后抛):
            结果对象 = 移动文件(str(源), str(落点), 开工ID="夹具所有者")
        self.assertFalse(结果对象.成功, "★ 写半截抛错被伪装成了成功")
    ...
    def test_失败在建出中间层后中间层也必须上锁(self) -> None:
        ...
        self.assertTrue(锁.是锁着的(self.根 / "中间一" / "中间二"),
                        "★ 收工释放后第二层中间目录也必须锁上（它不在 查询() 的扫描面里，必须直读 flags）")
...
class Test查询即懒回收_过期窗口第三态(锁夹具):
    """★ 2026-09-24 批P·P-1（O-17 的活体第三态）：`查询()` 必须先顺手回收过期租约，再分档。

    缺陷本体：一条写租约的**到期是时间的函数、不是事件** —— `申请文件租约` 认领成功即
    `解锁供写入(目标 ＋ 父链)` 开窗，而回锁只在**下一次认领的懒回收**里发生。于是
    「租约已过期、窗口还开着、没人来收」这段里，`活跃租约开窗集()` 的 `是活跃`
    （状态活跃 **且** 未过期）把它判成「既不是开窗、也不是写腿被杀留下的真缺口」
    ⇒ 落进 `缺口数` ⇒ `pre-commit` 按判据一拒提交（主会话活体实证：未锁 29 / 开窗 0 / 缺口 29）。

    修法（华哥裁决「判据不动，接已有节点」）：把**已有的**回收腿
    （`回收过期文件租约`，经 `文件租约存储.回收过期租约并报数` 注册进
    `写入授权.设租约回收器`）挂到 `查询()` 这个本来就有的唤醒点上。
    **判据本体一个字不改** —— 本组第 1、4 条专门钉这一点（过期租约**仍不进**豁免面）。

    夹具：租约账**跑真身**（真认领、真过期、真回锁），只把账目录指到夹具根之外的临时目录
    —— 桩掉账本就没有第三态可验了；放夹具根之内则会被 `上锁全仓()` 锁死，连第一个租约文件
    都写不进去。★ 2026-09-24 批Q·Q-k：账目录改走**生产自带的注入口**（租约这一族腿本来就
    收的 `存储目录` 参数 ＋ 两个注册口），不再 `mock.patch.object` 换掉
    `租约边界.默认存储目录` —— 那是替换被测模块的**本体成员**（`测试伪装门禁` 规则1·P1），
    等于测的是假对象。
    """
    ...
    def _换账目录(self) -> str:
        """给本组开一本**夹具自己的**租约账（临时目录），路径记在 `self.账目录`。

        为什么要真账本（不桩）：本组要验的正是「**真**租约**真**过期 ⇒ 真回锁」这条链，
        桩掉账本就没有第三态可验了（判据本体仍跑真身）。
        为什么放在夹具根**之外**：夹具根会被 `上锁全仓()` 整棵锁死，账目录若在里面，
        连第一个租约文件都写不进去（锁住的目录里建不了新条目）。

        ★ 账目录怎么生效：**不再 patch 模块级 `默认存储目录`**，改把 `self.账目录` 显式传给
        每一个碰账的调用点（`_认领` / `_账上记录` ＋ 下面 `_接真租约账` 里的两个注册口）。
        这不是另起一套：`存储目录` 本来就是租约这一族腿的通用口径（`申请文件租约` /
        `查询文件租约` / `回收过期文件租约` 都收它），本组只是把「哪一本账」明说出来。
        """
        临时 = tempfile.TemporaryDirectory(prefix="租约账_")
        self.addCleanup(临时.cleanup)
        self.账目录 = 临时.name
        return 临时.name

    def _接真租约账(self) -> None:
        """把**两个**注册口都接回真腿，并把它们绑到本组那本账（`self.账目录`）。

        基类 `setUp` 默认把账本桩成空集（`桩租约账`），那是给「不碰租约」的用例用的；
        本组必须读真账，故在此换回真腿；基类登记的还原在本函数之前（`addCleanup` 后进先出）。

        两个口都要绑（**缺一即假绿**）：`活跃租约开窗集()` 走**事实源**读开窗集，
        而 `查询()` 的顺手回收走**回收器**；只绑事实源 ⇒ 回收器仍去改判平台默认那本账
        （别的会话正在用的账），本组的「过期 ⇒ 回锁」读数就与账本无关了。
        """
        from 公共契约.运行时 import 写入授权 as 授权
        ...
        旧事实源 = 授权.写租约事实源
        旧回收器 = 授权.租约回收器
        授权.设写租约事实源(lambda: 写租约所有者表(存储目录=账目录))
        授权.设租约回收器(lambda 根: 回收过期租约并报数(根, 存储目录=账目录))
        self.addCleanup(lambda: 授权.设写租约事实源(旧事实源))
        self.addCleanup(lambda: 授权.设租约回收器(旧回收器))

    def _建树(self) -> tuple[Path, Path]:
        """建 `开发文档/分析.md` ＋ `开发文档/缺口.txt`（＋ `在写.md` 对照）并整仓上锁。

        三个文件都必须在 `上锁全仓()` **之前**建好：目录一旦锁上，里面就建不出新条目。
        """
        目录 = self.根 / "开发文档"
        目录.mkdir()
        for 名 in ("分析.md", "缺口.txt", "在写.md"):
            (目录 / 名).write_text("原始内容\n", encoding="utf-8")
        subprocess.run(["git", "init", "-q"], cwd=self.根, check=False, capture_output=True)
        锁.上锁全仓(self.根)
        self.assertEqual(0, 锁.查询(self.根)["缺口数"], "前置：夹具根应已全锁")
        return 目录 / "分析.md", 目录 / "缺口.txt"

    def _认领(self, 相对: str, 持有秒: float, 任务: str) -> None:
        """真认领一条租约（走真能力 `申请文件租约`，含它自己的懒回收）。

        账目录**显式**传 `self.账目录`（本组那本隔离账）：不传就落平台默认账，
        本组要验的「真认领 ⇒ 真过期 ⇒ 真回锁」就与读数无关了。
        """
        from 平台控制面.能力目录.实现.文件租约申请 import 申请文件租约

        结果 = 申请文件租约(修改路径=[相对], 所有者="回归锁-夹具", 任务=任务,
                            持有秒=持有秒, 项目根=str(self.根), 存储目录=self.账目录)
        self.assertTrue(getattr(结果, "成功", 假), f"前置：认领 {相对} 应成功，实际 {结果}")
        self.assertEqual(1, (结果.值 or {}).get("认领数"), f"前置：应认领 1 条，实际 {结果.值}")

    def _账上记录(self, 相对: str) -> dict:
        """直接读租约账（**取证**用，不是判据）：确认盘上状态与过期时间。

        读的是**本组那本账**（`self.账目录`）—— 与 `_认领` 写的那一本必须是同一本，
        否则「盘上状态仍写着活跃」这条前置就在读别的树/别的会话的账。
        """
        ...
        with 存储.独占():
            快照, 读问题 = 存储.读取()
        self.assertEqual("", 读问题, "前置：租约账应可读")
        命中 = [记录 for 记录 in 快照.values()
                if str((记录 or {}).get("路径") or "").strip("/") == 相对]
        self.assertEqual(1, len(命中), f"前置：账上应有且只有 {相对} 一条，实际 {sorted(快照)}")
        return 命中[0]

    def _造第三态(self, 相对: str) -> dict:
        """造出 O-17 的**活体第三态**：窗口已开、租约已过期、还没人回收。"""
        self._认领(相对, 0.0, "P-1 第三态：持有秒 0")
        目标 = self.根 / 相对
        self.assertFalse(锁.是锁着的(目标), "前置：认领应已打开写窗口（目标未锁）")
        self.assertFalse(锁.是锁着的(目标.parent), "前置：父链也应已放开")
        记录 = self._账上记录(相对)
        self.assertEqual("活跃", str(记录.get("状态") or ""), "前置：盘上状态仍写着活跃")
        self.assertLessEqual(float(记录.get("过期时间")), time.time(), "前置：租约应已过期")
        return 记录

    def _数回收器(self) -> list:
        """给**真**回收器套一层计数（观测接线，不换实现）：返回调用记录表。"""
        from 公共契约.运行时 import 写入授权 as 授权

        真回收器 = 授权.租约回收器
        self.assertIsNotNone(真回收器, "前置：平台控制面侧应已注册租约回收器")
        调用: list = []
        授权.设租约回收器(lambda 根: (调用.append(str(根)), 真回收器(根))[1])
        self.addCleanup(lambda: 授权.设租约回收器(真回收器))
        return 调用

    def test_判据本体未改宽_过期租约不进豁免面(self) -> None:
        """★ 「判据不动」的机器根据：同一段代码，差异只来自**时间**。

        一条已过期（第三态）、一条未过期（在写）—— 过期的**仍不进**豁免面（落缺口），
        未过期的仍是开窗（连祖先链）。判据本体若被改宽（例如把 `是活跃` 里「未过期」
        那一半去掉），本用例当场变红。
        """
        self._建树()
        self._认领("开发文档/在写.md", 600.0, "P-1 对照：未过期")
        self._造第三态("开发文档/分析.md")
        集合, 问题 = 锁.活跃租约开窗集()
        self.assertEqual("", 问题)
        self.assertNotIn("开发文档/分析.md", 集合, "★ 过期租约不许进豁免面（判据一个字不改）")
        self.assertIn("开发文档/在写.md", 集合, "对照：未过期租约仍是开窗")
        self.assertIn("开发文档", 集合, "对照：开窗连祖先链一起豁免")
        分 = 锁.未锁分档(["开发文档/分析.md", "开发文档/在写.md"], 豁免清单=集合)
        self.assertEqual((1, 1), (分["开窗数"], 分["缺口数"]),
                         "★ 过期那条落缺口、未过期那条算开窗")
        self.assertEqual(["开发文档/分析.md"], 分["缺口样本"])

    def test_第三态被查询收回_缺口归零(self) -> None:
        """★ 修后形状：`查询()` 顺手回收 ⇒ 过期窗口当场回锁 ⇒ 未锁 0 / 开窗 0 / 缺口 0。

        主会话修前活体读数（同一形状）：未锁 29 / 开窗 0 / 缺口 29 ⇒ `pre-commit` 拒提交。
        """
        目标, _ = self._建树()
        self._造第三态("开发文档/分析.md")
        调用 = self._数回收器()
        结果 = 锁.查询(self.根)
        self.assertEqual(1, len(调用), "★ 查询应顺手回收一次（挂在本来就有的唤醒点上）")
        self.assertTrue(锁.是锁着的(目标), "★ 过期窗口必须回锁（写窗口已静默关闭）")
        self.assertEqual(0, 结果["未锁数"], "★ 第三态被收回 ⇒ 未锁归零")
        self.assertEqual(0, 结果["开窗数"])
        self.assertEqual(0, 结果["缺口数"], "★ 不再误报真缺口（pre-commit 不再误拒）")
        self.assertIn("顺手回收过期租约 1 条", 结果["说明"], "读数如实带出")
        self.assertEqual("已过期", str(self._账上记录("开发文档/分析.md").get("状态") or ""),
                         "盘上记录应已改判「已过期」")

    def test_真缺口不因回收被抹平(self) -> None:
        """★ 反向样本一：写腿中途被杀留下的**真缺口**照样判红（缺口数 不许被永久做成 0）。"""
        目标, 缺口 = self._建树()
        self._造第三态("开发文档/分析.md")
        锁.解锁(缺口)                       # 真缺口：人为 nouchg，账上没有任何租约
        结果 = 锁.查询(self.根)
        self.assertEqual(1, 结果["未锁数"], "回收只该收回第三态那一条（目标 ＋ 父链）")
        self.assertEqual(1, 结果["缺口数"], "★ 真缺口仍判红（安全判据没被作废）")
        self.assertEqual(["开发文档/缺口.txt"], 结果["缺口样本"])
        self.assertTrue(锁.是锁着的(目标), "第三态那条已被回锁")
        self.assertIn("顺手回收过期租约 1 条", 结果["说明"])

    def test_摘掉回收器即回到修前形状_过期窗口算缺口(self) -> None:
        """★ 反向样本二：摘掉回收器 ⇒ 第三态原样落进缺口（这正是修前那条 `pre-commit` 误拒）。

        也钉住「绿不是判据改宽来的」：绿只在**回收真跑过**时出现；没有回收腿，读数与修前同形
        （未锁 == 缺口、开窗 0）。摘掉后 `_取回收器()` 的懒加载**不会再注册**（本进程已载入
        平台控制面），故这是一次真的「没有回收腿」。
        """
        from 公共契约.运行时 import 写入授权 as 授权
        ...
        旧 = 授权.租约回收器
        授权.设租约回收器(None)
        self.addCleanup(lambda: 授权.设租约回收器(旧))
        self.assertIsNone(授权._取回收器()[0], "前置：懒加载不会再注册 ⇒ 真·没有回收腿")
        结果 = 锁.查询(self.根)
        self.assertEqual(3, 结果["未锁数"], "第三态两条（目标 ＋ 父链）＋ 真缺口一条")
        self.assertEqual(0, 结果["开窗数"], "★ 过期租约仍不算开窗（判据本体没被改宽）")
        self.assertEqual(3, 结果["缺口数"], "★ 没有回收腿 ⇒ 过期窗口落缺口（修前形状）")
        self.assertIn("顺手回收未成功", 结果["说明"], "回收没跑成必须如实留痕，不许静默")

    def test_全绿不调回收器_零代价(self) -> None:
        """全绿（没有未锁）这条极常见路径**连回收器都不调**：零写盘、零额外依赖。"""
        self._建树()
        调用 = self._数回收器()
        结果 = 锁.查询(self.根)
        self.assertEqual([], 调用, "★ 没有未锁就不跑回收（回锁对象就是这些未锁条目，白跑一趟写盘）")
        self.assertEqual(0, 结果["未锁数"])
        self.assertEqual(0, 结果["缺口数"])
        self.assertNotIn("顺手回收", 结果["说明"])
