...
class 登记与流水测试(文件租约测试基类):
    ...
    def test_冲突策略整体拒绝是默认老行为(self) -> None:
        ...
        申请文件租约(修改路径=[文件1], 所有者="会话1", 任务="占住", 存储目录=self.存储目录)
    ...
    def test_两个指纹拉出中途全部流水(self) -> None:
        ...
        登记改动意图(修改路径=[{"路径": 文件1, "指纹": 旧指纹}], 所有者="会话1",
                   任务="想改", 存储目录=self.存储目录)
        认领 = 申请文件租约(修改路径=[{"路径": 文件1, "指纹": 旧指纹}], 所有者="会话1",
                         任务="动手改", 存储目录=self.存储目录, 校验指纹=真)
        self.assertTrue(认领.成功)
        租约id = 值(认领)["租约id清单"][0]
        文件租约留言(租约id=租约id, 作者="会话1", 留言="加了一行注释", 存储目录=self.存储目录)
        目标文件 = 仓库根 / 文件1
        原内容 = 目标文件.read_bytes()
        try:
    ...
    def test_登记参数不合法一律拒绝(self) -> None:
        ...
        for 参数 in ({"所有者": "", "任务": "有任务"},
                    {"所有者": "会话1", "任务": ""}):
            结果 = 登记改动意图(修改路径=[文件1], 存储目录=self.存储目录, **参数)
            self.assertFalse(结果.成功)
            self.assertEqual(结果.错误码, "参数不合法")
