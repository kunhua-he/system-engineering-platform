...
def 建服务() -> tuple[统一能力服务, Path]:
    目录 = Path(tempfile.mkdtemp(prefix="维护者视图测试_"))
    return 统一能力服务(目录), 目录
...
class Test维护者视图核心(unittest.TestCase):
    ...
    def test_冲突裁决写入能力条目可读回(self):
        self.服务.目录.登记能力(能力id="能力甲", 契约={"能力id": "能力甲"},
                             组件="甲", 领域="领域甲")
        结果 = self.视图.冲突裁决(self.令牌, "能力甲", "复用")
        self.assertTrue(结果["成功"], 结果.get("消息"))
        记录 = self.服务.状态.读取记录("能力条目", "能力id", "能力甲")
        self.assertEqual(记录["复用裁决"], "复用")
        self.assertEqual(记录["裁决状态"], "已裁决")
        # 裁决有真实证据
        证据 = self.服务.状态.查询证据(主题="能力甲", 类型="能力目录")
        self.assertEqual(证据[0]["内容"]["裁决"], "复用")
        # 非法裁决值被拒
        结果 = self.视图.冲突裁决(self.令牌, "能力甲", "任意值")
...
class Test维护者权限边界(unittest.TestCase):
    ...
    def test_维护者回滚被授权拦截(self):
        ...
        self.服务.发布.激活(发布id=发布id, 目标="摘要甲")
    ...
    def test_未授予身份访问视图被拒(self):
        ...
        结果 = self.视图.查看依赖(访客令牌, "能力甲")
        self.assertFalse(结果["成功"])
        self.assertEqual(结果["错误码"], "PERMISSION_DENIED")
        结果 = self.视图.冲突裁决(访客令牌, "能力甲", "复用")
        self.assertEqual(结果["错误码"], "PERMISSION_DENIED")
...
class Test维护者视图只读视图(unittest.TestCase):
    ...
    def test_查看资源返回真实句柄与线程(self):
        ...
        视图 = 维护者视图(服务)
        令牌 = 提权(服务, "维护者", "平台维护者")
        服务.状态.保存句柄(句柄id="句柄甲", 句柄类型="文件", 资源id="资源甲",
