...
class Test可信仓库元数据(unittest.TestCase):
    ...
    def test_生成快照校验通过_版本回退被阻断(self):
        仓库 = 可信仓库元数据(self.目录)
        仓库.初始化()
        仓库.发布目标("包甲", "1", 内容摘要("内容甲".encode("utf-8")), 制品内容="内容甲".encode("utf-8"))
        快照1 = 仓库.生成快照()
        快照2 = 仓库.生成快照()
        self.assertEqual(快照2["元数据版本"], 快照1["元数据版本"] + 1, "快照版本必须单调递增")
        self.assertIn("包甲@1", 快照2["目标表"])
        ...
        篡改快照 = dict(快照2)
        篡改快照["目标表"] = {"包甲@1": 内容摘要("别的摘要".encode("utf-8"))}
    ...
    def test_错误公钥验证失败_签名无效(self):
        仓库甲 = 可信仓库元数据(self.目录 / "甲")
        仓库甲.初始化()
        目标 = 仓库甲.发布目标("包乙", "1", 内容摘要(b"x"), 制品内容=b"x")
        仓库乙 = 可信仓库元数据(self.目录 / "乙")
        仓库乙.初始化()
        成功, 原因 = 仓库乙.校验元数据("目标", 目标)
