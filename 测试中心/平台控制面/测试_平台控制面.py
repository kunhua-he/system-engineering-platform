...
class Test需求与CAS(unittest.TestCase):
    ...
    def test_需求变化生成新版本不覆盖旧快照(self):
        快照1 = self.服务.需求.登记需求(目标="目标甲")
        快照2 = self.服务.需求.登记需求(目标="目标乙")
...
class Test复用治理与占用(unittest.TestCase):
    ...
    def test_相同契约指纹阻断重复实现(self):
        ...
        成功, _ = self.服务.目录.登记能力(能力id="文件.读取", 契约=契约, 组件="组件甲", 领域="文件")
        self.assertTrue(成功)
        成功2, 消息2 = self.服务.目录.登记能力(能力id="文件.读取2", 契约=契约, 组件="组件乙", 领域="文件")
    ...
    def test_占用租约过期后新Agent可接管(self):
        成功, _, 租约id = self.服务.目录.申请占用(
            能力id="b.能力", 领域="领域", 契约指纹="指纹", 任务="任务甲", 所有者="Agent甲")
        self.assertTrue(成功)
        ...
        成功2, _, _ = self.服务.目录.申请占用(
            能力id="b.能力", 领域="领域", 契约指纹="指纹", 任务="任务乙", 所有者="Agent乙")
        self.assertTrue(成功2, "租约过期后新 Agent 可接管")

    def test_疑似重叠未裁决不能发布(self):
        契约 = {"能力id": "c.能力"}
        指纹 = 契约指纹(契约)
        self.服务.目录.登记能力(能力id="c.能力", 契约=契约, 组件="甲", 领域="域")
...
class Test证据账本(unittest.TestCase):
    ...
    def test_证据只追加不覆盖(self):
        服务, _ = 建服务()
        证据id1 = 服务.状态.追加证据(类型="测试", 主题="主题甲", 内容={"值": 1})
        证据id2 = 服务.状态.追加证据(类型="测试", 主题="主题甲", 内容={"值": 2})
