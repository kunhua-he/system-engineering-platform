...
def 执行单文件记录依赖(路径: str) -> int:
    ...
    """
    from 测试中心.文件访问采集器 import 安装采集器, 静态扫描弱依赖
    ...
    依赖清单 = 采集器.提取依赖清单()
    # stat 类存在性探测（exists/is_file/getsize 等）不触发 audit 事件，
    # 静态扫描测试源码命中即标弱依赖（探测结果决定分支但内容未被读取）
    静态弱依赖特征 = 静态扫描弱依赖(文件路径.read_text(encoding="utf-8"))
