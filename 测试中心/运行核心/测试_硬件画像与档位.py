...
from 运行核心.自适应选型.档位表 import 内存水位硬上限比例, 并发旋钮按核数档
...
def _该档期望性能参数(核数档名: str) -> dict:
    """该核数档的**期望性能参数** = `标定参数集` 被 `并发旋钮按核数档` 覆盖。

    **为什么不再「逐档等于 `标定参数集`」**（2026-09-23 修，事实已变、判据跟着走）：
    `档位表._构建档位表` 的 docstring 明载「2026-09-21：从「每档都等于标定值」改为
    **按核数档分化并发旋钮**」（华哥裁决「动态判断电脑性能去调控」），分化面**只有**
    `并发旋钮按核数档` 列出的三个键（`网关工作线程`/`网关请求队列上限`/`流式并发通道`），
    其余字段仍取标定值。原断言写死「任何档位 == 标定参数集」，随该分化而过期。

    **判据强度不变**：仍是逐字段**精确相等**，只是期望值从常量改为「标定值 + 该核数档
    的并发旋钮」—— 分化面的任何一个键被改错、或多调了分化面之外的键，本判据照样判红。
    """
    期望 = dict(标定参数集)
    期望.update(并发旋钮按核数档[核数档名])
    return 期望


class Test首版标定与现值一致(unittest.TestCase):
    """★并发旋钮按核数档分化（2026-09-21）：除并发旋钮三键外逐字段对账**真实源码**现值。"""
    ...
    def test_每档性能参数都等于标定参数集(self) -> None:
        """并发旋钮按核数档分化（2026-09-21）：**任何**档位槽位的性能参数都必须
        逐字段等于**该核数档的期望值**（`标定参数集` + `并发旋钮按核数档`）。"""
        for 键, 槽位 in 档位表().items():
            self.assertEqual(槽位.性能参数, _该档期望性能参数(键.核数档),
                             f"档位 {键.文本()} 的性能参数偏离了该核数档的期望值")

    def test_并发旋钮按核数档真分化(self) -> None:
        """反向的另一半（防「分化表退化成一张同值表」）：小核/中核/大核的并发旋钮
        必须**互不相同**，且分化面**只有**这三个键 —— 否则上面的对账可能恒真。

        与 `档位表._构建档位表` 的 docstring 同源：「分化面**只有并发**，不做
        「顺手多调几个」的扩张（那样无法解释每个值的来源）」。
        """
        小核 = 并发旋钮按核数档["小核"]
        中核 = 并发旋钮按核数档["中核"]
        大核 = 并发旋钮按核数档["大核"]
        self.assertNotEqual(小核, 中核, "小核与中核的并发旋钮不得同值（分化退化为空转）")
        self.assertNotEqual(中核, 大核, "中核与大核的并发旋钮不得同值（分化退化为空转）")
        分化面 = set(小核)
        self.assertEqual({"网关工作线程", "网关请求队列上限", "流式并发通道"}, 分化面,
                         "分化面只允许这三个并发键（多调即超出可解释来源）")
        # 分化只落在并发旋钮上：其余字段逐档仍等于标定值。
        标定外的键 = set(档位表()[next(iter(档位表()))].性能参数) - set(标定参数集)
        self.assertEqual(set(), 标定外的键, f"出现了标定参数集之外的性能键：{标定外的键}")

    def test_任意画像的最终性能参数都等于现值(self) -> None:
        """换画像只换**档位**，不换该档位的性能参数 —— 参数由所选档位的核数档唯一决定。"""
        for 名称, 画像 in 规定画像表.items():
            for 形态 in 部署形态域:
                参数集 = 按画像定档位(画像, 形态)
                self.assertEqual(参数集.性能参数, _该档期望性能参数(参数集.选自档位.核数档),
                                 f"{名称}/{形态} 的性能参数偏离了该核数档的期望值")
