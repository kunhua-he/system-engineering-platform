...
class 网关类型收口验收(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.后端.注册能力("测试.自造名回显", _回显,
                          参数=[{"名称": "X", "类型": "字符串型", "必填": 假}])
        cls.后端.注册能力("测试.句柄回显", _回显,
                          参数=[{"名称": "句柄", "类型": "句柄型", "必填": 假}])
    ...
    def test_已撤除别名数值型按非正式类型名被拒(self) -> None:
        ...
        同时保留原强度断言：换用正式名 `双精度数型` 后**小数与整数都放行**、布尔/文本仍被拒，
        证明别名撤除**只收紧名字口径、不改变正式名强度**（别名还在时应有的行为全在这）。

        整数放行是 2026-09-21 的刻意放宽：JSON 只有一种数值类型，`120` 与 `120.0` 都是
        合法 JSON 数字，落到 Python 侧是 int 还是 float 由传输层决定。此前只收 float，
        等于把传输细节变成调用方的错 —— 实测 Agent 传 `超时秒: 120` 连错 3 次。
        int→double 无损故放行；`布尔`（bool 是 int 子类）与 `文本` 仍必须拒。
        """
        小数 = self.调用("测试.数值秒回显", {"等待秒": 0.5})
        self.assertEqual(小数["状态码"], 200, f"正式名双精度数型的小数必须放行: {小数}")
        整数 = self.调用("测试.数值秒回显", {"等待秒": 2})
        self.assertEqual(整数["状态码"], 200,
                         f"双精度数型必须接受整数（JSON 数字无整数/小数之分）: {整数}")
        self.assertTrue(整数["体"]["成功"], 整数)
        布尔 = self.调用("测试.数值秒回显", {"等待秒": 真})
