...
#: 标定常量的**真值所在模块**（拆分后的单一来源）：模块级可变常量必须打到它，
#: 打到门面 `超时执行`（按名 re-export）改不到真值 ⇒ 用例静默失真（20260919 实测）。
from 运行核心.能力调用.超时执行 import 基础 as 基础模块
...
class Test线程侧有界准入(unittest.TestCase):
    ...
    def test_每能力闸门登记有上限_超限走兜底不无界增长(self) -> None:
        ...
        # ★ 模块级可变常量**必须打到真值所在模块**：拆分后 `超时执行.py` 已是一层
        #   门面（同名目录包按名 re-export），给门面属性赋值改不到包内单元里的真值，
        #   消费方读的仍是原值 ⇒ 上限卡不住而本用例静默失真（20260919 实测）。
        #   单来源：`运行核心/能力调用/超时执行/基础.py`。
        原始上限 = 基础模块.每能力闸门数上限
        基础模块.每能力闸门数上限 = 2
        try:
