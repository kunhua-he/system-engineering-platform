...
- 反向破坏：删除 包声明/契约/锁 任一项，生产链必须真实整体阻断
  （删契约经 S0.4 组件合规 13/13 权威门禁证明，删锁经提供者依赖锁
  fail-closed 强制校验证明）；删除 注册入口/实现 属单包级半成品问题，
  只跳过该包并告警、其余包照常装配，且被跳过包的能力对调用方不可见
  （架构项A，2026-09-15：单包问题不阻断全平台，但绝不放行半装配）。
"""
...
# 的 `from 支持库.适配层 import 签名` 会打到桩上 ImportError——那是桩的缺口，
# 不是平台缺陷，所以按最小口径把公开导出补齐（2026-09-15）。
影子适配层入口 = '''"""冷启动影子适配层：只含系统探针/脱敏模式与签名公开入口（桩）。"""

from __future__ import annotations

import base64

from 支持库.适配层.密码签名提供者 import 内容摘要 as _提供者内容摘要
from 支持库.适配层.密码签名提供者 import 生成密钥对 as _提供者生成密钥对
from 支持库.适配层.密码签名提供者 import 签名 as _提供者签名
from 支持库.适配层.密码签名提供者 import 验证签名 as _提供者验证签名
...
class 冷启动反向门禁基础(unittest.TestCase):
    ...
    def 复制真实制品(self) -> None:
        """复制已安装真实制品：文件系统/系统核心 支持库 + 文件管理 模块。
        ...
        for 源, 目标 in (
            ...
            (支持库根 / "后端" / "系统核心支持库", self.根 / "支持库" / "后端" / "系统核心支持库"),
            # 并发控制支持库：无第三方依赖（包声明 依赖=[]）且无依赖锁，只 import 公共契约，
            # 复制它成本低。**必须复制**：运行核心/能力调用/超时执行.py 的线程准入优先走
            # 本库的容量闸门（等待秒=0，满载直接拒绝），不复制则该路径在冷启动下取不到提供者、
            # 只能降级成内置标准库闸门 —— 冷启动覆盖不到「准入走平台原语」这条真实链路（覆盖缺口）。
            (支持库根 / "后端" / "并发控制支持库", self.根 / "支持库" / "后端" / "并发控制支持库"),
...
class Test冷启动子进程(冷启动反向门禁基础):
    ...
    def test_装配后调用成功且证据齐全(self) -> None:
        ...
        请求 = self.装配请求(
            ("文件系统支持库.文件操作.写入文件", {"文件路径": str(写入路径), "内容": "冷启动成功", "编码": "utf-8"}),
            ("文件系统支持库.文件操作.判断存在", {"文件路径": str(写入路径)}),
        ...
        # 支持库直接调用
        self.assertTrue(结果表["文件系统支持库.文件操作.写入文件"]["成功"])
        self.assertTrue(结果表["文件系统支持库.文件操作.判断存在"]["成功"])
        self.assertTrue(结果表["文件系统支持库.文件操作.判断存在"]["值"], "写入后应真实存在")
    ...
    def test_进程重启重新装配(self) -> None:
        ...
        self.复制真实制品()
        请求 = self.装配请求(
            ("文件系统支持库.文件操作.写入文件", {"文件路径": str(self.根 / "重启.txt"), "内容": "重启", "编码": "utf-8"}),
        )
    ...
    def test_单包实现缺失只跳过该包不留半成品(self) -> None:
        """模块实现缺失（文件管理/实现/文件管理.py 被删）→ 只跳过该包并告警。

        新规（架构项A，2026-09-15）：实现缺失是「这个包自己的半成品问题」，
        只跳过该包并告警，其余包照常装配；跳过的包绝不允许以半装配形态对
        调用方可见（其能力必须直接 能力不存在）。
        """
        self.复制真实制品()
        (self.根 / "模块库" / "文件管理" / "实现" / "文件管理.py").unlink()
        请求 = self.装配请求(
            ("文件系统支持库.文件操作.写入文件", {"文件路径": str(self.根 / "残留.txt"), "内容": "残留", "编码": "utf-8"}),
            ("文件管理.写入文件", {"文件路径": str(self.根 / "半成品.txt"), "内容": "半成品"}),
        )
        数据 = self.运行脚本("装配", str(self.根), 请求)
        self.assertTrue(数据["装配成功"], str(数据["问题列表"]))
        self.assertEqual(数据["问题列表"], [], "单包问题不得整体阻断")
        self.assertTrue(数据["跳过包列表"], "跳过包必须有明确告警")
        告警文本 = "；".join(数据["跳过包列表"])
        self.assertIn("文件管理", 告警文本)
        self.assertIn("装配失败", 告警文本)
        # 生命周期留痕：被跳过的包必须有回滚记录（证据不丢）
        self.assertTrue(
            any(记录["操作名称"] == "回滚" and 记录["错误码"] == "单包级跳过"
                for 记录 in 数据["生命周期记录"]),
            "单包级跳过必须留下回滚生命周期记录",
        )
        结果表 = {项["能力id"]: 项 for 项 in 数据["调用结果表"]}
        # 被跳过模块的能力对调用方不可见（零半装配）
        self.assertFalse(结果表["文件管理.写入文件"]["成功"])
        self.assertEqual(结果表["文件管理.写入文件"]["错误码"], "能力不存在")
        # 其余包照常装配并可真实调用（支持库能力不受影响）
        self.assertTrue(结果表["文件系统支持库.文件操作.写入文件"]["成功"],
                        "单包被跳过不得影响其余包的能力")
    ...
    def test_重复能力冲突被拒绝(self) -> None:
        ...
        self.复制真实制品()
        self.写合成支持库("重复包1", "重复包1.能力", 注册能力id="冷启动.重复能力")
        self.写合成支持库("重复包2", "重复包2.能力", 注册能力id="冷启动.重复能力")
        ...
        self.复制真实制品()
        self.写合成支持库("声明包1", "冷启动.声明重复")
        self.写合成支持库("声明包2", "冷启动.声明重复")
        数据 = self.运行脚本("装配", str(self.根), self.装配请求())
    ...
    def test_版本锁漂移拒绝(self) -> None:
        ...
        self.写合成支持库(
            "漂移模块", "冷启动.漂移能力",
            依赖=[{"能力": "文件系统支持库.文件操作.读取文件", "版本": ">=2.0.0"}],
        )
        数据 = self.运行脚本("装配", str(self.根), self.装配请求())
...
class Test冷启动反向破坏(冷启动反向门禁基础):
    """反向破坏证明：删包声明/删契约/删锁 仍必须整体阻断（fail-closed 不放松）；
    删入口/删实现 属单包级半成品问题 → 只跳过该包并告警、其余包照常装配
    （架构项A，2026-09-15），且被跳过包的能力必须对调用方直接 能力不存在。"""
    ...
    def test_删注册入口单包级跳过且能力不可用(self) -> None:
        """删除 注册入口（入口.py）→ 该包被单包级跳过并告警，其余包照常装配。

        新规（架构项A，2026-09-15）：入口缺失属「实现缺失」单包级问题；无仓库
        孪生回退仍然成立——该包不能借同名包/缓存跑通，其能力必须 能力不存在。
        """
        self.复制真实制品()
        ...
        数据 = self.运行脚本("装配", str(self.根), self.装配请求(
            ("冷启动.删入口能力", {}),
            ("文件系统支持库.文件操作.写入文件",
             {"文件路径": str(self.根 / "删入口后.txt"), "内容": "其余包照常", "编码": "utf-8"}),
        ))
        self.assertTrue(数据["装配成功"], str(数据["问题列表"]))
        告警文本 = "；".join(数据["跳过包列表"])
        self.assertIn("支持库.适配层.删入口", 告警文本)
        self.assertIn("缺少入口文件", 告警文本)
        结果表 = {项["能力id"]: 项 for 项 in 数据["调用结果表"]}
        self.assertFalse(结果表["冷启动.删入口能力"]["成功"], "删入口后该能力必须不可用")
        self.assertEqual(结果表["冷启动.删入口能力"]["错误码"], "能力不存在")
        self.assertTrue(结果表["文件系统支持库.文件操作.写入文件"]["成功"],
                        "其余包必须照常装配并可真实调用")

    def test_删实现单包级跳过且能力不可用(self) -> None:
        """删除 实现/ 实现文件 → 该包被单包级跳过并告警，其余包照常装配。

        新规（架构项A，2026-09-15）：实现缺失属单包级半成品问题，只跳过该包
        并告警，不再让全平台装配失败；但该包的能力绝不允许被注册——删实现后
        ...
        数据 = self.运行脚本("装配", str(self.根), self.装配请求(
            ("冷启动.删实现能力", {}),
            ("文件系统支持库.文件操作.写入文件",
             {"文件路径": str(self.根 / "删实现后.txt"), "内容": "其余包照常", "编码": "utf-8"}),
        ))
        self.assertTrue(数据["装配成功"], str(数据["问题列表"]))
        告警文本 = "；".join(数据["跳过包列表"])
        self.assertIn("支持库.适配层.删实现", 告警文本)
        self.assertIn("装配失败", 告警文本)
        self.assertTrue(
            any(记录["操作名称"] == "回滚" and 记录["错误码"] == "单包级跳过"
                for 记录 in 数据["生命周期记录"]),
        ...
        结果表 = {项["能力id"]: 项 for 项 in 数据["调用结果表"]}
        self.assertFalse(结果表["冷启动.删实现能力"]["成功"], "删实现后该能力必须不可用")
        self.assertEqual(结果表["冷启动.删实现能力"]["错误码"], "能力不存在")
        self.assertTrue(结果表["文件系统支持库.文件操作.写入文件"]["成功"],
                        "其余包必须照常装配并可真实调用")
    ...
    def test_删契约生产链失败(self) -> None:
        ...
        数据 = self.运行脚本("装配", str(self.根), self.装配请求(
            ("文件系统支持库.文件操作.写入文件", {"文件路径": str(self.根 / "契约.txt"), "内容": "契约", "编码": "utf-8"}),
        ))
