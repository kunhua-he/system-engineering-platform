...
from 运行核心.任务调度.任务进程 import (
    ...
    每条任务fd数, 进程常驻fd余量, 由fd预算推历史上限, 历史上限设计上限, 独立任务)
...
class Test任务进程池有界(unittest.TestCase):
    ...
    def test_被驱逐任务重建后仍带本会话话语权(self):
        """K1 判据：`本会话活动` 是**池级会话事实**的派生视图，重建不得失真。

        批K 现场：旧实现把它存成对象字段，`查询` 从磁盘快照重建后恒为假 ⇒ 门面的
        终态回写闸门**永久关闭** —— 池快照已写「状态=成功 + 结果齐全」，门面运行库行
        却永远停在「结果=null」（实测任务 `6e392e5e99fd485a` / `659105a5f4554deb`）。
        判据锚**重建后的派生读数**：把已收敛的本会话任务驱逐出 `任务表`，再 `查询`
        （必走快照重建）——`本会话活动` 必须仍为真。
        """
        池 = self._新池(最大活动数=2, 最大排队数=4, 历史上限=1)
        池.注册执行函数("任务.快", 任务瞬返)
        先 = 池.提交(能力id="任务.快")
        self.assertTrue(等待条件(lambda: 先.状态 in 终态集合), "先提交的任务必须真实收敛")
        self.assertIn(先.任务id, 池.本会话任务id,
                      "本会话真起过工作进程 ⇒ 必须登记进会话事实源（唯一写入口）")
        # 再提交一条（走同一写入口 `_登记任务表`），把已收敛的先挤出 `任务表`
        后 = 池.提交(能力id="任务.快")
        self.assertTrue(等待条件(lambda: 后.状态 in 终态集合))
        self.assertNotIn(先.任务id, 池.任务表, "上界=1 时先提交的必须已被驱逐（判据前提）")
        self.assertTrue(等待条件(lambda: (池.存储目录 / f"{先.任务id}.json").is_file()),
                        "被驱逐任务的磁盘快照必须存在（重建靠它）")

        重建 = 池.查询(先.任务id)

        self.assertIsNot(重建, 先, "查询必须新建对象（走磁盘快照重建路径）")
        self.assertEqual(重建.状态, "成功")
        self.assertTrue(重建.本会话活动,
                        "重建对象必须仍带本会话话语权 —— 否则门面终态永久回写不出去（批K）")
        self.assertIsNone(重建.进程, "重建对象不持有任何句柄（登记它不省 fd）")

    def test_反向_会话事实若存成对象字段则重建即失真(self):
        """反向样本（K1 的变异判据）：把「本会话」事实存成对象字段 ⇒ 重建后必然失真。

        两个读数合起来才把「派生视图」与「恰好为真」区分开：① 快照 `转字典()` **不许**
        携带会话级事实（携带 = 让陈旧快照获得话语权，B-08 复活）；② 没有事实源时（对象
        独立构造），派生读数必须为**假** —— 跨会话重建的历史任务正是这个形状。
        """
        本体 = 独立任务(任务id="会话事实对照id", 能力id="示例.快")
        快照 = json.loads(json.dumps(本体.转字典(), ensure_ascii=False))
        self.assertNotIn("本会话活动", 快照,
                         "快照不许携带会话级事实（携带即让陈旧快照获得话语权）")
        重建 = 独立任务(任务id=快照["任务id"], 能力id=快照["能力id"])
        self.assertFalse(重建.本会话活动,
                         "没有会话事实源时，派生读数必须为假（跨会话历史任务形状）")
