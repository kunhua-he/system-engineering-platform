...
class 计数与排序(unittest.TestCase):
    ...
    def test_按次数倒序且上限生效(self):
        for _ in range(3):
            self.账.记一次调用("示例.能力", {"甲": "x"})
        for _ in range(7):
            self.账.记一次调用("示例.能力", {"乙": "y"})
        self.账.记一次调用("示例.能力", {"丙": "z", "丁": 1})
    ...
    def test_能力id隔离(self):
        self.账.记一次调用("甲.能力", {"甲": 1})
        self.账.记一次调用("乙.能力", {"乙": 2})
        self.assertEqual(self.账.查常用参数("甲.能力")["组合数"], 1)
        self.assertEqual(self.账.查常用参数("甲.能力")["常用参数组合"][0]["能力id"], "甲.能力")

    def test_跨能力汇总每段各取前N(self):
        for _ in range(3):
            self.账.记一次调用("甲.能力", {"甲": 1})
        self.账.记一次调用("甲.能力", {"甲": 2})
        for _ in range(5):
            self.账.记一次调用("乙.能力", {"乙": 1})
        查 = self.账.查常用参数("", 1)
    ...
    def test_空入参不入账(self):
        ...
        self.assertFalse(self.账.记一次调用("", {"甲": 1}))
...
class 反向验证(unittest.TestCase):
    ...
    def test_退回升序缺陷态_次数倒序失效(self):
        ...
        夹具要选得让**主键序与次数序相反**（`甲` 码点大于 `乙`），否则撤掉排序后顺序
        碰巧不变，样本就测不到生效点（实测踩过：第一版夹具 `乙` 既是次数冠军又主键靠前，
        撤掉 ORDER BY 首位仍是 `乙`，样本假绿）。
        ...
        **为什么改方向而不是删 ORDER BY**（实测踩过第二次）：本表在 `(能力id, 调用次数 DESC)`
        ...
        try:
            ...
            for _ in range(7):
                账.记一次调用("示例.能力", {"甲": "x"})
            # 先确认正常口径下冠军确实是「甲」（否则样本本身不成立）
            self.assertEqual(
                调用参数账(临时根 / "账.sqlite3").查常用参数("示例.能力", 2)
                ["常用参数组合"][0]["入参形状"], "甲:文本型")
            查 = 账.查常用参数("示例.能力", 2)
            首位 = 查["常用参数组合"][0]["入参形状"]
            self.assertNotEqual(首位, "甲:文本型",
