...
2. 四张表的**模块级字面量赋值者**全仓唯一，就是 `网关核心.py`
   （门禁 `公开调用完整性门禁`/`错误码文案派生`/`运行发布门禁` 都按「文件路径 +
...
# 四张协议表的唯一正本
正本路径 = "运行核心/统一网关/网关核心.py"
表名集 = ("能力参数别名表", "允许操作表", "操作权限表", "公开错误说明表")
# 副本曾丢失的 5 个码（分叉实证）
...
class 网关协议表唯一性(unittest.TestCase):
    def test_重复副本文件不得复活(self) -> None:
        self.assertFalse(
            副本路径.exists(),
            f"重复副本 {副本路径.relative_to(系统根)} 又出现了：它与 {正本路径} 是同名"
