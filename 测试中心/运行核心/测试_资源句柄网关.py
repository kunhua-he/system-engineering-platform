...
class 资源句柄网关测试(unittest.TestCase):
    ...
    def test_返回句柄后可查询并关闭(self) -> None:
        ...
        self.assertEqual(返回["句柄"], "428101")

        状态 = self.网关.处理(网关请求(
            操作="资源状态", 句柄="428101", 项目id="项目甲", 用户id="用户甲",
            权限范围=["查询"],
        )).转字典()
        ...
        读取 = self.网关.处理(网关请求(
            ...
            参数={"资源句柄": "428101"}, 项目id="项目甲", 用户id="用户甲",
            权限范围=["调用"],
        )).转字典()
        self.assertTrue(读取["成功"])
        self.assertEqual(读取["值"]["收到句柄"], "428101")

        关闭 = self.网关.处理(网关请求(
            操作="资源关闭", 句柄="428101", 项目id="项目甲", 用户id="用户甲",
            权限范围=["调用"],
        )).转字典()
    ...
    def test_跨项目查询拒绝(self) -> None:
        ...
        self.assertTrue(返回["成功"])
        状态 = self.网关.处理(网关请求(
            操作="资源状态", 句柄="428101", 项目id="项目乙", 用户id="用户甲",
            权限范围=["查询"],
        )).转字典()
