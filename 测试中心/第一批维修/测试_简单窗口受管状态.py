...
class 测试简单窗口受管状态(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 2026-09-19（开工-20260919-193541-2a8e）：简单窗口模块已按「断第二条腿」
        # 删除 `设置HTTP连接器` 第二入口，只经 `获取能力调用器()` 组合公开能力，
        # 故此处不再注入模块级连接器（网关仍由 setUp 装配的注册表提供能力）。
    ...
    def test_模块只通过统一调用能力接口(self) -> None:
        """实现必须经**唯一调用器**（`获取能力调用器().调用能力`）组合公开能力。

        2026-09-19（开工-20260919-193541-2a8e）：12 个模块包按「断第二条腿」改为
        `获取能力调用器().调用能力(...)`，不再持有模块级 `_连接器`。断言随之更新为
        新姿势（原断言 `_连接器.调用能力` 已随实现作废）。
        """
        源码 = (系统根 / "模块库" / "简单窗口" / "实现" / "简单窗口.py").read_text(encoding="utf-8")
        self.assertIn("获取能力调用器().调用能力", 源码)
        for 专用接口 in (
    ...
    def test_创建窗口经唯一HTTP调用接口接受冻结类型(self) -> None:
        连接器 = HTTP连接器(网关端口=self.网关.端口)
        响应 = 连接器.调用能力("简单窗口.创建窗口", {
            "窗口id": "HTTP验收窗", "标题": "HTTP验收", "宽度": 640, "高度": 480,
        })
        self.assertTrue(响应.get("成功"), 响应)
        self.assertIsInstance(响应.get("值", {}).get("句柄"), int)
