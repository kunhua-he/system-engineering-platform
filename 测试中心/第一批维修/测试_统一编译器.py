...
class 统一编译器入口与小单元测试(unittest.TestCase):
    ...
    def test_文件能力组件三类小单元计算契约依赖边界和影响闭包(self):
        with tempfile.TemporaryDirectory(prefix=f"隔离用例_{os.getpid()}_", dir="/tmp") as 临时:
            ...
            页面甲 = 页面目录 / "甲.json"
            页面乙 = 页面目录 / "乙.json"
            页面甲.write_text(json.dumps({
                "页面id": "甲", "标题": "甲", "路由": "/甲",
                "组件列表": [{"组件id": "按钮甲", "类型": "按钮", "属性": {"能力id": "能力.甲"}}],
            }, ensure_ascii=False), encoding="utf-8")
            页面乙.write_text(json.dumps({
                "页面id": "乙", "标题": "乙", "路由": "/乙",
                "组件列表": [{"组件id": "按钮乙", "类型": "按钮", "属性": {"能力id": "能力.乙"}}],
            }, ensure_ascii=False), encoding="utf-8")
            文件闭包 = 计算影响闭包(项目, 文件=页面甲)
            能力闭包 = 计算影响闭包(项目, 能力="能力.乙")
            组件闭包 = 计算影响闭包(项目, 组件="按钮甲")
            self.assertEqual(文件闭包["受影响页面"], ["甲.json"])
            self.assertEqual(能力闭包["受影响页面"], ["乙.json"])
            self.assertEqual(组件闭包["受影响页面"], ["甲.json"])
