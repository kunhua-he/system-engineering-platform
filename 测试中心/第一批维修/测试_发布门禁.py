...
def 建制品(根: Path, *, 能力id: str = "模块甲.求和") -> Path:
    制品 = 根 / "候选制品"
    ...
    契约 = {
        ...
        "能力契约": [{
            ...
            "参数": [
                {"名称": "甲", "类型": "整数型", "必填": True, "默认值": 1},
                {"名称": "乙", "类型": "整数型", "必填": True, "默认值": 2},
            ],
            "返回": {"类型": "结果型", "值结构": {"和": "整数型"}},
            "错误码": ["参数不合法"],
            "调用示例": {"能力id": 能力id, "参数": {"甲": 1, "乙": 2}},
            "行为": {"副作用": "纯计算", "资源释放": "无资源残留"},
            "提供者": {"默认": "模块库.模块甲", "版本": ">=1.0.0"},
        }],
    }
    写JSON(制品 / "模块库" / "模块甲" / "能力契约" / "参数契约.json", 契约)
    写JSON(制品 / "模块库" / "模块甲" / "包声明.json", {
        "包id": "模块库.模块甲", "版本": "1.0.0", "类型": "模块库",
        "能力": [{"能力id": 能力id}],
    })
...
def 建通过矩阵(制品: Path, 能力id: str = "模块甲.求和") -> dict:
    return {
        "制品路径": str(制品.resolve()),
        "场景总数": 2, "通过数": 2, "失败数": 0,
        "结果列表": [
            {
                "场景id": f"能力.{能力id}.成功", "能力id": 能力id, "通过": True,
                "请求": {"能力id": 能力id, "参数": {"甲": 1, "乙": 2}},
                "状态码": 200, "返回": {"成功": True, "值": {"和": 3}},
                "契约对账": {
                    "参数": True, "返回": True, "错误码": True, "行为": True,
                    "提供者": "模块库.模块甲", "版本": "1.0.0",
                },
                "资源释放": {"进程残留": 0, "文件残留": 0, "监听端口残留": 0},
            },
            {
                "场景id": f"能力.{能力id}.错误.参数不合法", "能力id": 能力id, "通过": True,
                "请求": {"能力id": 能力id, "参数": {}}, "状态码": 400,
                "返回": {"成功": False, "错误码": "参数不合法", "错误说明": "缺参数"},
                "契约对账": {"错误码": True},
                "资源释放": {"进程残留": 0, "文件残留": 0, "监听端口残留": 0},
            },
        ],
        "资源释放": {"制品进程已退出": True, "进程残留": 0, "文件残留": 0, "监听端口残留": 0},
    }
...
class 契约与HTML矩阵测试(unittest.TestCase):
    def test_正式HTML矩阵逐能力对账成功请求错误码业务值与释放(self):
        with tempfile.TemporaryDirectory() as 临时:
            制品 = 建制品(Path(临时))
    ...
    def test_坏矩阵只有AST式名称计数不得假绿(self):
        with tempfile.TemporaryDirectory() as 临时:
            ...
            假矩阵 = {"制品路径": str(制品), "场景总数": 1, "通过数": 1, "失败数": 0,
                    "结果列表": [{"场景id": "模块甲", "能力id": "模块甲.求和", "通过": True}]}
            通过, 详情, _ = 门禁.校验契约与HTML矩阵(制品, 假矩阵)
            self.assertFalse(通过)
            self.assertRegex(详情, "请求|状态码|业务值|资源释放")
