"""系统级提供者健康探针：独立子进程检查外部应用/系统工具 + 系统内存汉化原子能力。
...
出现（支持库经本能力的汉化返回值取真值，不得直连第三方）。本能力是本目录下
根级适配器的内部原语，按依赖防火墙口径「不注册能力契约、不发布、不被模块调用」，
因此不登记 能力定义.json，也不进 包发现 的正式包表。
"""
...
def 读取系统内存() -> 系统内存快照:
    ...
    """
    if psutil is None:
        return 系统内存快照(可用=False, 不可用原因=内存探针不可用原因)
    try:
        ...
        return 系统内存快照(可用=False, 不可用原因=f"psutil 读取系统内存失败: {错误}")
    return 系统内存快照(
        可用=True, 总量字节=总量字节, 可用字节=可用字节, 已用字节=已用字节,
