"""reportlab 隔离执行管理器：把 reportlab 下沉到隔离子进程执行。

主进程（本文件）只做三件事：参数预校验、拉起一次性子进程、把子进程响应映射成
统一结果 —— **本进程绝不 import reportlab**（第三方库只在 实现/子进程入口.py 内加载）。

渲染逻辑不在本文件重复实现：渲染唯一实现在同包 实现/生成PDF.py，子进程内经同包
公开入口调用（同一份逻辑只有一个实现）。

错误码严格保持本包契约声明的三项（参数不合法 / 提供者不可用 / 生成失败）：子进程
超时与崩溃按「生成失败」返回、真实原因写进 错误说明，不新增未声明的错误码。
释放语义：子进程独占进程组启动，调用返回前一律回收（终止 → 宽限 → 强杀 → 复查），
不留残留进程；不落盘、不持跨调用状态。

本文件由 后端腿 `支持库/后端/文档转换支持库/PDF生成/实现/生成PDF.py` 转调承载：
两腿该模块名指向本模块对象（同一模块对象），故本文件是隔离子进程形态的唯一实现。
"""
    ...
    try:
        标准输出, _标准错误, 已超时, 输出超限 = 受限通信(
            ...
            终止回调=lambda: 进程终止.结束并留痕(
                进程, 位置="reportlab提供者.执行任务", 宽限秒=1.0, 等待秒=1.0),
        )
    finally:
        try:
            if 进程.poll() is None:
