...
def 解析PDF为字典(
    ...
    关闭失败: dict[str, Any] | None = None
    try:
        try:
            pdf = 状态["pdfplumber"].open(str(路径))
        except (Exception,) as 错误:
            if _疑似加密(错误):
                结果 = {"错误码": "文件加密", "错误说明": f"PDF 已加密：{错误}"}
            else:
                结果 = {"错误码": "文件损坏", "错误说明": f"pdfplumber 打开失败: {错误}"}
        if 结果 is None:
            try:
                if 最大页数 > 0 and len(pdf.pages) > 最大页数:
                    结果 = {"错误码": "超出限制", "错误说明": f"PDF 共 {len(pdf.pages)} 页超过上限 {最大页数} 页"}
                else:
                    块列表: list[dict] = []
                    资源列表: list[dict] = []
                    警告表: list[str] = []
                ...
                try:
                    ...
                    关闭失败 = {"错误码": "文件损坏", "错误说明": f"PDF 关闭失败: {错误}"}
    except (Exception,) as 错误:
        if _疑似加密(错误):
            结果 = {"错误码": "文件加密", "错误说明": f"PDF 已加密：{错误}"}
        else:
            结果 = {"错误码": "文件损坏", "错误说明": f"PDF 解析失败: {错误}"}
    finally:
        if fitz文档 is not None:
            try:
                fitz文档.close()
            except (Exception,) as 错误:
                关闭失败 = {"错误码": "文件损坏", "错误说明": f"fitz 文档关闭失败: {错误}"}
    if 关闭失败 is not None:
        return 关闭失败
    if 结果 is None:
        return {"错误码": "文件损坏", "错误说明": "PDF 解析未产生结果"}
    return 结果
