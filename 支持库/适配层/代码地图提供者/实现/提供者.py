...
# 自举前只能用标准库：本文件必须先把**自己所在那棵树**插进 `sys.path`，下面那几行
# `from 公共契约…`（制品态是 `from 平台客户端.公共契约…`）才成立，故此刻**不能**调
# `公共契约.运行时.导入前缀`（循环依赖）。判据仍是**同一套锚目录**（同时含 `支持库` 与
# `模块库` 的最近祖先，与共享模块 `导入前缀.取系统根` 逐字同判；不是 `parents[N]` 那种
# 一改目录结构就静默指错树的层数写法）。形状与同目录族先例一致：本包
# `实现/子进程入口.py`、`支持库/适配层/PDF隔离提供者/实现/子进程入口.py`。
#
# ★ 为什么不许写死源码树绝对路径（2026-09-24 修，实测）：制品构建的 `复制并重写`
#   只重写 import **语句**，改不到字符串字面量 ⇒ 写死值在制品拷贝里**原样保留**，于是
#   跑制品时这一行把**源码树**插进 `sys.path`：制品不是「自足」而是「悄悄依赖源码树」，
#   源码树一改名/搬走，制品当场 ModuleNotFoundError。同族事故照它改：
#   `支持库/适配层/Git提供者/实现/仓库根白名单.py:37-48`（原先按 `parents[4]` 推根，在
#   制品态指错树，导致源码树提交被判 `路径越界`）。
# ★ 判据选**锚目录判据（本文件所在那棵树的包根）**，不是 `.git` 仓库根：本行要回答的是
#   「`公共契约…` 从哪棵树解析」，是**包根**问题。`.git` 判据在这里会反向出错 —— 制品位于
#   源码仓 `工程缓存/` 内、自身无 `.git` ⇒ 它命中的恰是**最外层源码仓根**，等于把要修的
#   症状写进判据，且 `平台客户端.公共契约…` 那条导入腿仍不成立。要「Git 仓库根」的是仓库根
#   白名单（越界提交防护），两处需求不同，判据因此不同。
系统根 = next(
    祖先 for 祖先 in Path(__file__).resolve().parents
    if (祖先 / "支持库").is_dir() and (祖先 / "模块库").is_dir()
)
# 制品态：本棵树的包根是 `<制品外>/平台客户端`，被导入的包名是 `平台客户端.公共契约…`
# ⇒ 入路径的必须是它的**上一级**（源码态不上抬，与旧行为逐字相同）。
    ...
    try:
        输出, _, 已超时, 已超限 = 受限通信(
            ...
            终止回调=lambda: 进程终止.结束并留痕(
                进程, 位置="代码地图提供者.执行任务", 宽限秒=1.0, 等待秒=1.0),
        )
        if 已超时:
