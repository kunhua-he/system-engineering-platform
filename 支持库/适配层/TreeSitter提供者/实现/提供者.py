...
def 停止() -> None:
    ...
    **状态：预留入口（2026-09-16 债务 F-2 处置：保留 + 标注预留，不接入卸载链）**

    为什么保留而不是按 1.4「空转即杀」删掉（三条现场事实）：
    ① `生命周期契约.json` 的 `停止入口` 指向本函数，审计 `_停止入口真实存在()` 按 AST 核对它
       真在实现里；删掉就得把契约改指另一个真实函数，而本层没有第二个释放动作可指；
    ② 真实调用方已存在：`测试中心/支持库/核心/测试_语法索引.py` 的 F-2 回归真跑
       解析 → `停止()` 清空 → 再解析重建，证明它不是空函数、也不是一次性自毁；
    ③ 接进卸载链必须改 `运行核心/加载器/生命周期管理/管理器.py` 的 停止系统/卸载系统
       （该文件现场只有**包状态机**「可运行→已停止→已卸载」与能力注册表清空，
       **不存在**包级资源释放钩子调用链，没有任何一处会回调提供者的 停止()），
       而本路的允许修改域只含 `支持库/适配层/*提供者/**` 与 `模块库/自修复工具/**`——不动域外文件。
       **补齐这项属越域**：需要的落点是 `运行核心/加载器/生命周期管理/` 与
       `支持库/后端/代码解析支持库/语法索引/`，已登记待主会话派单；本函数不预先改形状。

    另两条现场事实：本提供者被 `代码解析支持库.语法索引` 在**主进程内**惰性导入使用
    （不走独立进程隔离），没有「进程退出即回收」以外的运行时卸载路径；进程退出时解析器
    缓存随解释器回收，当前不构成资源泄漏。

    接入点（将来补卸载链时只改这一处调用关系，不改本函数）：给 加载器 停止/卸载 增加
    「包级释放钩子」调用（约定包 `__init__` 导出 停止 即调用），或在 `语法索引` 释放自身
    缓存时连带调用本入口。
    """
    with _缓存锁:
        _解析器缓存.clear()
