"""已废弃：正式代码迁移到 密码签名提供者。历史兼容保留。

本文件直连 cryptography（Rust 原生扩展），不再被任何正式代码引用。
正式代码一律经 支持库.适配层 公开入口调用（内部经 密码签名提供者
隔离子进程执行）。仅测试与历史脚本仍可直接引用本文件。
密码适配层：Ed25519 签名/验证/密钥生成（成熟实现 cryptography，
第三方强制英文接口全部集中在适配层），正式代码只调用中文外壳。
"""
