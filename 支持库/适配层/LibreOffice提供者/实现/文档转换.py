...
def 转换办公文件(输入路径: str, 目标格式: str, *, 超时秒: float = 默认超时秒,
    ...
    try:
        ...
        # 归一到已登记码 `提供者不可用`（503）：受管池配置无效 ⇒ 本提供者当前不可用。
        # **不新造 `提供者配置错误`** —— 网关两表无此码，且与 503 族的调用方处理
        # 方式完全相同（都是「先修环境再重试」），新造只会让错误码面多一个无独立
        # 处理价值的细分码（哲学 1.2 禁止第二实现同族）。真因（配置项名与越界值）
        # 完整保留在 `消息` 里，诊断信息不丢。
        return _失败(
            "提供者不可用",
            f"LibreOffice 受管池配置无效: {错误}",
            详情={"原因": "配置无效", "配置项": str(错误)[:200]},
        )
    if 输出目录:
