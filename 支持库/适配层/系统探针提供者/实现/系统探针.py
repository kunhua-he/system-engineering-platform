...
def 终止探针子进程(进程: subprocess.Popen, *, 宽限秒: float = 终止宽限秒) -> None:
    """探针子进程的**唯一停止入口**：先 terminate、超时再 kill，并回收僵尸。

    探针的两条退出路径（超时终止回调、finally 兜底）都走本函数，
    保证「一个子进程一个停止定义点」；本函数也是 生命周期契约.json
    「停止入口」指向的真实函数。
    """
