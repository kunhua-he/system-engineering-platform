...
**落点（#96 包化后）**：本件是 `支持库/适配层/模型服务提供者/` 的实现载体。
对外入口仍是根下平铺件 `支持库/适配层/模型服务.py`（自举 + 等价再导出腿），
`模型连接器` 的本地启动链按**路径**拉起它（`取系统根(__file__) / 支持库/适配层/模型服务.py`），
路径与命令行参数**逐字不变**。

## 相对安全声明（哲学 9.5②「不许包装成绝对边界」，2026-09-20 销 #179）

**本服务是适配层内部件**（登记见 `支持库/适配层/依赖登记.json` 与
`模型服务提供者/依赖锁.json`），不是公开能力 —— 调用方**无法经唯一能力调用入口把
`模型路径` 传进来**；它只由 `模型连接器` 的本地启动链拉起。

### 一、`trust_remote_code=True`（`模型服务.加载` 与 `模型服务._加载决策` 两处）—— **这是相对安全，不是绝对边界**
...
def 停止服务(服务: 模型服务) -> None:
    """显式释放入口（生命周期契约.json 的「停止入口」指向本函数）。

    做两件真事：① 摘掉分词器/权重引用；② 归还平台加速器缓存（MPS / CUDA）。
    进程级停止（SIGTERM → 子进程组回收）仍由启动方 `模型连接器` 负责 ——
    本服务是**独立进程**，进程内的「停止」只能是释放资源，不能自我 kill。
    缓存归还失败不影响语义（引用已摘除，进程退出即全部归还），故此处不抛异常。
    """
    服务.模型 = None
    服务.分词器 = None
    try:
    ...
    except Exception:  # 缓存归还是尽力而为，失败不改变「已释放」的事实
        pass


def 创建应用(服务: 模型服务) -> FastAPI:
    # 关闭钩子挂 停止服务：uvicorn 收到退出信号时先释放权重与加速器缓存，再退进程。
    # 用 lifespan（FastAPI 现行口径）而不是 `@应用.on_event`（已废弃）或
    # `应用.add_event_handler`（本机 fastapi 实测无此属性）。
    @asynccontextmanager
    async def _服务生命周期(_应用: FastAPI) -> AsyncIterator[None]:
        yield
        停止服务(服务)

    应用 = FastAPI(title="底座内部模型服务", lifespan=_服务生命周期)
