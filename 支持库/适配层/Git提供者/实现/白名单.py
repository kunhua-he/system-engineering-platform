...
def 授权拒绝(绝对路径表, 开工ID: object) -> 结果 | None:
    """把「本次 git 操作会改写的路径」逐条交给**唯一判据**
    `公共契约/运行时/写入授权.校验写入授权`（判据本体唯一在那一处，本函数不另写一份）：
    受管路径必须被一条 `所有者 == 开工ID` 的活跃写租约覆盖，否则回 `越界`；
    非受管路径（`.git/`、`工程缓存/`、仓库外）该判据恒通过 ⇒ 本函数不拦。

    为什么 git 能力要过这道门（2026-09-25）：变更类 git 命令（worktree add/remove、
    switch、merge、revert、cherry-pick）**直接改写工作区受管文件**，而它们走的是
    `git` 子进程、**绕开平台写入腿**（`文件操作`/`文本补丁`/`资源管理`）⇒ 若不在这里
    补一道，这些能力就是「不经写入授权、也无凭证」的写受管面通道。本函数只做
    「逐条接线 + 取第一条失败说明」，**不自行判拒**（错误码与说明由那一处给）。

    `绝对路径表` 必须是**绝对路径**（调用方按本次 `仓库路径` 把 git 回带的相对路径
    拼成绝对）：判据内部经 `平台适配.解析路径` 解析，相对写法会落到进程 cwd 兜底，
    不是本次仓库 —— 拼绝对由调用方负责（与 `归一化仓库内路径` 同一口径）。
    """
