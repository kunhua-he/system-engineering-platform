...
def 底座仓库根() -> Path:
    """底座仓库根：从本文件向上定位到的**真实 Git 仓库根**（含其下 `工程缓存/` 内的仓与 worktree）。

    ★ 为什么不能按位置数层（2026-09-22 修，实测踩过）：原先写 `Path(__file__).resolve().parents[4]`
    （本文件位于 `<仓库根>/支持库/适配层/Git提供者/实现/`），**只在「跑源码树」时成立**。
    网关 40007 现在跑的是激活制品（launchd `WorkingDirectory` =
    `工程缓存/制品仓库/平台客户端环境/平台客户端`），那份拷贝的 `parents[4]` = **制品根**，
    于是「对源码树提交」被判 `路径越界`（白名单里只有制品根 + 系统临时目录）——
    **纯 MCP 的源码树 Git 提交因此根本不可能，只能回终端**，正是「每用一次终端＝一处能力缺失」。
    改为向上找**最近一个含 `.git` 的祖先目录**（worktree 里 `.git` 是文件，故目录/文件都认）：
    制品拷贝位于源码仓 `工程缓存/` 内、自身无 `.git` ⇒ 命中源码仓根，源码树与制品两种跑法
    拿到同一个仓库根，安全边界不变（仍只放行「跑着的这份代码所在的那个仓」+ 环境变量显式加白
    + 系统临时目录）。找不到任何 `.git` 祖先时回落位置推导（保持旧行为，不静默失败）。
    """
    起始 = Path(__file__).resolve()
    for 候选 in 起始.parents:
        if (候选 / ".git").exists():
            return 候选
    return 起始.parents[4]
