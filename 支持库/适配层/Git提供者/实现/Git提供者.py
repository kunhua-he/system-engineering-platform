...
from 支持库.适配层.Git提供者.实现.白名单 import (
    失败结果, 校验分支名, 校验仓库路径, 校验起始点, 校验路径文本, 校验超时,
)
...
def 关闭工作区(仓库路径: str, 目标路径: str, 超时秒: float = 默认超时秒) -> 结果:
    """git worktree remove 非强制：存在未提交修改 → 未提交修改 拒绝。"""
    校验 = 校验仓库路径(仓库路径) or 校验路径文本(目标路径) or 校验超时(超时秒)
    if 校验:
        return 校验
    ...
    执行 = 命令结果(执行git(仓库路径, ["worktree", "remove", 目标路径], 超时秒))
    if not 执行.成功:
        return 执行
    if Path(目标路径).exists():
        return 失败结果("命令失败", "worktree remove 后目录仍存在")
    return 结果.成功结果({"已关闭": 目标路径})
