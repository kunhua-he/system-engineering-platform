...
子进程组启动标志 独立进程组；终止一律走 进程终止.强制结束子进程（终止→宽限→强杀），
    ...
    *,
    ...
    try:
        进程 = subprocess.Popen(
            ...
            stderr=subprocess.PIPE, **平台适配.子进程组启动标志(), cwd=工作目录,
        )
    ...
    try:
        ...
        若仍存活 = 进程.poll() is None
        if 若仍存活:
            # 零残留：进程组必死。统一口径：回收结果不得丢弃——
            # `结束并留痕` 已把失败登记进 进程终止 的回收失败留痕（位置可溯源），
            # 这里再把「未确认收敛」带进本次调用的错误说明与值，调用方看得见。
            if not 进程终止.结束并留痕(
                    进程, 位置="Tesseract受管进程.执行命令", 宽限秒=终止宽限秒,
                    等待秒=终止宽限秒):
                回收失败 = ("外部命令进程组**未确认**回收（进程可能仍在存活，"
                            "见 进程终止.回收失败留痕快照()）")
        泵.等待()
    ...
    if 终止结果 is not None or 泵.超限:
        终结 = 终止结果 or _失败("超出限制",
                                 f"外部命令输出超过上限 {输出上限字节} 字节")
        if 回收失败:
            return _失败(终结.错误码, f"{终结.错误说明}；{回收失败}",
                        可重试=终结.可重试)
        return 终结
    标准输出, 标准错误 = 泵.文本()
    if 回收失败:
        # 防御分支：走到这里说明「终止结果 为空但回收未确认」——按现有控制流不可达
        # （回收失败要求 finally 时进程仍存活，与循环退出条件矛盾，除非循环体内抛异常，
        # 那时异常本身照常外抛、不会走到这一行）。保留它是为了让「回收失败绝不静默」
        # 在本函数里无例外；错误码取**已声明**的 `命令失败`（不新造未登记码），
        # 未确认回收的事实写在说明里，也已在 进程终止.回收失败留痕 里留痕。
        return _失败("命令失败", f"外部命令未能确认回收：{回收失败}")
    return 结果.成功结果({"退出码": 进程.returncode, "标准输出": 标准输出,
                          "标准错误": 标准错误})


def 等待并收集(进程列表: list[subprocess.Popen], 超时秒: float = 10.0) -> list[str]:
    """批量等待并强制清理子进程（测试与收口用），幂等。

    统一口径：``强制结束子进程`` 的返回值一律经 ``进程终止.结束并留痕`` 处理——
    回收失败会在收口层留下可读留痕（``进程终止.回收失败留痕快照()``），不再静默；
    本函数再把本批的**回收失败位置**回传给调用方（空表 = 全部确认回收），
    使「丢弃返回值」这件事在本文件里没有落点。
    """
    回收失败位置: list[str] = []
    for 进程 in 进程列表:
        try:
            if 进程.poll() is None:
                if not 进程终止.结束并留痕(
                        进程, 位置="Tesseract受管进程.等待并收集",
                        宽限秒=超时秒, 等待秒=超时秒):
                    回收失败位置.append("Tesseract受管进程.等待并收集")
        except (OSError, ValueError):
            pass
    return 回收失败位置
