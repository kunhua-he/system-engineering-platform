"""跨平台语音转写独立提供者：配置驱动 + 受管子进程执行（**一份实现 + 按平台选后端**）。
后端由 `公共契约/运行时/平台适配.转写后端()` 唯一判定：Apple Silicon → mlx_whisper；
Windows / Linux → faster_whisper（后端差异在 实现/子进程解析.py 内部吸收，对外结构不变）。
未配置模型 → 如实返回 未配置模型；模型目录缺失 → 模型缺失；绝不伪装可用、
绝不模拟转写成功。转写库只在隔离子进程加载；转写覆盖 超时/取消/进程组回收/输出上限。
...
def 转写音频文件(文件路径: str, 超时秒: float = 默认超时秒,
    ...
    附加术语 非空时作为领域术语提示下传后端转写库（mlx / faster-whisper 的 initial_prompt）。
    """
