...
import json, threading, webbrowser, html
...
def 启动网页服务(*, 标题: str = "底座网页服务", 页面说明: str = "", 调用函数: Callable[[str], Any] | None = None, 端口: int = 45080, 自动打开: bool = False) -> tuple[ThreadingHTTPServer, str]:
    ...
    标题 = html.escape(str(标题), quote=True)
    页面说明 = html.escape(str(页面说明), quote=True)
    ...
    class 处理器(BaseHTTPRequestHandler):
        def setup(self) -> None:
            super().setup()
            # 请求行/请求头阶段也必须有硬截止，避免半请求长期占用线程和 FD。
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if 长度 < 0 or 长度 > 1024 * 1024:
                    raise ValueError("请求体超过上限或缺少 Content-Length")
                if "application/json" not in self.headers.get("Content-Type", "").lower():
                    raise ValueError("请求正文必须使用 JSON")
                参数 = json.loads(self.rfile.read(长度).decode("utf-8"))
                if not isinstance(参数, dict):
                    raise ValueError("请求正文必须是对象")
                响应 = 调用函数(str(参数.get("文本", "")))
            except Exception as 错误:  # noqa: BLE001
                响应 = {"成功": False, "错误码": "参数不合法", "错误说明": "请求参数不符合契约"}
            正文 = json.dumps(响应, ensure_ascii=False).encode("utf-8"); self.send_response(200 if 响应.get("成功") else 400); self.send_header("Content-Type", "application/json; charset=utf-8"); self.send_header("Content-Length", str(len(正文))); self.end_headers(); self.wfile.write(正文)
    try:
