...
#
# `会话`（2026-09-15 MCP 收敛批次 B4 重新登记；**语义已换，注意同名双义**）：
# - 本库 `会话` 域承载**会话态** = 子任务临时上下文（原落 `工程缓存/MCP临时上下文/<开工id>.json`
#   文件式），主键 = `开工id`；这是「运行态一律入库」的落点（口径见 `开发文档/未完成事项.md:40`、
#   华哥裁决 `开发文档/临时文档/20260915_MCP收敛/03_总表与处置.md:31`）。
# - **LLM 会话元数据**（`会话id`/`用户id`/`模型名`/`标题`/`压缩次数`/`成本统计`/`最后消息时间`）
#   的写入侧仍在 `大语言模型支持库.会话存储.db`（专用库），那才是 LLM 会话事实的唯一权威；
#   两者**不是一回事**，故 LLM 侧字段不在本域登记（登记了就成同名第二个权威，违反第 1 条 3 项）。
# - 历史遗留：正式库里那张 `会话` 表由旧的「十域全量遍历建表」建出，带 LLM 专用列且 **0 行**；
#   本批**不删表**，改由 `_补齐缺失列` 在首次写入前把会话态列补上（旧列恒空、不混数据），
#   否则 `CREATE TABLE IF NOT EXISTS` 空操作会让首次写入报 `table 会话 has no column named 开工id`。
运行库表定义 = {
    "任务": {
    ...
    # 会话态专用列只镜像高频查询字段；**会话内容全文走 `载荷`**（写入语义保证完整原始记录入载荷），
    # 不再另设「会话内容」列，避免同一事实两份（哲学第 1 条 3 项）。列表型字段（允许目录/定向记忆查询/
    # 已确认事实/验证计划）与 `协作状态.子任务列表` 同口径：库表是 TEXT 列，**调用方自己 `json.dumps`
    # 成文本**再写（先例 `MCP工具箱/协作状态.py:128-141`）；直接塞 list 会被 sqlite3 拒成
    # 「运行态写入失败，已回滚：Error binding parameter ... type 'list' is not supported」（实测）。
    # `过期时间` = epoch 秒（浮点），与临时上下文现有 JSON 的 `过期时间` 逐字同义；是否有效由写入侧按
    # TTL 判定，读取侧只看字段（本层不做过期删除，避免读路径隐式写）。
    "会话": {
        "字段": ["开工id", "项目", "父任务", "角色", "允许目录", "定向记忆查询",
                 "已确认事实", "验证计划", "过期时间"],
        "索引": ["状态", "开工id", "项目", "过期时间"],
    },
}
...
def _字段类型(名称: str) -> str:
    """运行态字段的 SQLite 存储类型：逻辑型必须 INTEGER、数值型必须 REAL，其余 TEXT。

    为什么单列函数（2026-09-15 审计修复，两种坑都是实测）：TEXT 列会让读到的 "0.25" 变成文本；
    逻辑字段落成 TEXT `"0"/"1"` 会让 Python 端 `if "0"` 判成真，成功/失败/超时统计整体算错。
    """
    if 名称 in {"取消标记", "结果已截断", "成功", "超时"}:
        return "INTEGER"
    if 名称 in {"进度", "登记时间", "关闭时间", "灰度比例", "耗时毫秒", "观察窗口秒",
                "请求总数", "成功数", "失败数", "超时数", "过期时间"}:
        return "REAL"
    return "TEXT"


def _补齐缺失列(连接对象: sqlite3.Connection, 域: str, 定义: dict) -> None:
    """给**已存在**的旧表补齐登记字段（幂等，只加列）。

    为什么必须有：`CREATE TABLE IF NOT EXISTS` 对已存在的表是空操作。正式库里的 `会话` 表由旧的
    「十域全量遍历建表」建出（带 LLM 专用列、0 行），登记会话态字段后若不做列级同步，首次写入会以
    `table 会话 has no column named 开工id` 报「事务失败」。本函数只 `ALTER TABLE ... ADD COLUMN`：
    不改已有列的类型、不删列、不动任何数据。
    """
    已有列 = {行[1] for 行 in 连接对象.execute(f"PRAGMA table_info({_标识符(域)})")}
    for 名称 in 定义["字段"]:
        if 名称 in 已有列:
            continue
        连接对象.execute(
