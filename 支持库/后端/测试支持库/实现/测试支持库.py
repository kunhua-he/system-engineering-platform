...
#: 允许的解释器（U-5 解释器口径收口，2026-09-24 华哥裁决「不强制 3.14」）：`python3.14`
#: 是**优先项**不是**强制项** —— 取不到 3.14 的机器回落 `python3` 也放行。准入层只判
#: 「可执行在不在白名单」，跑不跑得动由执行腿按真实退出码如实报（准入不替它下结论）。
允许可执行 = ("python3.14", "python3")
测试模块前缀 = "测试中心."
HTML验证入口 = "开发工具.HTML验证.验证器"
#: 唯一发布命令的**规范写法**（展示用，仍写优先项 3.14）。识别**不按解释器判**：见下方
#: `验证命令白名单` 按**入口**比对（U-5 之前按整条比对，没装 3.14 的机器用 `python3`
#: 跑发布门禁会被误判成「验证入口不在白名单」）。
...
def _校验unittest形态(命令: list[Any], 根: Path | None, 校验存在: bool) -> dict[str, Any] | None:
    ...
    if len(命令) != 3 or 命令[1] != "-m":
        return _拒绝(f"开发期回归只允许 {' / '.join(允许可执行)} -m <精确模块>")
    模块 = 命令[2]
    段表 = 模块.split(".")
...
def 验证命令白名单(命令: list[Any], 仓库根目录: str = "", 校验存在: bool = True) -> 结果:
    ...
    # U-5（2026-09-24 华哥裁决「不强制 3.14」）：唯一发布命令按**入口**识别、不按解释器
    # 识别 —— 同一发布入口用 `python3.14` 还是回落成 `python3` 跑，都是「正式发布」形态。
    if 命令[1:] == 唯一发布命令[1:]:
        形态 = "正式发布"
