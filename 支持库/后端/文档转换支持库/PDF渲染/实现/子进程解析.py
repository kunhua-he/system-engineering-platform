"""子进程解析（PDF 渲染）：唯一实现在 支持库/适配层/PyMuPDF提供者（D-2 收口）。

本文件原与 `支持库/适配层/PyMuPDF提供者/实现/子进程解析.py` **逐字节相同**
（结构债 D-2 第 ⑥ 组，sha256 f26cb539…）。同一份逻辑只能有一个实现，故本文件
改为**转调**：让 `支持库.后端.文档转换支持库.PDF渲染.实现.子进程解析`
与适配层腿那唯一实现成为**同一个模块对象**（`sys.modules[__name__] = 唯一实现`），
本包子进程入口（实现/子进程入口.py）照旧从此路径导入，fitz 仍只在子进程内加载。
...
唯一实现名 = f"{__name__.split('支持库.', 1)[0]}支持库.适配层.PyMuPDF提供者.实现.子进程解析"
系统根 = next(
...
if 唯一实现名 not in sys.modules:  # 兜底：公开入口未加载该子模块时按文件路径显式载入
    唯一实现文件 = 系统根 / "支持库" / "适配层" / "PyMuPDF提供者" / "实现" / "子进程解析.py"
