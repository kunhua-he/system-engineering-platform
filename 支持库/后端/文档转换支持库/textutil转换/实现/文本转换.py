...
本文件原与 `支持库/适配层/textutil提供者/实现/文本转换.py` 逻辑一致，差异仅在包路径深度
（后端腿比适配层腿多一层目录，故 `parents[N]` 相差 1）与一处内部薄委托（`_终止进程组`）。
...
唯一实现名 = f"{__name__.split('支持库.', 1)[0]}支持库.适配层.textutil提供者.实现.文本转换"
系统根 = next(
