"""子进程解析（PDF 隔离）：唯一实现在 支持库/适配层/PDF隔离提供者（D-2 收口）。

本文件原与 `支持库/适配层/PDF隔离提供者/实现/子进程解析.py` **逐字节相同**
（结构债 D-2 第 ② 组，sha256 f77d070a…）。同一份逻辑只能有一个实现，故本文件
改为**转调**：让 `支持库.后端.文档转换支持库.PDF隔离提供者.实现.子进程解析`
与适配层腿那唯一实现成为**同一个模块对象**（`sys.modules[__name__] = 唯一实现`），
本包子进程入口（实现/子进程入口.py）照旧从此路径导入，进程边界不变。
...
import 支持库.适配层.PDF隔离提供者  # noqa: F401 —— 公开入口（同层，合规）

# 前缀感知：制品把本模块注册成 `平台客户端.支持库.…`，而字面量不随导入前缀改写，
# 故按 `__name__` 派生本树前缀（源码树为空串、制品为 `平台客户端.`），
# 使下面的兜底判据在两种形态下都成立（债务 #216②）。
唯一实现名 = f"{__name__.split('支持库.', 1)[0]}支持库.适配层.PDF隔离提供者.实现.子进程解析"
系统根 = next(
...
if 唯一实现名 not in sys.modules:  # 兜底：公开入口未加载该子模块时按文件路径显式载入
    唯一实现文件 = 系统根 / "支持库" / "适配层" / "PDF隔离提供者" / "实现" / "子进程解析.py"
