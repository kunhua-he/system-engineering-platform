...
- XLSX 原生解析经受管提供者能力 `办公文档支持库.表格文档.解析表格文档`
  （openpyxl提供者）执行，本模块（主进程）绝不 import openpyxl；
- 旧格式 XLS 先经受管提供者能力 `文档转换支持库.LibreOffice转换.转换办公文件`
  转 XLSX 再解析；
...
来源 = "表格文档"
# 后端公开能力与 Provider 实现能力分离；公开 owner 仍是本包，Provider
# 只通过内部 id 接收受管调用，避免同能力 id 递归回到本公开实现。
解析表格文档能力id = "内部.表格文档.解析"
