...
def 合并文本文件(文件列表: list = None, 输出路径: str = None, 分隔符: str = None,
                开工ID: str = None) -> 结果:
    """合并多个文本文件到输出文件。返回 {输出路径, 合并文件数, 总字节数}。

    `开工ID`（写入凭证，**只做形态归一、不自行判拒**）：`输出路径` 由调用方入参决定，
    若落在仓库受管面内，必须被一条 `所有者 == 开工ID` 的活跃写租约覆盖 —— 判据唯一在
    `公共契约/运行时/写入授权.校验写入授权`（本腿只转调它）。`输出路径` 在仓库外/
    `工程缓存/`/临时目录时该判据恒通过。
    """
    ...
    通过, 授权说明 = 校验写入授权(输出路径, str(开工ID or "").strip())
    if not 通过:
        return 结果.失败("越界", 授权说明, 来源="办公文档")
    分隔 = 分隔符 or "\n"
    总字节 = 0
    合并数 = 0
    try:
