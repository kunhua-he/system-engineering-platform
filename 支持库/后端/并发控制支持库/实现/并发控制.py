...
def 创建互斥锁() -> 结果:
    ...
    **无参数**：句柄超时不由能力参数决定，一律按本包 `包声明.json` 的 `句柄超时秒` 申报值
    （未申报则默认 1800 秒）。历史上本函数收过一个 `超时秒` 形参但从未使用，
    而契约里又声明了它 —— 那是「能力级覆盖包级」的第二条腿，2026-09-22 已删，只留包级一条。
    """
    return _创建("互斥锁", threading.Lock(), "互斥锁已创建；持句柄 获取锁/释放锁")


def 创建信号量(初始值: int = None) -> 结果:
    """创建信号量。返回句柄。句柄超时按本包 `包声明.json` 的 `句柄超时秒`（历史上收过未使用的 `超时秒` 形参，2026-09-22 已删）。"""
    值 = 初始值 if isinstance(初始值, int) and 初始值 > 0 else 1
    return _创建("信号量", threading.Semaphore(值), f"信号量已创建（初始 {值}）")


def 创建线程池(最大线程数: int = None) -> 结果:
    """创建线程池。返回句柄。句柄超时按本包 `包声明.json` 的 `句柄超时秒`（历史上收过未使用的 `超时秒` 形参，2026-09-22 已删）。"""
