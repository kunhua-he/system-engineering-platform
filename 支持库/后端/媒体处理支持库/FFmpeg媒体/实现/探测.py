"""FFmpeg 媒体提供者·探测侧（FFmpeg媒体）：唯一实现在 支持库/适配层/FFmpeg提供者（B 档收口）。

本文件原与 `支持库/适配层/FFmpeg提供者/实现/探测.py` 逻辑一致，差异仅在取根方式
（后端腿**不再按层数**、改按锚目录判据取根，见 `公共契约/运行时/导入前缀`；适配层腿仍是 `parents[4]`）。同一份逻辑只能有一个实现，
故本文件改为**转调**：让
`支持库.后端.媒体处理支持库.FFmpeg媒体.实现.探测` 与适配层腿那唯一实现成为
**同一个模块对象**（`sys.modules[__name__] = 唯一实现`）。本包 `__init__.py` 照旧从
本路径导入 `检查提供者 / 探测媒体`，本包 `实现/处理.py`、`实现/拼接.py` 也照旧按
本路径 import 探测 —— **对外 import 路径零改动**。

为什么不直接 `import ...实现.探测`：跨包导入 `实现/` 被
...
唯一实现名 = 取根前缀(__name__) + "支持库.适配层.FFmpeg提供者.实现.探测"
系统根 = 取系统根(__file__)

if 唯一实现名 not in sys.modules:  # 兜底：公开入口未加载该子模块时按文件路径显式载入
    唯一实现文件 = 系统根 / "支持库" / "适配层" / "FFmpeg提供者" / "实现" / "探测.py"
