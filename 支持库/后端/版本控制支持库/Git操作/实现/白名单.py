"""白名单：唯一实现在 支持库/适配层/Git提供者（D-1 收口，本包不放第二份）。

本文件原与 `支持库/适配层/Git提供者/实现/白名单.py` **逐字同源**（仅包路径前缀不同）。
同一份逻辑只能有一个实现，故本文件改为**转调**：让
`支持库.后端.版本控制支持库.Git操作.实现.白名单` 与适配层腿那唯一实现成为
**同一个模块对象**（`sys.modules[__name__] = 唯一实现`）。本包 `__init__.py` 照旧从
本路径导入 —— **对外 import 路径零改动**。

为什么保留同名模块（不能删）：本包 `实现/受管执行.py`、`实现/仓库根白名单.py`、`实现/提交回滚.py` 都按这个名字导入，改名会让这些入口失效。

为什么不直接 `import ...实现.白名单`：跨包导入 `实现/` 被
...
if 唯一实现名 not in sys.modules:  # 兜底：公开入口未加载该子模块时按文件路径显式载入
    唯一实现文件 = 系统根 / "支持库" / "适配层" / "Git提供者" / "实现" / "白名单.py"
