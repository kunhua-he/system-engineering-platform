...
def _被忽略(相对路径: str, 名称: str, 忽略模式表: tuple) -> bool:
    """条目是否命中忽略模式（E1，2026-09-21 批 4）：**整段相对路径与名称都参与匹配**。

    为什么要匹配相对路径而不只名称：忽略清单的自然写法是 `工程缓存/**`、`*/归档/*`
    这类**带目录的**模式；只看名称就表达不了（`节点_modules` 与 `a/node_modules` 分不开）。
    同时兼容只写名称的简单写法（`*.log`）—— 两种写法都能用。
    """
    for 模式 in 忽略模式表:
...
def _估算token(文本: str) -> int:
    """估文本 token 数（E1，2026-09-21 批 4）：口径与仓库地图一致 —— **字符数 / 3**。

    为什么口径要从外界抄（而不自进一套）：同仓已有两把尺子
    （`代码解析支持库.仓库地图` 用 字符/3；`大语言模型支持库.上下文压缩` 用
    中文字数×0.8+英文字符数/4），本能力若再整个第三套，调用方就无法把
    「树占多少 token」与「地图占多少」放在一起比，反而误导。
    故这里**显式选地图那把尺子**（同属「代码仓形状」类返回，可比性最重要），
    并在字段名与说明里写明口径，不让人猜。
    """
    return len(文本) // 3
...
def 目录树(目录路径: str = None, 最大深度: int = 默认最大深度, 包含文件: bool = 真,
           ...
           含大小: bool = 假, 忽略模式: list = None) -> 结果:
    """一次调用返回整棵树。返回 {树文本, 节点数, 目录数, 文件数, 已截断, 截断处, 估算token}。
    ...
    `忽略模式`（2026-09-21 批 4，**只增的可选参数**）：glob 列表，命中的条目（目录连子树）
    一律不进树；不传＝行为与改前**逐字一致**。它与 `名称模式` 方向相反 —— 那个是「只留
    匹配的」（含型白名单，同 `tree -P`），这个是「去掉匹配的」（忽略型，同 `.gitignore` 的
    直觉）；两者可同时用，先忽略再白名单。
    """
    ...
    if 忽略模式 is not None and not isinstance(忽略模式, list):
        return 结果.失败("参数不合法", "忽略模式 必须是 glob 字符串列表（不传 = 不忽略）", 来源="文件系统")
    忽略模式表 = tuple(str(项) for 项 in (忽略模式 or []) if str(项))
    ...
    def 遍历(目录: pathlib.Path, 前缀: str, 相对前缀: str, 层级: int) -> None:
        ...
        for 条目 in _列直接条目(目录, 排除集):
            相对候选 = f"{相对前缀}/{条目['名称']}" if 相对前缀 else 条目["名称"]
            if 忽略模式表 and _被忽略(相对候选, 条目["名称"], 忽略模式表):
                continue
            if not 条目["是目录"]:
                if not 包含文件:
                    continue
    ...
    return 结果.成功结果({
        ...
        "已截断": 截断["已截断"],
        "截断处": 截断["截断处"],
        # 只增字段（2026-09-21 批 4）：让调用方**知道该不该继续下钻**，省一次试探。
        "估算token": _估算token(树文本),
        "估算口径": "字符数 / 3（与 代码解析支持库.仓库地图 同一把尺子）",
    })
