...
def _定位项目根() -> pathlib.Path:
    ...
    相对路径的生效根一律由调用方**显式**传（`解析路径(路径, _定位项目根())`），
    不把根留空交给「环境变量 → 进程 cwd」兜底（2026-09-24 修根）。
    """
...
def 删除目录(目录路径: str = None, 允许危险路径: bool = False,
    ...
    try:
        ...
        # ★ 2026-09-25 本批：落盘窗口本身也过授权判据（`仓库只读锁._要求写入授权`）⇒ 开窗
        #   必须带调用方的开工ID，否则受管路径上空开工ID 会让窗口自己抛 PermissionError。
        with 临时解锁树(目标, 开工ID=str(开工ID or "").strip()):
            shutil.rmtree(目标)
    except OSError:
...
def 追加写入(路径: str = None, 内容: str = None,
             允许危险路径: bool = False, 开工ID: str | None = None) -> 结果:
    ...
    try:
        ...
        #
        # ★ 2026-09-24（未完成事项 D3）：窗口内按写前状态恢复，**窗口外**补
        #   `对齐目标锁态(路径)` —— 活跃写租约窗口里「写前状态」正是认领时刚解开的
        #   「未锁」⇒ 不补这一步，追加过的落点会停在未锁，`pre-commit` 钩子
        #   「未锁 > 0 即拒提交」当场判红。口径与 `文件系统.py::_原子写` 同一条，
        #   不在本文件另立一份判据（哲学 1.2）。
        try:
            # ★ 2026-09-25 本批：开窗带开工ID（窗口本身也过授权判据，缺它受管路径必抛）。
            with 临时解锁(路径, 开工ID=str(开工ID or "").strip()):
                with open(路径, "ab") as f:
                    f.write(字节缓冲)
        finally:
            # ★ 窗口**外**收口，且只收口「窗口退出后仍未锁」的那一档：写前就锁着的
            #   文件由 `临时解锁` 自己按写前状态复原，无条件对齐会把它解锁掉。
            写腿收口锁态(路径)
        # ★ 2026-09-26 治根（华哥「MCP 错误的，就先修 mcp，想办法治根先」）：追加写 `.md`
        #   **也必须登记写入凭据**。改前本腿是**唯一一条改了 md 却不登记**的写入腿，后果不是
        #   「少一条流水」，而是 `pre-commit` 的「拒绝手写」判据把**工具写的文件判成手写**并
        #   拒提交（实测：`开发文档/未完成事项.md` 三次追加后提交被拒，判据报「印记 1 份在／
        #   写入凭据 0 份命中」；又因留痕只增，它还被过期原因码误归成「凭据登记失败」）。
        #   与上方 `_身份拒绝` 那条注释是**同一类缺口**（「同一个包里的腿没挂同一道口」），
        #   故按本仓纪律「发现一个问题先排查同类」在此补齐，而不是逐个再补。
...
def 压缩文件(源路径: str = None, 目标路径: str = None,
    ...
    # ★ 2026-09-25 补挂（本批开工-20260925-161955-849f）：压缩会创建/覆盖**目标** zip
    #   （源路径只读），属变更类写入腿，此前只有 `_身份拒绝` 一道闸。补第二判据，
    #   只判目标（与危险路径护栏既有口径一致：源只读、不构成系统性破坏）。
    授权 = 授权拒绝结果(目标路径, "文件系统", 开工ID)
    if 授权 is not None:
        return 授权
    # 「相对 → 绝对」只算一次、只走一条腿：`公共契约.运行时.平台适配.解析路径`；
    # **根显式给出**（`_定位项目根()`，判据本体在 `公共契约/运行时/导入前缀.取系统根`，
    # 唯一实现）—— 不留空转交「环境变量 `系统平台_项目根` → 进程 cwd」三级兜底：
    # 网关跑制品时进程 cwd 就是制品根，相对源路径会静默遍历错那棵树而不报错
    # （2026-09-24 修根）。`abspath` 只保留「规范化」那一半（去 `..`/`.`、**不解析
    # 符号链接** —— 遍历产出的成员名要与旧行为逐字一致）。本函数后面每一处对 `源路径`
    # 的判据与遍历都用这个绝对形态（同一入参若有的按 cwd、有的按生效根，就是「校验看
    # 一棵树、遍历看另一棵树」的半腿；2026-09-24 实测：只改遍历基时，设了环境变量会
    # 先回「源不存在」；而若同名路径恰在 cwd 下存在，就会静默遍历错那棵树）。
    源绝对 = os.path.abspath(解析路径(源路径, _定位项目根()))
    ...
    try:
        ...
        # ★ 2026-09-24 批R-R24：窗口只按**调用方给的目标**开 —— `临时解锁(目标)` 本来就
        #   一路解锁到「第一个已存在的祖先（含）」，而这两个派生件都建在 `目标.parent` 里，
        #   故父目录链已把它们整片覆盖；再把派生件逐条列进来是**多余的第二套说法**。
        #   更硬的理由：它们是**本腿自己造的名字**（`.__压缩_*` / `.__压缩备份_*`），
        #   调用方**永远认领不到** ⇒ 一旦被 `写入授权.校验写入授权` 逐条判，
        #   这条腿在「目标已存在」时就结构性必炸（实测：认领了成品.zip 仍回
        #   `越界：.__压缩备份_xxx`，PermissionError 从 finally 逃出去，清理与还原全丢）。
        with 临时解锁(目标, 开工ID=str(开工ID or "").strip()):
...
def 解压文件(源路径: str = None, 目标目录: str = None,
             ...
             最大压缩比: float = None, 允许危险路径: bool = False,
             开工ID: str | None = None) -> 结果:
    ...
    # ★ 2026-09-25 补挂（本批开工-20260925-161955-849f）：解压会向目标目录整树落盘，
    #   属变更类写入腿，此前只有 `_身份拒绝` 一道闸。补第二判据（只判目标目录）。
    授权 = 授权拒绝结果(目标目录, "文件系统", 开工ID)
    if 授权 is not None:
        return 授权
    ...
    try:
        with zipfile.ZipFile(源路径, "r") as 压缩包:
            ...
            # ★ 2026-09-25 本批：开窗带开工ID（窗口本身也过授权判据，缺它受管路径必抛）。
            with 临时解锁树(目标根, 开工ID=str(开工ID or "").strip()):
