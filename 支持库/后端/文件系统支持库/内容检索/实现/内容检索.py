...
def 正则搜索(
    ...
    **紧凑返回（2026-09-24 批R R-15 只增开关，默认 假）**：真 时去掉 `命令` 回显
    ...
    """
    # ★ 2026-09-24 批R R-32：`根目录` 省略/空串 ＝ **调用方 `项目根`**（契约 必填 真→假）。
    #   为什么：平台口径是「普通路径参数一律项目根相对」，而本能力改前把空 `根目录` 判成
    #   `参数不合法`，调用方只能靠猜一个基准（与 R-21/R-27 各白跑一轮同一形态）。
    #   这里**不自写基准**：空串时用 `"."` 表达「基准本身」，交给全平台唯一那条解析腿的
    #   兜底（本次调用基准 ＝ 调用方顶层 `项目根`，网关派发前写入，见
    #   `公共契约.运行时.平台适配.设置本次调用基准`），与 `解析路径(路径, 根=None)` 同源。
    根目录文本 = 根目录.strip() if isinstance(根目录, str) else ""
