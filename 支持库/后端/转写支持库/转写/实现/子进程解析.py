"""子进程解析逻辑：唯一实现在 支持库/适配层/MLXWhisper提供者（D-3 收口，本包不放第二份）。

本文件原与 `支持库/适配层/MLXWhisper提供者/实现/子进程解析.py` **逐段一致**，
唯一差异是一处注释措辞（「既有测试与说明文档」↔「既有测试与设计说明」）。同一份逻辑
只能有一个实现，故本文件改为**转调**：让
`支持库.后端.转写支持库.转写.实现.子进程解析` 与适配层腿那唯一实现成为
**同一个模块对象**（`sys.modules[__name__] = 唯一实现`）。

为什么要保留同名模块（不能删）：`支持库/后端/转写支持库/转写/实现/子进程入口.py`
与 `测试中心/支持库/测试_转写跨平台后端.py` 都按这个名字导入（后者还拿它做两腿
「分段键名逐字一致」「附加术语两后端都传 initial_prompt」的对照），改名会让这些入口失效。
...
唯一实现名 = f"{__name__.split('支持库.', 1)[0]}支持库.适配层.MLXWhisper提供者.实现.子进程解析"
系统根 = next(
...
if 唯一实现名 not in sys.modules:  # 兜底：公开入口未加载该子模块时按文件路径显式载入
    唯一实现文件 = 系统根 / "支持库" / "适配层" / "MLXWhisper提供者" / "实现" / "子进程解析.py"
