...
def 连接LLM(模型: str = None, 提供者: str = None, 部署形态: str = None,
           ...
           协议: str = 默认协议, 额外请求头: dict = None, 超时秒: int = None,
           内存估算系数: float = None, 内存安全阈值: float = None) -> 结果:
    """连接大语言模型，返回句柄。连接阶段只接受连接配置与协议参数；流式输出属于生成对话选项，未知关键字（包括连接阶段流式输出）由函数签名拒绝。

    ★ 2026-09-23 签名名序对齐真契约：形参名序此前为 `(…, 上下文长度, 超时秒, 协议, 额外请求头, …)`，
    与真契约 `能力契约/参数契约.json` 的 `[…, 上下文长度, 协议, 额外请求头, 超时秒, …]` 不一致。
    ...
    if 形态 == "本地" and 本地路径:
        ...
        return 启动本地模型(模型路径=本地路径, 启动器=启动器, 模型类型="LLM",
                           模型大小字节=模型大小字节,
...
def 连接向量模型(模型: str = None, 提供者: str = None, 部署形态: str = None,
    ...
    配置 = {"模型名": 模型, "提供者": 提供者 or "本地", "部署形态": 形态, "本地路径": 本地路径,
            "启动器": 启动器, "模型大小字节": 模型大小字节, "url": url, "api_key": api_key}
    if 形态 == "本地" and 本地路径:
        return 启动本地模型(模型路径=本地路径, 启动器=启动器, 模型类型="向量",
                           模型大小字节=模型大小字节, 超时秒=超时秒)
...
def 连接重排模型(模型: str = None, 提供者: str = None, 部署形态: str = None,
    ...
    配置 = {"模型名": 模型, "提供者": 提供者 or "本地", "部署形态": 形态, "本地路径": 本地路径,
            "启动器": 启动器, "模型大小字节": 模型大小字节, "url": url, "api_key": api_key}
    if 形态 == "本地" and 本地路径:
        return 启动本地模型(模型路径=本地路径, 启动器=启动器, 模型类型="重排",
                           模型大小字节=模型大小字节, 超时秒=超时秒)
...
def 连接决策模型(模型: str = None, 提供者: str = None, 部署形态: str = None,
    ...
    配置 = {"模型名": 模型, "提供者": 提供者 or "本地", "部署形态": 形态, "本地路径": 本地路径,
            "启动器": 启动器, "模型大小字节": 模型大小字节, "url": url, "api_key": api_key}
    if 形态 == "本地" and 本地路径:
        return 启动本地模型(模型路径=本地路径, 启动器=启动器, 模型类型="决策",
                           模型大小字节=模型大小字节, 超时秒=超时秒)
...
def 启动本地模型(模型路径: str | None = None, 模型类型: str | None = None, 启动器: str | None = None,
                端口: int | None = None, 模型大小字节: int | None = None, 超时秒: int | None = None, 参数: dict | None = None) -> 结果:
    """串行启动本地模型；同一路径的后续请求由内部索引复用全局句柄。

    ★ 2026-09-23 签名名序对齐真契约：形参名序此前为 `(模型路径, 启动器, 模型类型, …, 参数, 超时秒)`，
    与真契约 `能力契约/参数契约.json` 的 `[模型路径, 模型类型, 启动器, 端口, 模型大小字节, 超时秒, 参数]` 不一致。
    运行期一律关键字调用故无功能影响；参数事实源只有契约一处（哲学 12.1）。
    下方转调 `_启动本地模型` 仍按它自己的形参序按位置传参，与本次调整无关。
    """
    with 本地启动锁:
...
def _回收看守名下模型进程(模型路径: str, 看守进程号: int) -> None:
    ...
    if 进程终止.进程组存活(模型进程号):
        ...
        # 看守用 子进程组启动标志() 拉起模型进程（独立组长），只判组长句柄会漏掉
        # 「组长已回收、同组子孙仍在」。回收失败已在收口层登记留痕。
        已收敛 = 进程终止.结束并留痕(
            模型进程号, 位置="模型连接器.回收看守名下模型进程", 宽限秒=5.0, 等待秒=5.0)
        if not 已收敛:
            降级记录表.append(
                f"看守名下模型进程 {模型进程号} 强杀后仍未收敛（见 进程终止.回收失败留痕快照()）")
    try:
