...
def _打分(汇总项: dict, 全记录: list[dict], 任务类型: str, 权重: dict, 样本门槛: int,
          基准耗时: float, 基准成本: float) -> dict:
    """按四维加权给单个候选打分（纯函数，同输入同输出）。

    ★ 基准耗时/基准成本 由调用方在**候选集合全体记录**上算好后传入（见 `选择模型`）。
      实测教训（2026-09-18）：早期实现用「该模型自己的中位数」当基准，
      而该模型自己的平均值≈自己的中位数 ⇒ 比值恒≈1 ⇒ 效率与性价比恒得 0，
      两维形同虚设、分数只剩成功率一项。基准必须跨候选集合取，才产生区分度。
    """
    条数 = 汇总项["样本数"]
    if 条数 < 样本门槛:
        # ★ 样本不足不给分 —— 不编造、不默认满信心
...
def 选择模型(
    ...
    if 智能 and 存活:
        # ★ 基准必须跨「候选集合全体记录」取（不是每个模型自己的中位数）——
        #   否则该模型的平均值≈自己的中位数 ⇒ 效率/性价比恒 0，两维形同虚设（实测坑）。
        全体耗时 = [条["耗时毫秒"] for 条 in 全记录 if 条["耗时毫秒"] is not None]
        全体成本 = [条["成本"] for 条 in 全记录 if 条["成本"] is not None]
        基准耗时 = _中位数(全体耗时) or 1.0
        基准成本 = _中位数(全体成本) or 1.0
