...
自带完整边界（只依赖标准库 + 结果类型 + 连接器基元的 `_失败`），
与「句柄/连接/启动」三类职责无耦合。
拆出后该文件由 1667 行降到约 1440 行；**对外零变化** ——
`归一化对话响应` / `归一化流式分块` 仍在 `模型连接器.py` 里 re-export（见该文件尾部），
`__init__.py` 的导入路径不变。

**2026-09-23 补漏**：上面这次拆分只搬了函数体，把 `_失败` 留在了旧文件
（`_失败` 现居 `实现/模型连接基元.py`，是全族唯一的失败结果构造）。于是本文件
6 处错误分支全是 `NameError`：`归一化对话响应` / `归一化流式分块` 的
「入参不是字典型/逻辑型」与「形状无法识别」—— 而这四条**正是契约明文要求的行为**
（能力契约「空值」：*空或非字典/非逻辑型入参返回 参数不合法*；错误码含
`参数不合法` / `归一化失败`）。既有用例只走正常形状，故长期潜伏。修法＝按名导入
唯一实现（见文件末尾收口导入），**不另写一份 `_失败`**。
"""
...
# ── 收口导入（放文件末尾；与 `模型连接基元.py` 末尾同一手法，避开环形导入）──────────
# `_失败` 的唯一实现在 `实现/模型连接基元.py`（来源固定「模型连接器」，全族共用）：
# 本文件不另写第二份（同一件事两套实现即缺陷），按名导入。
# **位置必须在文件末尾**：`模型连接基元.py` 末尾也反向导入本文件的 `_归一化响应体`，
# 若把本导入放文件头，则「先导入本文件」的顺序会拿到**半初始化**的基元模块
# （`_归一化响应体` 那时还没定义）而 ImportError。放末尾则两种导入顺序都成立
# （基元在第 84 行就已定义 `_失败`，早于它末尾的反向导入）。
from 支持库.后端.大语言模型支持库.模型连接器.实现.模型连接基元 import (  # noqa: E402
    _失败,
)
