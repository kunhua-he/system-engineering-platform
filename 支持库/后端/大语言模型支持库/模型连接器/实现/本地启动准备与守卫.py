...
def 回收看守残留(模型路径: str) -> dict:
    ...
    # 升级顺序（终止→宽限→强杀→复查）由收口层一处持有；复查口径是**整组**。
    已收敛 = 进程终止.结束并留痕(
        模型进程号, 位置="本地启动准备与守卫.回收看守残留", 宽限秒=5.0, 等待秒=5.0)
    if 已收敛:
        _写看守账本(看守账本路径(模型路径), {**账本, "状态": "已回收"})
        return {"已回收": 真,
                "原因": f"归属与世代校验通过，已回收模型进程 {模型进程号}（端口 {端口}）"}
    降级记录表.append(f"看守残留回收失败：模型进程 {模型进程号} 强杀后仍未收敛")
    return {"已回收": 假, "原因": f"模型进程 {模型进程号} 强杀后仍未收敛"}
...
def 运行本地模型看守(端口: int, 空闲秒: int, 命令: list[str], 日志路径: str,
    ...
    try:
        ...
        if 进程 is not None and 进程.poll() is None:
            ...
            # 升级顺序由收口层一处持有；手上有 Popen 句柄就直接传句柄（能拿到真实退出码）。
            进程终止.结束并留痕(
                进程, 位置="本地启动准备与守卫.看守收尾", 宽限秒=5.0, 等待秒=5.0)
            try:
                进程.wait(timeout=5)
            except Exception:
                pass
