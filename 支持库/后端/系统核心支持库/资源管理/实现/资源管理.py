...
def 执行逆序回滚(*, 事务id: str = None, 回滚器: dict = None) -> 结果:
    """按逆序执行全部已登记步骤的回滚动作。回滚器 = {"函数名": "...", "参数模板": {...}}。

    0限制设计：底座不内置回滚实现，回滚器由调用者提供（如 V3 侧的业务释放函数）；
    底座只保证「逆序 + 全部执行 + 统计结果」，回滚器不可用时标记失败但不中断。
    """
    try:
        事务 = str(事务id or "").strip()
        if not 事务:
        ...
        with _回滚锁:
            ...
            条目["已回滚"] = True
        return 结果.成功结果({
            "事务id": 事务, "回滚数": len(回滚结果列表),
            "回滚列表": 回滚结果列表, "状态": "已回滚",
            "说明": "按登记逆序全部标记执行；实际释放动作由调用者的回滚器完成（底座0限制只编排）",
        })
    except Exception as 异常:
