...
def _授权拒绝(路径, 开工ID: str | None = None):
    """变更类删除腿的授权收口：不过即回 `结果.失败("越界", …)`，过则 `None`。

    判据本体与失败说明的唯一来源是 `公共契约/运行时/写入授权.授权拒绝结果`
    （它再转 `校验写入授权`）—— 本模块不另写授权语义、也不另写「越界」说明文本
    （哲学 1.2：同一条腿两处实现即缺陷）。`开工ID` 由变更类能力从公开入参原样透传：
    受管路径必须被**本凭证**认领的活跃写租约覆盖，缺失或属他人即拒（fail-closed）。

    `原子写入` 家族（含 `原子替换`/`比较并交换`）**不**在此收口：它们整条腿转调唯一
    写入腿 `文件系统支持库.文件操作.写入文件`，由那条腿自己过同一道判据（本模块不重复判）。
    本函数只服务 `安全释放` 这条**删除腿**（它不写内容，故不走唯一写入腿）。
    """
    from 公共契约.运行时.写入授权 import 授权拒绝结果

    return 授权拒绝结果(str(路径), "资源管理", str(开工ID or "").strip())
...
def 原子写入(
    ...
    """原子写入（**唯一写入腿的转发口**；2026-09-24 批M-1 收口，未完成事项 M-2）。

    ★ 本函数**不再自带落盘骨架**：此前它是**素腿** —— 临时件 + `fsync` + `os.replace`，
    成熟腿四件骨架一样都没有（① 身份对账 / ② 租约授权 / ③ 内核解锁窗口 / ④ md 写入凭据）。
    而权威能力 `模块库/项目文档/实现/写文档.py:9`（docstring 明写经本函数写 md）走的正是它
    ⇒ **平台自己写出的 md 对不上任何一次「经工具写入」，被 `开发工具/git钩子/pre-commit`
    的 `拒绝手写md` 判红**（批L 亲历一次）。

    现整条腿**转调唯一写入腿** `文件系统支持库.文件操作.写入文件`（同层内部导入，
    `运行核心/依赖防火墙.py:719`「允许方向检查（同层内部导入允许）」明文允许），
    四件骨架全部由它提供：

        ① `平台适配.MCP身份准入()`（不过即 `权限不足`，不落盘）；
        ② `写入授权.授权拒绝结果(路径)`（受管路径必须被一条活跃写租约覆盖，
           否则回 `结果.失败("越界")`，说明里带可照抄的 `开工即占` 调用）；
        ③ `仓库只读锁.临时解锁(目标)` 窗口 + 窗口外 `写腿收口锁态(目标)`；
        ④ `写入流水.登记md写入凭据(路径, 内容, 来源)`（md 唯一凭据登记节点）。

    **为什么不 import `文件系统支持库` 的 `_原子写` 私有骨架**：跨包 import `实现/` 目录是
    `运行核心/依赖防火墙.py:640` 的**强制拒绝**项（同包 `__init__` 导自身实现才合法），
    合法通路只有包级入口 —— 故本文件只 `from 支持库.后端.文件系统支持库.文件操作 import 写入文件`，
    骨架一份都不复制（「不许两个文件各留一份骨架」）。

    **返回形状**（本批同批收口）：成功回 `结果.成功结果(真)`（与真契约
    `返回.值结构.值 = 成功标志（真值）` 逐字一致）；失败回唯一腿的
    `结果.失败(错误码, 说明)`。此前失败只能靠抛 `OSError` 外溢，调用方拿不到
    「为什么被拒」（是越界？还是身份没对上？），能力面还会把它落成 `内部错误`。
    进程内调用方读 `.成功`，能力面调用方拿到同一份信封
    （`运行核心/能力调用/唯一能力调用._规范化结果` 对 `结果` 原样透传、不再包装）。

    **权限位口径**：`保留权限` 形参保留（签名是既定调用契约，批M-1 明文要求一字不改），
    但权限位治理现在只有唯一腿一处 —— 它**恒按「目标已存在则保持原权限位」恢复**，
    故「不保留」这一分支不再存在：传 `假` 即明确回 `参数不合法`（**不静默降级**：
    权限位承载安全语义，装作收下了却按保留执行才是真缺陷）。
    连带口径变化：新建文件的权限位由唯一腿的临时件决定（`0600`），不再随 umask。

    - 内容 与 内容字节 二选一：文本走 内容（utf-8），二进制走 内容字节。
    - 二进制经 `latin-1` 逐字节往返（该编码是 `0..255` 与码位的双射，落盘字节逐字相同）：
      唯一腿只收文本，故这是**传输编码**，不是第二条骨架。
    - 危险路径护栏随唯一腿生效（默认拒绝系统目录 / 用户凭据与 shell 启动配置）；
      本签名没有 `允许危险路径`，确需放行的调用方请直接用
      `文件系统支持库.文件操作.写入文件`。

    **开工ID（2026-09-25 底座权限面收口）**：本能力是唯一写入腿的转发口，`开工ID`
    原样透传给 `写入文件` —— 受管路径必须被**本凭证**认领的活跃写租约覆盖，缺失或属
    他人即由那条腿回 `越界`；写 `.md` 时也由那条腿把它写进写入凭据。
    """
    if 保留权限 is not 真:
        return 结果.失败(
            "参数不合法",
            "保留权限=假 已随唯一写入腿收口（2026-09-24 批M-1）：权限位治理只在写入腿一处，"
            "唯一腿恒按「目标已存在则保持原权限位」恢复，故不再提供「不保留」分支；"
            "本形参保留仅为签名与既有调用点兼容，请传 真（缺省即 真）。",
            来源="资源管理",
        )
    数据 = 内容字节 if 内容字节 is not None else 内容
    if isinstance(数据, (bytes, bytearray)):
        文本, 编码 = bytes(数据).decode("latin-1"), "latin-1"
    else:
        文本, 编码 = 数据, "utf-8"
    # 包级入口只在真被调用时导入：本模块属 `系统核心支持库`，不为一次落盘在模块顶层
    # 拉起另一个支持库（同层内部导入，判据见 docstring 引的依赖防火墙两行）。
    from 支持库.后端.文件系统支持库.文件操作 import 写入文件

    回执 = 写入文件(文件路径=str(目标路径), 内容=文本, 编码=编码, 开工ID=开工ID)
    if not 回执.成功:
        return 回执
    return 结果.成功结果(真)
...
def 原子替换(目标路径: Path, 新内容: str | bytes, 期望版本: str = "",
         开工ID: str | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        # 落盘腿收口后 `原子写入` 回统一结果信封（成功 `结果.成功结果(真)`）：本函数必须
        # **按 `.成功` 判成败并原样上抛错误码**，不许再「调完就当成功」——否则唯一腿拒绝
        # （越界 / 身份没对上）时本函数会假报「原子替换完成」（2026-09-24 批M-1）。
        落盘 = 原子写入(目标路径, 新内容, 开工ID=开工ID)
        if not 落盘.成功:
            return 假, _带错误码(落盘.错误码, 落盘.错误说明)
        return 真, "原子替换完成"
    finally:
        短锁.释放()


def 比较并交换(目标路径: Path, 期望值: Any, 新值: Any,
          开工ID: str | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        # 判成败口径同 `原子替换`：`原子写入` 现回统一结果信封，不许「调完就当成功」。
        落盘 = 原子写入(目标路径, json.dumps(现有, ensure_ascii=False, indent=2), 开工ID=开工ID)
        if not 落盘.成功:
            return 假, _带错误码(落盘.错误码, 落盘.错误说明)
        return 真, "CAS 提交成功"
    finally:
...
def 安全释放资源(路径: Path, 开工ID: str | None = None) -> tuple[bool, str]:
    """安全释放资源：文件删除或目录递归删除；不存在视为幂等成功。

    **开工ID（2026-09-25 底座权限面收口）**：这是本模块的**删除腿**，与
    `文件系统支持库.文件操作.删除文件/删除目录` 同一判据、同一份实现来源 ——
    受管路径必须被**本凭证**认领的活跃写租约覆盖，否则拒绝（不删一个字节）。
    非受管路径（`工程缓存/`、`/tmp` 等临时资源）本判据没有话说，行为不变。
    """
    路径 = Path(路径)
    拒绝 = _授权拒绝(路径, 开工ID)
    if 拒绝 is not None:
        return 假, _带错误码(拒绝.错误码, 拒绝.错误说明)
    if not 路径.exists():
        return 真, "资源不存在（释放幂等）"
    try:
        if 路径.is_dir():
