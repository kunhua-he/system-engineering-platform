...
def 创建不可变快照(来源目录: Path, 快照目录: Path) -> Path:
    """创建不可变快照（复制 + 快照摘要 + 只读标记）；返回快照摘要文件的 Path。

    ★ 注解修正（2026-09-23 P8）：原注解写 `-> str`，实返 `快照目录 / "快照摘要.json"`（Path），
    属「注解与实返不符」的注解瑕疵（非契约谎 —— 契约 `能力契约/参数契约.json` 说的是
    「成功返回快照摘要路径」，并未承诺 str）。全仓 AST 扫描同类（注解写标量/实返 Path）仅此一处，
    无兄弟漏改。返回真身是 Path，故注解改为 `-> Path`。
    """
    来源目录 = Path(来源目录)
    快照目录 = Path(快照目录)
    if not 来源目录.is_dir():
...
def 原子写入(
    ...
    ★ 2026-09-23 签名名序对齐真契约：形参名序此前为 `(目标路径, 内容, 保留权限, 内容字节)`，
    与真契约 `能力契约/参数契约.json` 的 `[目标路径, 内容, 内容字节, 保留权限]` 不一致。
    运行期一律关键字调用故无功能影响；参数事实源只有契约一处（哲学 12.1）。
