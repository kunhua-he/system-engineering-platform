...
**敏感键名白名单 + 默认掩码**收口：键名（忽略大小写、含子串即命中）
命中唯一腿 `公共契约/基础类型/字段名册.py::敏感键名表` 时，值以 `默认掩码`（`***`）
回显；需明文时由调用方显式传 包含敏感值=True（可选参数，缺省 False，向后兼容）。
...
# 敏感键名判定走唯一腿（`公共契约/基础类型/字段名册.py` 的 `敏感键名表` + `是敏感键名`）：
# 本模块**不自持关键词表** —— 2026-09-23 收口前它自持 `敏感键关键词表`（全大写口径），
# 是当时五份互不覆盖的敏感键名表之一。收口后表取五份的**并集**（中文凭据命名 + 英文通用命名），
# 本包原有的 `KEY/TOKEN/SECRET/PASSWORD/PASSWD/PWD/CREDENTIAL/AUTH/凭据/凭证/密钥/私钥/口令/密码`
# 全部包含在内，且口径不变（忽略大小写、含子串即命中，失败安全：宁可多掩码，不漏码）。
...
def _回显值(名称: str, 值, 包含敏感值: bool):
    ...
    敏感键（判定见唯一腿 `公共契约.基础类型.字段名册.是敏感键名`）的值掩码为
    `默认掩码`。空值原样返回空串（无敏感内容可泄露，也避免把「空」误显示成「有值」）。
    """
    if 包含敏感值 or not 是敏感键名(名称):
        return 值
...
def 获取环境变量(名称: str = None, 包含敏感值: bool = False) -> 结果:
    ...
    敏感键（判定见唯一腿 `公共契约/基础类型/字段名册.是敏感键名`）的值默认以
    `***` 回显；需明文时显式传 包含敏感值=True。命中脱敏时 值 恒为 `***`
    （不回显长度/前缀）。
    """
