...
def 启动进程(命令: str = None, 参数: list = None, 工作目录: str = None,
    ...
    ``环境变量`` —— ★ **合并**（2026-09-23 华哥裁决，依据 `决策记录 0061` 第 ③ 项）：
    在**当前进程环境**（``os.environ``）之上逐键覆盖 —— 即「保留既有 PATH ＋ 传参覆盖
    同名键」；留空或空字典则原样继承当前进程环境。
    改前是**整体替换**（``Popen(env=环境变量)``）—— 实测传 ``{"X": "1"}`` 会让子进程
    **丢掉 PATH**：子进程环境只剩 ``X=1``、``python3.14`` 找不到（`[Errno 2] No such
    file or directory`），看着像解释器没装。**调用方不再需要自己带上 PATH**。
    """
    ...
    # 直启腿（经shell 留空/假）遇到 shell 语法一律**显式拒绝**，不再静默把它当普通参数
    # 传给程序（2026-09-24 R-27 华哥口径「未对齐就报，不许静默」）。判据本体是收口层
    # 唯一实现 `平台适配.shell语法问题()`，本处不另写一套识别。
    if not 经shell开关:
        语法问题 = 平台适配.shell语法问题(命令)
        if 语法问题:
            return 结果.失败("参数不合法", 语法问题, 来源="进程管理")
    try:
        # 经shell=真 的解释器 argv（含 POSIX/Windows 差异）由收口层唯一实现，
        # 本处不含任何平台判断。
        cmd = (平台适配.经shell命令表(命令, 参数) if 经shell开关
               else 命令.split() + list(参数 or []))
        # 环境变量口径＝合并（2026-09-23 裁决，决策记录 0061 第③项）：在当前进程环境
        # 之上逐键覆盖，不再整体替换（替换会静默丢 PATH）。留空/空字典 ⇒ None＝原样继承。
        合并环境 = {**os.environ, **环境变量} if 环境变量 else None
        # 独立进程组：平台差异（POSIX setsid / Windows 新建进程组标志）只在
        # 平台适配.子进程组启动标志() 内判定，调用点不写平台判断。
        进程 = subprocess.Popen(cmd, cwd=工作目录, env=合并环境,
...
def 终止进程(句柄: int | None = None, 强制: bool = None) -> 结果:
    ...
    try:
        # 升级顺序（终止→宽限→强杀→复查）由收口层一处持有；能力契约声明 已终止「固定 真」
        # （见 能力定义.json），故此处按契约不把返回值换成 已终止；回收失败仍由收口层登记留痕。
        进程终止.结束并留痕(
            进程, 位置="进程管理.终止进程",
            宽限秒=0.0 if bool(强制) else 2.0, 等待秒=2.0)
...
def 执行命令(命令: str = None, 超时秒: float = None, 工作目录: str = None,
    ...
    **``经shell`` 两种取值下都在直启/解释器启动之前拦截**（shell 形态尤其需要前置策略）。
    ...
    if 经shell原因:
        return 结果.失败("参数不合法", 经shell原因, 来源="进程管理")
    # 直启腿（经shell 留空/假）遇到 shell 语法一律**显式拒绝**：2026-09-24 R-27 实测
    # `执行命令("ls /tmp 2>&1 | head -3")` 在旧形态下退出码 1、错误输出报 `ls: |: ...`、
    # 标准输出却是**未过滤的完整目录** —— 既不报错也不给对结果。判据本体是收口层唯一实现。
    if not 经shell开关:
        语法问题 = 平台适配.shell语法问题(命令)
        if 语法问题:
            return 结果.失败("参数不合法", 语法问题, 来源="进程管理")
    ...
    try:
        ...
        stdout, stderr, 已超时, 已超限 = 受限通信(
            ...
            终止回调=lambda: 进程终止.结束并留痕(
                进程, 位置="进程管理.执行命令", 宽限秒=0.0, 等待秒=2.0),
        ...
        if 进程 is not None:
            if not 进程终止.结束并留痕(
                    进程, 位置="进程管理.执行命令清理", 宽限秒=0.0, 等待秒=2.0):
                清理说明 = "（强制回收未确认收敛，见 进程终止.回收失败留痕快照()）"
...
def 释放句柄(句柄: int | None = None) -> 结果:
    ...
    if 进程对象 is not None:
        try:
            if not 进程终止.结束并留痕(
                    进程对象, 位置="进程管理.释放句柄", 宽限秒=0.0, 等待秒=2.0):
                降级记录表.append("释放句柄：进程组回收未确认收敛（见 进程终止.回收失败留痕快照()）")
        except Exception as 错误:
            降级记录表.append(str(错误))
        finally:
    ...
    try:
        with 输出文件.open("wb") as 出, 错误文件.open("wb") as 错:
            ...
            try:
                ...
                超时标志 = True
                进程终止.结束并留痕(
                    进程, 位置="进程管理.沙箱执行命令超时", 宽限秒=0.0, 等待秒=2.0)
        标准输出 = _读受限(输出文件, 上限字节)
        错误输出 = _读受限(错误文件, 上限字节)
        ...
        if 进程 is not None:
            if not 进程终止.结束并留痕(
                    进程, 位置="进程管理.沙箱执行命令清理", 宽限秒=0.0, 等待秒=2.0):
                清理说明 = "（强制回收未确认收敛，见 进程终止.回收失败留痕快照()）"
        return 结果.失败("执行失败", f"{错误}{清理说明}", 来源="进程管理")
    finally:
        for 文件 in (输出文件, 错误文件):
