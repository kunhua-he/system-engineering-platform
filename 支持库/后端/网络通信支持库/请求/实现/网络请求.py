...
def 下载文件(*, 地址: str = None, 保存路径: str = None, 请求头: dict = None,
             ...
             最大字节数: int = 默认最大字节数, 开工ID: str = None) -> 结果:
    """从 URL 下载文件（内容先攒在内存、有界，再整块交平台唯一写入腿落盘）。
    返回 {状态码, 文件路径, 字节数}。

    落盘口径（2026-09-25 单腿铁律收口）：**不再自建 `.part 临时件 + os.replace` 的
    第二条写实现** —— 整块内容交 `系统核心支持库.资源管理.原子写入`（同层内部导入），
    由它转调唯一写入腿 `文件系统支持库.文件操作.写入文件`：原子替换 + 身份对账 +
    租约授权 + 内核解锁窗口四件骨架全在那一处，本能力只负责把字节凑齐。
    「任何失败路径 `保存路径` 保持原样」这条语义由写腿的原子替换保证（超限在调用
    写腿之前就返回，压根没落盘）。历史实现直接 `open(保存路径, "wb")`：一打开就把
    旧文件清空，失败后旧文件已毁、目录里只剩半截新内容。

    内存有界：`最大字节数` 缺省 5 MiB，且超限在读到超额块时立即返回，缓冲不会无界增长。

    `开工ID`（写入凭证，**只做形态归一、不自行判拒**）：两处消费 ——
    ① 网络 I/O 之前先过 `公共契约/运行时/写入授权.校验写入授权`（同一条判据，
       目的是「没被授权的落点不发起请求」，不是第二套判据）；
    ② 原样透传给写腿 `资源管理.原子写入(开工ID=…)`，由写腿按
       「活跃写租约 `所有者 == 开工ID`」再判一次（判据唯一在那一处）。
    `保存路径` 在仓库外/`工程缓存/`/临时目录时该判据恒通过。
    """
    try:
        if not isinstance(地址, str) or 地址.strip() == "":
            return 结果.失败("参数不合法", "地址为空", 来源="网络请求")
        ...
        from 公共契约.运行时.写入授权 import 校验写入授权
        通过, 授权说明 = 校验写入授权(保存路径, str(开工ID or "").strip())
        if not 通过:
            return 结果.失败("越界", 授权说明, 来源="网络请求")
        地址 = 编码中文地址(地址)
        _校验协议与SSRF(地址, 允许回环)
        ...
        # 内容先攒在内存（有界：见 docstring），再整块交平台唯一写入腿 ——
        # 本能力不再自建「.part 临时件 + os.replace」这条第二条写实现。
        缓冲 = bytearray()
        try:
            响应对象 = 打开器.open(请求, timeout=超时秒)
            with 响应对象 as 响应:
                ...
                while True:
                    ...
                    if not 块:
                        break
                    if len(缓冲) + len(块) > 最大字节数:
                        # 超限：写腿还没被调用，保存路径原样（没落盘就没有半截）
                        return 结果.失败(
        ...
        # 落盘：唯一写入腿（原子替换 + 身份对账 + 租约授权 + 内核解锁窗口四件骨架
        # 全在那一处）；`开工ID` 原样透传（形态归一），受管路径由它按
        # 「活跃写租约 所有者 == 开工ID」判 —— 本能力不自行判拒。
        from 支持库.后端.系统核心支持库.资源管理 import 原子写入
        落盘 = 原子写入(目标路径=str(保存路径), 内容字节=bytes(缓冲),
                        开工ID=开工ID or "")
        if not 落盘.成功:
            return 结果.失败(落盘.错误码, 落盘.错误说明, 来源="网络请求")
        return 结果.成功结果({"状态码": 状态码, "文件路径": 保存路径,
                                "字节数": len(缓冲), "错误信息": ""})
    except ValueError as 错误:
        return 结果.失败("参数不合法", str(错误), 来源="网络请求")
    except OSError as 错误:
