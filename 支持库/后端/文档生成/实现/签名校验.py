...
def 校验签名(格式: str, 字节: bytes) -> 结果:
    ...
    if not isinstance(字节, bytes):
        return 结果.失败("参数不合法", "字节必须是二进制数据", 来源="文档生成")
    格式 = str(格式 or "").lower().lstrip(".")
    if 格式 in OOXML必要成员表:
        return _校验OOXML(格式, 字节)
    if 格式 == "pdf":
        return _校验PDF(字节)
    return 结果.失败("参数不合法", f"不支持的签名校验格式 '{格式}'", 来源="文档生成")
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(字节)) as 压缩包:
            if 压缩包.testzip() is not None:
                return 结果.失败("生成失败", "OOXML 压缩包存在损坏成员", 来源="文档生成")
            名称表 = set(压缩包.namelist())
    except (zipfile.BadZipFile, OSError) as 错误:
        return 结果.失败("生成失败", f"不是有效 ZIP 压缩包: {错误}", 来源="文档生成")
...
def _校验PDF(字节: bytes) -> 结果:
    ...
    if not 字节.startswith(b"%PDF"):
        return 结果.失败("生成失败", "PDF 缺少 %PDF 文件头", 来源="文档生成")
    尾部 = 字节[-2048:].rstrip(b"\r\n \t")
    if not 尾部.endswith(b"%%EOF") and b"%%EOF" not in 尾部:
        return 结果.失败("生成失败", "PDF 缺少 %%EOF 文件尾", 来源="文档生成")
    页数 = _尝试重开(字节)
    if 页数 is None:
        return 结果.失败("生成失败", "PDF 无法重新打开解析", 来源="文档生成")
    return 结果.成功结果([f"PDF 头尾齐全，可重新打开（{页数} 页）"])


def _尝试重开(字节: bytes) -> int | None:
    """经 PDF 隔离提供者在独立子进程中重新打开 PDF，返回页数。

    主进程不加载 fitz/pdfplumber（PyMuPDF SWIG 崩溃隔离）；
    子进程崩溃/不可用时返回 None（签名校验降级为仅头尾检查）。
    """
    from 支持库.适配层.PDF隔离提供者 import 校验PDF隔离
