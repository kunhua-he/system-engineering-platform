...
def 生成支持库模板(*, 输出目录: str | Path, 包id: str, 名称: str, 能力清单: list[dict],
                   ...
                   测试文件路径: str | Path | None = None, 开工ID: str | None = None) -> 结果:
    """按能力清单生成九要素支持库包模板；失败返回结构化错误码。

    `开工ID`（2026-09-25 开工-20260925-161955-849f）：本次改动的开工ID，逐跳透传到
    `模板公共件.写产物` → `文件系统支持库.文件操作.写入文件`，供写腿第二判据核对活跃写租约
    的所有者；受管路径缺它即 `越界`（写腿拒绝经 `写入被拒` 原样上抛，不收敛成别的码）。
    """
    输出目录 = Path(输出目录)
    ...
    _写(输出目录 / "待作者补.md", 生成待作者补清单(
        包目录=str(输出目录), 名称=名称, 能力清单=能力清单, 第三方说明=第三方), 开工ID)
    _写(输出目录 / "完整性摘要.json", _转JSON文本(
        生成完整性摘要(输出目录, 包id=包id, 版本="1.0.0")), 开工ID)
