...
def 生成模块模板(模块名: Any = None, 模块类型: Any = "基础模块", 能力清单: Any = None,
                ...
                测试中心根: Any = None, 开工ID: Any = None) -> 结果:
...
def 生成并写入完整性摘要(包目录: Any = None, 开工ID: Any = None) -> 结果:
    ...
    try:
        摘要 = _生成并写入完整性摘要(目标, 开工ID=str(开工ID or "").strip())
    except 写入被拒 as 错误:
        # 写腿第二判据（写入授权）的拒绝：**原样透出真码**（`越界`）——它继承 ValueError，
        # 不提前拦下会被下面的 `except ValueError` 误报成 `包无正式文件`（真因被洗掉）。
        return 结果.失败(错误.错误码, 错误.错误说明, 来源=来源)
