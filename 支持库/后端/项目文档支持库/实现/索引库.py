...
from .检索粒度 import 收敛细节级别, 细节级别表, 投影记录
...
def 查询索引(项目名: str = None, 关键词: str = None, 返回条数: int = 20,
            库文件: str = None, 细节级别: str = None, 搜索词: list = None,
            截断上限: int = 0) -> 结果:
    ...
    **如实报「未核验」而不是默认当新鲜**。
    ...
    if not 组结果:
        return 结果.成功结果({
            ...
            "是否截断": 假, "细节级别": 档, "生效搜索词": [], "搜索词组数": len(词组表),
            "扩词记录": 扩词记录,
    ...
    return 结果.成功结果({
        ...
        "命中数": len(条目表), "是否截断": len(条目表) > 上限, "细节级别": 档,
