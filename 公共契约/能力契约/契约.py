...
def _取返回类型位(名称: Any) -> str:
    ...
    2. **字符串化对象形态** `"{'类型': '结果型', '值结构': {...}}"`：
       `公共契约/包声明/声明.py::从字典构建` 用 `返回=str(条目.get("返回", ""))` 把对象
...
class 未知参数错误(TypeError):
    ...
    进程内腿（`唯一能力调用` / 契约层）对**未声明键是拒绝的**；网关边界对未对齐参数
    是**剔除**（在边界剔除，**不判红、不失败**，且信封回带 `被忽略参数`）——**两层口径不同，别混**。
