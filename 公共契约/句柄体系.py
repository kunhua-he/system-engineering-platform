...
def _监听枚举工具() -> str | None:
    ...
    **B-11 收口点**：端口回收要靠 ``lsof`` 把「监听某端口的进程」枚举出来，再与句柄
    ...
    收口层复用说明：同仓 ``公共契约/运行时/平台适配.py`` 只提供平台判定、虚拟环境
...
class 句柄体系:
    def __init__(self) -> None:
        ...
        self.失效id留痕: deque[int] = deque(maxlen=失效id留痕上限)   # 已淘汰句柄的 id（幂等判定）
...
__all__ = ["句柄", "句柄体系", "是合法句柄id", "生成句柄id", "句柄位数", "句柄绑定资源上限", "回收证据上限", "失效句柄保留数", "失效句柄保留秒", "失效id留痕上限", "句柄类型_读取", "句柄类型_修改事务", "句柄类型_资源", "句柄类型_任务", "句柄类型_会话", "状态_已创建", "状态_有效", "状态_已失效", "失效原因_过期", "失效原因_回收", "失效原因_释放", "失效原因_超时"]
