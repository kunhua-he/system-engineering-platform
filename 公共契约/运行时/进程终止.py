...
- **整组收敛面（2026-09-24 批R·R-6 补齐，审计 R3）**：``进程组存活`` / ``等待进程组退出`` /
  ``发进程组信号`` 三个原语把「整组回收」的判定与发信号收成唯一实现，``强制结束子进程``
  的宽限与强杀**复查也改判整组**（组长已退出、同组子孙仍在 ⇒ 未收敛，继续升级强杀）。
  此前这套流程在 `独立进程_进程组收口面`、`进程提供者`、`任务进程_回收落盘面`、
  `HTML验证/制品进程` 各写一份，各份收尾稳健度并不相同 —— 同源副本真泄漏过（差异记录见
  `支持库/后端/媒体处理支持库/FFmpeg媒体/实现/进程管理.py`）。
...
# --------------------------------------------------------------------------- #
# 进程组收敛面（2026-09-24 批R·R-6 补齐，审计 R3）：组存活 / 等组退出 / 两发组信号
#
# 这一簇是「整组回收」的唯一实现。此前同一套流程在 `独立进程_进程组收口面`、`进程提供者`、
# `任务进程_回收落盘面`、`HTML验证/制品进程` 各写一份，各份收尾稳健度并不相同 ——
# 同源副本真泄漏过（差异记录见 `支持库/后端/媒体处理支持库/FFmpeg媒体/实现/进程管理.py`）。
# --------------------------------------------------------------------------- #

def 信号已发出(收口结果) -> bool:
    """判定 ``终止进程组`` / ``按组号终止`` / ``发进程组信号`` 的信封是否**真发出信号**。

    只读信封里的 ``已发出信号`` 字段（由本模块按平台事实填写），调用点不猜平台：
    成功类结论在 ``.值``，失败类结论在 ``.详细信息``。
    """
...
def _按入参组探活(进程) -> bool:
    """按「入参进程号 == 组号」的约定探活该组（**不看句柄是否已回收**）。

    约定来源：``子进程组启动标志()``（POSIX ``setsid``）建立的独立会话组长，其组号 == 组长 pid。
    非法入参一律 ``假``。
    """
    if _是进程句柄(进程):
        进程ID = 进程.pid
...
def 进程组存活(进程) -> bool:
    """整组成员是否仍存活；**组长已回收但同组子孙仍在也算存活**。非法入参返回 ``假``。

    为什么不能只看组长句柄：``Popen.poll()/wait()`` 一旦观察到组长退出就把它**回收**，
    回收后 ``os.getpgid(组长pid)`` 必然失败，而 ``终止进程组`` / ``进程存活`` 都以它为前提
    —— 「组长正常退出、同组子孙仍在」这条语义只有按组号原语（``按组号探活``）判得出来。

    判据（顺序即口径）
        1. 传句柄：``poll() is None`` → 真（句柄未回收即未退出，组必在）；已回收 → ``按组号探活(进程.pid)``；
        2. 传裸进程号（约定：组号 == 组长 pid）：先 ``进程存活`` 看进程本身，再 ``按组号探活``
           看该组 —— 只看组会把「非组长进程仍活着」漏判；
        3. 非 POSIX 平台无组概念（``按组号探活`` 如实回假）→ 只有句柄存活才算存活，不伪造。

    僵尸计为存活（``进程存活`` 的如实语义）。
    """
    if 进程 is None or isinstance(进程, bool):
        return 假
    if _是进程句柄(进程):
        try:
            if 进程.poll() is None:
...
def 等待进程组退出(进程, 超时秒: float = 默认等待秒) -> bool:
    """轮询等待**整组**收敛；返回是否已收敛（超时返回 ``假``）。不抛异常。

    收敛判据 = 「组长句柄已回收（或裸进程号已消失）**且** 该组无残留成员」：

    - 句柄路径先 ``wait(timeout=轮询间隔秒)``（组长退出即回收，顺带收僵尸），等到了才问
      「组还在不在」（``按组号探活``）—— 组长已回收但同组子孙仍在 ⇒ **未收敛**；
    - 裸进程号路径每轮先 ``_回收自有子进程``（否则自有子进程的僵尸会让组探活一直为真）；
    - 与 ``等待进程消失`` 的区别就在第二条判据：后者只问「进程在不在」，本函数问「整组在不在」。
    """
    if isinstance(超时秒, bool) or not isinstance(超时秒, (int, float)) or 超时秒 < 0:
        return 假
    截止 = time.monotonic() + float(超时秒)
...
def 发进程组信号(进程, 信号: str = "终止") -> 结果[dict[str, Any]]:
    """向**整组**发一次信号，返回结果信封，**永不抛异常**。

    两发都走本模块的组原语，调用点不做任何平台判断：

    1. ``终止进程组(进程ID)``：组长仍是活着的进程（含僵尸）时按组长身份一次整组终止；
       不是组长时退化为单进程（不误伤同组其他进程）；
    2. 第一发没真发出信号时，改按**组号**补发一次（``按组号终止``）—— 组长已被 ``Popen``
       回收时取不到组号（只能如实回「进程不存在」），而组内子孙可能仍在；**自保护**：目标
       组号与本进程组号相同时拒绝补发，绝不向自己所在的组发信号（组号由 ``进程组号()`` 取，
       非 POSIX 平台如实回 ``None``，这不是平台判断而是「不要打自己」的安全闸）；
    3. 两发都没发出且手上有句柄时，退化为句柄信号（与原实现 ``killpg`` 抛错后回退同语义）。

    返回 ``结果.值``：``{结论, 进程ID, 信号, 平台, 已发出信号, 补发腿, 说明表, 说明}``。
    已发出信号时结论为 ``已终止``；未发出时错误码沿用第一发的收口结论（``无权限`` /
    ``不支持`` / ``进程不存在`` / ``终止失败``），调用方据此区分「没送达」与「已送达」。
    """
    if 信号 not in 信号表:
        return _失败结果(结论_参数错误, f"信号必须是 {信号表} 之一，收到: {信号!r}")
    if 进程 is None or isinstance(进程, bool):
        return _失败结果(结论_参数错误, f"进程 不接受 {进程!r}，须是有 pid 的句柄或正整数进程号")
    if _是进程句柄(进程):
        进程ID = 进程.pid
    elif isinstance(进程, int):
        进程ID = 进程
        ...
        return _失败结果(
            结论_参数错误,
            f"进程 必须是有 pid 的句柄或正整数进程号，收到 {type(进程).__name__}",
        )
    if not isinstance(进程ID, int) or 进程ID <= 0:
        return _失败结果(结论_参数错误, f"无法从入参取得合法进程号: {进程ID!r}")

    说明表: list[str] = []
    第一发 = 终止进程组(进程ID, 信号=信号)
    已发出 = 信号已发出(第一发)
    补发腿 = 假
    if 已发出:
        说明表.append(f"按组长身份整组发信号：{(第一发.值 or {}).get('说明') or 信号}")
    else:
        第一发说明 = (第一发.值 or 第一发.详细信息 or {}).get("说明") or 第一发.错误说明 or 第一发.错误码
        说明表.append(f"按组长身份发信号未送达：{第一发说明}")
        本进程组号 = 进程组号(os.getpid())
        if 本进程组号 is None or 进程ID != 本进程组号:
            第二发 = 按组号终止(进程ID, 信号=信号)
            if 信号已发出(第二发):
                已发出 = 真
                补发腿 = 真
                说明表.append(f"按组号补发：{(第二发.值 or {}).get('说明') or 信号}")
            else:
                第二发说明 = (第二发.值 or 第二发.详细信息 or {}).get("说明") or 第二发.错误说明 or 第二发.错误码
                说明表.append(f"按组号补发未送达：{第二发说明}")
        else:
            说明表.append("拒绝按组号补发：目标组号与本进程同组（不向自己所在的组发信号）")
    if not 已发出 and _是进程句柄(进程):
        try:
            if 进程.poll() is None:
                if 信号 == "终止":
                    进程.terminate()
                    ...
                    进程.kill()
                已发出 = 真
                说明表.append("句柄兜底：已向进程句柄发出信号")
        except (OSError, ProcessLookupError) as 错误:
            说明表.append(f"句柄兜底失败：{type(错误).__name__}: {错误}")
    说明 = "；".join(说明表)
    值 = {
        "结论": 结论_已终止 if 已发出 else (第一发.错误码 or 结论_终止失败),
        "进程ID": 进程ID,
        "信号": 信号,
        "平台": _当前平台名(),
        "已发出信号": 已发出,
    ...
    if 已发出:
        return 结果.成功结果(值)
    return _失败结果(值["结论"], 说明 or "发进程组信号未送达", 详情=值)
...
def _结束子进程核心(
    ...
    **复查口径是「整组」**（2026-09-24 批R·R-6）：幂等判定走 ``进程组存活``，宽限与强杀后的
    等待走 ``等待进程组退出``，两发信号走 ``发进程组信号``（含「组长已被回收时按组号补发」）。
    「组长已退出但同组子孙仍在」判**未收敛**并升级强杀 —— 此前只看组长句柄死透，而
    ``Popen.poll()/wait()`` 一看到组长退出就把它回收，同组子孙就在这一眼里漏掉。
    ...
    if not 进程组存活(进程):
        说明表.append("目标进程组此前已收敛（组长已结束且组内无存活成员），无需终止（幂等）")
        return _组装结束(结论_进程不存在, 已使用强杀, 说明表, *参数)

    # 阶段一：优雅终止（整组两发：先按组长身份，组长已被回收时按组号补发）
    第一步 = 发进程组信号(进程, 信号="终止")
    if not 第一步.成功:
        if 第一步.错误码 == 结论_进程不存在:
    ...
    说明表.append(f"终止阶段：{第一步.值.get('说明') if 第一步.值 else '已发出终止信号'}")

    if 等待进程组退出(进程, 宽限秒):
        说明表.append(f"{宽限秒} 秒宽限内整组已收敛")
        return _组装结束(结论_已终止, 已使用强杀, 说明表, *参数)
    说明表.append(f"{宽限秒} 秒宽限内整组未收敛，升级为强杀")
    ...
    if 等待进程组退出(进程, 等待秒):
        说明表.append(f"强杀后 {等待秒} 秒内整组已收敛")
        return _组装结束(结论_已终止, 已使用强杀, 说明表, *参数)

    说明表.append(f"强杀后 {等待秒} 秒整组仍未收敛（僵尸计为存活；组长已回收但同组子孙仍在也算未收敛）")
...
__all__ = [
    ...
    "等待进程消失",
    "终止进程组",
    # 2026-09-24 批R·R-6 补齐：整组收敛面（组存活 / 等组退出 / 两发组信号 / 信号已发出判定）
    "信号已发出",
    "进程组存活",
    "等待进程组退出",
    "发进程组信号",
