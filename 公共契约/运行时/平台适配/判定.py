"""平台判定（跨平台收口层的**最底层**：全仓唯一允许读 `sys.platform` / `os.name` / `platform.*` 的地方）。"""
...
def 原始宿主标志() -> str:
    """返回 ``os.name`` 原始值（``posix``/``nt``/``java``…），**仅用于诊断与自检对账**。

    与 `原始平台标志()` 成对：``sys.platform`` 是**解释器口径**（解释器构建时定），
    ``os.name`` 是**宿主口径**（``os`` 模块按宿主平台定）—— 两个口径**同源不同路**，
    故可互为交叉验证。本函数是 ``os.name`` 的**全仓唯一取值点**（铁律：平台判断只许出现在
    收口层；调用点裸读 ``os.name`` 会被 `开发工具/验证门禁/平台判断越界检测.py` 判红）。

    **为什么必须有这个原语（能力缺口登记，2026-09-23）**：`开发工具/环境自检/` 的两处自检项
    （`探测_环境能力.py` 的「虚拟环境解释器布局」与 `自检_装配与外部.py` 的装配安全闸）要拿
    **独立于 ``sys.platform`` 的**事实源，去校验 `虚拟环境解释器相对路径()` 这类由收口层给出的
    判定 —— 那是「对账」，不是「分叉」。若把期望值改成由 `当前平台()` 推出来，期望值与待验证的
    判定就同源了（自证恒真、闸门恒不触发）：`自检_装配与外部.py` 已记录该后果的实测代价 ——
    按错误的平台判定去跑真实装配，会把 `工程缓存/提供者运行环境/` 下本机有效的受管环境删掉
    （要联网重装第三方依赖才能恢复）。故本原语只提供**取值**，不提供判定。

    **使用约束**：只许用于**诊断、日志与自检对账**（把两个口径摊开给人看/做交叉验证）；
    **行为分叉一律不用它** —— 那是 `当前平台()` / `本机系统名()` 与收口层分支原语的职责。
    """
    return os.name
