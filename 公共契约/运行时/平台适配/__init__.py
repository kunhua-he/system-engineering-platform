...
__all__ = [
    ...
    "平台不支持错误",
    "原始平台标志",
    # 2026-09-23 平台判断越界收口：`原始宿主标志()`（``os.name`` 的唯一取值点）是**新公开原语**
    # ——`开发工具/环境自检/` 的两处自检项（虚拟环境解释器布局、装配安全闸）要用**独立于
    # ``sys.platform`` 的**事实源做对账，裸读 ``os.name`` 会被 `平台判断越界检测` 判红。
    # 未列入 `__all__` 时型检会报 `reportAttributeAccessIssue`（运行时可用，但公开面自述与实际不符）
    # —— 与既有各次补列同一处置：新原语必须同时进公开面。
    "原始宿主标志",
    "本机系统名",
    "当前平台标识",
    "当前平台",
    ...
    "经shell命令表",
    # 2026-09-24 R-27：`shell语法问题` 是「命令文本含未加引号 shell 语法」判据的
    # **全平台唯一实现**（`进程管理.启动进程/执行命令` 的直启腿跨层调用它），
    # 未列入 `__all__` 时型检会报 `reportAttributeAccessIssue` —— 与既有补列同一处置。
    "shell语法问题",
    "进程内存RSS字节",
    "平台稳定缓存根",
    ...
    "项目根环境变量名",
    "解析路径",
    # 2026-09-24 批R R-32：`设置本次调用基准` 是「把**调用方 `项目根`** 交给唯一解析腿」
    # 的**唯一写入口**（`运行核心/统一网关/核心处理/网关执行面.py` 派发前跨层调用它）——
    # 未列入 `__all__` 时型检会报 `reportAttributeAccessIssue`，与既有补列同一处置。
    "设置本次调用基准",
    ...
    "脚本入口准入MCP",
    # 2026-09-25 复制原语收口：`复制文件不传标志` 是「复制文件且**不传播内核不可变标志**」的
    # 全仓唯一实现（`shutil.copy2` 会把源的 `uchg` 一并复制 ⇒ 副本改不动也删不掉）。
    # 两个调用点（`开发工具/HTML验证/动态值.py::夹具文件复制`、
    # `开发工具/统一验证/验证工作区.py::复制输入快照`）都经 `平台适配` 公开面调用它；
    # 未列入 `__all__` 时型检会报 `reportAttributeAccessIssue` —— 与既有补列同一处置。
    "复制文件不传标志",
]
