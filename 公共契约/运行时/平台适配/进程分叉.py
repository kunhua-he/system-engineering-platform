...
def shell语法问题(命令: str) -> str:
    """检测命令串里**未加引号**的 shell 专有语法；命中即返回原因文本（空串=没命中）。

    为什么必须收口在这里：`拆分命令文本()`（argv 直启）与 `经shell命令表()`（整串交解释器）
    是「同一份命令文本、两种解释口径」。调用方写的是 shell 语法却走了 argv 直启那条路时，
    `|` / `&&` / `2>&1` 会被**当成普通参数原样传给程序** —— 2026-09-24 R-27 实测
    `执行命令("ls /tmp 2>&1 | head -3")`：退出码 1、`错误输出` 是 `ls: |: No such file or directory`、
    而 `标准输出` 却是**未过滤的完整目录**。既没报错也没给对结果 —— 典型**静默失效**。
    华哥口径「未对齐就报，不许静默」：这类命令文本一律**显式拒绝**，并把
    「要 shell 语义请传 经shell=真」写进说明。

    判定口径（保守，宁少报不误伤）—— 只认**未加引号**的这几类，因为它们出现在 argv 里
    几乎不可能是本意：
    ...
    **不报**的（argv 里合法且常见）：裸 ``&``（URL 查询串）、``$变量``、通配 ``*``/``?``、
    引号本身；引号内的同名字符一律不报（``rg "a|b" 文件`` 是正常写法）。

    返回的是**原因文本**而不是布尔：调用方要把它原样拼进 `参数不合法` 的说明里，
    与 `_解析超时秒` / `_解析经shell` 同一形状。
    """
    if not isinstance(命令, str) or not 命令:
        return ""
    命中: list[str] = []
    引号 = ""
    位置 = 0
    ...
    if not 命中:
        return ""
    return (f"命令含未加引号的 shell 语法（{'、'.join(命中)}）：本能力默认 argv 直启、"
            "不经 shell，这些字符会被当成普通参数原样传给程序（静默失效）；"
            "要 shell 语义（管道/重定向/&&/变量展开/shell 内建）请传 经shell=真。")
