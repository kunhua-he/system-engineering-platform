...
# 锁定格式常量：写入侧（本模块）与校验侧（锁定校验）共用同一来源。
# 原先两处各写各的字面量 "1.0.0"，校验侧拿它跟写入侧硬编码值比，恒等 → 该判定项
# 永远发现不了任何东西。共用常量后，它至少能真实发现「旧格式锁/被改过的版本字段」。
锁定版本常量 = "1.0.0"
# 契约版本不写字面量：见文件顶部 `from 公共契约.版本规则.契约版本 import 契约版本 as 契约版本常量`。
# 现存旧锁（契约版本 1.0.0）会被 锁定校验 判「契约版本不符」——这是预期效果
# （它就是要能发现旧格式锁），但必须同批用 生成依赖锁定 重新生成锁，否则假红。
能力版本常量 = "1.0.0"
...
def 生成依赖锁定(项目根目录: Path, 系统根目录: Path) -> 锁定结果:
    ...
    # 落盘走**唯一原子写腿**（2026-09-25 核心批·写腿下沉）：本层按依赖防火墙不得调用
    # `支持库.后端.文件系统支持库.文件操作.写入文件`（唯一分层方案：项目适配层只可依赖
    # 公共契约 与 运行核心），而写腿已下沉到 `公共契约/运行时/` ⇒ 本层够得到，不再自建
    # 「解锁 → 临时件 + replace → 收口锁态」三拍骨架（哲学 1.2：一类事情只有一条腿）。
    # `OSError` 原样逸出，口径不变。
    from 公共契约.运行时.写文件 import 原子写文件
    原子写文件(锁定路径, json.dumps(锁定数据, ensure_ascii=False, indent=2), "utf-8", "")
