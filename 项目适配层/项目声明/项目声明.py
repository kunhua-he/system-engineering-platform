...
def 写入项目声明(声明: 项目声明, 声明路径: Path) -> None:
    ...
    落盘走**唯一原子写腿** `公共契约.运行时.写文件.原子写文件`（2026-09-25 核心批·写腿
    下沉）：本层按依赖防火墙不得调用 `支持库.后端.文件系统支持库.文件操作.写入文件`
    （唯一分层方案：项目适配层只可依赖 公共契约 与 运行核心），而写腿已下沉到
    `公共契约/运行时/` ⇒ 本层够得到，不再自建「解锁 → 临时件 + replace → 收口锁态」
    三拍骨架（哲学 1.2：一类事情只有一条腿）。`OSError` 原样逸出，口径不变。
    """
    from 公共契约.运行时.写文件 import 原子写文件
    原子写文件(声明路径, json.dumps(声明.转字典(), ensure_ascii=False, indent=2), "utf-8", "")
