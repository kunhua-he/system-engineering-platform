...
**当前实测规模**（本机 macOS / arm64 / Python 3.14；每条都能一分钟自证，取证命令见文末「审计者事实基线」）：

| 指标 | 数值 |
|---|---|
| 正式公开能力 | **662** |
| 包（支持库 103 / 模块库 18 / 技能库 1 / 平台控制面 15 / 运行核心 1 …） | **133** |
| 包级验证场景 | **827** |
| 依赖锁（第三方边界，全部锁到具体版本） | **32** 份 |
| 外部调用入口 | **1 个网关 · 9 条路径 · 13 项操作** |
| 提交历史 | 865 |
...
| **工具目录**（可选） | 要看全貌时列清单 | 包 → 能力的分级目录（渐进读取，首次只给 60 字简介） |

## 它现在能做什么（能力地图）

下表是**支持库 + 模块库 + 技能库**的公开能力（**565 条**，本机实测；网关公开能力总计 **662 条**，另含平台自用能力）。
每组都能现场核：用关键词搜一下就有真实条目。

| 你想干的事 | 能力域（条数） | 代表能力（真实能力 id） |
|---|---|---|
| 读写文件、搜文件内容 | 文件系统支持库（24） | `文件系统支持库.文件操作.读取文件`、`.写入文件`、`.正则搜索` |
| 处理 Word / Excel / PPT / PDF | 办公文档支持库（24）、文档转换支持库（15） | `办公文档支持库.文档生成.生成DOCX`、`.生成XLSX`、`.生成PPTX`、`文档转换支持库.PDF文本表格.提取表格` |
| 图片识别与处理 | OCR识别支持库（3）、图像处理支持库（9） | `OCR识别.识别图片`、`图像处理支持库.图像解码.生成缩略图` |
| 音视频处理 | 媒体处理支持库（7） | `媒体处理支持库.FFmpeg媒体.探测媒体`、`.转码`、`.提取音频`、`.抽取帧` |
| 语音转文字 | 转写支持库（3） | `转写支持库.转写.转写音频文件` |
| 调大模型 | 大语言模型支持库（50） | `大语言模型支持库.模型连接器.生成对话`、`.上下文压缩.压缩历史`、`.重排服务.重排` |
| 连数据库 | 数据库连接支持库（13） | `数据库连接支持库.SQLite数据库.事务执行`、`.初始化运行数据库` |
| 用 Git | 版本控制支持库（13） | `版本控制支持库.Git操作.提交`、`.创建工作区`、`.回滚`、`.当前状态` |
| 发请求、抓网页 | 网络通信支持库（7） | `网络通信支持库.请求.发送请求`、`.网页解析.提取网页正文` |
| 看代码结构 | 代码解析支持库（11） | `代码解析支持库.解析代码文件`、`.代码地图.查询节点` |
| 并发与排队 | 并发控制支持库（39） | `并发控制支持库.lane排入`、`.lane领取`、`.lane完成` |
| 安全守卫 | 系统核心支持库（78） | `系统核心支持库.命令安全.检测危险命令`、`.协议白名单.校验端点` |
| 浏览器自动化 | 浏览器自动化支持库（7） | `浏览器自动化.导航页面`、`.读取页面`、`.获取截图` |
| 组合流程（一步出结果） | 模块库（61） | `OCR.识别图片文字`、`文件管理.写入文件`、`直播逐字稿.全自动精校` |
| 第三方技术接入 | 适配层（58） | `FFmpeg媒体.转码`、`LibreOffice转换.转换办公文件`、`PDF渲染.渲染整页` |
...
| **调用面唯一** | 一个网关、9 条路径、13 项操作；不允许第二执行腿 | `rg -n '"/' 运行核心/统一网关/本地网关.py` |
...
[`开发文档/架构总览.md`](./开发文档/架构总览.md)（5 张图、每个节点带 `文件:行号`）和
[`开发文档/项目说明.md`](./开发文档/项目说明.md)（设计哲学：12 条主条 + 子项）进。
...
[装配告警] 已跳过 支持库.适配层.YAML提供者｜入口装配｜装配失败（PyYAML 未安装：请通过 pip 安装 pyyaml…）
[装配] 成功= True   能力数= 661   跳过包= 1
```

**结论**：底座能装配起来（661/662 个能力可用），**依赖第三方的包会被单独跳过，其余照常跑**；
跳过的包会**明确告诉你缺什么、怎么装**，不会静默降级，也不会让整台机器起不来。
调用缺库的能力时返回的是明确错误（如「提供者不可用：reportlab 未安装，无法生成 PDF」），不是崩溃。

### 事实二：第三方依赖清单（共 23 项，全部锁到精确版本）
...
> **一句话**：平台**框架层**零第三方、随时可起；**能力层**封装了 23 项第三方技术，
...
| 正式公开能力数 | 同上（「装配冒烟」行） | 662（本机当前含在途能力时为 664） |
| 包数 / 分层 | `find . -name 包声明.json -not -path './工程缓存/*' \| wc -l` | 133 |
| 依赖锁 | `find 支持库 -name 依赖锁.json \| wc -l` | 32 |
| 第三方依赖总数 | `python3.14 开发工具/依赖生命周期审计/运行依赖生命周期审计.py` | 23 项（pip 包 + 外部应用），逐项有台账 |
| 不装第三方能否起 | `python3.14 -S -c "…后端核心.启动()…"` | 能起：661/662 能力，依赖第三方的包被单独跳过并说明缺什么 |
| 唯一调用入口 | `rg -n '"/' 运行核心/统一网关/本地网关.py` | 9 条允许路径 |
| 允许操作 | `rg -n "允许操作表" 运行核心/统一网关/网关核心.py` | 13 项 |
...
| 真实 HTTP 覆盖 | 门禁内 HTML 黑盒（827 个包级场景） | `目标能力 662，步骤 991，清理失败 0，资源残留 0` |
