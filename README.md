...
```mermaid
flowchart TB
    S0["① 取代码<br/>git clone + cd"] --> S1["② 环境自检<br/>开发工具.环境自检<br/>有【不支持】即非零退出"]
    S1 -->|通过| S2["③ 装配冒烟<br/>后端核心.启动()<br/>真断言：成功 + 能力数>0 + 无跳过包"]
    S1 -->|不通过| S9["按报错点名处理：<br/>解释器 / 架构 / 依赖锁 / 外部命令"]
    S9 --> S1
    S2 --> S3["④ 拉起唯一网关<br/>127.0.0.1:40007<br/>要求凭证=真"]
    S3 --> S4{"谁来调用？"}
    S4 -->|Coding Agent| S5["MCP 薄壳（stdio，无端口）<br/>3 工具：目录 / 搜索 / 调用"]
    S4 -->|业务项目 / 脚本 / CI| S6["HTTP：POST /网关/调用<br/>传 能力id + 参数"]
    S4 -->|网页 / 页面工具| S7["HTML/JS 传参<br/>fetch 到同一网关"]
    S5 --> S8["统一信封返回<br/>成功 / 值 / 错误码 / 说明 / 请求id / 耗时"]
    S6 --> S8
    S7 --> S8
    S8 -.->|"后端不改：只需拉取更新"| S3
    S8 -.->|"每个产物可单独选择更新或不更新"| S10["见「产物更新策略」一节"]
```

> **图 A 的三个要点**：① 后端**不用管** —— 平台对业务方而言只是「拉取更新 + 网关常驻」；
> ② **每个产物可选择更新或不更新**（「产物更新策略」一节给了可执行判据）；
...
```mermaid
flowchart LR
    Q1["搜索能力（自然语言）"] --> Q2{"命中了吗"}
    Q2 -->|命中| Q3["直接调用，结束"]
    Q2 -->|没命中| Q4{"是你缺还是平台缺"}
    Q4 -->|"只是没找对词"| Q5["换说法 / 用工具目录翻包<br/>→ 回到搜索"]
    Q4 -->|"平台真的缺"| Q6["提「能力需求」<br/>POST /平台/能力反馈"]
    Q4 -->|"你会写，想加"| Q7["提 PR：新增能力包<br/>（支持库 / 模块库 / 技能库）"]
    Q6 --> Q8["维护者裁定分层与命名"]
    Q7 --> Q8
    Q8 --> Q9["合入 → 编译 → 发布门禁 → 热接入生效"]
    Q9 -.->|"增量装配，免重启；单包失败保持旧版继续跑"| Q3
```
