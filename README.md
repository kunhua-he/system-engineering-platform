...
> 最后更新：2026-09-18
> 口径：**只写不变式与取证命令，不写会漂的具体数字**（包数/能力数/提交数/场景数每建一个包就变；引用前必须重跑取证命令）
> 维护者：对外定位（唯一权威处）
...
**结论**：底座能装配起来（**只有依赖第三方的少数包会被跳过**，其余照常跑）；`能力数` 与 `跳过包` 的具体值以现场跑上面这条命令的输出为准，
本文不写死（在途包会逐分钟改变它们，写死必漂；`跳过包` 必须为 0 才算装配冒烟通过）。
...
> **支持范围（准入口径，2026-09-18 落地）**：本平台当前**只支持 macOS / arm64（Apple Silicon）**。
> 这句话不是描述，是**入口硬拦截**：`运行核心/启动运行核心网关.py` 在**任何装配之前**调用
> `公共契约/运行时/平台适配.校验支持范围()`，非 macOS arm64 时**明确报错并退出码 1**（不做静默降级、
> 不假装可用）。所以 Windows / Linux / x86 机器上启动底座，拿到的是**一句中文支持范围说明**，
> 不是底层 ImportError。复现方式见本文末「事实基线」里的 `平台准入` 一条。

| 平台 | 状态 | 依据 |
|---|---|---|
| **macOS / arm64 / Python 3.14.x** | **完整可用（唯一支持范围，入口强校验）** | 维护者开发与发布环境；依赖锁按此锁定；环境自检 / 装配冒烟 / HTML 黑盒 / 发布门禁均在此实测 |
| Linux / x86_64 | **未验收（入口直接拒绝）** | 代码层已尽量走 `公共契约/运行时/` 收口层，但**无验收记录**；准入判据不认它 |
| **Windows / x64** | **未完成（入口直接拒绝）** | 三条链路仍是 POSIX 专有实现（见下），且 `公共契约/运行时/平台适配.py` 自述「本模块的 Windows 分支**未经真机实测**」 |
...
- **依赖锁当前锁 `macOS / arm64`（份数以 `find 支持库 -name 依赖锁.json | wc -l` 现场输出为准；纯标准库包不建锁）**（本项目在 macOS（Apple Silicon）上开发与验证）：换到 Windows、Linux 或 x86 机器，先按当前环境重建依赖锁再运行。所谓「环境对得上就能部署」，对的就是这一条。锁文件自带证据：`支持库/适配层/*/依赖锁.json` 的 `环境` 字段写明 `Python 3.14.4 / macOS / CPU arm64`。
...
| 平台准入（只支持 macOS arm64） | `unset PYTHONPATH && python3.14 -c "import platform,runpy,sys; sys.platform='win32'; platform.system=lambda:'Windows'; platform.machine=lambda:'AMD64'; sys.argv=['x']; runpy.run_path('运行核心/启动运行核心网关.py', run_name='__main__')"` | **退出码 1** + 中文说明「当前环境不在支持范围内…只支持 macOS / arm64（Apple Silicon）」；**不会继续装配**（仓库根目录下执行） |
